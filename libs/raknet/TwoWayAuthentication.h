...
/// This can be used to determine what permissions are should be allowed to the other system
/// If the other system should not send any data until authentication passes, you can use the MessageFilter plugin for this. Call
/// MessageFilter::SetAllowMessageID() including ID_TWO_WAY_AUTHENTICATION_NEGOTIATION when doing so. Also attach MessageFilter first in the list of plugins
...
class RAK_DLL_EXPORT TwoWayAuthentication : public PluginInterface2 {
  public:
    ...
    /// \details After calling Challenge, you will get back ID_TWO_WAY_AUTHENTICATION_SUCCESS, ID_TWO_WAY_AUTHENTICATION_OUTGOING_CHALLENGE_TIMEOUT, or
    /// ID_TWO_WAY_AUTHENTICATION_OUTGOING_CHALLENGE_FAILED ID_TWO_WAY_AUTHENTICATION_SUCCESS will be returned if and only if the other system has called
    /// AddPassword() with the same identifier\password pair as this system. \param[in] identifier A unique identifier representing this password. This is
    /// transmitted in plaintext and should be considered insecure \return True on success, false on remote system not connected, or identifier not previously
    /// added with AddPassword()
