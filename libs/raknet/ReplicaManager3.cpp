...
void ReplicaManager3::AutoCreateConnectionList(DataStructures::List<RakNetGUID> &participantListIn, DataStructures::List<Connection_RM3 *> &participantListOut,
                                               WorldId worldId) {
...
void Connection_RM3::AutoConstructByQuery(ReplicaManager3 *replicaManager3, WorldId worldId) {
        ...
        while (index < queryToConstructReplicaList.Size()) {
            ...
            if (constructionState == RM3CS_ALREADY_EXISTS_REMOTELY || constructionState == RM3CS_ALREADY_EXISTS_REMOTELY_DO_NOT_CONSTRUCT) {
                        ...
                        res!=RM3SR_SERIALIZED_UNIQUELY)
                {
                        ...
                        if (SendSerialize(replica, allIndices, sp.outputBitstream, sp.messageTimestamp, sp.pro, replicaManager3->GetRakPeerInterface(),
                replicaManager3->GetWorldID())==SSICR_SENT_DATA) lsr->replica->whenLastSerialized=RakNet::GetTimeMS();
                }
                */
    ...
    SendConstruction(constructedReplicasCulled, destroyedReplicasCulled, replicaManager3->defaultSendParameters, replicaManager3->rakPeerInterface, worldId,
                     replicaManager3);
}
...
PluginReceiveResult ReplicaManager3::OnConstruction(Packet *packet, unsigned char *packetData, int packetDataLength, RakNetGUID senderGuid,
...
PluginReceiveResult ReplicaManager3::OnSerialize(Packet *packet, unsigned char *packetData, int packetDataLength, RakNetGUID senderGuid, RakNet::Time timestamp,
                                                 unsigned char packetDataOffset, WorldId worldId) {
    Connection_RM3 *connection = GetConnectionByGUID(senderGuid, worldId);
    if (connection == 0)
        return RR_CONTINUE_PROCESSING;
...
PluginReceiveResult ReplicaManager3::OnDownloadStarted(Packet *packet, unsigned char *packetData, int packetDataLength, RakNetGUID senderGuid,
                                                       unsigned char packetDataOffset, WorldId worldId) {
    Connection_RM3 *connection = GetConnectionByGUID(senderGuid, worldId);
    if (connection == 0)
        return RR_CONTINUE_PROCESSING;
...
PluginReceiveResult ReplicaManager3::OnDownloadComplete(Packet *packet, unsigned char *packetData, int packetDataLength, RakNetGUID senderGuid,
                                                        unsigned char packetDataOffset, WorldId worldId) {
    Connection_RM3 *connection = GetConnectionByGUID(senderGuid, worldId);
    if (connection == 0)
        return RR_CONTINUE_PROCESSING;
...
void ReplicaManager3::BroadcastDestructionList(DataStructures::List<Replica3 *> &replicaListSource, const SystemAddress &exclusionAddress, WorldId worldId) {
    ...
    for (j = 0; j < world->connectionList.Size(); j++) {
        ...
        if (cnt > 0) {
            ...
            rakPeerInterface->Send(&bsOut, defaultSendParameters.priority, defaultSendParameters.reliability, defaultSendParameters.orderingChannel,
                                   world->connectionList[j]->GetSystemAddress(), false, defaultSendParameters.sendReceipt);
        }
    }
}
...
Connection_RM3::Connection_RM3(const SystemAddress &_systemAddress, RakNetGUID _guid) : systemAddress(_systemAddress), guid(_guid) {
    isValidated = false;
...
SendSerializeIfChangedResult Connection_RM3::SendSerialize(RakNet::Replica3 *replica, bool indicesToSend[RM3_NUM_OUTPUT_BITSTREAM_CHANNELS],
                                                           ...
                                                           PRO sendParameters[RM3_NUM_OUTPUT_BITSTREAM_CHANNELS], RakPeerInterface *rakPeer,
...
SendSerializeIfChangedResult Connection_RM3::SendSerializeIfChanged(LastSerializationResult *lsr, SerializeParameters *sp, RakNet::RakPeerInterface *rakPeer,
    ...
    if (replica->forceSendUntilNextUpdate) {
        ...
        return SendSerialize(replica, replica->lastSentSerialization.indicesToSend, replica->lastSentSerialization.bitStream, sp->messageTimestamp, sp->pro,
                             rakPeer, worldId, curTime);
    }
...
void Connection_RM3::SendConstruction(DataStructures::List<Replica3 *> &newObjects, DataStructures::List<Replica3 *> &deletedObjects, PRO sendParameters,
    ...
    for (newListIndex = 0; newListIndex < newObjects.Size(); newListIndex++) {
        ...
        RM3SerializationResult res = replica->Serialize(&sp);
        if (res != RM3SR_NEVER_SERIALIZE_FOR_THIS_CONNECTION && res != RM3SR_DO_NOT_SERIALIZE && res != RM3SR_SERIALIZED_UNIQUELY) {
            bool allIndices[RM3_NUM_OUTPUT_BITSTREAM_CHANNELS];
            for (int z = 0; z < RM3_NUM_OUTPUT_BITSTREAM_CHANNELS; z++) {
                sp.bitsWrittenSoFar += sp.outputBitstream[z].GetNumberOfBitsUsed();
                allIndices[z] = true;
