    ...
    while (i < (int)natDetectionAttempts.Size()) {
        if (time > natDetectionAttempts[i].nextStateTime) {
            ...
            case STATE_TESTING_FULL_CONE_2:
                ...
                // S2P3 sends to C1 (Different address, different port, to previously used port on client). If received, Full-cone nat. Done.  (Else S2P3
                // potentially banned, do not use again).
                saOut = natDetectionAttempts[i].systemAddress;
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void NatTypeDetectionServer::DeallocRNS2RecvStruct(RNS2RecvStruct *s, const char *file, unsigned int line) { RakNet::OP_DELETE(s, file, line); }
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
RNS2RecvStruct *NatTypeDetectionServer::AllocRNS2RecvStruct(const char *file, unsigned int line) { return RakNet::OP_NEW<RNS2RecvStruct>(file, line); }
