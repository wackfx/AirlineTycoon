...
class RAK_DLL_EXPORT RelayPlugin : public PluginInterface2 {
  public:
    ...
    /// \param[in] key A string to identify the target's RakNetGUID. This is so the sending system does not need to know the RakNetGUID of the target system.
    /// The key should be unique among all guids added. If the key is not unique, only one system will be sent to (at random). \param[in] guid The RakNetGuid of
    /// the system to send to. If this system disconnects, it is removed from the internal hash \return RPE_ADD_CLIENT_TARGET_NOT_CONNECTED,
    /// RPE_ADD_CLIENT_NAME_ALREADY_IN_USE, or RPE_ADD_CLIENT_OK
    ...
    /// \param[in] relayPluginServerGuid the RakNetGUID of the system running RelayPlugin
    /// \param[in] destinationGuid The key value passed to AddParticipant() earlier on the server. If this was not done, the server will not relay the message
    /// (it will be silently discarded). \param[in] bitStream The data to relay \param[in] priority See the parameter of the same name in
    /// RakPeerInterface::Send() \param[in] reliability See the parameter of the same name in RakPeerInterface::Send() \param[in] orderingChannel See the
    /// parameter of the same name in RakPeerInterface::Send()
    void SendToParticipant(const RakNetGUID &relayPluginServerGuid, const RakString &destinationGuid, BitStream *bitStream, PacketPriority priority,
                           PacketReliability reliability, char orderingChannel);

    void SendGroupMessage(const RakNetGUID &relayPluginServerGuid, BitStream *bitStream, PacketPriority priority, PacketReliability reliability,
