...
/// Furthermore, this must be done before sending packets that use this class, since the strings are ordered for fast lookup.  Adding after that time would mess
/// up all the indices so don't do it.<BR> Don't use this class to write strings which were not previously registered with AddString, since you just waste
/// bandwidth then.  Use StringCompressor instead. \brief Writes a string index, instead of the whole string
class RAK_DLL_EXPORT StringTable {
  public:
    ...
    /// The RakPeer constructor adds a reference to this class, so don't call this until an instance of RakPeer exists, or unless you call AddReference
    /// yourself. \return the unique instance of the StringTable
    ...
    /// Relies on the StringCompressor class, which is automatically reference counted in the constructor and destructor in RakPeer.  You can call the reference
    /// counting functions yourself if you wish too. \param[in] input Pointer to an ASCII string \param[in] maxCharsToWrite The size of \a input \param[out]
    /// output The bitstream to write the compressed string to
    ...
    /// Relies on the StringCompressor class, which is automatically reference counted in the constructor and destructor in RakPeer.  You can call the reference
    /// counting functions yourself if you wish too. \param[out] output A block of bytes to receive the output \param[in] maxCharsToWrite Size, in bytes, of \a
    /// output .  A NULL terminator will always be appended to the output string.  If the maxCharsToWrite is not large enough, the string will be truncated.
  ...
  protected:
    ...
    /// \param[in] maxCharsToWrite Size, in bytes, of \a output .  A NULL terminator will always be appended to the output string.  If the maxCharsToWrite is
    /// not large enough, the string will be truncated.
