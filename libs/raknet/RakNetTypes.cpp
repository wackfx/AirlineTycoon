...
bool SystemAddress::EqualsExcludingPort(const SystemAddress &right) const {
    return (address.addr4.sin_family == AF_INET && address.addr4.sin_addr.s_addr == right.address.addr4.sin_addr.s_addr)
           ...
           || (address.addr4.sin_family == AF_INET6 &&
               memcmp(address.addr6.sin6_addr.s6_addr, right.address.addr6.sin6_addr.s6_addr, sizeof(address.addr6.sin6_addr.s6_addr)) == 0)
#endif
        ;
}
...
unsigned long SystemAddress::ToInteger(const SystemAddress &sa) {
    unsigned int lastHash =
        SuperFastHashIncremental((const char *)&sa.address.addr4.sin_port, sizeof(sa.address.addr4.sin_port), sizeof(sa.address.addr4.sin_port));
#if RAKNET_SUPPORT_IPV6 == 1
...
void SystemAddress::FixForIPVersion(const SystemAddress &boundAddressToSocket) {
    ...
    if (strcmp(str, IPV6_LOOPBACK) == 0) {
        ...
        // 		if (boundAddressToSocket.GetIPVersion()==4)
        // 		{
        // 			// Some kind of bug with sendto: returns "The requested address is not valid in its context." if loopback doesn't have the same IP
        // address 			address.addr4.sin_addr.s_addr=boundAddressToSocket.address.addr4.sin_addr.s_addr;
        // 		}
    }
}
...
bool SystemAddress::FromString(const char *str, char portDelineator, int ipVersion) {
    ...
    if (ipVersion == 4 && strcmp(str, IPV6_LOOPBACK) == 0) {
        ...
        for (; i < sizeof(ipPart) && str[i] != 0 && str[i] != portDelineator; i++) {
            if ((str[i] < '0' || str[i] > '9') && (str[i] < 'a' || str[i] > 'f') && (str[i] < 'A' || str[i] > 'F') && str[i] != '.' && str[i] != ':' &&
                str[i] != '%' && str[i] != '-' && str[i] != '/')
                break;
...
unsigned long RakNetGUID::ToUint32(const RakNetGUID &g) { return ((unsigned long)(g.g >> 32)) ^ ((unsigned long)(g.g & 0xFFFFFFFF)); }
