...
1. Sender calls ConnectInternal(). A ConnnectRequest structure is allocated and stored in the connectionRequests list, containing a list of every system we are
connected to. ID_ROUTER_2_QUERY_FORWARDING is sent to all connected systems.

2. Upon the router getting ID_ROUTER_2_QUERY_FORWARDING, ID_ROUTER_2_REPLY_FORWARDING is sent to the sender indicating if that router is connected to the
endpoint, along with the ping from the router to the endpoint.

3. Upon the sender getting ID_ROUTER_2_REPLY_FORWARDING, the connection request structure is looked up in Router2::UpdateForwarding. The ping is stored in that
structure. Once all systems have replied, the system continues to the next state. If every system in step 1 has been exhausted, and routing has occured at least
once, then ID_CONNECTION_LOST is returned. If every system in step 1 has been exhausted and routing has never occured, then ID_ROUTER_2_FORWARDING_NO_PATH is
returned. Otherwise, the router with the lowest ping is chosen, and RequestForwarding() is called with that system, which sends ID_ROUTER_2_REQUEST_FORWARDING
to the router.

4. When the router gets ID_ROUTER_2_REQUEST_FORWARDING, a MiniPunchRequest structure is allocated and stored in the miniPunchesInProgress list. The function
SendOOBMessages() sends ID_ROUTER_2_REPLY_TO_SENDER_PORT from the routing sockets to both the sender and endpoint. It also sends
ID_ROUTER_2_REPLY_TO_SPECIFIED_PORT through the regular RakNet connection.

5. The sender and endpoint should get ID_ROUTER_2_REPLY_TO_SENDER_PORT and/or ID_ROUTER_2_REPLY_TO_SPECIFIED_PORT depending on what type of router they have. If
ID_ROUTER_2_REPLY_TO_SENDER_PORT arrives, then this will reply back to the routing socket directly. If ID_ROUTER_2_REPLY_TO_SPECIFIED_PORT arrives, then the
reply port is modified to be the port specified by the router system. In both cases, ID_ROUTER_2_MINI_PUNCH_REPLY is sent. As the router has already setup the
forwarding, ID_ROUTER_2_MINI_PUNCH_REPLY will actually arrive to the endpoint from the sender, and from the sender to the endpoint.

6. When ID_ROUTER_2_MINI_PUNCH_REPLY arrives, ID_ROUTER_2_MINI_PUNCH_REPLY_BOUNCE will be sent to the router. This is to tell the router that the forwarding has
succeeded.

7. When ID_ROUTER_2_MINI_PUNCH_REPLY_BOUNCE arrives on the router, the router will find the two systems in the miniPunchesInProgress list, which was added in
step 4 (See OnMiniPunchReplyBounce()). gotReplyFromSource or gotReplyFromEndpoint will be set to true, depending on the sender. When both gotReplyFromSource and
gotReplyFromEndpoint have replied, then ID_ROUTER_2_REROUTE is sent to the endpoint, and ID_ROUTER_2_FORWARDING_ESTABLISHED is sent to the sender.

8. When the endpoint gets ID_ROUTER_2_REROUTE, the system address is updated for the guid of the sender using RakPeer::ChangeSystemAddress(). This happens in
OnRerouted().

9. When the sender gets ID_ROUTER_2_FORWARDING_ESTABLISHED, then in OnForwardingSuccess() the endpoint is removed from the connectionRequest list and moved to
the forwardedConnectionList.

10. In OnClosedConnection(), for the sender, if the closed connection is the endpoint, then the endpoint is removed from the forwardedConnectionList (this is a
graceful disconnect). If the connection was instead lost to the router, then ConnectInternal() gets called, which goes back to step 1. If instead this was a
connection requset in progress, then UpdateForwarding() gets called, which goes back to step 3.

11. When the user connects the endpoint and sender, then the sender will get ID_CONNECTION_REQUEST_ACCEPTED. The sender will look up the endpoint in the
forwardedConnectionList, and send ID_ROUTER_2_INCREASE_TIMEOUT to the endpoint. This message will call SetTimeoutTime() on the endpoint, so that if the router
disconnects, enough time is available for the reroute to complete.
*/
...
bool Router2::ConnectInternal(RakNetGUID endpointGuid, bool returnConnectionLostOnFailure) {
        ...
        if (guids[i] != endpointGuid) {
            ...
            if (debugInterface) {
                ...
                debugInterface->ShowDiagnostic(
                    FormatStringTS(buff, "Router2::ConnectInternal: at %s:%i [else ..].: %I64d==%I64d", __FILE__, __LINE__, guids[i].g, endpointGuid.g));
            }
        }
    }
    connectionRequestsMutex.Lock();
    ...
    if (debugInterface) {
        char buff[512];
        debugInterface->ShowDiagnostic(
            FormatStringTS(buff, "Broadcasting ID_ROUTER_2_QUERY_FORWARDING to %I64d at %s:%i\n", endpointGuid.g, __FILE__, __LINE__));
    }
    ...
    if (packet->data[0] == ID_ROUTER_2_INTERNAL) {
        ...
        case ID_ROUTER_2_REQUEST_FORWARDING: {
            ...
            if (debugInterface) {
                ...
                debugInterface->ShowDiagnostic(FormatStringTS(buff, "Got ID_ROUTER_2_REQUEST_FORWARDING on ip %s from %I64d, ", buff2, packet->guid.g));
            }
        ...
        case ID_ROUTER_2_REPLY_TO_SENDER_PORT: {
            ...
            if (debugInterface) {
                ...
                debugInterface->ShowDiagnostic(
                    FormatStringTS(buff, "Got ID_ROUTER_2_REPLY_TO_SENDER_PORT %i on address %s, replying with ID_ROUTER_2_MINI_PUNCH_REPLY at %s:%i\n",
...
void Router2::Update(void) {
    ...
    while (connectionRequestIndex < connectionRequests.Size()) {
        ...
        if (connectionRequest->requestState == R2RS_REQUEST_STATE_QUERY_FORWARDING && connectionRequest->pingTimeout < curTime) {
...
void Router2::OnClosedConnection(const SystemAddress &systemAddress, RakNetGUID rakNetGUID, PI2_LostConnectionReason lostConnectionReason) {
    ...
    while (forwardedConnectionIndex < forwardedConnectionList.Size()) {
        if (forwardedConnectionList[forwardedConnectionIndex].endpointGuid == rakNetGUID) {
            ...
            // Lost connection to intermediary. Restart process to connect to endpoint. If failed, push ID_CONNECTION_LOST. Also remove connection request if it
            // already is in the list, to restart it
            connectionRequestsMutex.Lock();
            ...
            if (debugInterface) {
                char buff[512];
                debugInterface->ShowDiagnostic(
                    FormatStringTS(buff, "Closed connection %I64d, restarting forwarding at %s:%i\n", rakNetGUID.g, __FILE__, __LINE__));
            }
        ...
        if (connectionRequestGuidIndex != (unsigned int)-1) {
            ...
            if (UpdateForwarding(cr) == false) // If returns false, no connection request systems left
            {
                if (debugInterface) {
                    char buff[512];
                    debugInterface->ShowDiagnostic(
                        FormatStringTS(buff, "Aborted connection to the %I64d, aborted forwarding at %s:%i\n", rakNetGUID.g, __FILE__, __LINE__));
                }

                RemoveConnectionRequest(connectionRequestIndex);
            } else // Else a system in the connection request list dropped. If cr->requestState==R2RS_REQUEST_STATE_QUERY_FORWARDING then we are still waiting
                   // for other systems to respond.
            {
                ...
                //                 if(volatile bool is_my_fix_a_truth = true) { // A system in the list of potential systems to try routing to dropped. There is
                //                 no need to restart the whole process.
                //                     connectionRequestsMutex.Lock();
...
bool Router2::UpdateForwarding(ConnnectRequest *connectionRequest) {
    ...
    if (connectionRequest->connectionRequestSystems.Size() == 0) {
        ...
        if (connectionRequest->returnConnectionLostOnFailure) {
            ...
            //             sendDisconnectionNotification); RAKNET_DEBUG_PRINTF(__FUNCTION__": call rakPeerInterface->CloseConnection(%I64d)" ,
...
void Router2::RequestForwarding(ConnnectRequest *connectionRequest) {
    ...
    for (connectionRequestGuidIndex = 0; connectionRequestGuidIndex < connectionRequest->connectionRequestSystems.Size(); connectionRequestGuidIndex++) {
        ...
        commandList.Insert(connectionRequest->connectionRequestSystems[connectionRequestGuidIndex],
                           connectionRequest->connectionRequestSystems[connectionRequestGuidIndex], true, _FILE_AND_LINE_);
    }
    connectionRequest->connectionRequestSystemsMutex.Unlock();
...
void Router2::OnQueryForwarding(Packet *packet) {
    ...
    if (debugInterface) {
        char buff[512];
        debugInterface->ShowDiagnostic(
            FormatStringTS(buff, "Sending ID_ROUTER_2_REPLY_FORWARDING to the %I64d at %s:%i\n", packet->guid.g, __FILE__, __LINE__));
    }
}
void Router2::OnQueryForwardingReply(Packet *packet) {
    ...
    if (debugInterface) {
        ...
        debugInterface->ShowDiagnostic(FormatStringTS(buff, "Got ID_ROUTER_2_REPLY_FORWARDING on address %s(%I64d -> %I64d) canForward=%i at %s:%i\n", buff2,
                                                      packet->guid.g, endpointGuid.g, canForward, __FILE__, __LINE__));
    }
...
void Router2::OnRequestForwarding(Packet *packet) {
    ...
    UDPForwarderResult result =
        udpForwarder->StartForwarding(packet->systemAddress, endpointSystemAddress, 30000, 0, socketFamily, &forwardingPort, &forwardingSocket);
    ...
    if (result == UDPFORWARDER_FORWARDING_ALREADY_EXISTS) {
        ...
        if (debugInterface)
            debugInterface->ShowFailure(FormatStringTS(buff,
                                                       "Router2 failed at %s:%i with UDPFORWARDER_INVALID_PARAMETERS, packet->systemAddress=%s, "
                                                       "endpointSystemAddress=%s, forwardingPort=%i, forwardingSocket=%i\n",
                                                       __FILE__, __LINE__, buff2, buff3, forwardingPort, forwardingSocket));
        SendFailureOnCannotForward(packet->guid, endpointGuid);
    ...
    while (i < miniPunchesInProgress.Size()) {
        if (miniPunchesInProgress[i].sourceGuid == packet->guid || miniPunchesInProgress[i].endpointGuid == packet->guid) {
            ...
            if (debugInterface) {
                char buff[512];
                debugInterface->ShowDiagnostic(
                    FormatStringTS(buff, "Processing ID_ROUTER_2_MINI_PUNCH_REPLY_BOUNCE, gotReplyFromSource=%i gotReplyFromEndpoint=%i at %s:%i\n",
                                   miniPunchesInProgress[i].gotReplyFromSource, miniPunchesInProgress[i].gotReplyFromEndpoint, __FILE__, __LINE__));
            }

            if (miniPunchesInProgress[i].gotReplyFromEndpoint == true && miniPunchesInProgress[i].gotReplyFromSource == true) {
                SendForwardingSuccess(ID_ROUTER_2_REROUTED, miniPunchesInProgress[i].endpointGuid, miniPunchesInProgress[i].sourceGuid,
                                      miniPunchesInProgress[i].forwardingPort);
                SendForwardingSuccess(ID_ROUTER_2_FORWARDING_ESTABLISHED, miniPunchesInProgress[i].sourceGuid, miniPunchesInProgress[i].endpointGuid,
                                      miniPunchesInProgress[i].forwardingPort);
                miniPunchesInProgress.RemoveAtIndexFast(i);
            } else {
                i++;
    ...
    if (forwardingIndex < forwardedConnectionList.Size()) {
        ...
        if (debugInterface) {
            ...
            debugInterface->ShowDiagnostic(FormatStringTS(
                buff, "Got ID_ROUTER_2_REROUTE, returning ID_ROUTER_2_REROUTED, Calling RakPeer::ChangeSystemAddress at %s:%i\n", __FILE__, __LINE__));
        }
    }
}
bool Router2::OnForwardingSuccess(Packet *packet) {
    ...
    if (forwardingIndex < forwardedConnectionList.Size()) {
        ...
        if (debugInterface) {
            char buff[512];
            debugInterface->ShowDiagnostic(
                FormatStringTS(buff, "Got ID_ROUTER_2_FORWARDING_ESTABLISHED, returning ID_ROUTER_2_REROUTED, Calling RakPeer::ChangeSystemAddress at %s:%i\n",
                               _FILE_AND_LINE_));
        }
