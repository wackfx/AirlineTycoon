...
template <class MemoryBlockType> class RAK_DLL_EXPORT MemoryPool {
  public:
    struct Page;
...
template <class MemoryBlockType> MemoryPool<MemoryBlockType>::MemoryPool() {
...
template <class MemoryBlockType> void MemoryPool<MemoryBlockType>::SetPageSize(int size) { memoryPoolPageSize = size; }

template <class MemoryBlockType> MemoryBlockType *MemoryPool<MemoryBlockType>::Allocate(const char *file, unsigned int line) {
...
template <class MemoryBlockType> void MemoryPool<MemoryBlockType>::Release(MemoryBlockType *m, const char *file, unsigned int line) {
#ifdef _DISABLE_MEMORY_POOL
...
template <class MemoryBlockType> void MemoryPool<MemoryBlockType>::Clear(const char *file, unsigned int line) {
#ifdef _DISABLE_MEMORY_POOL
    return;
#else
    Page *cur, *freed;
...
template <class MemoryBlockType> int MemoryPool<MemoryBlockType>::BlocksPerPage(void) const { return memoryPoolPageSize / sizeof(MemoryWithPage); }
template <class MemoryBlockType> bool MemoryPool<MemoryBlockType>::InitPage(Page *page, Page *prev, const char *file, unsigned int line) {
    int i = 0;
