...
struct RAK_DLL_EXPORT PunchthroughConfiguration {
    ...
    /// This sometimes works because the remote router was looking for an incoming message on a higher numbered port before responding to a lower numbered port
...
class RAK_DLL_EXPORT NatPunchthroughClient : public PluginInterface2 {
  public:
    ...
    /// This function is also called automatically when you call OpenNAT - however, calling it earlier when you are connected to the facilitator will speed up
    /// the process \param[in] destination The system to punch. Must already be connected to \a facilitator
    ...
    /// You will get ID_NAT_TARGET_NOT_CONNECTED, ID_NAT_TARGET_UNRESPONSIVE, ID_NAT_CONNECTION_TO_TARGET_LOST, ID_NAT_ALREADY_IN_PROGRESS, or
    /// ID_NAT_PUNCHTHROUGH_FAILED on failures of various types However, if you lose connection to the facilitator, you may not necessarily get above
  ...
  protected:
    ...
    void UpdateGroupPunchOnNatResult(SystemAddress facilitator, RakNetGUID targetSystem, SystemAddress targetSystemAddress, int result); // 0=failed, 1=success,
    2=ignore
    */
};
