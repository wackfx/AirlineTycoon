...
// int RAK_DLL_EXPORT SystemAddressAndIndexComp( const SystemAddress &key, const RemoteSystemIndex &data ); // GCC requires RakPeer::RemoteSystemIndex or it
// won't compile
...
class RAK_DLL_EXPORT RakPeer : public RakPeerInterface, public RNS2EventHandler {
  public:
    ...
    // --------------------------------------------------------------------------------------------Major Low Level Functions - Functions needed by most
    // users--------------------------------------------------------------------------------------------
    ...
    /// \param[in] maxConnections Maximum number of connections between this instance of RakPeer and another instance of RakPeer. Required so that the network
    /// can preallocate and for thread safety. A pure client would set this to 1.  A pure server would set it to the number of allowed clients.A hybrid would
    /// set it to the sum of both types of connections. \param[in] localPort The port to listen for connections on. On linux the system may be set up so thast
    /// ports under 1024 are restricted for everything but the root user. Use a higher port for maximum compatibility. \param[in] socketDescriptors An array of
    /// SocketDescriptor structures to force RakNet to listen on a particular IP address or port (or both).  Each SocketDescriptor will represent one unique
    /// socket.  Do not pass redundant structures.  To listen on a specific port, you can pass SocketDescriptor(myPort,0); such as for a server.  For a client,
    /// it is usually OK to just pass SocketDescriptor(); However, on the XBOX be sure to use IPPROTO_VDP \param[in] socketDescriptorCount The size of the \a
    /// socketDescriptors array.  Pass 1 if you are not sure what to pass. \param[in] threadPriority Passed to the thread creation routine. Use
    /// THREAD_PRIORITY_NORMAL for Windows. For Linux based systems, you MUST pass something reasonable based on the thread priorities for your application.
    ...
    /// \param[in] bRequireClientKey: Should be set to false for most servers.  Allows the server to accept a public key from connecting clients as a proof of
    ...
    /// This is a way to set a low level password for all incoming connections.  To selectively reject connections, implement your own scheme using
    /// CloseConnection() to remove unwanted connections. \param[in] passwordData A data block that incoming connections must match.  This can be just a
    /// password, or can be a stream of data. Specify 0 for no password data \param[in] passwordDataLength The length in bytes of passwordData
    ...
    /// ID_CONNECTION_REQUEST_ACCEPTED. If the connection is not successful, such as a rejected connection or no response then neither of these things will
    /// happen. \pre Requires that you first call Startup(). \param[in] host Either a dotted IP address or a domain name. \param[in] remotePort Port to connect
    /// to on the remote machine. \param[in] passwordData A data block that must match the data block on the server passed to SetIncomingPassword().  This can
    /// be a string or can be a stream of data.  Use 0 for no password. \param[in] passwordDataLength The length in bytes of passwordData. \param[in] publicKey
    /// The public key the server is using. If 0, the server is not using security. If non-zero, the publicKeyMode member determines how to connect \param[in]
    /// connectionSocketIndex Index into the array of socket descriptors passed to socketDescriptors in RakPeer::Startup() to determine the one to send on.
    ...
    /// sendConnectionAttemptCount number of attempts, ID_CONNECTION_ATTEMPT_FAILED is returned. Under low bandwidth conditions with multiple simultaneous
    /// outgoing connections, this value should be raised to 1000 or higher, or else the MTU detection can overrun the available bandwidth. \param[in]
    /// timeoutTime Time to elapse before dropping the connection if a reliable message could not be sent. 0 to use the default value from
    /// SetTimeoutTime(UNASSIGNED_SYSTEM_ADDRESS); \return CONNECTION_ATTEMPT_STARTED on successful initiation. Otherwise, an appropriate enumeration indicating
    /// failure. \note CONNECTION_ATTEMPT_STARTED does not mean you are already connected! \note It is possible to immediately get back
    /// ID_CONNECTION_ATTEMPT_FAILED if you exceed the maxConnections parameter passed to Startup(). This could happen if you call CloseConnection() with
    /// sendDisconnectionNotificaiton true, then immediately call Connect() before the connection has closed.
    ConnectionAttemptResult Connect(const char *host, unsigned short remotePort, const char *passwordData, int passwordDataLength, PublicKey *publicKey = 0,
    ...
    /// \param[in] passwordData A data block that must match the data block on the server passed to SetIncomingPassword().  This can be a string or can be a
    /// stream of data.  Use 0 for no password. \param[in] passwordDataLength The length in bytes of passwordData. \param[in] socket A bound socket returned by
    /// another instance of RakPeerInterface. \param[in] sendConnectionAttemptCount Number of datagrams to send to the other system to try to connect.
    /// \param[in] timeBetweenSendConnectionAttemptsMS Time to elapse before a datagram is sent to the other system to try to connect. After
    /// sendConnectionAttemptCount number of attempts, ID_CONNECTION_ATTEMPT_FAILED is returned.. Under low bandwidth conditions with multiple simultaneous
    /// outgoing connections, this value should be raised to 1000 or higher, or else the MTU detection can overrun the available bandwidth. \param[in]
    /// timeoutTime Time to elapse before dropping the connection if a reliable message could not be sent. 0 to use the default from
    /// SetTimeoutTime(UNASSIGNED_SYSTEM_ADDRESS); \return CONNECTION_ATTEMPT_STARTED on successful initiation. Otherwise, an appropriate enumeration indicating
    /// failure. \note CONNECTION_ATTEMPT_STARTED does not mean you are already connected!
    virtual ConnectionAttemptResult ConnectWithSocket(const char *host, unsigned short remotePort, const char *passwordData, int passwordDataLength,
                                                      RakNetSocket2 *socket, PublicKey *publicKey = 0, unsigned sendConnectionAttemptCount = 6,
                                                      unsigned timeBetweenSendConnectionAttemptsMS = 1000, RakNet::TimeMS timeoutTime = 0);
    ...
    /// \param[in] passwordData A data block that must match the data block on the server passed to SetIncomingPassword().  This can be a string or can be a
    stream of data.  Use 0 for no password.
    ...
    /// \param[in] blockDuration Wait time(milli seconds) for all remaining messages to go out, including ID_DISCONNECTION_NOTIFICATION.  If 0, it doesn't wait
    /// at all. \param[in] orderingChannel Channel on which ID_DISCONNECTION_NOTIFICATION will be sent, if blockDuration > 0. \param[in]
    /// disconnectionNotificationPriority Priority of sending ID_DISCONNECTION_NOTIFICATION. If set to 0, the disconnection notification won't be sent.
    ...
    /// \param[out] remoteSystems An array of SystemAddress structures, to be filled with the SystemAddresss of the systems we are connected to. Pass 0 to
    /// remoteSystems to get the number of systems we are connected to. \param[in, out] numberOfSystems As input, the size of remoteSystems array.  As output,
    /// the number of elements put into the array.
    ...
    /// \param[in] orderingChannel When using ordered or sequenced messages, the channel to order these on. Messages are only ordered relative to other messages
    /// on the same stream. \param[in] systemIdentifier Who to send this packet to, or in the case of broadcasting who not to send it to. Pass either a
    /// SystemAddress structure or a RakNetGUID structure. Use UNASSIGNED_SYSTEM_ADDRESS or to specify none \param[in] broadcast True to send this packet to all
    /// connected systems. If true, then systemAddress specifies who not to send the packet to. \param[in] forceReceipt If 0, will automatically determine the
    /// receipt number to return. If non-zero, will return what you give it. \return 0 on bad input. Otherwise a number that identifies this message. If \a
    /// reliability is a type that returns a receipt, on a later call to Receive() you will get ID_SND_RECEIPT_ACKED or ID_SND_RECEIPT_LOSS with bytes 1-4
    /// inclusive containing this number
    uint32_t Send(const char *data, const int length, PacketPriority priority, PacketReliability reliability, char orderingChannel,
                  const AddressOrGUID systemIdentifier, bool broadcast, uint32_t forceReceiptNumber = 0);
    ...
    /// \param[in] priority Priority level to send on.  See PacketPriority.h
    /// \param[in] reliability How reliably to send this data.  See PacketPriority.h
    /// \param[in] orderingChannel Channel to order the messages on, when using ordered or sequenced messages. Messages are only ordered relative to other
    /// messages on the same stream. \param[in] systemIdentifier System Address or RakNetGUID to send this packet to, or in the case of broadcasting, the
    /// address not to send it to.  Use UNASSIGNED_SYSTEM_ADDRESS to specify none. \param[in] broadcast True to send this packet to all connected systems. If
    /// true, then systemAddress specifies who not to send the packet to. \param[in] forceReceipt If 0, will automatically determine the receipt number to
    /// return. If non-zero, will return what you give it. \return 0 on bad input. Otherwise a number that identifies this message. If \a reliability is a type
    /// that returns a receipt, on a later call to Receive() you will get ID_SND_RECEIPT_ACKED or ID_SND_RECEIPT_LOSS with bytes 1-4 inclusive containing this
    /// number \note COMMON MISTAKE: When writing the first byte, bitStream->Write((unsigned char) ID_MY_TYPE) be sure it is casted to a byte, and you are not
    /// writing a 4 byte enumeration.
    uint32_t Send(const RakNet::BitStream *bitStream, PacketPriority priority, PacketReliability reliability, char orderingChannel,
                  const AddressOrGUID systemIdentifier, bool broadcast, uint32_t forceReceiptNumber = 0);
    ...
    /// that returns a receipt, on a later call to Receive() you will get ID_SND_RECEIPT_ACKED or ID_SND_RECEIPT_LOSS with bytes 1-4 inclusive containing this
    /// number
    uint32_t SendList(const char **data, const int *lengths, const int numParameters, PacketPriority priority, PacketReliability reliability,
                      char orderingChannel, const AddressOrGUID systemIdentifier, bool broadcast, uint32_t forceReceiptNumber = 0);
    ...
    /// \note COMMON MISTAKE: Be sure to call this in a loop, once per game tick, until it returns 0. If you only process one packet per game tick they will
    /// buffer up. \sa RakNetTypes.h contains struct Packet.
    Packet *Receive(void);
    ...
    void CloseConnection(const AddressOrGUID target, bool sendDisconnectionNotification, unsigned char orderingChannel = 0,
    ...
    /// \param[out] addresses All system addresses. Size of the list is the number of connections. Size of the \a addresses list will match the size of the \a
    /// guids list. \param[out] guids All guids. Size of the list is the number of connections. Size of the list will match the size of the \a addresses list.
    ...
    // --------------------------------------------------------------------------------------------Pinging Functions - Functions dealing with the automatic ping
    // mechanism--------------------------------------------------------------------------------------------
    ...
    /// \details The remote system, if it is Initialized, will respond with ID_PONG followed by sizeof(RakNet::TimeMS) containing the system time the ping was
    /// sent. Default is 4 bytes - See __GET_TIME_64BIT in RakNetTypes.h System should reply with ID_PONG if it is active \param[in] host Either a dotted IP
    /// address or a domain name.  Can be 255.255.255.255 for LAN broadcast. \param[in] remotePort Which port to connect to on the remote machine. \param[in]
    /// onlyReplyOnAcceptingConnections Only request a reply if the remote system is accepting connections \param[in] connectionSocketIndex Index into the array
    /// of socket descriptors passed to socketDescriptors in RakPeer::Startup() to send on. \return true on success, false on failure (unknown hostname)
    ...
    // --------------------------------------------------------------------------------------------Static Data Functions - Functions dealing with API defined
    // synchronized memory--------------------------------------------------------------------------------------------
    ...
    /// \param[in] systemAddress Use UNASSIGNED_SYSTEM_ADDRESS to get your behind-LAN address. Use a connected system to get their behind-LAN address. This does
    /// not return the port. \param[in] index When you have multiple internal IDs, which index to return? Currently limited to MAXIMUM_NUMBER_OF_INTERNAL_IDS
    /// (so the maximum value of this variable is MAXIMUM_NUMBER_OF_INTERNAL_IDS-1) \return Identifier of your system internally, which may not be how other
    /// systems see if you if you are behind a NAT or proxy.
    ...
    /// \param[in] client_public_key The connected client's public key is copied to this address.  Buffer must be cat::EasyHandshake::PUBLIC_KEY_BYTES bytes in
    /// length.
    ...
    // -------------------------------------------------------------------------------------------- Plugin
    // Functions--------------------------------------------------------------------------------------------
    /// \brief Attaches a Plugin interface to an instance of the base class (RakPeer or PacketizedTCP) to run code automatically on message receipt in the
    /// Receive call. If the plugin returns false from PluginInterface::UsesReliabilityLayer(), which is the case for all plugins except PacketLogger, you can
    /// call AttachPlugin() and DetachPlugin() for this plugin while RakPeer is active. \param[in] messageHandler Pointer to the plugin to attach.
    ...
    /// If the plugin returns false from PluginInterface::UsesReliabilityLayer(), which is the case for all plugins except PacketLogger, you can call
    /// AttachPlugin() and DetachPlugin() for this plugin while RakPeer is active. \param[in] messageHandler Pointer to a plugin to detach.
    ...
    /// \param[in] pushAtHead True to push the packet at the start of the queue so that the next receive call returns it.  False to push it at the end of the
    ...
    /// The smart pointer reference counts the RakNetSocket object, so the socket will remain active as long as the smart pointer does, even if RakNet were to
    /// shutdown or close the connection. \note This sends a query to the thread and blocks on the return value for up to one second. In practice it should only
    /// take a millisecond or so. \param[in] target Which system. \return A smart pointer object containing the socket information about the target. Be sure to
    /// check IsNull() which is returned if the update thread is unresponsive, shutting down, or if this system is not connected.
    ...
    /// To simulate bi-directional ping and packet loss, you should call this on both the sender and the recipient, with half the total ping and packetloss
    /// value on each. You can exclude network simulator code with the _RELEASE #define to decrease code size \deprecated Use
    /// http://www.jenkinssoftware.com/forum/index.php?topic=1671.0 instead. \note Doesn't work past version 3.6201 \param[in] packetloss Chance to lose a
    /// packet. Ranges from 0 to 1. \param[in] minExtraPing The minimum time to delay sends. \param[in] extraPingVariance The additional random time to delay
    /// sends.
    ...
    /// \param[in] maxBitsPerSecond Maximum bits per second to send.  Use 0 for unlimited (default). Once set, it takes effect immedately and persists until
    /// called again.
    ...
    /// \param[in] rns If you supply this structure,the network statistics will be written to it. Otherwise the method uses a static struct to write the data,
    /// which is not threadsafe. \return 0 if the specified system can't be found. Otherwise a pointer to the struct containing the specified system's network
    /// statistics. \sa RakNetStatistics.h
    ...
    virtual void GetStatisticsList(DataStructures::List<SystemAddress> &addresses, DataStructures::List<RakNetGUID> &guids,
    ...
    // --------------------------------------------------------------------------------------------EVERYTHING AFTER THIS COMMENT IS FOR INTERNAL USE
    // ONLY--------------------------------------------------------------------------------------------
    ...
    struct RemoteSystemStruct {
        ...
        RakNet::Time lastReliableSend; /// When did the last reliable send occur.  Reliable sends must occur at least once every timeoutTime/2 units to notice
                                       /// disconnects
  ...
  protected:
    ...
    friend bool ProcessOfflineNetworkPacket(SystemAddress systemAddress, const char *data, const int length, RakPeer *rakPeer, RakNetSocket2 *rakNetSocket,
                                            bool *isOfflineMessage, RakNet::TimeUS timeRead);
    ...
    friend void ProcessNetworkPacket(const SystemAddress systemAddress, const char *data, const int length, RakPeer *rakPeer, RakNetSocket2 *rakNetSocket,
    ...
    ConnectionAttemptResult SendConnectionRequest(const char *host, unsigned short remotePort, const char *passwordData, int passwordDataLength,
                                                  PublicKey *publicKey, unsigned connectionSocketIndex, unsigned int extraData,
                                                  unsigned sendConnectionAttemptCount, unsigned timeBetweenSendConnectionAttemptsMS, RakNet::TimeMS timeoutTime,
    ...
    RemoteSystemStruct *AssignSystemAddressToRemoteSystemList(const SystemAddress systemAddress, RemoteSystemStruct::ConnectMode connectionMode,
    ...
    /// activeSystemList holds a list of pointers and is preallocated to be the same size as remoteSystemList. It is updated only by the network thread, but
    /// read by both threads When the isActive member of RemoteSystemStruct is set to true or false, that system is added to this list of pointers Threadsafe
    /// because RemoteSystemStruct is preallocated, and the list is only added to, not removed from
...
//	char random[16];
    ...
    void CloseConnectionInternal(const AddressOrGUID &systemIdentifier, bool sendDisconnectionNotification, bool performImmediate,
                                 unsigned char orderingChannel, PacketPriority disconnectionNotificationPriority);
    void SendBuffered(const char *data, BitSize_t numberOfBitsToSend, PacketPriority priority, PacketReliability reliability, char orderingChannel,
                      const AddressOrGUID systemIdentifier, bool broadcast, RemoteSystemStruct::ConnectMode connectionMode, uint32_t receipt);
    void SendBufferedList(const char **data, const int *lengths, const int numParameters, PacketPriority priority, PacketReliability reliability,
                          char orderingChannel, const AddressOrGUID systemIdentifier, bool broadcast, RemoteSystemStruct::ConnectMode connectionMode,
                          uint32_t receipt);
    bool SendImmediate(char *data, BitSize_t numberOfBitsToSend, PacketPriority priority, PacketReliability reliability, char orderingChannel,
