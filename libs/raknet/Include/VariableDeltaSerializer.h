...
/// Usage:<BR>
///< BR>
/// 1. Call BeginUnreliableAckedSerialize(), BeginUniqueSerialize(), or BeginIdenticalSerialize(). In the case of Replica3, this would be in the Serialize()
/// call<BR>
...
class RAK_DLL_EXPORT VariableDeltaSerializer {
  ...
  public:
    ...
    /// The last value of each variable will be saved per remote system. Additionally, a history of \a _sendReceipts is stored to determine what to resend on
    /// packetloss. When variables are lost, they will be flagged dirty and always resent to the system that lost it Disadvantages: Every variable for every
    /// remote system is copied internally, in addition to a history list of what variables changed for which \a _sendReceipt. Very memory and CPU intensive for
    /// multiple connections. Advantages: When data needs to be resent by RakNet, RakNet can only resend the value it currently has. This allows the application
    /// to control the resend, sending the most recent value of the variable. The end result is that bandwidth is used more efficiently because old data is
    /// never sent. \pre Upon getting ID_SND_RECEIPT_LOSS or ID_SND_RECEIPT_ACKED call OnMessageReceipt() \pre AddRemoteSystemVariableHistory() and
    /// RemoveRemoteSystemVariableHistory() must be called for new and lost connections \param[in] context Holds the context of this group of serialize calls.
    /// This can be a stack object just passed to the function. \param[in] _guid Which system we are sending to \param[in] _bitSteam Which bitStream to write to
    /// \param[in] _sendReceipt Returned from RakPeer::IncrementNextSendReceipt() and passed to the Send() or SendLists() function. Identifies this update for
    /// ID_SND_RECEIPT_LOSS and ID_SND_RECEIPT_ACKED
    ...
    /// \brief Call before doing one or more SerializeVariable calls for data that may be sent differently to every remote system (such as an invisibility flag
    /// that only teammates can see) The last value of each variable will be saved per remote system. Unlike BeginUnreliableAckedSerialize(), send receipts are
    /// not necessary Disadvantages: Every variable for every remote system is copied internally. Very memory and CPU intensive for multiple connections.
    ...
    /// \brief Call before doing one or more SerializeVariable calls for data that is sent with the same value to every remote system (such as health, position,
    /// etc.) This is the most common type of serialization, and also the most efficient Disadvantages: A copy of every variable still needs to be held,
    /// although only once Advantages: After the first serialization, the last serialized bitStream will be used for subsequent sends \pre Call
    /// OnPreSerializeTick() before doing any calls to BeginIdenticalSerialize() for each of your objects, once per game tick \param[in] context Holds the
    /// context of this group of serialize calls. This can be a stack object just passed to the function. \param[in] _isFirstSerializeToThisSystem Pass true if
    /// this is the first time ever serializing to this system (the initial download). This way all variables will be written, rather than checking against
    /// prior sent values. \param[in] _bitSteam Which bitStream to write to
    void BeginIdenticalSerialize(SerializationContext *context, bool _isFirstSerializeToThisSystem, BitStream *_bitStream);

    /// \brief Call after BeginUnreliableAckedSerialize(), BeginUniqueSerialize(), or BeginIdenticalSerialize(), then after calling SerializeVariable() one or
    /// more times \param[in] context Same context pointer passed to BeginUnreliableAckedSerialize(), BeginUniqueSerialize(), or BeginIdenticalSerialize()
    ...
    template <class VarType> void SerializeVariable(SerializationContext *context, const VarType &variable) {
        if (context->newSystemSend) {
            ...
            context->anyVariablesWritten |= context->variableHistory->variableListDeltaTracker.WriteVarToBitstream(
                variable, context->bitStream, context->changedVariables->bitField, context->changedVariables->bitWriteIndex++);
        } else {
            if (context->variableHistoryIdentical) {
                // Identical serialization to a number of systems
                if (didComparisonThisTick == false)
                    context->anyVariablesWritten |= context->variableHistory->variableListDeltaTracker.WriteVarToBitstream(variable, context->bitStream);
    ...
    /// \param[in] context Same context pointer passed to BeginDeserialize()
    /// \param[in] variable A variable to write to the bitStream passed to \a context
    template <class VarType> bool DeserializeVariable(DeserializationContext *context, VarType &variable) {
  ...
  protected:
    ...
    // Also if a send does not arrive (ID_SND_RECEIPT_LOSS) we use updatedVariablesHistory to mark those variables as dirty, to resend them unreliably with the
    // current values
    struct RemoteSystemVariableHistory {
        RakNetGUID guid;
