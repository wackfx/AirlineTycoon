...
class ReliabilityLayer //<ReliabilityLayer>
{
  public:
    ...
    /// \param[in] receipt This number will be returned back with ID_SND_RECEIPT_ACKED or ID_SND_RECEIPT_LOSS and is only returned with the reliability types
    /// that contain RECEIPT in the name \return True or false for success or failure.
    bool Send(char *data, BitSize_t numberOfBitsToSend, PacketPriority priority, PacketReliability reliability, unsigned char orderingChannel,
    ...
    /// \param[in] messageHandlerList A list of registered plugins
    void Update(RakNetSocket2 *s, SystemAddress &systemAddress, int MTUSize, CCTimeType time, unsigned bitsPerSecondLimit,
  ...
  private:
    ...
    unsigned RemovePacketFromResendListAndDeleteOlderReliableSequenced(const MessageNumberType messageNumber, CCTimeType time,
    ...
    InternalPacket *BuildPacketFromSplitPacketList(SplitPacketIdType splitPacketId, CCTimeType time, RakNetSocket2 *s, SystemAddress &systemAddress,
    ...
    void AllocInternalPacketData(InternalPacket *internalPacket, InternalPacketRefCountedData **refCounter, unsigned char *externallyAllocatedPtr,
