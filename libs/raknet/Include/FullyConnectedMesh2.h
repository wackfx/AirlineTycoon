...
class RAK_DLL_EXPORT FullyConnectedMesh2 : public PluginInterface2 {
  public:
    ...
    /// If \a attemptConnection is false, you can manually connect to all systems listed in ID_REMOTE_NEW_INCOMING_CONNECTION with
    /// ConnectToRemoteNewIncomingConnections() \note This will not work on any console. It will also not work if NAT punchthrough is needed. Generally, this
    /// should be false and you should connect manually. It is here for legacy reasons. \param[in] attemptConnection If true, we try to connect to any systems
    /// we are notified about with ID_REMOTE_NEW_INCOMING_CONNECTION, which comes from the ConnectionGraph2 plugin. Defaults to true. \param[in] pw The password
    /// to use to connect with. Only used if \a attemptConnection is true
    ...
    /// includeCalculating should be true if you are taking action based on another system becoming host, because not all host calculations may complete at the
    /// exact same time \sa ConnectionGraph2::GetLowestAveragePingSystem() . If you need one system in the peer to peer group to relay data, have the host call
    /// this function after host migration, and use that system \return System address of whichever system is host.
    ...
    /// When there is a risk of failure, such as if relying on NATPunchthroughClient, you may not want to call AddParticipant() until are connections have
    /// completed to all other particpants StartVerifiedJoin() can manage the overhead of the negotiation involved so the programmer only has to deal with
    /// overall success or failure
    ///
    /// Processing:
    /// 1. Send the RakNetGUID and SystemAddress values of GetParticipantList() to the client with ID_FCM2_VERIFIED_JOIN_START
    /// 2. The client, on ID_FCM2_VERIFIED_JOIN_START, can execute NatPunchthroughClient::OpenNAT() (optional), followed by RakPeerInterface::Connect() if
    /// punchthrough success, for each system returned from GetVerifiedJoinRequiredProcessingList()
    /// 3. After all participants in step 2 have connected, failed to connect, or failed NatPunchthrough, the client automatically sends the results to the
    /// server.
    /// 4. The server compares the results of the operations in step 2 with the values from GetParticpantList().
    /// 4A. If the client failed to connect to a current participant, return ID_FCM2_VERIFIED_JOIN_FAILED to the client. CloseConnection() is automatically
    /// called on the client for the failed participants. 4B. If AddParticipant() was called between steps 1 and 4, go back to step 1, transmitting new
    /// participants. 4C. If the client successfully connected to all participants, the server gets ID_FCM2_VERIFIED_JOIN_CAPABLE. The server programmer, on the
    /// same frame, should execute RespondOnVerifiedJoinCapable() to either accept or reject the client.
    /// 5. If the client got ID_FCM2_VERIFIED_JOIN_ACCEPTED, AddParticipant() is automatically called for each system in the mesh.
    /// 6. If the client got ID_FCM2_VERIFIED_JOIN_REJECTED, CloseConnection() is automatically called for each system in the mesh. The connection is NOT
    /// automatically closed to the original host that sent StartVerifiedJoin().
    /// 7. If the client's connection to the server was lost before getting ID_FCM2_VERIFIED_JOIN_ACCEPTED or ID_FCM2_VERIFIED_JOIN_REJECTED, return to the
    /// programmer ID_FCM2_VERIFIED_JOIN_FAILED and call RakPeerInterface::CloseConnection()
    ///
    ...
    /// \endcode
    /// \param[in] packet The system that sent ID_FCM2_VERIFIED_JOIN_CAPABLE. Based on \accept, ID_FCM2_VERIFIED_JOIN_ACCEPTED or ID_FCM2_VERIFIED_JOIN_REJECTED
    /// will be sent in reply \param[in] accept True to accept, and thereby automatically call AddParticipant() on all systems on the mesh. False to reject, and
    /// call CloseConnection() to all mesh systems on the target \param[in] additionalData Any additional data you want to add to the
    /// ID_FCM2_VERIFIED_JOIN_ACCEPTED or ID_FCM2_VERIFIED_JOIN_REJECTED messages \sa WriteVJCUserData()
    ...
    virtual void GetVerifiedJoinAcceptedAdditionalData(Packet *packet, bool *thisSystemAccepted, DataStructures::List<RakNetGUID> &systemsAccepted,
    ...
    /// \param[in] userContext The data set with SetMyContext() for that system. May be empty. To properly write userContext, you will need to first write
    /// userContext->GetNumberOfBitsUsed(), followed by bsOut->Write(userContext);
  ...
  protected:
    ...
    void CategorizeVJIP(VerifiedJoinInProgress *vjip, DataStructures::List<RakNetGUID> &participatingMembersOnClientSucceeded,
                        DataStructures::List<RakNetGUID> &participatingMembersOnClientFailed, DataStructures::List<RakNetGUID> &participatingMembersNotOnClient,
