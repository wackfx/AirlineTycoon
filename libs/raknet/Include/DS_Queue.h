...
template <class queue_type> class RAK_DLL_EXPORT Queue {
  public:
    Queue();
    ~Queue();
...
template <class queue_type> inline unsigned int Queue<queue_type>::Size(void) const {
    if (head <= tail)
        return tail - head;
    else
...
template <class queue_type> inline bool Queue<queue_type>::IsEmpty(void) const { return head == tail; }

template <class queue_type> inline unsigned int Queue<queue_type>::AllocationSize(void) const { return allocation_size; }

template <class queue_type> Queue<queue_type>::Queue() {
    // allocation_size = 16;
...
template <class queue_type> inline queue_type Queue<queue_type>::Pop(void) {
#ifdef _DEBUG
    RakAssert(head != tail);
#endif
...
template <class queue_type> inline queue_type Queue<queue_type>::PopTail(void) {
#ifdef _DEBUG
    RakAssert(head != tail);
#endif
    if (tail != 0) {
...
template <class queue_type> inline queue_type Queue<queue_type>::PopDeref(void) {
    if (++head == allocation_size)
        head = 0;
...
template <class queue_type> void Queue<queue_type>::PushAtHead(const queue_type &input, unsigned index, const char *file, unsigned int line) {
...
template <class queue_type> inline queue_type Queue<queue_type>::Peek(void) const {
#ifdef _DEBUG
    RakAssert(head != tail);
#endif
...
template <class queue_type> inline queue_type Queue<queue_type>::PeekTail(void) const {
#ifdef _DEBUG
    RakAssert(head != tail);
#endif
    if (tail != 0)
...
template <class queue_type> void Queue<queue_type>::Push(const queue_type &input, const char *file, unsigned int line) {
    if (allocation_size == 0) {
...
template <class queue_type> Queue<queue_type>::Queue(Queue &original_copy) {
...
template <class queue_type> bool Queue<queue_type>::operator=(const Queue &original_copy) {
...
template <class queue_type> inline void Queue<queue_type>::Clear(const char *file, unsigned int line) {
    if (allocation_size == 0)
        return;
...
template <class queue_type> void Queue<queue_type>::Compress(const char *file, unsigned int line) {
    queue_type *new_array;
...
template <class queue_type> bool Queue<queue_type>::Find(const queue_type &q) {
    if (allocation_size == 0)
        return false;
...
template <class queue_type> void Queue<queue_type>::ClearAndForceAllocation(int size, const char *file, unsigned int line) {
    RakNet::OP_DELETE_ARRAY(array, file, line);
    if (size > 0)
...
template <class queue_type> inline queue_type &Queue<queue_type>::operator[](unsigned int position) const {
#ifdef _DEBUG
...
template <class queue_type> void Queue<queue_type>::RemoveAtIndex(unsigned int position) {
#ifdef _DEBUG
    RakAssert(position < Size());
    RakAssert(head != tail);
#endif
