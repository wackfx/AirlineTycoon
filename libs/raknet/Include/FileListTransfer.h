...
class RAK_DLL_EXPORT FileListTransfer : public PluginInterface2 {
  public:
    ...
    /// \param[in] threadPriority Passed to the thread creation routine. Use THREAD_PRIORITY_NORMAL for Windows. For Linux based systems, you MUST pass
    ...
    /// \return A set ID value, which should be passed as the \a setID value to the Send() call on the other system.  This value will be returned in the
    /// callback and is unique per file set.  Returns 65535 on failure (not connected to sender)
    ...
    /// \param[in] _incrementalReadInterface If a file in \a fileList has no data, _incrementalReadInterface will be used to read the file in chunks of size \a
    /// chunkSize \param[in] _chunkSize How large of a block of a file to read/send at once. Large values use more memory but transfer slightly faster.
    void Send(FileList *fileList, RakNet::RakPeerInterface *rakPeer, SystemAddress recipient, unsigned short setID, PacketPriority priority,
              char orderingChannel, IncrementalReadInterface *_incrementalReadInterface = 0, unsigned int _chunkSize = 262144 * 4 * 16);
