...
class RAK_DLL_EXPORT FileListProgress {
  public:
    ...
    virtual void OnFilePush(const char *fileName, unsigned int fileLengthBytes, unsigned int offset, unsigned int bytesBeingSent, bool done,
...
class RAK_DLL_EXPORT FileList {
  public:
    ...
    void AddFilesFromDirectory(const char *applicationDirectory, const char *subDirectory, bool writeHash, bool writeData, bool recursive,
                               FileListNodeContext context);
    ...
    /// \details For each file that is missing or different, add that file to \a missingOrChangedFiles. Note: the file contents are not written, and only the
    /// hash if written if \a alwaysWriteHash is true alwaysWriteHash and neverWriteHash are optimizations to avoid reading the file contents to generate the
    /// hash if not necessary because the file is missing or has different lengths anyway. \param[in] applicationDirectory The first part of the path. This is
    /// not stored as part of the filename. Use \ as the path delineator. \param[out] missingOrChangedFiles Output list written to \param[in] alwaysWriteHash If
    /// true, and neverWriteHash is false, will hash the file content of the file on disk, and write that as the file data with a length of SHA1_LENGTH bytes.
    /// If false, if the file length is different, will only write the filename. \param[in] neverWriteHash If true, will never write the hash, even if
    /// available. If false, will write the hash if the file lengths are the same and it was forced to do a comparison.
    ...
    void AddFile(const char *filename, const char *fullPathToFile, const char *data, const unsigned dataLength, const unsigned fileLength,
                 FileListNodeContext context, bool isAReference = false, bool takeDataPointer = false);
    ...
    /// \param[in] cb A pointer to an externally defined instance of FileListProgress. This pointer is held internally, so should remain valid as long as this
    /// class is valid.
