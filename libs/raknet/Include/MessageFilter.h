...
class RAK_DLL_EXPORT MessageFilter : public PluginInterface2 {
  public:
    ...
    /// \param[in] allow True to allow this message ID, false to disallow. By default, all messageIDs except the noted types are disallowed.  This includes
    /// messages from other plugins! \param[in] messageIDStart The first ID_* message to allow in the range.  Inclusive. \param[in] messageIDEnd The last ID_*
    /// message to allow in the range.  Inclusive. \param[in] filterSetID A user defined ID to represent a filter set.  If no filter with this ID exists, one
    ...
    void SetDisallowedMessageCallback(int filterSetID, void *userData,
                                      void (*invalidMessageCallback)(RakPeerInterface *peer, AddressOrGUID addressOrGUID, int filterSetID, void *userData,
    ...
    /// \param[in] addressOrGUID The system we are referring to
    /// \param[in] filterSetID A user defined ID to represent a filter set.  If no filter with this ID exists, one will be created with default settings.  If
    /// -1, the system will be removed from all filter sets.
