...
class RAK_DLL_EXPORT UDPForwarder {
  public:
    ...
    /// \pre Call Startup()
    /// \note RakNet's protocol will ensure a message is sent at least every 15 seconds, so if routing RakNet messages, it is a reasonable value for
    /// timeoutOnNoDataMS, plus an some extra seconds for latency \param[in] source The source IP and port \param[in] destination Where to forward to (and
    /// vice-versa) \param[in] timeoutOnNoDataMS If no messages are forwarded for this many MS, then automatically remove this entry. \param[in]
    /// forceHostAddress Force binding on a particular address. 0 to use any. \param[in] socketFamily IP version: For IPV4, use AF_INET (default). For IPV6, use
    /// AF_INET6. To autoselect, use AF_UNSPEC. \param[out] forwardingPort New opened port for forwarding \param[out] forwardingSocket New opened socket for
    /// forwarding \return UDPForwarderResult
    UDPForwarderResult StartForwarding(SystemAddress source, SystemAddress destination, RakNet::TimeMS timeoutOnNoDataMS, const char *forceHostAddress,
                                       unsigned short socketFamily, unsigned short *forwardingPort, __UDPSOCKET__ *forwardingSocket);
