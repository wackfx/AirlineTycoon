...
/// For each message that arrives on an instance of RakPeer, the plugins get an opportunity to process them first. This enumeration represents what to do with
/// the message \ingroup PLUGIN_INTERFACE_GROUP
...
class RAK_DLL_EXPORT PluginInterface2 {
  ...
  protected:
    ...
    void SendUnified(const RakNet::BitStream *bitStream, PacketPriority priority, PacketReliability reliability, char orderingChannel,
                     const AddressOrGUID systemIdentifier, bool broadcast);
    void SendUnified(const char *data, const int length, PacketPriority priority, PacketReliability reliability, char orderingChannel,
                     const AddressOrGUID systemIdentifier, bool broadcast);
    bool SendListUnified(const char **data, const int *lengths, const int numParameters, PacketPriority priority, PacketReliability reliability,
