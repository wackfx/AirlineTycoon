...
enum DefaultMessageIDTypes {
    ...
    /// Informs user of a new host GUID. Packet::Guid contains this new host RakNetGuid. The old host can be read out using BitStream->Read(RakNetGuid) starting
    /// on byte 1 This is not returned until connected to a remote system If the oldHost is UNASSIGNED_RAKNET_GUID, then this is the first time the host has
    /// been determined
    ID_FCM2_NEW_HOST,
    ...
    /// \note If you fail to connect to a system, this does not automatically mean you will get ID_FCM2_VERIFIED_JOIN_FAILED as that system may have been
    /// shutting down from the host too \sa FullyConnectedMesh2::StartVerifiedJoin()
    ...
    /// The password we used to challenge the other system passed, meaning the other system has called TwoWayAuthentication::AddPassword() with the same
    /// password we passed to TwoWayAuthentication::Challenge() You can read the identifier used to challenge as follows: RakNet::BitStream bs(packet->data,
    /// packet->length, false); bs.IgnoreBytes(sizeof(RakNet::MessageID)); RakNet::RakString password; bs.Read(password);
    ...
    /// If the other system must pass the challenge to stay connected, you should call RakPeer::CloseConnection() to terminate the connection to the other
    /// system.
    ...
    /// The other system did not respond within a timeout threshhold. Either the other system is not running the plugin or the other system was blocking on some
    /// operation for a long time. You can read the identifier used to challenge as follows: RakNet::BitStream bs(packet->data, packet->length, false);
    /// bs.IgnoreBytes(sizeof(MessageID)); RakNet::RakString password; bs.Read(password);
