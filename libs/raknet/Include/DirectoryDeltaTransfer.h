...
class RAK_DLL_EXPORT DirectoryDeltaTransfer : public PluginInterface2 {
  public:
    ...
    /// \param[in] prependAppDirToOutputSubdir True to prepend outputSubdir with pathToApplication when determining the final output path.  Usually you want
    /// this to be true. \param[in] host The address of the remote system to send the message to. \param[in] onFileCallback Callback to call per-file
    /// (optional).  When fileIndex+1==setCount in the callback then the download is done \param[in] _priority See RakPeerInterface::Send() \param[in]
    /// _orderingChannel See RakPeerInterface::Send() \param[in] cb Callback to get progress updates. Pass 0 to not use. \return A set ID, identifying this
    /// download set.  Returns 65535 on host unreachable.
    unsigned short DownloadFromSubdirectory(const char *subdir, const char *outputSubdir, bool prependAppDirToOutputSubdir, SystemAddress host,
                                            FileListTransferCBInterface *onFileCallback, PacketPriority _priority, char _orderingChannel, FileListProgress *cb);
    ...
    /// _orderingChannel See RakPeerInterface::Send() \param[in] cb Callback to get progress updates. Pass 0 to not use. \return A set ID, identifying this
    /// download set.  Returns 65535 on host unreachable.
    unsigned short DownloadFromSubdirectory(FileList &localFiles, const char *subdir, const char *outputSubdir, bool prependAppDirToOutputSubdir,
                                            SystemAddress host, FileListTransferCBInterface *onFileCallback, PacketPriority _priority, char _orderingChannel,
                                            FileListProgress *cb);
    ...
    /// \param[in] _incrementalReadInterface If a file in \a fileList has no data, filePullInterface will be used to read the file in chunks of size \a
    /// chunkSize \param[in] _chunkSize How large of a block of a file to send at once
