...
/// \details This should not be used at the same time as RPC3. This is a less functional version of RPC3, and is here for users that do not want the Boost
/// dependency of RPC3. \ingroup PLUGINS_GROUP
...
class RAK_DLL_EXPORT RPC4 : public PluginInterface2 {
  public:
    ...
    /// When a call is made to call this function from the \a Call() or CallLoopback() function, the function pointer will be invoked with the passed bitStream
    /// to Call() and the actual Packet that RakNet got. \sa RegisterPacketCallback() \param[in] uniqueID Identifier to be associated with \a functionPointer.
    /// If this identifier is already in use, the call will return false. \param[in] functionPointer C function pointer to be called \return True if the hash of
    /// uniqueID is not in use, false otherwise.
    ...
    /// \param[in] callPriority Slots are called by order of the highest callPriority first. For slots with the same priority, they are called in the order they
    /// are registered
    ...
    void Call(const char *uniqueID, RakNet::BitStream *bitStream, PacketPriority priority, PacketReliability reliability, char orderingChannel,
    ...
    bool CallBlocking(const char *uniqueID, RakNet::BitStream *bitStream, PacketPriority priority, PacketReliability reliability, char orderingChannel,
                      const AddressOrGUID systemIdentifier, RakNet::BitStream *returnData);
    ...
    void Signal(const char *sharedIdentifier, RakNet::BitStream *bitStream, PacketPriority priority, PacketReliability reliability, char orderingChannel,
                const AddressOrGUID systemIdentifier, bool broadcast, bool invokeLocal);
  ...
  protected:
    ...
    DataStructures::Hash<RakNet::RakString, void (*)(RakNet::BitStream *, RakNet::BitStream *, Packet *), 64, RakNet::RakString::ToInteger>
