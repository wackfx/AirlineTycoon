...
template <class range_type> struct RangeNode {
    RangeNode() {}
    ~RangeNode() {}
...
template <class range_type> int RangeNodeComp(const range_type &a, const RangeNode<range_type> &b) {
    if (a < b.minIndex)
        return -1;
    if (a == b.minIndex)
        return 0;
...
template <class range_type> class RAK_DLL_EXPORT RangeList {
  public:
    RangeList();
    ~RangeList();
...
template <class range_type> RakNet::BitSize_t RangeList<range_type>::Serialize(RakNet::BitStream *in, RakNet::BitSize_t maxBits, bool clearSerialized) {
...
template <class range_type> bool RangeList<range_type>::Deserialize(RakNet::BitStream *out) {
...
template <class range_type> RangeList<range_type>::RangeList() { RangeNodeComp<range_type>(0, RangeNode<range_type>()); }

template <class range_type> RangeList<range_type>::~RangeList() { Clear(); }

template <class range_type> void RangeList<range_type>::Insert(range_type index) {
    if (ranges.Size() == 0) {
...
template <class range_type> void RangeList<range_type>::Clear(void) { ranges.Clear(true, _FILE_AND_LINE_); }

template <class range_type> unsigned RangeList<range_type>::Size(void) const { return ranges.Size(); }

template <class range_type> unsigned RangeList<range_type>::RangeSum(void) const {
    unsigned sum = 0, i;
    for (i = 0; i < ranges.Size(); i++)
