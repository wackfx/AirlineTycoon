...
namespace RakNet {
/// This class allows you to write and read native types as a string of bits.  BitStream is used extensively throughout RakNet and is designed to be used by
/// users as well. \sa BitStreamSample.txt
class RAK_DLL_EXPORT BitStream {
  ...
  public:
    ...
    /// \return true if \a writeToBitstream is true.  true if \a writeToBitstream is false and the read was successful.  false if \a writeToBitstream is false
    ...
    /// \note A static is used so that the required number of bits for (maximum-minimum) is only calculated once. This does require that \a minimum and \maximum
    /// are fixed values for a given line of code for the life of the program \param[in] writeToBitstream true to write from your data to this bitstream.  False
    /// to read from this bitstream and write to your data \param[in] value Integer value to write, which should be between \a minimum and \a maximum \param[in]
    /// minimum Minimum value of \a value \param[in] maximum Maximum value of \a value \param[in] allowOutsideRange If true, all sends will take an extra bit,
    /// however value can deviate from outside \a minimum and \a maximum. If false, will assert if the value deviates
    template <class templateType>
    bool SerializeBitsFromIntegerRange(bool writeToBitstream, templateType &value, const templateType minimum, const templateType maximum,
    ...
    template <class templateType> // templateType for this function must be a float or double
    bool SerializeOrthMatrix(bool writeToBitstream, templateType &m00, templateType &m01, templateType &m02, templateType &m10, templateType &m11,
    ...
    /// \note A static is used so that the required number of bits for (maximum-minimum) is only calculated once. This does require that \a minimum and \maximum
    /// are fixed values for a given line of code for the life of the program \param[in] value Integer value to write, which should be between \a minimum and \a
    /// maximum \param[in] minimum Minimum value of \a value \param[in] maximum Maximum value of \a value \param[in] allowOutsideRange If true, all sends will
    /// take an extra bit, however value can deviate from outside \a minimum and \a maximum. If false, will assert if the value deviates. This should match the
    ...
    /// \param[in] requiredBits Primarily for internal use, called from above function() after calculating number of bits needed to represent maximum-minimum
    template <class templateType>
    void WriteBitsFromIntegerRange(const templateType value, const templateType minimum, const templateType maximum, const int requiredBits,
                                   bool allowOutsideRange = false);
    ...
    /// Lossy, although the result is renormalized
    template <class templateType> // templateType for this function must be a float or double
    void WriteOrthMatrix(templateType m00, templateType m01, templateType m02, templateType m10, templateType m11, templateType m12, templateType m20,
    ...
    /// \note A static is used so that the required number of bits for (maximum-minimum) is only calculated once. This does require that \a minimum and \maximum
    /// are fixed values for a given line of code for the life of the program \param[in] value Integer value to read, which should be between \a minimum and \a
    /// maximum \param[in] minimum Minimum value of \a value \param[in] maximum Maximum value of \a value \param[in] allowOutsideRange If true, all sends will
    /// take an extra bit, however value can deviate from outside \a minimum and \a maximum. If false, will assert if the value deviates. This should match the
    ...
    /// \param[in] requiredBits Primarily for internal use, called from above function() after calculating number of bits needed to represent maximum-minimum
    template <class templateType>
    bool ReadBitsFromIntegerRange(templateType &value, const templateType minimum, const templateType maximum, const int requiredBits,
                                  bool allowOutsideRange = false);
    ...
    /// \return true on success, false on failure.
    template <class templateType> // templateType for this function must be a float or double
    bool ReadOrthMatrix(templateType &m00, templateType &m01, templateType &m02, templateType &m10, templateType &m11, templateType &m12, templateType &m20,
                        templateType &m21, templateType &m22);
    ...
    template <> void Write(const bool &var);

    /// Write a systemAddress to a bitstream
    /// \param[in] var The value to write
    template <> void Write(const SystemAddress &var);

    /// Write a uint24_t to a bitstream
    /// \param[in] var The value to write
    template <> void Write(const uint24_t &var);

    /// Write a RakNetGUID to a bitsteam
    /// \param[in] var The value to write
    template <> void Write(const RakNetGuid &var);

    /// Write a string to a bitstream
    /// \param[in] var The value to write
    template <> void Write(const char *const &var);
    template <> void Write(const unsigned char *const &var);
    template <> void Write(char *const &var);
    template <> void Write(unsigned char *const &var);
    template <> void Write(const RakString &var);
    template <> void Write(const RakWString &var);
    ...
    /// \param[in] currentValue The current value to write
    /// \param[in] lastValue The last value to compare against
    template <> void WriteDelta(const SystemAddress &currentValue, const SystemAddress &lastValue);

    template <> void WriteDelta(const uint24_t &currentValue, const uint24_t &lastValue);

    template <> void WriteDelta(const RakNetGUID &currentValue, const RakNetGUID &lastValue);

    /// \brief Write a bool delta.
    /// \details Same thing as just calling Write
    /// \param[in] currentValue The current value to write
    /// \param[in] lastValue The last value to compare against
    template <> void WriteDelta(const bool &currentValue, const bool &lastValue);

    template <> void WriteCompressed(const SystemAddress &var);

    template <> void WriteCompressed(const uint24_t &var);

    template <> void WriteCompressed(const RakNetGUID &var);

    template <> void WriteCompressed(const bool &var);

    /// For values between -1 and 1
    template <> void WriteCompressed(const float &var);

    /// For values between -1 and 1
    template <> void WriteCompressed(const double &var);

    /// Compressed string
    template <> void WriteCompressed(const char *var);
    template <> void WriteCompressed(const unsigned char *var);
    template <> void WriteCompressed(char *var);
    template <> void WriteCompressed(unsigned char *var);
    template <> void WriteCompressed(const RakString &var);
    template <> void WriteCompressed(const RakWString &var);

    /// \brief Write a bool delta.
    /// \details Same thing as just calling Write
    /// \param[in] currentValue The current value to write
    /// \param[in] lastValue The last value to compare against
    template <> void WriteCompressedDelta(const bool &currentValue, const bool &lastValue);

    /// \brief Save as WriteCompressedDelta(bool currentValue, const templateType &lastValue)
    /// when we have an unknown second bool
    template <> void WriteCompressedDelta(const bool &currentValue);

    /// \brief Read a bool from a bitstream.
    /// \param[in] var The value to read
    /// \return true on success, false on failure.
    template <> bool Read(bool &var);

    /// \brief Read a systemAddress from a bitstream.
    /// \param[in] var The value to read
    /// \return true on success, false on failure.
    template <> bool Read(SystemAddress &var);

    template <> bool Read(uint24_t &var);

    template <> bool Read(RakNetGUID &var);

    /// \brief Read a String from a bitstream.
    /// \param[in] var The value to read
    /// \return true on success, false on failure.
    template <> bool Read(char *&var);
    template <> bool Read(wchar_t *&var);
    template <> bool Read(unsigned char *&var);
    template <> bool Read(RakString &var);
    template <> bool Read(RakWString &var);

    /// \brief Read a bool from a bitstream.
    /// \param[in] var The value to read
    /// \return true on success, false on failure.
    template <> bool ReadDelta(bool &var);

    template <> bool ReadCompressed(SystemAddress &var);

    template <> bool ReadCompressed(uint24_t &var);

    template <> bool ReadCompressed(RakNetGUID &var);

    template <> bool ReadCompressed(bool &var);

    template <> bool ReadCompressed(float &var);

    /// For values between -1 and 1
    /// \return true on success, false on failure.
    template <> bool ReadCompressed(double &var);

    template <> bool ReadCompressed(char *&var);
    template <> bool ReadCompressed(wchar_t *&var);
    template <> bool ReadCompressed(unsigned char *&var);
    template <> bool ReadCompressed(RakString &var);
    template <> bool ReadCompressed(RakWString &var);

    /// \brief Read a bool from a bitstream.
    /// \param[in] var The value to read
    /// \return true on success, false on failure.
    template <> bool ReadCompressedDelta(bool &var);
#endif
  ...
  private:
    ...
    /// BitStreams that use less than BITSTREAM_STACK_ALLOCATION_SIZE use the stack, rather than the heap to store data.  It switches over if
    /// BITSTREAM_STACK_ALLOCATION_SIZE is exceeded
    unsigned char stackData[BITSTREAM_STACK_ALLOCATION_SIZE];
};

template <class templateType> inline bool BitStream::Serialize(bool writeToBitstream, templateType &inOutTemplateVar) {
    if (writeToBitstream)
...
template <class templateType> inline bool BitStream::SerializeDelta(bool writeToBitstream, templateType &inOutCurrentValue, const templateType &lastValue) {
    if (writeToBitstream)
...
template <class templateType> inline bool BitStream::SerializeDelta(bool writeToBitstream, templateType &inOutCurrentValue) {
    if (writeToBitstream)
...
template <class templateType> inline bool BitStream::SerializeCompressed(bool writeToBitstream, templateType &inOutTemplateVar) {
    if (writeToBitstream)
...
// Stoppedhere
template <class templateType> inline bool BitStream::SerializeCompressedDelta(bool writeToBitstream, templateType &inOutCurrentValue) {
    if (writeToBitstream)
...
template <class serializationType, class sourceType> bool BitStream::SerializeCasted(bool writeToBitstream, sourceType &value) {
    if (writeToBitstream)
...
template <class templateType>
bool BitStream::SerializeBitsFromIntegerRange(bool writeToBitstream, templateType &value, const templateType minimum, const templateType maximum,
...
template <class templateType> inline bool BitStream::SerializeNormVector(bool writeToBitstream, templateType &x, templateType &y, templateType &z) {
    if (writeToBitstream)
...
template <class templateType> inline bool BitStream::SerializeVector(bool writeToBitstream, templateType &x, templateType &y, templateType &z) {
    if (writeToBitstream)
        WriteVector(x, y, z);
    else
...
template <class templateType>
inline bool BitStream::SerializeOrthMatrix(bool writeToBitstream, templateType &m00, templateType &m01, templateType &m02, templateType &m10, templateType &m11,
                                           templateType &m12, templateType &m20, templateType &m21, templateType &m22) {
    if (writeToBitstream)
...
inline bool BitStream::SerializeBits(bool writeToBitstream, unsigned char *inOutByteArray, const BitSize_t numberOfBitsToSerialize,
...
template <class templateType> inline void BitStream::Write(const templateType &inTemplateVar) {
#ifdef _MSC_VER
...
template <class templateType> inline void BitStream::WritePtr(templateType *inTemplateVar) {
#ifdef _MSC_VER
#pragma warning(disable : 4127) // conditional expression is constant
#endif
...
/// \param[in] inTemplateVar The value to write
template <> inline void BitStream::Write(const bool &inTemplateVar) {
    if (inTemplateVar)
        Write1();
    else
        Write0();
...
/// \param[in] inTemplateVar The value to write
template <> inline void BitStream::Write(const SystemAddress &inTemplateVar) {
...
template <> inline void BitStream::Write(const uint24_t &inTemplateVar) {
...
template <> inline void BitStream::Write(const RakNetGUID &inTemplateVar) { Write(inTemplateVar.g); }

/// \brief Write a string to a bitstream.
/// \param[in] var The value to write
template <> inline void BitStream::Write(const RakString &inTemplateVar) { inTemplateVar.Serialize(this); }
template <> inline void BitStream::Write(const RakWString &inTemplateVar) { inTemplateVar.Serialize(this); }
template <> inline void BitStream::Write(const char *const &inStringVar) { RakString::Serialize(inStringVar, this); }
template <> inline void BitStream::Write(const wchar_t *const &inStringVar) { RakWString::Serialize(inStringVar, this); }
template <> inline void BitStream::Write(const unsigned char *const &inTemplateVar) { Write((const char *)inTemplateVar); }
template <> inline void BitStream::Write(char *const &inTemplateVar) { Write((const char *)inTemplateVar); }
template <> inline void BitStream::Write(unsigned char *const &inTemplateVar) { Write((const char *)inTemplateVar); }
...
/// \param[in] currentValue The current value to write
/// \param[in] lastValue The last value to compare against
template <class templateType> inline void BitStream::WriteDelta(const templateType &currentValue, const templateType &lastValue) {
...
/// \param[in] currentValue The current value to write
/// \param[in] lastValue The last value to compare against
template <> inline void BitStream::WriteDelta(const bool &currentValue, const bool &lastValue) {
    (void)lastValue;
...
/// \brief WriteDelta when you don't know what the last value is, or there is no last value.
/// \param[in] currentValue The current value to write
template <class templateType> inline void BitStream::WriteDelta(const templateType &currentValue) {
    Write(true);
    Write(currentValue);
}
...
/// If you are not using __BITSTREAM_NATIVE_END the opposite is true for types larger than 1 byte
/// \param[in] inTemplateVar The value to write
template <class templateType> inline void BitStream::WriteCompressed(const templateType &inTemplateVar) {
#ifdef _MSC_VER
#pragma warning(disable : 4127) // conditional expression is constant
#endif
    if (sizeof(inTemplateVar) == 1)
...
template <> inline void BitStream::WriteCompressed(const SystemAddress &inTemplateVar) { Write(inTemplateVar); }

template <> inline void BitStream::WriteCompressed(const RakNetGUID &inTemplateVar) { Write(inTemplateVar); }

template <> inline void BitStream::WriteCompressed(const uint24_t &var) { Write(var); }

template <> inline void BitStream::WriteCompressed(const bool &inTemplateVar) { Write(inTemplateVar); }

/// For values between -1 and 1
template <> inline void BitStream::WriteCompressed(const float &inTemplateVar) {
...
/// For values between -1 and 1
template <> inline void BitStream::WriteCompressed(const double &inTemplateVar) {
...
/// Compress the string
template <> inline void BitStream::WriteCompressed(const RakString &inTemplateVar) { inTemplateVar.SerializeCompressed(this, 0, false); }
template <> inline void BitStream::WriteCompressed(const RakWString &inTemplateVar) { inTemplateVar.Serialize(this); }
template <> inline void BitStream::WriteCompressed(const char *const &inStringVar) { RakString::SerializeCompressed(inStringVar, this, 0, false); }
template <> inline void BitStream::WriteCompressed(const wchar_t *const &inStringVar) { RakWString::Serialize(inStringVar, this); }
template <> inline void BitStream::WriteCompressed(const unsigned char *const &inTemplateVar) { WriteCompressed((const char *)inTemplateVar); }
template <> inline void BitStream::WriteCompressed(char *const &inTemplateVar) { WriteCompressed((const char *)inTemplateVar); }
template <> inline void BitStream::WriteCompressed(unsigned char *const &inTemplateVar) { WriteCompressed((const char *)inTemplateVar); }
...
/// \param[in] currentValue The current value to write
/// \param[in] lastValue The last value to compare against
template <class templateType> inline void BitStream::WriteCompressedDelta(const templateType &currentValue, const templateType &lastValue) {
    if (currentValue == lastValue) {
        Write(false);
    } else {
        Write(true);
...
/// \param[in] currentValue The current value to write
/// \param[in] lastValue The last value to compare against
template <> inline void BitStream::WriteCompressedDelta(const bool &currentValue, const bool &lastValue) {
    (void)lastValue;
...
template <class templateType> inline void BitStream::WriteCompressedDelta(const templateType &currentValue) {
    Write(true);
    WriteCompressed(currentValue);
}
...
/// \brief Save as WriteCompressedDelta(bool currentValue, const templateType &lastValue)
/// when we have an unknown second bool
template <> inline void BitStream::WriteCompressedDelta(const bool &currentValue) { Write(currentValue); }

/// \brief Read any integral type from a bitstream.  Define __BITSTREAM_NATIVE_END if you need endian swapping.
/// \param[in] outTemplateVar The value to read
template <class templateType> inline bool BitStream::Read(templateType &outTemplateVar) {
#ifdef _MSC_VER
#pragma warning(disable : 4127) // conditional expression is constant
#endif
...
/// \brief Read a bool from a bitstream.
/// \param[in] outTemplateVar The value to read
template <> inline bool BitStream::Read(bool &outTemplateVar) {
...
/// \brief Read a systemAddress from a bitstream.
/// \param[in] outTemplateVar The value to read
template <> inline bool BitStream::Read(SystemAddress &outTemplateVar) {
...
template <> inline bool BitStream::Read(uint24_t &outTemplateVar) {
...
template <> inline bool BitStream::Read(RakNetGUID &outTemplateVar) { return Read(outTemplateVar.g); }

template <> inline bool BitStream::Read(RakString &outTemplateVar) { return outTemplateVar.Deserialize(this); }
template <> inline bool BitStream::Read(RakWString &outTemplateVar) { return outTemplateVar.Deserialize(this); }
template <> inline bool BitStream::Read(char *&varString) { return RakString::Deserialize(varString, this); }
template <> inline bool BitStream::Read(wchar_t *&varString) { return RakWString::Deserialize(varString, this); }
template <> inline bool BitStream::Read(unsigned char *&varString) { return RakString::Deserialize((char *)varString, this); }
...
/// ReadDelta is only valid from a previous call to WriteDelta
/// \param[in] outTemplateVar The value to read
template <class templateType> inline bool BitStream::ReadDelta(templateType &outTemplateVar) {
    bool dataWritten;
    bool success;
...
/// \brief Read a bool from a bitstream.
/// \param[in] outTemplateVar The value to read
template <> inline bool BitStream::ReadDelta(bool &outTemplateVar) { return Read(outTemplateVar); }
...
/// If you are not using __BITSTREAM_NATIVE_END the opposite is true for types larger than 1 byte
/// \param[in] outTemplateVar The value to read
template <class templateType> inline bool BitStream::ReadCompressed(templateType &outTemplateVar) {
#ifdef _MSC_VER
#pragma warning(disable : 4127) // conditional expression is constant
#endif
    if (sizeof(outTemplateVar) == 1)
...
template <> inline bool BitStream::ReadCompressed(SystemAddress &outTemplateVar) { return Read(outTemplateVar); }

template <> inline bool BitStream::ReadCompressed(uint24_t &outTemplateVar) { return Read(outTemplateVar); }

template <> inline bool BitStream::ReadCompressed(RakNetGUID &outTemplateVar) { return Read(outTemplateVar); }

template <> inline bool BitStream::ReadCompressed(bool &outTemplateVar) { return Read(outTemplateVar); }

/// For values between -1 and 1
template <> inline bool BitStream::ReadCompressed(float &outTemplateVar) {
...
/// For values between -1 and 1
template <> inline bool BitStream::ReadCompressed(double &outTemplateVar) {
...
/// For strings
template <> inline bool BitStream::ReadCompressed(RakString &outTemplateVar) { return outTemplateVar.DeserializeCompressed(this, false); }
template <> inline bool BitStream::ReadCompressed(RakWString &outTemplateVar) { return outTemplateVar.Deserialize(this); }
template <> inline bool BitStream::ReadCompressed(char *&outTemplateVar) { return RakString::DeserializeCompressed(outTemplateVar, this, false); }
template <> inline bool BitStream::ReadCompressed(wchar_t *&outTemplateVar) { return RakWString::Deserialize(outTemplateVar, this); }
template <> inline bool BitStream::ReadCompressed(unsigned char *&outTemplateVar) {
...
/// ReadCompressedDelta is only valid from a previous call to WriteDelta
/// \param[in] outTemplateVar The value to read
template <class templateType> inline bool BitStream::ReadCompressedDelta(templateType &outTemplateVar) {
    bool dataWritten;
    bool success;
    success = Read(dataWritten);
    if (dataWritten)
...
/// \brief Read a bool from a bitstream.
/// \param[in] outTemplateVar The value to read
template <> inline bool BitStream::ReadCompressedDelta(bool &outTemplateVar) { return Read(outTemplateVar); }

template <class destinationType, class sourceType> void BitStream::WriteCasted(const sourceType &value) {
...
template <class templateType>
void BitStream::WriteBitsFromIntegerRange(const templateType value, const templateType minimum, const templateType maximum, const int requiredBits,
                                          bool allowOutsideRange) {
...
template <class templateType> // templateType for this function must be a float or double
void BitStream::WriteOrthMatrix(templateType m00, templateType m01, templateType m02, templateType m10, templateType m11, templateType m12, templateType m20,
                                templateType m21, templateType m22) {
...
template <class serializationType, class sourceType> bool BitStream::ReadCasted(sourceType &value) {
...
template <class templateType>
bool BitStream::ReadBitsFromIntegerRange(templateType &value, const templateType minimum, const templateType maximum, const int requiredBits,
                                         bool allowOutsideRange) {
    RakAssert(maximum >= minimum);
    if (allowOutsideRange) {
        bool isOutsideRange;
...
template <class templateType> // templateType for this function must be a float or double
bool BitStream::ReadOrthMatrix(templateType &m00, templateType &m01, templateType &m02, templateType &m10, templateType &m11, templateType &m12,
...
template <class templateType> BitStream &operator<<(BitStream &out, templateType &c) {
    out.Write(c);
    return out;
}
template <class templateType> BitStream &operator>>(BitStream &in, templateType &c) {
