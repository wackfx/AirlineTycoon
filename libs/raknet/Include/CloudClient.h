...
/// \details The CloudServer plugins operates on requests from the CloudClient plugin. The servers are in a fully connected mesh topology, which the clients are
/// connected to any server. Clients can interact with each other by posting and subscribing to memory updates, without being directly connected or even knowing
/// about each other. \ingroup PLUGINS_GROUP

/// \brief Performs Post() and Get() operations on CloudMemoryServer
/// \details A CloudClient is a computer connected to one or more servers in a cloud configuration. Operations by one CloudClient can be received and subscribed
/// to by other instances of CloudClient, without those clients being connected, even on different servers. \ingroup CLOUD_GROUP
class RAK_DLL_EXPORT CloudClient : public PluginInterface2 {
  public:
    ...
    /// \details Pointers to CloudAllocator and CloudClientCallback can be stored by the system if desired. If a callback is not provided to OnGetReponse(),
    /// OnSubscriptionNotification(), OnSubscriptionDataDeleted(), the callback passed here will be used instead. \param[in] _allocator An instance of
    /// CloudAllocator \param[in] _callback An instance of CloudClientCallback
    ...
    /// \details If a remote system has subscribed to one or more of the \a keys uploaded, they will get ID_CLOUD_SUBSCRIPTION_NOTIFICATION notifications
    /// containing the last value uploaded before deletions \param[in] cloudKey Identifies the data to release. It is possible to remove uploads from multiple
    /// Post() calls at once. \param[in] systemIdentifier A remote system running CloudServer that we are already connected to.
    ...
    /// CloudQuery::maxRowsToReturn is used to limit the number of rows returned. The number of rows returned may also be limited by
    /// CloudServer::SetMaxBytesPerDownload(); CloudQuery::subscribeToResults if set to true, will cause ID_CLOUD_SUBSCRIPTION_NOTIFICATION to be returned to us
    /// when any of the keys in the query are updated or are deleted. ID_CLOUD_GET_RESPONSE will be returned even if subscribing to the result list. Only later
    /// updates will return ID_CLOUD_SUBSCRIPTION_NOTIFICATION. Calling Get() with CloudQuery::subscribeToResults false, when you are already subscribed, does
    /// not remove the subscription. Use Unsubscribe() for this. Resubscribing using the same CloudKey but a different or no \a specificSystems overwrites the
    /// subscribed systems for those keys. \param[in] cloudQuery One or more keys, and optional parameters to perform with the Get \param[in] systemIdentifier A
    /// remote system running CloudServer that we are already connected to. \param[in] specificSystems It is possible to get or subscribe to updates only for
    /// specific uploading CloudClient instances. Pass the desired instances here. The overload that does not have the specificSystems parameter is treated as
    /// subscribing to all updates from all clients.
    ...
    /// You cannot unsubscribe specific systems when previously subscribed to updates from any system. To do this, first Unsubscribe() from all systems, and
    /// call Get() with the \a specificSystems parameter explicilty listing the systems you want to subscribe to.
...
class RAK_DLL_EXPORT CloudClientCallback {
  public:
    ...
    /// \param[out] deallocateRowsAfterReturn CloudQueryResult::rowsReturned will be deallocated after the function returns by default. Set to false to not
    /// deallocate these pointers. The pointers are allocated through CloudAllocator.
    ...
    /// \param[out] wasUpdated If true, the row was updated. If false, it was deleted. \a result will contain the last value just before deletion
    /// \param[out] deallocateRowAfterReturn \a result will be deallocated after the function returns by default. Set to false to not deallocate these pointers.
    /// The pointers are allocated through CloudAllocator.
