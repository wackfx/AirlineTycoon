...
class RAK_DLL_EXPORT RakString {
  public:
    ...
    /// \param[in] writeLanguageId encode the languageId variable in the stream. If false, 0 is assumed, and DeserializeCompressed will not look for this
    /// variable in the stream (saves bandwidth) \pre StringCompressor::AddReference must have been called to instantiate the class (Happens automatically from
    /// RakPeer::Startup())
