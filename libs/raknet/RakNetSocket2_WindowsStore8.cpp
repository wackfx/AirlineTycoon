...
task<IOutputStream^>(socket->GetOutputStreamAsync(eventArguments->RemoteAddress, eventArguments->RemotePort)).then([this, socket, eventArguments]
...
RNS2BindResult RNS2_WindowsStore8::Bind(Platform::String ^ localServiceName) {
    ...
    listener->MessageReceived +=
        ref new TypedEventHandler<DatagramSocket ^, DatagramSocketMessageReceivedEventArgs ^>(listenerContext, &ListenerContext::OnMessage);
...
void RNS2_WindowsStore8::DomainNameToIP(const char *domainName, char ip[65]) {
    ...
    op->Completed = ref new AsyncOperationCompletedHandler<Windows::Foundation::Collections::IVectorView<Windows::Networking::EndpointPair ^> ^>(
        [&](IAsyncOperation<Windows::Foundation::Collections::IVectorView<Windows::Networking::EndpointPair ^> ^> ^ result, AsyncStatus status) {
...
int RNS2_WindowsStore8::WinRTSetSockOpt(Windows::Networking::Sockets::DatagramSocket ^ s, int level, int optname, const char *optval, socklen_t optlen) {
    RakAssert("WinRTSetSockOpt Unsupported" && 0);
    return 0;
}

int RNS2_WindowsStore8::WinRTIOCTLSocket(Windows::Networking::Sockets::DatagramSocket ^ s, long cmd, unsigned long *argp) {
    RakAssert("WinRTIOCTLSocket Unsupported" && 0);
    return 0;
}

int RNS2_WindowsStore8::WinRTGetSockName(Windows::Networking::Sockets::DatagramSocket ^ s, struct sockaddr *name, socklen_t *namelen) {
