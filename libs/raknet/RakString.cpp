...
bool RakString::operator==(const RakString &rhs) const { return strcmp(sharedString->c_str, rhs.sharedString->c_str) == 0; }
...
int RakString::StrNCmp(const RakString &rhs, size_t num) const { return strncmp(sharedString->c_str, rhs.C_String(), num); }
...
RakNet::RakString &RakString::SQLEscape(void) {
    ...
    readIndex = strLen;
    while (readIndex >= 0) {
        if (sharedString->c_str[readIndex] == '\'' || sharedString->c_str[readIndex] == '"' || sharedString->c_str[readIndex] == '\\') {
            sharedString->c_str[writeIndex--] = sharedString->c_str[readIndex--];
