    ...
    case CS_CONNECTED: {
        ...
        if (currentProcessingCommand.isPost) {
            request.Set("POST %s HTTP/1.0\r\n"
                        ...
                        "\r\n"
                        "%s",
                        currentProcessingCommand.remotePath.C_String(), host.C_String(), port, currentProcessingCommand.contentType.C_String(),
                        (unsigned)currentProcessingCommand.data.GetLength(), currentProcessingCommand.data.C_String());
        } else {
            // request.Set("GET %s\r\n", host.C_String());
...
void HTTPConnection::ProcessTCPPacket(Packet *packet) {
    ...
    if (packet->systemAddress == server) {
        ...
        if (
            ...
            connectionState == CS_PROCESSING) {
            /*
            // The stupid programmer that wrote this originally didn't think that just because the header contains this value doesn't mean you got the whole
