...
class RAK_DLL_EXPORT TeamBalancer : public PluginInterface2 {
  public:
    ...
    /// SetTeamSizeLimit() can be called on all systems if desired - for example, in a P2P environment you may wish to call it on all systems in advanced in
    /// case you become host. \param[in] team Which team to set the limit for \param[in] limit The maximum number of people on this team
    ...
    /// If teams are uneven at the time that SetForceEvenTeams(true) is called, players at randomly will be switched, and will be notified of
    /// ID_TEAM_BALANCER_TEAM_ASSIGNED If players disconnect from the host such that teams would not be even, and teams are not locked, then a player from the
    /// largest team is randomly moved to even the teams. Defaults to false \note SetLockTeams(true) takes priority over SetForceEvenTeams(), so if teams are
    /// currently locked, this function will have no effect until teams become unlocked. \param[in] force True to force even teams. False to allow teams to not
    /// be evenly matched
    ...
    /// \details However, if those functions are called and you do not have a team, then you will be assigned to a default team according to
    /// SetDefaultAssignmentAlgorithm() and possibly SetForceEvenTeams(true) If \a lock is false, and SetForceEvenTeams() was called with \a force as true, and
    /// teams are currently uneven, they will be made even, and those players randomly moved will get ID_TEAM_BALANCER_TEAM_ASSIGNED Defaults to false
    ...
    /// In the case of ID_TEAM_BALANCER_REQUESTED_TEAM_CHANGE_PENDING the request will stay in memory. ID_TEAM_BALANCER_SET_TEAM will be returned when someone
    /// on the desired team leaves or wants to switch to your team. If SetLockTeams(true) is called while you have a request pending, you will get
    /// ID_TEAM_BALANCER_TEAMS_LOCKED \pre Call SetTeamSizeLimits() on the host and call SetHostGuid() on this system. If the host is not running the
    /// TeamBalancer plugin or did not have SetTeamSizeLimits() called, then you will not get any response. \param[in] memberId If there is more than one player
    /// per computer, this number identifies that player. Use any consistent value, such as UNASSIGNED_NETWORK_ID if there is only one player. \param[in]
    /// desiredTeam An index representing your team number. The index should range from 0 to one less than the size of the list passed to SetTeamSizeLimits() on
    /// the host. You can also pass UNASSIGNED_TEAM_ID to not be on any team (such as if spectating)
    void RequestSpecificTeam(NetworkID memberId, TeamId desiredTeam);

    /// If ID_TEAM_BALANCER_REQUESTED_TEAM_CHANGE_PENDING is returned after a call to RequestSpecificTeam(), the request will stay in memory on the host and
    /// execute when available, or until the teams become locked. You can cancel the request by calling CancelRequestSpecificTeam(), in which case you will stay
    /// on your existing team. \note Due to latency, even after calling CancelRequestSpecificTeam() you may still get ID_TEAM_BALANCER_SET_TEAM if the packet
    /// was already in transmission. \param[in] memberId If there is more than one player per computer, this number identifies that player. Use any consistent
    /// value, such as UNASSIGNED_NETWORK_ID if there is only one player.
    ...
    /// \param[in] memberId If there is more than one player per computer, this number identifies that player. Use any consistent value, such as
    /// UNASSIGNED_NETWORK_ID if there is only one player.
    ...
    /// \pre For this to return anything other than UNASSIGNED_TEAM_ID, connect to a properly initialized host and RequestSpecificTeam() or RequestAnyTeam()
    /// first \param[in] memberId If there is more than one player per computer, this number identifies that player. Use any consistent value, such as
    /// UNASSIGNED_NETWORK_ID if there is only one player. \return UNASSIGNED_TEAM_ID for no team. Otherwise, the index should range from 0 to one less than the
    /// size of the list passed to SetTeamSizeLimits() on the host
  ...
  protected:
    ...
    bool TeamWouldBeOverpopulatedOnAddition(
        TeamId teamId,
        unsigned int teamMemberSize); // Accounting for team balancing and team limits, would this team be overpopulated if a member was added to it?
