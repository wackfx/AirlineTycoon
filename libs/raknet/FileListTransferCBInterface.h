...
class FileListTransferCBInterface {
  public:
    ...
    struct FileProgressStruct {
        /// \param[out] onFileStruct General information about this file, such as the filename and the first \a partLength bytes. You do NOT need to save this
        /// data yourself. The complete file will arrive normally.
        ...
        /// \param[out] partTotal The total number of parts this file was split into. Each part will be roughly the MTU size, minus the UDP header and RakNet
        /// headers
        unsigned int partTotal;
        /// \param[out] dataChunkLength How many bytes long firstDataChunk and iriDataChunk are
        unsigned int dataChunkLength;
        /// \param[out] firstDataChunk The first \a partLength of the final file. If you store identifying information about the file in the first \a partLength
        /// bytes, you can read them while the download is taking place. If this hasn't arrived yet, firstDataChunk will be 0
        char *firstDataChunk;
        /// \param[out] iriDataChunk If the remote system is sending this file using IncrementalReadInterface, then this is the chunk we just downloaded. It
        /// will not exist in memory after this callback. You should either store this to disk, or in memory. If it is 0, then the file is smaller than one
        /// chunk, and will be held in memory automatically
        char *iriDataChunk;
    ...
    /// \details This is called in one of two circumstances: Either the transport layer is returning ID_PROGRESS_NOTIFICATION, or you got a block via
    /// IncrementalReadInterface If the transport layer is returning ID_PROGRESS_NOTIFICATION (see RakPeer::SetSplitMessageProgressInterval()) then
    /// FileProgressStruct::iriDataChunk will be 0. If this is a block via IncrementalReadInterface, then iriDataChunk will point to the block just downloaded.
    /// If not using IncrementalReadInterface, then you only care about partCount and partTotal to tell how far the download has progressed. YOu can use
    /// firstDataChunk to read the first part of the file if desired. The file is usable when you get the OnFile callback. If using IncrementalReadInterface and
    /// you let RakNet buffer the files in memory (default), then it is the same as above. The file is usable when you get the OnFile callback. If using
    /// IncrementalReadInterface and you do not let RakNet buffer the files in memory, then set allocateIrIDataChunkAutomatically to false. Write the file to
    /// disk whenever you get OnFileProgress and iriDataChunk is not 0, and ignore OnFile.
