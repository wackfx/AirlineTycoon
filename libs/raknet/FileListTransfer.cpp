...
void FileListTransfer::Send(FileList *fileList, RakNet::RakPeerInterface *rakPeer, SystemAddress recipient, unsigned short setID, PacketPriority priority,
                            char orderingChannel, IncrementalReadInterface *_incrementalReadInterface, unsigned int _chunkSize) {
...
void FileListTransfer::GetCallbacks(DataStructures::List<FileListProgress *> &callbacks) { callbacks = fileListProgressCallbacks; }
...
void FileListTransfer::OnReferencePush(Packet *packet, bool isTheFullFile) {
    ...
    if (isTheFullFile == false) {
        ...
        // 2/16/2012 I care because a reference push is 16 megabytes by default. Also, if it is the last file "if (ftpr->filesToPush.Size()<2)" or total file
        // size exceeds smallFileTotalSize it always sends a reference push.
        //		return;
    }
    ...
    *returnOutput = false;
    ...
    for (ftpIndex = 0; ftpIndex < fileListTransfer->fileToPushRecipientList.Size(); ftpIndex++) {
        ...
        if (ftpr->systemAddress == systemAddress && ftpr->setId == setId) {
            ...
            bytesRead = ftp->incrementalReadInterface->GetFilePart(ftp->fileListNode.fullPathToFile, ftp->currentOffset, ftp->chunkSize, buff,
            ...
            while (done && ftp->currentOffset == 0 && smallFileTotalSize < ftp->chunkSize) {
                ////ftpr->filesToPushMutex.Lock();
                // The reason for 2 is that ID_FILE_LIST_REFERENCE_PUSH gets ID_FILE_LIST_REFERENCE_PUSH_ACK. WIthout ID_FILE_LIST_REFERENCE_PUSH_ACK,
            ...
            for (unsigned int flpcIndex = 0; flpcIndex < fileListTransfer->fileListProgressCallbacks.Size(); flpcIndex++)
                fileListTransfer->fileListProgressCallbacks[flpcIndex]->OnFilePush(ftp->fileListNode.filename, ftp->fileListNode.fileLengthBytes,
                                                                                   ftp->currentOffset - bytesRead, bytesRead, done, systemAddress, setId);
