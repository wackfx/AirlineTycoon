            ...
            if (sp.attemptCount > pc.MAX_PREDICTIVE_PORT_RANGE) {
                if (natPunchthroughDebugInterface) {
                    ...
                    natPunchthroughDebugInterface->OnClientMessage(
                        RakNet::RakString("Likely bidirectional punchthrough failure to guid %s, system address %s.", guidString, ipAddressString));
                }
        ...
        if (sp.testMode == SendPing::PUNCHING_FIXED_PORT) {
            ...
            if (++sp.retryCount >= sp.punchingFixedPortAttempts) {
                if (natPunchthroughDebugInterface) {
                    ...
                    sp.targetGuid.ToString(guidString);
                    natPunchthroughDebugInterface->OnClientMessage(
                        RakNet::RakString("Likely unidirectional punchthrough failure to guid %s, system address %s.", guidString, ipAddressString));
                }
...
void NatPunchthroughClient::OnPunchthroughFailure(void) {
    if (pc.retryOnFailure == false) {
        if (natPunchthroughDebugInterface) {
            ...
            sp.targetGuid.ToString(guidString);
            natPunchthroughDebugInterface->OnClientMessage(
                RakNet::RakString("Failed punchthrough once. Returning failure to guid %s, system address %s to user.", guidString, ipAddressString));
        }
    ...
    for (i = 0; i < failedAttemptList.Size(); i++) {
        if (failedAttemptList[i].guid == sp.targetGuid) {
            if (natPunchthroughDebugInterface) {
                ...
                sp.targetGuid.ToString(guidString);
                natPunchthroughDebugInterface->OnClientMessage(
                    RakNet::RakString("Failed punchthrough twice. Returning failure to guid %s, system address %s to user.", guidString, ipAddressString));
            }
    ...
    if (natPunchthroughDebugInterface) {
        ...
        sp.targetGuid.ToString(guidString);
        natPunchthroughDebugInterface->OnClientMessage(
            RakNet::RakString("First punchthrough failure on guid %s, system address %s. Reattempting.", guidString, ipAddressString));
    }
...
PluginReceiveResult NatPunchthroughClient::OnReceive(Packet *packet) {
    ...
    case ID_NAT_RESPOND_BOUND_ADDRESSES: {
        ...
        if (boundAddressCount < 2) {
            if (natPunchthroughDebugInterface)
                natPunchthroughDebugInterface->OnClientMessage(
                    RakString("INCAPABLE_PORT_STRIDE. My external ID is %s", rakPeerInterface->GetExternalID(packet->systemAddress).ToString()));
        ...
        for (int i = 0; i < boundAddressCount && i < MAXIMUM_NUMBER_OF_INTERNAL_IDS; i++) {
            ...
            if (boundAddresses[i] != packet->systemAddress) {
                ...
                rakPeerInterface->SendOutOfBand((const char *)boundAddresses[i].ToString(false), boundAddresses[i].GetPort(),
                                                (const char *)outgoingBs.GetData(), outgoingBs.GetNumberOfBytesUsed());
                break;
            }
        }
    }
    ...
    case ID_OUT_OF_BAND_INTERNAL:
        if (packet->length >= 2 && packet->data[1] == ID_NAT_PONG) {
            ...
            if (natPunchthroughDebugInterface)
                natPunchthroughDebugInterface->OnClientMessage(
                    RakString("HAS_PORT_STRIDE %i. First external port %i. Second external port %i.", portStride, externalPort, externalPort2));

            SendQueuedOpenNAT();
            return RR_STOP_PROCESSING_AND_DEALLOCATE;
        } else if (packet->length >= 2 && (packet->data[1] == ID_NAT_ESTABLISH_UNIDIRECTIONAL || packet->data[1] == ID_NAT_ESTABLISH_BIDIRECTIONAL) &&
                   sp.nextActionTime != 0) {
            RakNet::BitStream bs(packet->data, packet->length, false);
            bs.IgnoreBytes(2);
            uint16_t sessionId;
            ...
            if (packet->data[1] == ID_NAT_ESTABLISH_UNIDIRECTIONAL && sp.targetGuid == packet->guid) {
                ...
                if (sp.testMode != SendPing::PUNCHING_FIXED_PORT) {
                    ...
                    if (natPunchthroughDebugInterface) {
                        ...
                        sp.targetGuid.ToString(guidString);
                        natPunchthroughDebugInterface->OnClientMessage(RakNet::RakString(
                            "PUNCHING_FIXED_PORT: Received ID_NAT_ESTABLISH_UNIDIRECTIONAL from guid %s, system address %s.", guidString, ipAddressString));
                    }
                } else {
                    if (natPunchthroughDebugInterface) {
                        char guidString[128];
                        sp.targetGuid.ToString(guidString);
                        natPunchthroughDebugInterface->OnClientMessage(
                            RakNet::RakString("Received ID_NAT_ESTABLISH_UNIDIRECTIONAL from guid %s, system address %s.", guidString, ipAddressString));
                    }
                }
                ...
                if (mostRecentExternalPort == 0) {
                    ...
                    if (natPunchthroughDebugInterface) {
                        natPunchthroughDebugInterface->OnClientMessage(
                            RakNet::RakString("ID_NAT_ESTABLISH_BIDIRECTIONAL mostRecentExternalPort first time set to %i", mostRecentExternalPort));
                    }
                } else {
                    if (sp.testMode != SendPing::TESTING_INTERNAL_IPS && sp.testMode != SendPing::WAITING_FOR_INTERNAL_IPS_RESPONSE) {
                        if (hasPortStride != HAS_PORT_STRIDE) {
                            ...
                            if (natPunchthroughDebugInterface) {
                                natPunchthroughDebugInterface->OnClientMessage(RakString("ID_NAT_ESTABLISH_BIDIRECTIONAL: Estimated port stride from incoming "
                        ...
                        if (natPunchthroughDebugInterface) {
                            natPunchthroughDebugInterface->OnClientMessage(
                                RakString("ID_NAT_ESTABLISH_BIDIRECTIONAL: New mostRecentExternalPort %i", mostRecentExternalPort));
                        }
                    }
                }
                ...
                if (natPunchthroughDebugInterface) {
                    ...
                    if (removedFromFailureQueue)
                        natPunchthroughDebugInterface->OnClientMessage(
                            RakNet::RakString("Punchthrough to guid %s, system address %s succeeded on 2nd attempt.", guidString, ipAddressString));
                    else
                        natPunchthroughDebugInterface->OnClientMessage(
                            RakNet::RakString("Punchthrough to guid %s, system address %s succeeded on 1st attempt.", guidString, ipAddressString));
                }
            }
    ...
    case ID_NAT_ALREADY_IN_PROGRESS: {
        ...
        if (natPunchthroughDebugInterface) {
            ...
            natPunchthroughDebugInterface->OnClientMessage(
                RakNet::RakString("Punchthrough retry to guid %s failed due to ID_NAT_ALREADY_IN_PROGRESS. Returning failure.", guidString));
        }
...
void NatPunchthroughClient::OnGetMostRecentPort(Packet *packet) {
    ...
    if (mostRecentExternalPort == 0) {
        ...
        if (natPunchthroughDebugInterface) {
            natPunchthroughDebugInterface->OnClientMessage(
                RakNet::RakString("OnGetMostRecentPort mostRecentExternalPort first time set to %i", mostRecentExternalPort));
        }
    }
