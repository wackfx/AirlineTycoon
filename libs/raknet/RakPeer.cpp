...
// \param[in] maxConnections The maximum number of connections between this instance of RakPeer and another instance of RakPeer. Required so the network can
// preallocate and for thread safety. A pure client would set this to 1.  A pure server would set it to the number of allowed clients.- A hybrid would set it to
// the sum of both types of connections \param[in] localPort The port to listen for connections on. \param[in] _threadSleepTimer How many ms to Sleep each
// internal update cycle. With new congestion control, the best results will be obtained by passing 10. \param[in] socketDescriptors An array of
// SocketDescriptor structures to force RakNet to listen on a particular IP address or port (or both).  Each SocketDescriptor will represent one unique socket.
// Do not pass redundant structures.  To listen on a specific port, you can pass &socketDescriptor, 1SocketDescriptor(myPort,0); such as for a server.  For a
// client, it is usually OK to just pass SocketDescriptor(); \param[in] socketDescriptorCount The size of the \a socketDescriptors array.  Pass 1 if you are not
// sure what to pass. \return False on failure (can't create socket or thread), true on success.
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
StartupResult RakPeer::Startup(unsigned int maxConnections, SocketDescriptor *socketDescriptors, unsigned socketDescriptorCount, int threadPriority) {
    ...
    for (i = 0; i < socketDescriptorCount; i++) {
        ...
        {
                rns = SocketLayer::CreateBoundSocket( this, socketDescriptors[i].port, socketDescriptors[i].blockingSocket, addrToBind, 100,
socketDescriptors[i].extraSocketOptions, socketDescriptors[i].socketFamily, socketDescriptors[i].chromeInstance );
        }
        else
        {
#if defined(_PS3) || defined(__PS3__) || defined(SN_TARGET_PS3) || defined(_PS4)
                rns = SocketLayer::CreateBoundSocket_PS3Lobby( socketDescriptors[i].port, socketDescriptors[i].blockingSocket, addrToBind,
socketDescriptors[i].socketFamily ); #elif  defined(SN_TARGET_PSP2) rns = SocketLayer::CreateBoundSocket_PSP2( socketDescriptors[i].port,
...
// bRequireClientKey: Should be set to false for most servers.  Allows the server to accept a public key from connecting clients as a proof of identity but eats
// twice as much CPU time as a normal connection
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void RakPeer::SetMaximumIncomingConnections(unsigned short numberAllowed) { maximumIncomingConnections = numberAllowed; }
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
ConnectionAttemptResult RakPeer::Connect(const char *host, unsigned short remotePort, const char *passwordData, int passwordDataLength, PublicKey *publicKey,
                                         unsigned connectionSocketIndex, unsigned sendConnectionAttemptCount, unsigned timeBetweenSendConnectionAttemptsMS,
...
ConnectionAttemptResult RakPeer::ConnectWithSocket(const char *host, unsigned short remotePort, const char *passwordData, int passwordDataLength,
                                                   RakNetSocket2 *socket, PublicKey *publicKey, unsigned sendConnectionAttemptCount,
...
// Returns:
// \return 0 on bad input. Otherwise a number that identifies this message. If \a reliability is a type that returns a receipt, on a later call to Receive() you
// will get ID_SND_RECEIPT_ACKED or ID_SND_RECEIPT_LOSS with bytes 1-4 inclusive containing this number
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
uint32_t RakPeer::Send(const RakNet::BitStream *bitStream, PacketPriority priority, PacketReliability reliability, char orderingChannel,
    ...
    SendBuffered((const char *)bitStream->GetData(), bitStream->GetNumberOfBitsUsed(), priority, reliability, orderingChannel, systemIdentifier, broadcast,
...
// \param[in] orderingChannel When using ordered or sequenced messages, what channel to order these on. Messages are only ordered relative to other messages on
// the same stream \param[in] systemIdentifier Who to send this packet to, or in the case of broadcasting who not to send it to. Pass either a SystemAddress
// structure or a RakNetGUID structure. Use UNASSIGNED_SYSTEM_ADDRESS or to specify none \param[in] broadcast True to send this packet to all connected systems.
// If true, then systemAddress specifies who not to send the packet to. \return False if we are not connected to the specified recipient.  True otherwise
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    for (i=0; i < socketList.Size(); i++)
    {
            while (1)
            {
                    ...
                    SocketLayer::RecvFromBlocking(
                            ...
                            recvFromStruct->extraSocketOptions, recvFromStruct->data, &recvFromStruct->bytesRead, &recvFromStruct->systemAddress,
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void RakPeer::CloseConnection(const AddressOrGUID target, bool sendDisconnectionNotification, unsigned char orderingChannel,
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
int RakPeer::GetIndexFromSystemAddress(const SystemAddress systemAddress) const { return GetIndexFromSystemAddress(systemAddress, false); }
...
// pushAtHead True to push the packet so that the next receive call returns it.  False to push it at the end of the queue (obviously pushing it at the end makes
...
// To simulate bi-directional ping and packet loss, you should call this on both the sender and the recipient, with half the total ping and maxSendBPS value on
// each.
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
bool RakPeer::GenerateConnectionRequestChallenge(RequestedConnectionStruct *rcs, PublicKey *publicKey) {
    ...
    case PKM_USE_KNOWN_PUBLIC_KEY:
        ...
        if (!rcs->client_handshake->Initialize(publicKey->remoteServerPublicKey) || !rcs->client_handshake->GenerateChallenge(rcs->handshakeChallenge)) {
        ...
        if (doClientKey == 1) {
            ...
            if (_require_client_public_key) {
                ...
                if (!_server_handshake->VerifyInitiatorIdentity(remoteSystem->answer, ident, remoteSystem->client_public_key)) {
                    RakNet::BitStream bitStream;
                    bitStream.Write((MessageID)ID_REMOTE_SYSTEM_REQUIRES_PUBLIC_KEY); // Report an error since the client is not providing an identity when it
...
void RakPeer::OnConnectionRequest(RakPeer::RemoteSystemStruct *remoteSystem, RakNet::Time incomingTimestamp) {
    ...
    SendImmediate((char *)bitStream.GetData(), bitStream.GetNumberOfBitsUsed(), IMMEDIATE_PRIORITY, RELIABLE_ORDERED, 0, remoteSystem->systemAddress, false,
...
void RakPeer::NotifyAndFlagForShutdown(const SystemAddress systemAddress, bool performImmediate, unsigned char orderingChannel,
    ...
    if (performImmediate) {
        SendImmediate((char *)temp.GetData(), temp.GetNumberOfBitsUsed(), disconnectionNotificationPriority, RELIABLE_ORDERED, orderingChannel, systemAddress,
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
RakPeer::RemoteSystemStruct *RakPeer::AssignSystemAddressToRemoteSystemList(const SystemAddress systemAddress, RemoteSystemStruct::ConnectMode connectionMode,
                                                                            RakNetSocket2 *incomingRakNetSocket, bool *thisIPConnectedRecently,
                ...
                if (1 || foundIndex == (unsigned int)-1) {
                    ...
                    if (socketListIndex==socketList.Size())
                    {
                            ...
                            if (incomingRakNetSocket->GetRemotePortRakNetWasStartedOn()==0)
                                    rns = SocketLayer::CreateBoundSocket( this, bindingAddress.GetPort(), incomingRakNetSocket->GetBlockingSocket(),
ipListFoundIndexStr, 0, incomingRakNetSocket->GetExtraSocketOptions(), incomingRakNetSocket->GetSocketFamily(), incomingRakNetSocket->GetChromeInstance() );
                            else
                                    rns = SocketLayer::CreateBoundSocket_PS3Lobby( bindingAddress.GetPort(), incomingRakNetSocket->GetBlockingSocket(),
ipListFoundIndexStr, incomingRakNetSocket->GetSocketFamily() );
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void RakPeer::AddToActiveSystemList(unsigned int remoteSystemListIndex) { activeSystemList[activeSystemListSize++] = remoteSystemList + remoteSystemListIndex; }
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void RakPeer::RemoveFromActiveSystemList(const SystemAddress &sa) {
    ...
    RakAssert("activeSystemList invalid, entry not found in RemoveFromActiveSystemList. Ensure that AddToActiveSystemList and RemoveFromActiveSystemList are "
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
bool RakPeer::AllowIncomingConnections(void) const { return GetNumberOfRemoteInitiatedConnections() < GetMaximumIncomingConnections(); }
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void RakPeer::SendBuffered(const char *data, BitSize_t numberOfBitsToSend, PacketPriority priority, PacketReliability reliability, char orderingChannel,
...
//			int debugIndex = GetRemoteSystemIndex(systemIdentifier.systemAddress);
    ...
    for (sendListIndex = 0; sendListIndex < sendListSize; sendListIndex++) {
        ...
        remoteSystemList[sendList[sendListIndex]].reliabilityLayer.Send(data, numberOfBitsToSend, priority, reliability, orderingChannel, useData == false,
                                                                        remoteSystemList[sendList[sendListIndex]].MTUSize, currentTime, receipt);
        if (useData)
            callerDataAllocationUsed = true;

        if (reliability == RELIABLE || reliability == RELIABLE_ORDERED || reliability == RELIABLE_SEQUENCED || reliability == RELIABLE_WITH_ACK_RECEIPT ||
    ...
    // Return value only meaningful if true was passed for useCallerDataAllocation.  Means the reliability layer used that data copy, so the caller should not
    // deallocate it
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
namespace RakNet {
bool ProcessOfflineNetworkPacket(SystemAddress systemAddress, const char *data, const int length, RakPeer *rakPeer, RakNetSocket2 *rakNetSocket,
    ...
    if (length <= 2) {
        ...
        *isOfflineMessage =
            memcmp(data + sizeof(unsigned char) + sizeof(RakNet::Time) + RakNetGUID::size(), OFFLINE_MESSAGE_DATA_ID, sizeof(OFFLINE_MESSAGE_DATA_ID)) == 0;
    ...
    if (*isOfflineMessage) {
        ...
        // UNCONNECTED MESSAGE Pong with no data.
            ...
            packet = rakPeer->AllocPacket(
                (unsigned int)(length - sizeof(OFFLINE_MESSAGE_DATA_ID) - RakNetGUID::size() - sizeof(RakNet::Time) + sizeof(RakNet::TimeMS)), _FILE_AND_LINE_);
            ...
            bsOut.Write(pingMS);
            bsOut.WriteAlignedBytes((const unsigned char *)data + sizeof(unsigned char) + sizeof(RakNet::Time) + RakNetGUID::size() +
       ...
       // Message does not contain a challenge
                    ...
                    // SocketLayer::SendTo( rakPeer->socketList[rcs->socketIndex], (const char*) bsOut.GetData(), bsOut.GetNumberOfBytesUsed(),
                    // rcs->systemAddress, _FILE_AND_LINE_ );
            ...
            for (i = 0; i < rakPeer->requestedConnectionQueue.Size(); i++) {
                ...
                if (rcs->systemAddress == systemAddress) {
                    ...
                    if (remoteSystem == 0) {
                        if (rcs->socket == 0) {
                            remoteSystem =
                                rakPeer->AssignSystemAddressToRemoteSystemList(systemAddress, RakPeer::RemoteSystemStruct::UNVERIFIED_SENDER, rakNetSocket,
                                                                               &thisIPConnectedRecently, bindingAddress, mtu, guid, doSecurity);
                        } else {
                            remoteSystem =
                                rakPeer->AssignSystemAddressToRemoteSystemList(systemAddress, RakPeer::RemoteSystemStruct::UNVERIFIED_SENDER, rcs->socket,
                                                                               &thisIPConnectedRecently, bindingAddress, mtu, guid, doSecurity);
                        }
                    }
                        ...
                        if (remoteSystem) {
                            ...
                            if (rcs->client_handshake) {
                                ...
                                if (rcs->publicKeyMode == PKM_USE_TWO_WAY_AUTHENTICATION) {
                                    if (!rcs->client_handshake->ProcessAnswerWithIdentity(answer, ident,
                            ...
                            rakPeer->SendImmediate((char *)temp.GetData(), temp.GetNumberOfBitsUsed(), IMMEDIATE_PRIORITY, RELIABLE, 0, systemAddress, false,
                                                   false, timeRead, 0);
                        } else {
            ...
            return true;
                   ...
                   (unsigned char)(data)[0] == (MessageID)ID_NO_FREE_INCOMING_CONNECTIONS || (unsigned char)(data)[0] == (MessageID)ID_CONNECTION_BANNED ||
                   (unsigned char)(data)[0] == (MessageID)ID_ALREADY_CONNECTED || (unsigned char)(data)[0] == (MessageID)ID_INVALID_PASSWORD ||
                   (unsigned char)(data)[0] == (MessageID)ID_IP_RECENTLY_CONNECTED || (unsigned char)(data)[0] == (MessageID)ID_INCOMPATIBLE_PROTOCOL_VERSION) {
            ...
            if (IPAddrInUse & GUIDInUse) {
                if (rssFromSA == rssFromGuid && rssFromSA->connectMode == RakPeer::RemoteSystemStruct::UNVERIFIED_SENDER) {
                    ...
                    // Note to self: If REQUESTED_CONNECTION, this means two systems attempted to connect to each other at the same time, and one finished
                    // first. Returns ID)_CONNECTION_REQUEST_ACCEPTED to one system, and ID_ALREADY_CONNECTED followed by ID_NEW_INCOMING_CONNECTION to another
                } else {
...
void ProcessNetworkPacket(SystemAddress systemAddress, const char *data, const int length, RakPeer *rakPeer, RakNetSocket2 *rakNetSocket,
    ...
    if (remoteSystem) {
        ...
        if (isOfflineMessage == false) {
            remoteSystem->reliabilityLayer.HandleSocketReceiveFromConnectedPlayer(data, length, systemAddress, rakPeer->pluginListNTS, remoteSystem->MTUSize,
...
bool RakPeer::RunRecvFromOnce( RakNetSocket *s )
{
        ...
        {
                recvFromStruct->s=s;
                SocketLayer::RecvFromBlocking(s, this, recvFromStruct->data, &recvFromStruct->bytesRead, &recvFromStruct->systemAddress,
...
bool RakPeer::RunUpdateCycle(BitStream &updateBitStream) {
    ...
    while ((recvFromStruct = PopBufferedPacket()) != 0) {
        ...
        if (socketListIndex!=socketList.Size())
        */
        ProcessNetworkPacket(recvFromStruct->systemAddress, recvFromStruct->data, recvFromStruct->bytesRead, this, recvFromStruct->socket,
        ...
        while (requestedConnectionQueueIndex < requestedConnectionQueue.Size()) {
            ...
            if (rcs->nextRequestTime < timeMS) {
                ...
                if (condition1 || condition2) {
                    ...
                    // if (SocketLayer::SendTo( socketToUse, (const char*) bitStream.GetData(), bitStream.GetNumberOfBytesUsed(), rcs->systemAddress,
                    // _FILE_AND_LINE_ )==-10040)
                    {
    ...
    // for ( remoteSystemIndex = 0; remoteSystemIndex < remoteSystemListSize; ++remoteSystemIndex )
    {
        ...
        if (timeMS > remoteSystem->lastReliableSend && timeMS - remoteSystem->lastReliableSend > remoteSystem->reliabilityLayer.GetTimeoutTime() / 2 &&
        ...
        remoteSystem->reliabilityLayer.Update(remoteSystem->rakNetSocket, systemAddress, remoteSystem->MTUSize, timeNS, maxOutgoingBPS, pluginListNTS, &rnr,
        ...
        if (remoteSystem->reliabilityLayer.IsDeadConnection() ||
            ((remoteSystem->connectMode == RemoteSystemStruct::DISCONNECT_ASAP || remoteSystem->connectMode == RemoteSystemStruct::DISCONNECT_ASAP_SILENTLY) &&
        ...
        while (bitSize > 0) {
            ...
            if (remoteSystem->connectMode == RemoteSystemStruct::UNVERIFIED_SENDER) {
                ...
                if ((unsigned char)(data)[0] == ID_CONNECTION_REQUEST) {
                    ...
                    // ((remoteSystem->connectMode!=RemoteSystemStruct::CONNECTED && time > remoteSystem->connectionTime && time - remoteSystem->connectionTime
                    // > 10000))
                    if (remoteSystem->connectMode == RemoteSystemStruct::REQUESTED_CONNECTION) {
                        ParseConnectionRequestPacket(remoteSystem, systemAddress, (const char *)data, byteSize);
                    } else {
                    ...
                    if (remoteSystem->connectMode == RemoteSystemStruct::HANDLING_CONNECTION_REQUEST) {
                        ...
                        // Bug: If A connects to B through R, A's firstExternalID is set to R. If A tries to send to R, sends to loopback because
                        // R==firstExternalID Correct fix is to specify in Connect() if target is through a proxy. However, in practice you have to connect to
                        // something else first anyway to know about the proxy. So setting once only is good enough
                    ...
                    SendImmediate((char *)outBitStream.GetData(), outBitStream.GetNumberOfBitsUsed(), IMMEDIATE_PRIORITY, UNRELIABLE, 0, systemAddress, false,
                                  false, RakNet::GetTimeUS(), 0);
                    ...
                    if (byteSize > sizeof(MessageID) + sizeof(unsigned int) + sizeof(unsigned short) + sizeof(SystemIndex) + sizeof(RakNet::Time) * 2) {
                        ...
                        if (allowConnection) {
                            ...
                            // Bug: If A connects to B through R, A's firstExternalID is set to R. If A tries to send to R, sends to loopback because
                            // R==firstExternalID Correct fix is to specify in Connect() if target is through a proxy. However, in practice you have to connect
                            // to something else first anyway to know about the proxy. So setting once only is good enough
                            if (firstExternalID == UNASSIGNED_SYSTEM_ADDRESS) {
                    ...
                    if ((data[0] >= (MessageID)ID_TIMESTAMP || data[0] == ID_SND_RECEIPT_ACKED || data[0] == ID_SND_RECEIPT_LOSS) && remoteSystem->isActive) {
                        packet = AllocPacket(byteSize, data, _FILE_AND_LINE_);
                        packet->bitSize = bitSize;
                        packet->systemAddress = systemAddress;
                        packet->systemAddress.systemIndex = remoteSystem->remoteSystemIndex;
