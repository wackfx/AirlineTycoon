...
// bool operator<( const DataStructures::MLKeyRef<UDPProxyClient::ServerWithPing> &inputKey, const UDPProxyClient::ServerWithPing &cls ) {return
// inputKey.Get().serverAddress < cls.serverAddress;} bool operator>( const DataStructures::MLKeyRef<UDPProxyClient::ServerWithPing> &inputKey, const
// UDPProxyClient::ServerWithPing &cls ) {return inputKey.Get().serverAddress > cls.serverAddress;} bool operator==( const
// DataStructures::MLKeyRef<UDPProxyClient::ServerWithPing> &inputKey, const UDPProxyClient::ServerWithPing &cls ) {return inputKey.Get().serverAddress ==
// cls.serverAddress;}
...
bool UDPProxyClient::RequestForwarding(SystemAddress proxyCoordinator, SystemAddress sourceAddress, RakNetGUID targetGuid, RakNet::TimeMS timeoutOnNoDataMS,
...
bool UDPProxyClient::RequestForwarding(SystemAddress proxyCoordinator, SystemAddress sourceAddress, SystemAddress targetAddressAsSeenFromCoordinator,
...
void UDPProxyClient::Update(void) {
    ...
    while (idx1 < pingServerGroups.Size()) {
        ...
        if (psg->serversToPing.Size() > 0 && RakNet::GetTimeMS() > psg->startPingTime + DEFAULT_UNRESPONSIVE_PING_TIME_COORDINATOR) {
        ...
        case ID_UDP_PROXY_FORWARDING_NOTIFICATION: {
            ...
            case ID_UDP_PROXY_IN_PROGRESS: {
                ...
                if (packet->data[1] == ID_UDP_PROXY_FORWARDING_SUCCEEDED) {
                    if (resultHandler)
                        resultHandler->OnForwardingSuccess(serverIP.C_String(), forwardingPort, packet->systemAddress, senderAddress, targetAddress, targetGuid,
                                                           this);
                } else if (packet->data[1] == ID_UDP_PROXY_IN_PROGRESS) {
                    if (resultHandler)
                        resultHandler->OnForwardingInProgress(serverIP.C_String(), forwardingPort, packet->systemAddress, senderAddress, targetAddress,
                                                              targetGuid, this);
                } else {
                    ...
                    if (resultHandler)
                        resultHandler->OnForwardingNotification(serverIP.C_String(), forwardingPort, packet->systemAddress, senderAddress, targetAddress,
                                                                targetGuid, this);
                }
            } break;
            case ID_UDP_PROXY_ALL_SERVERS_BUSY:
