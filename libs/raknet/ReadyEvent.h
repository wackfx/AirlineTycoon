...
/// \file
/// \brief Ready event plugin.  This enables a set of systems to create a signal event, set this signal as ready or unready, and to trigger the event when all
/// systems are ready
///
...
/// This plugin uses two stages to solve that problem, first, everyone I know about is ready. Second, everyone I know about is ready to everyone they know
/// about.<BR> The user will get ID_READY_EVENT_SET and ID_READY_EVENT_UNSET as the signal flag is set or unset<BR> The user will get ID_READY_EVENT_ALL_SET
/// when all systems are done waiting for all other systems, in which case the event is considered complete, and no longer tracked.<BR> \sa FullyConnectedMesh2
/// \ingroup READY_EVENT_GROUP
class ReadyEvent : public PluginInterface2 {
  public:
    ...
    /// When systems can call SetEvent() with isReady==false, it is possible for one system to return true from IsEventCompleted() while the other systems
    /// return false This can occur if a system SetEvent() with isReady==false while the completion message is still being transmitted. If your game has the
    /// situation where some action should be taken on all systems when IsEventCompleted() is true for any system, then call ForceCompletion() when the action
    /// begins. This will force all systems to return true from IsEventCompleted(). \param[in] eventId A user-defined identifier to immediately set as completed
    ...
    /// If you are waiting for a specific number of systems (such as players later connecting), also check GetRemoteWaitListSize(eventId) to be equal to 1 less
    /// than the total number of participants. \param[in] eventId A user-defined identifier \return True on completion. False (failure) on unknown eventId, or
    /// the set is not completed.
    ...
    /// \param[in] guid An address to wait for event replies from.  Pass UNASSIGNED_SYSTEM_ADDRESS for all currently connected systems. Until all systems in
    /// this list have called SetEvent with this ID and true, and have this system in the list, we won't get ID_READY_EVENT_COMPLETE \return True on success,
    /// false on unknown eventId (this should be considered an error)
