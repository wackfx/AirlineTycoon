...
bool NatPunchthroughServer::User::HasConnectionAttemptToUser(User *user) {
    ...
    for (index = 0; index < connectionAttempts.Size(); index++) {
        if (connectionAttempts[index]->recipient->guid == user->guid || connectionAttempts[index]->sender->guid == user->guid)
            return true;
    }
    return false;
}
...
void NatPunchthroughServer::SetDebugInterface(NatPunchthroughServerDebugInterface *i) { natPunchthroughServerDebugInterface = i; }
    ...
    if (time > lastUpdate + 250) {
        ...
        for (i = 0; i < users.Size(); i++) {
            ...
            for (j = 0; j < user->connectionAttempts.Size(); j++) {
                connectionAttempt = user->connectionAttempts[j];
                if (connectionAttempt->sender == user) {
                    if (connectionAttempt->attemptPhase != ConnectionAttempt::NAT_ATTEMPT_PHASE_NOT_STARTED && time > connectionAttempt->startTime &&
...
PluginReceiveResult NatPunchthroughServer::OnReceive(Packet *packet) {
    ...
    case ID_OUT_OF_BAND_INTERNAL:
        if (packet->length >= 2 && packet->data[1] == ID_NAT_PING) {
            ...
            rakPeerInterface->SendOutOfBand((const char *)packet->systemAddress.ToString(false), packet->systemAddress.GetPort(),
...
void NatPunchthroughServer::OnGetMostRecentPort(Packet *packet) {
    ...
    if (natPunchthroughServerDebugInterface) {
        ...
        log = RakNet::RakString("Got ID_NAT_GET_MOST_RECENT_PORT from systemAddress %s guid %s. port=%i. sessionId=%i. userFound=%i.", addr1, addr2,
                                mostRecentPort, sessionId, objectExists);
