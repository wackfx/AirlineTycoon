...
/// To use:<BR>
/// <OL>
/// <LI>Derive from Connection_RM3 and implement Connection_RM3::AllocReplica(). This is a factory function where given a user-supplied identifier for a class
/// (such as name) return an instance of that class. Should be able to return any networked object in your game. <LI>Derive from ReplicaManager3 and implement
/// AllocConnection() and DeallocConnection() to return the class you created in step 1. <LI>Derive your networked game objects from Replica3. All pure virtuals
/// have to be implemented, however defaults are provided for Replica3::QueryConstruction(), Replica3::QueryRemoteConstruction(), and
/// Replica3::QuerySerialization() depending on your network architecture. <LI>When a new game object is created on the local system, pass it to
/// ReplicaManager3::Reference(). <LI>When a game object is destroyed on the local system, and you want other systems to know about it, call
...
class RAK_DLL_EXPORT ReplicaManager3 : public PluginInterface2 {
  public:
    ...
    /// It is used if autoCreate is true via SetAutoManageConnections() (true by default). Otherwise, the function is not called, and you will have to call
    /// PushConnection() manually<BR> \note If you do not want a new network connection to immediately download game objects, SetAutoManageConnections() and
    /// PushConnection() are how you do this. \sa SetAutoManageConnections() \param[in] systemAddress Address of the system you are adding \param[in] rakNetGUID
    /// GUID of the system you are adding. See Packet::rakNetGUID or RakPeerInterface::GetGUIDFromSystemAddress() \return The new connection instance.
    ...
    /// It is used if autoDestroy is true via SetAutoManageConnections() (true by default). Otherwise, the function is not called and you would then be
    /// responsible for deleting your own connection objects. \param[in] connection The pointer instance to delete
    ...
    /// If you want to delay deleting a connection that has dropped, set \a autoDestroy to false. If you do this, then you must call PopConnection() to remove
    /// that connection from being internally tracked. You'll also have to delete the connection instance on your own.<BR> \param[in] autoCreate Automatically
    /// call ReplicaManager3::AllocConnection() for each new connection. Defaults to true. Also see AutoCreateConnectionList() \param[in] autoDestroy
    /// Automatically call ReplicaManager3::DeallocConnection() for each dropped connection. Defaults to true.
    ...
    void AutoCreateConnectionList(DataStructures::List<RakNetGUID> &participantListIn, DataStructures::List<Connection_RM3 *> &participantListOut,
                                  WorldId worldId = 0);
    ...
    /// \details On call, for each replica returned by GetReplicasCreatedByGuid(), QueryActionOnPopConnection() will be called. Depending on the return value,
    /// this may delete the corresponding replica.<BR> If autoDestroy is true in the call to SetAutoManageConnections() (true by default) then this is called
    /// automatically when the connection is lost. In that case, the returned connection instance is deleted.<BR> \param[in] guid of the connection to get.
    /// Passed to ReplicaManager3::AllocConnection() originally. \param[in] worldId Used for multiple worlds. World 0 is created automatically by default. See
    /// AddWorld()
    ...
    /// Remote systems already connected will potentially download this object the next time ReplicaManager3::Update() is called, which happens every time you
    /// call RakPeerInterface::Receive().<BR> You can also call ReplicaManager3::Update() manually to send referenced objects right away \param[in] replica3 The
    /// object to start tracking \param[in] worldId Used for multiple worlds. World 0 is created automatically by default. See AddWorld()
    ...
    /// This is called automatically from the destructor of Replica3, so you don't need to call it manually unless you want to stop tracking an object before it
    /// is destroyed. \param[in] replica3 The object to stop tracking \param[in] worldId Used for multiple worlds. World 0 is created automatically by default.
    /// See AddWorld()
    ...
    /// \details Originally created is defined as the value of Replica3::creatingSystemGUID, which is automatically assigned in
    /// ReplicaManager3::Reference().<BR> You do not have to be directly connected to that system to get the objects originally created by that system.<BR>
    ...
    /// \details This can be wrong, for example if that system locally deleted the outside the scope of ReplicaManager3, if QueryRemoteConstruction() returned
    /// false, or if DeserializeConstruction() returned false. \param[in] replica The replica to check against. \param[in] worldId Used for multiple worlds.
    /// World 0 is created automatically by default. See AddWorld() \param[out] connectionsThatHaveConstructedThisReplica Populated with connection instances
    /// that we believe have \a replica allocated
    ...
    /// A world with id 0 is created automatically. If you want multiple worlds, use this function, and ReplicaManager3::SetNetworkIDManager() to have a
    /// different NetworkIDManager instance per world \param[in] worldId A unique identifier for this world. User-defined
    ...
    /// \details WorldId 0 is created by default. Worlds will not necessarily be in the order added with AddWorld(). Edit RemoveWorld() changing
    /// RemoveAtIndexFast() to RemoveAtIndex() to preserve order. \param[in] index A value between 0 and GetWorldCount()-1 \return One of the WorldId values
    /// added with AddWorld()
  ...
  protected:
    ...
    PluginReceiveResult OnConstruction(Packet *packet, unsigned char *packetData, int packetDataLength, RakNetGUID senderGuid, unsigned char packetDataOffset,
                                       WorldId worldId);
    PluginReceiveResult OnSerialize(Packet *packet, unsigned char *packetData, int packetDataLength, RakNetGUID senderGuid, RakNet::Time timestamp,
                                    unsigned char packetDataOffset, WorldId worldId);
    PluginReceiveResult OnDownloadStarted(Packet *packet, unsigned char *packetData, int packetDataLength, RakNetGUID senderGuid,
                                          unsigned char packetDataOffset, WorldId worldId);
    PluginReceiveResult OnDownloadComplete(Packet *packet, unsigned char *packetData, int packetDataLength, RakNetGUID senderGuid,
                                           unsigned char packetDataOffset, WorldId worldId);
...
/// Represents the serialized data for an object the last time it was sent. Used by Connection_RM3::OnAutoserializeInterval() and
/// Connection_RM3::SendSerializeIfChanged() \ingroup REPLICA_MANAGER_GROUP3
...
/// \details Important function: AllocReplica() - must be overridden to create an object given an identifier for that object, which you define for all objects
/// in your game \ingroup REPLICA_MANAGER_GROUP3
class RAK_DLL_EXPORT Connection_RM3 {
  public:
    ...
    /// Return 0 if unable to create the intended object. Note, in that case the other system will still think we have the object and will try to serialize
    /// object updates to us. Generally, you should not send objects the other system cannot create.<BR> \sa Replica3::WriteAllocationID(). Sample
    /// implementation:<BR> {RakNet::RakString typeName; allocationIdBitstream->Read(typeName); if (typeName=="Soldier") return new Soldier; return 0;}<BR>
    ...
    enum ConstructionMode {
        ...
        /// For every object that does not exist on the remote system, call Replica3::QueryConstruction() every tick. Based on the call, the object may be sent
        /// to the other system. For every object that does exist on the remote system, call Replica3::QueryDestruction() every tick. Based on the call, the
        /// object may be deleted on the other system. Do not call Connection_RM3::QueryReplicaList()
    ...
    /// This enables the application to show a downloading splash screen on ID_REPLICA_MANAGER_DOWNLOAD_STARTED, a progress bar, and to close the splash screen
    /// and activate all objects on ID_REPLICA_MANAGER_DOWNLOAD_COMPLETE However, if the application was not set up for this then it would result in incomplete
    /// objects spread out over time, and cause problems If you return true from QueryGroupDownloadMessages(), then these messages will be returned all in one
    /// tick, returned only when the download is complete \note ID_REPLICA_MANAGER_DOWNLOAD_STARTED calls the callback DeserializeOnDownloadStarted() \note
    ...
    /// \details The default of calling QueryConstruction for every known object is easy to use, but not efficient, especially for large worlds where many
    /// objects are outside of the player's circle of influence.<BR> QueryDestruction is also not necessarily useful or efficient, as object destruction tends
    /// to happen in known cases, and can be accomplished by calling Replica3::BroadcastDestruction() QueryConstructionMode() allows you to specify more
    /// efficient algorithms than the default when overriden. \return How to get the list of objects that exist on the remote system. You should always return
    /// the same value for a given connection
    ...
    /// \details This advantage of this callback is if that there are many objects that a particular connection does not have, then we do not have to iterate
    /// through those objects calling QueryConstruction() for each of them.<BR>
    ///< BR>
    ...
    /// Example use case:
    /// We have more data to send then the bandwidth supports, so want to prioritize sends. For example enemies shooting are more important than animation
    /// effects When QuerySerializationList(), sort objects by priority, and write the list to \a replicasToSerialize, optionally skipping objects with a lower
    /// serialization frequency If you hit your bandwidth limit when checking SerializeParameters::bitsWrittenSoFar, you can return RM3SR_DO_NOT_SERIALIZE for
    /// all remaining items \note Only replicas written to replicasToSerialize are transmitted. Even if you returned RM3SR_SERIALIZED_ALWAYS a prior
    /// ReplicaManager3::Update() cycle, the replica will not be transmitted if it is not in replicasToSerialize \note If you do not know what objects are
    /// candidates for serialization, you can use queryToSerializeReplicaList as a source for your filtering or sorting operations \param[in]
    /// replicasToSerialize List of replicas to call QuerySerialization() on \return Return true to use replicasToSerialize (replicasToSerialize may be empty if
    /// desired). Otherwise return false.
    ...
    virtual SendSerializeIfChangedResult SendSerialize(RakNet::Replica3 *replica, bool indicesToSend[RM3_NUM_OUTPUT_BITSTREAM_CHANNELS],
                                                       RakNet::BitStream serializationData[RM3_NUM_OUTPUT_BITSTREAM_CHANNELS], RakNet::Time timestamp,
                                                       PRO sendParameters[RM3_NUM_OUTPUT_BITSTREAM_CHANNELS], RakNet::RakPeerInterface *rakPeer,
                                                       unsigned char worldId, RakNet::Time curTime);

    /// \internal
    /// \details Calls Connection_RM3::SendSerialize() if Replica3::Serialize() returns a different result than what is contained in \a
    /// lastSerializationResult.<BR> Used by autoserialization in Connection_RM3::OnAutoserializeInterval() \param[in] lsr Item in the
    /// queryToSerializeReplicaList \param[in] sp Controlling parameters over the serialization \param[in] rakPeer Instance of RakPeerInterface to send on
    /// \param[in] worldId Which world, see ReplicaManager3::AddWorld()
    /// \param[in] curTime The current time
    virtual SendSerializeIfChangedResult SendSerializeIfChanged(LastSerializationResult *lsr, SerializeParameters *sp, RakNet::RakPeerInterface *rakPeer,
                                                                unsigned char worldId, ReplicaManager3 *replicaManager, RakNet::Time curTime);
    ...
    virtual void SendConstruction(DataStructures::List<Replica3 *> &newObjects, DataStructures::List<Replica3 *> &deletedObjects, PRO sendParameters,
                                  RakNet::RakPeerInterface *rakPeer, unsigned char worldId, ReplicaManager3 *replicaManager3);
...
enum RM3SerializationResult {
    ...
    /// Can send unique data per connection if desired. If same data is sent to all connections, use RM3SR_SERIALIZED_ALWAYS_IDENTICALLY for even better
    /// performance Efficient for memory and speed, but not necessarily bandwidth
...
class RAK_DLL_EXPORT Replica3 : public NetworkIDObject {
  public:
    ...
    /// \details If ReplicaManager3::QueryConstructionMode() returns QUERY_CONNECTION_FOR_REPLICA_LIST or QUERY_REPLICA_FOR_CONSTRUCTION_AND_DESTRUCTION
    /// (default), then QueyrConstruction() is called once per tick from ReplicaManager3::Update() to determine if an object should exist on a given system.<BR>
    ...
    /// \note Defaults are provided: QueryConstruction_PeerToPeer(), QueryConstruction_ServerConstruction(), QueryConstruction_ClientConstruction(). Return one
    /// of these functions for a working default for the relevant topology. \param[in] destinationConnection Which system we will send to \param[in]
    /// replicaManager3 Plugin instance for this Replica3 \return What action to take
    ...
    /// then QueryDestruction() is called once per tick from ReplicaManager3::Update() to determine if an object that exists on a remote system should be
    /// destroyed for a given system.<BR> Based on the return value, a network message may be sent to the other system to destroy the object.<BR> Note that you
    /// can also destroy objects with BroadcastDestruction(), so this function is not useful unless you plan to delete objects for only a particular
    /// connection.<BR> If QueryConstructionMode() is overriden to return QUERY_CONNECTION_FOR_REPLICA_LIST, this function is unused.<BR> \param[in]
    /// destinationConnection Which system we will send to \param[in] replicaManager3 Plugin instance for this Replica3 \return What action to take. Only
    /// RM3CS_SEND_DESTRUCTION does anything at this time.
    ...
    /// \details By default, QueryRemoteConstruction_ServerConstruction() does not allow clients to create objects. The client will get
    /// Replica3::DeserializeConstructionRequestRejected().<BR> If you want the client to be able to potentially create objects for client/server, override
    /// accordingly.<BR> Other variants of QueryRemoteConstruction_* just return true. \note Defaults are provided: QueryRemoteConstruction_PeerToPeer(),
    /// QueryRemoteConstruction_ServerConstruction(), QueryRemoteConstruction_ClientConstruction(). Return one of these functions for a working default for the
    /// relevant topology. \param[in] sourceConnection Which system sent us the object creation request message. \return True to allow the object to pass onto
    /// DeserializeConstruction() (where it may also be rejected), false to immediately reject the remote construction request
    ...
    /// \details SerializeConstruction is used to write out data that you need to create this object in the context of your game, such as health, score, name.
    /// Use it for data you only need to send when the object is created.<BR> After SerializeConstruction() is called, Serialize() will be called immediately
    /// thereafter. However, they are sent in different messages, so Serialize() may arrive a later frame than SerializeConstruction() For that reason, the
    /// object should be valid after a call to DeserializeConstruction() for at least a short time.<BR> \note The object's NetworkID and allocation id are
    /// handled by the system automatically, you do not need to write these values to \a constructionBitstream \param[out] constructionBitstream Destination
    /// bitstream to write your data to \param[in] destinationConnection System that will receive this network message.
    ...
    /// <LI>ReplicaManager3::SetAutoManageConnections() was called autoDestroy true (which is the default setting), and a remote system that owns this object
    /// disconnected) OR
    /// <\OL>
    /// <BR>
    /// Override with {delete this;} to actually delete the object (and any other processing you wish).<BR>
    /// If you don't want to delete the object, just do nothing, however, the system will not know this. You may wish to call Dereference() if the object should
    /// no longer be networked, but remain in memory. You are responsible for deleting it yoruself later.<BR> destructionBitstream may be 0 if the object was
    /// deleted locally
    ...
    /// \details QuerySerialization() is a first pass query to check if a given object should serializable to a given system. The intent is that the user
    /// implements with one of the defaults for client, server, or peer to peer.<BR> Without this function, a careless implementation would serialize an object
    /// anytime it changed to all systems. This would give you feedback loops as the sender gets the same message back from the recipient it just sent to.<BR>
    /// If more than one system can serialize the same object then you will need to override to return true, and control the serialization result from
    /// Replica3::Serialize(). Be careful not to send back the same data to the system that just sent to you! \return True to allow calling
    /// Replica3::Serialize() for this connection, false to not call.
    ...
    virtual void OnSerializeTransmission(RakNet::BitStream *bitStream, RakNet::Connection_RM3 *destinationConnection,
                                         BitSize_t bitsPerChannel[RM3_NUM_OUTPUT_BITSTREAM_CHANNELS], RakNet::Time curTime) {
        (void)bitStream;
        (void)destinationConnection;
        (void)bitsPerChannel;
        (void)curTime;
    ...
    /// \param[in] destinationConnection destinationConnection parameter passed to QueryConstruction()
    /// \param[in] p2pMode If controlled only by this system ever, pass R3P2PM_SINGLE_OWNER. Otherwise pass R3P2PM_MULTI_OWNER_CURRENTLY_AUTHORITATIVE or
    /// R3P2PM_MULTI_OWNER_NOT_CURRENTLY_AUTHORITATIVE
    ...
    /// \details Use if the values you are serializing are generated by the client that owns the object. The serialization will be relayed through the server to
    /// the other clients. \param[in] destinationConnection destinationConnection parameter passed to QueryConstruction() \param[in] isThisTheServer True if
    /// this system is the server, false if not.
    ...
    /// \brief Default call for QuerySerialization().
    /// \details Use if the values you are serializing are generated only by the server. The serialization will be sent to all clients, but the clients will not
    /// send back to the server. \param[in] destinationConnection destinationConnection parameter passed to QueryConstruction() \param[in] isThisTheServer True
    ...
    /// \brief Default call for QuerySerialization().
    /// \details Use if the values you are serializing are on a peer to peer network. The peer that owns the object will send to all. Remote peers will not
    /// send. \param[in] destinationConnection destinationConnection parameter passed to QueryConstruction() \param[in] p2pMode If controlled only by this
    /// system ever, pass R3P2PM_SINGLE_OWNER. Otherwise pass R3P2PM_MULTI_OWNER_CURRENTLY_AUTHORITATIVE or R3P2PM_MULTI_OWNER_NOT_CURRENTLY_AUTHORITATIVE
...
template <class parent_type> class RAK_DLL_EXPORT Replica3Composite : public Replica3 {
  protected:
  ...
  public:
    ...
    virtual void OnSerializeTransmission(RakNet::BitStream *bitStream, RakNet::Connection_RM3 *destinationConnection,
                                         RakNet::BitSize_t bitsPerChannel[RakNet::RM3_NUM_OUTPUT_BITSTREAM_CHANNELS], RakNet::Time curTime) {
