...
/// <LI>Each datagram containing a user message has a sequence number which is set after calling OnSendBytes(). Set it by calling
/// GetAndIncrementNextDatagramSequenceNumber() <LI>System is designed to be used from a single thread. <LI>Each packet should have a timeout time based on
/// GetSenderRTOForACK(). If this time elapses, add the packet to the head of the send list for retransmission.
/// </OL>
///
/// Recommended:
/// <OL>
...
/// <LI>When you want to send or resend data, call GetNumberOfBytesToSend(). It will return you enough bytes to keep you busy for \a estimatedTimeToNextTick.
/// You can send more than this to fill out a datagram, or to send packet pairs <LI>Call OnSendBytes() when sending datagrams. <LI>When data arrives, record the
/// sequence number and buffer an ACK for it, to be sent from Update() if ShouldSendACKs() returns true <LI>Every 16 packets that you send, send two of them
/// back to back (a packet pair) as long as both packets are the same size. If you don't have two packets the same size, it is fine to defer this until you do.
/// <LI>When you get a packet, call OnGotPacket(). If the packet is also either of a packet pair, call OnGotPacketPair()
/// <LI>If you get a packet, and the sequence number is not 1 + the last sequence number, send a NAK. On the remote system, call OnNAK() and resend that
/// message. <LI>If you get an ACK, remove that message from retransmission. Call OnNonDuplicateAck(). <LI>If a message is not ACKed for
...
class CCRakNetUDT {
  public:
    ...
    bool OnGotPacket(DatagramSequenceNumberType datagramSequenceNumber, bool isContinuousSend, CCTimeType curTime, uint32_t sizeInBytes,
    ...
    /// If the time difference between when a message was last transmitted, and the current time is greater than RTO then packet is eligible for retransmission,
    /// pending congestion control RTO = (RTT + 4 * RTTVar) + SYN If we have been continuously sending for the last RTO, and no ACK or NAK at all, SND*=2; This
    /// is per message, which is different from UDT, but RakNet supports packetloss with continuing data where UDT is only RELIABLE_ORDERED Minimum value is 100
    /// milliseconds
  ...
  protected:
    ...
    /// However, if this would result in an immediate update yet again, it is set to SYN microseconds past the current time (in case the thread did not update
    /// for a long time)
    ...
    static BytesPerMicrosecond CalculateListMedianRecursive(const BytesPerMicrosecond inputList[CC_RAKNET_UDT_PACKET_HISTORY_LENGTH], int inputListLength,
