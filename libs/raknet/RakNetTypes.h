...
enum PublicKeyMode {
    ...
    /// I recommend not using this mode except for server-to-server communication as it significantly increases the CPU requirements during connections for both
    /// sides. Furthermore, when it is used, a connection password should be used as well to avoid DoS attacks.
...
struct RAK_DLL_EXPORT SocketDescriptor {
    ...
    /// IPV6 is the newer internet protocol. Instead of addresses such as natpunch.jenkinssoftware.com, you may have an address such as
    /// fe80::7c:31f7:fec4:27de%14. Encoding takes 16 bytes instead of 4, so IPV6 is less efficient for bandwidth. On the positive side, NAT Punchthrough is not
    /// needed and should not be used with IPV6 because there are enough addresses that routers do not need to create address mappings. RakPeer::Startup() will
    /// fail if this IP version is not supported. \pre RAKNET_SUPPORT_IPV6 must be set to 1 in RakNetDefines.h for AF_INET6
    short socketFamily;
...
/// This is not necessarily a unique identifier. For example, if a system has both LAN and internet connections, the system may be identified by either one,
/// depending on who is communicating<BR> Therefore, you should not transmit the SystemAddress over the network and expect it to identify a system, or use it to
/// connect to that system, except in the case where that system is not behind a NAT (such as with a dedciated server) Use RakNetGUID for a unique per-instance
/// of RakPeer to identify systems
struct RAK_DLL_EXPORT SystemAddress {
    ...
    /// \param[in] portDelineator if \a str contains a port, delineate the port with this character. portDelineator should not be '.', ':', '%', '-', '/', a
    /// number, or a-f \param[in] ipVersion Only used if str is a pre-defined address in the wrong format, such as 127.0.0.1 but you want ip version 6, so you
    /// can pass 6 here to do the conversion \note The current port is unchanged if a port is not specified in \a str \return True on success, false on
    /// ipVersion does not match type of passed string
...
/// Uniquely identifies an instance of RakPeer. Use RakPeer::GetGuidFromSystemAddress() and RakPeer::GetSystemAddressFromGuid() to go between SystemAddress and
/// RakNetGUID Use RakPeer::GetGuidFromSystemAddress(UNASSIGNED_SYSTEM_ADDRESS) to get your own GUID
