...
void BitStream::Write(BitStream *bitStream) { Write(bitStream, bitStream->GetNumberOfBitsUsed() - bitStream->GetReadOffset()); }
...
void BitStream::WriteBits(const unsigned char *inByteArray, BitSize_t numberOfBitsToWrite, const bool rightAlignedBits) {
    ...
    // do
    {
        ...
        if (numberOfBitsToWrite < 8 && rightAlignedBits) // rightAlignedBits means in the case of a partial byte, the bits are aligned from the right (bit 0)
                                                         // rather than the left (as in the normal internal representation)
...
void BitStream::WriteCompressed(const unsigned char *inByteArray, const unsigned int size, const bool unsignedData) {
    ...
    if ((unsignedData && ((*(inByteArray + currentByte)) & 0xF0) == 0x00) || (unsignedData == false && ((*(inByteArray + currentByte)) & 0xF0) == 0xF0)) {
        bool b = true;
        Write(b);
...
bool BitStream::ReadCompressed(unsigned char *inOutByteArray, const unsigned int size, const bool unsignedData) {
    ...
    // All but the first bytes are byteMatch.  If the upper half of the last byte is a 0 (positive) or 16 (negative) then what we read will be a 1 and the
    // remaining 4 bits. Otherwise we read a 0 and the 8 bytes
...
void BitStream::AddBitsAndReallocate(const BitSize_t numberOfBitsToWrite) {
    ...
    if (numberOfBitsToWrite + numberOfBitsUsed > 0 &&
        ((numberOfBitsAllocated - 1) >> 3) < ((newNumberOfBitsAllocated - 1) >> 3)) // If we need to allocate 1 or more new bytes
    {
#ifdef _DEBUG
