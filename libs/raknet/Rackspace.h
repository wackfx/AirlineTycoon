...
class RAK_DLL_EXPORT Rackspace {
  public:
    ...
    /// \param[in] _authenticationURL See http://docs.rackspacecloud.com/servers/api/v1.0/cs-devguide-20110112.pdf . US-based accounts authenticate through
    /// auth.api.rackspacecloud.com. UK-based accounts authenticate through lon.auth.api.rackspacecloud.com \param[in] _rackspaceCloudUsername Username you
    /// registered with Rackspace on their website \param[in] _apiAccessKey Obtain your API access key from the Rackspace Cloud Control Panel in the Your
    /// Account API Access section. \return The address of the authentication server, or UNASSIGNED_SYSTEM_ADDRESS if the connection attempt failed
    ...
    /// The server takes a while to build. Call GetServerDetails() to get the current build status. Server id to pass to GetServerDetails() is returned in the
    /// field <server ... id="1234"> \sa http://docs.rackspacecloud.com/servers/api/v1.0/cs-devguide-20110112.pdf \sa
    /// RackspaceEventCallback::OnCreateServerResult() \param[in] name Name of the server. Only alphanumeric characters, periods, and hyphens are valid. Server
    /// Name cannot start or end with a period or hyphen. \param[in] imageId Which image (harddrive contents, including OS) to use \param[in] flavorId Which
    /// flavor (hardware config) to use, primarily how much memory is available.
    ...
    /// \param[in] serverId Which server to get details on. You can call ListServers() to get the list of active servers.
    /// \param[in] newName The new server name. Leave blank to leave unchanged. Only alphanumeric characters, periods, and hyphens are valid. Server Name cannot
    /// start or end with a period or hyphen. \param[in] newPassword The new server password. Leave blank to leave unchanged.
