...
void CloudServer::OnGetRequest(Packet *packet) {
    ...
    for (unsigned int filterIndex = 0; filterIndex < queryFilters.Size(); filterIndex++) {
        if (queryFilters[filterIndex]->OnGetRequest(packet->guid, packet->systemAddress, getRequest->cloudQueryWithAddresses.cloudQuery,
    ...
    if (getRequest->cloudQueryWithAddresses.cloudQuery.subscribeToResults) {
        ...
        for (keyIndex = 0; keyIndex < getRequest->cloudQueryWithAddresses.cloudQuery.keys.Size(); keyIndex++) {
            ...
            for (specificSystemIndex = 0; specificSystemIndex < getRequest->cloudQueryWithAddresses.specificSystems.Size(); specificSystemIndex++) {
                keySubscriberId->specificSystemsSubscribedTo.Insert(getRequest->cloudQueryWithAddresses.specificSystems[specificSystemIndex],
                                                                    getRequest->cloudQueryWithAddresses.specificSystems[specificSystemIndex], true,
                                                                    _FILE_AND_LINE_);
            }
                ...
                if (subscribedKeysIndexExists) {
                    ...
                    for (specificSystemIndex = 0; specificSystemIndex < keySubscriberId->specificSystemsSubscribedTo.Size(); specificSystemIndex++) {
                        ...
                        unsigned int keyDataIndex =
                            cloudDataList->keyData.GetIndexFromKey(keySubscriberId->specificSystemsSubscribedTo[specificSystemIndex], &keyDataExists);
                        if (keyDataExists) {
...
void CloudServer::OnClosedConnection(const SystemAddress &systemAddress, RakNetGUID rakNetGUID, PI2_LostConnectionReason lostConnectionReason) {
    ...
    if (remoteSystemIndex.IsInvalid() == false) {
        ...
        for (subscribedKeysIndex = 0; subscribedKeysIndex < remoteCloudClient->subscribedKeys.Size(); subscribedKeysIndex++) {
            ...
            bool keyDataRepositoryExists;
            unsigned int keyDataRepositoryIndex =
                dataRepository.GetIndexFromKey(remoteCloudClient->subscribedKeys[subscribedKeysIndex]->key, &keyDataRepositoryExists);
            if (keyDataRepositoryExists) {
...
void CloudServer::WriteCloudQueryRowFromResultList(DataStructures::List<CloudData *> &cloudDataResultList, DataStructures::List<CloudKey> &cloudKeyResultList,
                                                   BitStream *bsOut) {
...
void CloudServer::WriteCloudQueryRowFromResultList(unsigned int i, DataStructures::List<CloudData *> &cloudDataResultList,
...
void CloudServer::NotifyClientSubscribersOfDataChange(CloudData *cloudData, CloudKey &key, DataStructures::OrderedList<RakNetGUID, RakNetGUID> &subscribers,
                                                      bool wasUpdated) {
    RakNet::BitStream bsOut;
...
void CloudServer::ProcessCloudQueryWithAddresses(CloudServer::CloudQueryWithAddresses &cloudQueryWithAddresses,
                                                 DataStructures::List<CloudData *> &cloudDataResultList, DataStructures::List<CloudKey> &cloudKeyResultList) {
    CloudQueryResult cloudQueryResult;
    CloudQueryRow cloudQueryRow;
...
void CloudServer::GetServersWithUploadedKeys(DataStructures::List<CloudKey> &keys, DataStructures::List<CloudServer::RemoteServer *> &remoteServersWithData) {
...
void CloudServer::UnsubscribeFromKey(RemoteCloudClient *remoteCloudClient, RakNetGUID remoteCloudClientGuid, unsigned int keySubscriberIndex,
    ...
    if (specificSystems.Size() == 0) {
        ...
        for (j = 0; j < specificSystems.Size(); j++) {
            ...
            specificSystemsSubscribedToIndex =
                keySubscriberId->specificSystemsSubscribedTo.GetIndexFromKey(specificSystems[j], &hasSpecificSystemsSubscribedTo);
