...
class RAK_DLL_EXPORT CommandParserInterface {
  public:
    ...
    virtual bool OnCommand(const char *command, unsigned numParameters, char **parameterList, TransportInterface *transport, const SystemAddress &systemAddress,
    ...
    static void ParseConsoleString(char *str, const char delineator, unsigned char delineatorToggle, unsigned *numParameters, char **parameterList,
    ...
    /// Registers a command.
    /// \param[in] parameterCount How many parameters your command requires.  If you want to accept a variable number of commands, pass
    /// CommandParserInterface::VARIABLE_NUMBER_OF_PARAMETERS \param[in] command A pointer to a STATIC string that has your command.  I keep a copy of the
    /// pointer here so don't deallocate the string. \param[in] commandHelp A pointer to a STATIC string that has the help information for your command.  I keep
    /// a copy of the pointer here so don't deallocate the string.
