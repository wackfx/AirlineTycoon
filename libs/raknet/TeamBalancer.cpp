        ...
        for (i = 0; i < teamMembers.Size(); i++) {
            if (teamMembers[i].requestedTeam != UNASSIGNED_TEAM_ID) {
                for (j = i + 1; j < teamMembers.Size(); j++) {
                    if (teamMembers[j].requestedTeam == teamMembers[i].currentTeam && teamMembers[i].requestedTeam == teamMembers[j].currentTeam) {
...
PluginReceiveResult TeamBalancer::OnReceive(Packet *packet) {
    ...
    // - This user does not already have a team. Assign a team as if the user called RequestAnyTeam(), with a preference for the requested team. Return
    // ID_TEAM_BALANCER_TEAM_ASSIGNED once the team has been assigned. If teams are not locked
    // - If even team balancing is on, only assign this user if this would not cause teams to be unbalanced. If teams WOULD be unbalanced, then flag this user
    // as wanting to join this team. Return ID_TEAM_BALANCER_REQUESTED_TEAM_CHANGE_PENDING
...
void TeamBalancer::OnStatusUpdateToNewHost(Packet *packet) {
    ...
    for (uint8_t i = 0; i < requestedTeamChangeListSize; i++) {
        ...
        if (memberIndex == (unsigned int)-1) {
            ...
            // Do not process balancing on requested teams, since we don't necessarily have all data from all systems yet and hopefully the state during the
            // host migration was stable.
...
bool TeamBalancer::WeAreHost(void) const { return hostGuid == rakPeerInterface->GetGuidFromSystemAddress(UNASSIGNED_SYSTEM_ADDRESS); }
...
bool TeamBalancer::TeamsWouldBeEvenOnSwitch(TeamId t1, TeamId t2) {
    ...
    return TeamWouldBeOverpopulatedOnAddition(t1, teamMembers.Size() - 1) == false && TeamWouldBeUnderpopulatedOnLeave(t2, teamMembers.Size() - 1) == false;
}
