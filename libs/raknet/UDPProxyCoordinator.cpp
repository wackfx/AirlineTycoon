...
// bool operator<( const DataStructures::MLKeyRef<unsigned short> &inputKey, const UDPProxyCoordinator::ServerWithPing &cls ) {return inputKey.Get() <
// cls.ping;} bool operator>( const DataStructures::MLKeyRef<unsigned short> &inputKey, const UDPProxyCoordinator::ServerWithPing &cls ) {return inputKey.Get()
// > cls.ping;} bool operator==( const DataStructures::MLKeyRef<unsigned short> &inputKey, const UDPProxyCoordinator::ServerWithPing &cls ) {return
// inputKey.Get() == cls.ping;}
...
void UDPProxyCoordinator::Update(void) {
    ...
    while (idx < forwardingRequestList.Size()) {
        ...
        if (fw->timeRequestedPings != 0 && curTime > fw->timeRequestedPings + DEFAULT_UNRESPONSIVE_PING_TIME_COORDINATOR) {
...
void UDPProxyCoordinator::SendForwardingRequest(SystemAddress sourceAddress, SystemAddress targetAddress, SystemAddress serverAddress,
...
void UDPProxyCoordinator::OnForwardingReplyFromServerToCoordinator(Packet *packet) {
    ...
    if (success == UDPFORWARDER_SUCCESS) {
        ...
        // 05/18/09 Keep the entry around for some time after success, so duplicates are reported if attempting forwarding from the target system before
...
void UDPProxyCoordinator::SendAllBusy(SystemAddress senderClientAddress, SystemAddress targetClientAddress, RakNetGUID targetClientGuid,
