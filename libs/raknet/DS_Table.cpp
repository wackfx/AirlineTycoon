...
Table::Cell::Cell(double numericValue, char *charValue, void *ptr, ColumnType type) { SetByType(numericValue, charValue, ptr, type); }
...
unsigned Table::ColumnIndex(char columnName[_TABLE_MAX_COLUMN_NAME_LENGTH]) const { return ColumnIndex((const char *)columnName); }
...
Table::Row *Table::AddRow(unsigned rowId, DataStructures::List<Cell *> &initialCellValues, bool copyCells) {
    ...
    for (rowIndex = 0; rowIndex < columns.Size(); rowIndex++) {
        if (rowIndex < initialCellValues.Size() && initialCellValues[rowIndex] && initialCellValues[rowIndex]->isEmpty == false) {
            if (copyCells == false)
                newRow->cells.Insert(RakNet::OP_NEW_4<Table::Cell>(_FILE_AND_LINE_, initialCellValues[rowIndex]->i, initialCellValues[rowIndex]->c,
    ...
    for (columnIndex = 0; columnIndex < columnIndices.Size(); columnIndex++) {
        if (row->cells[columnIndices[columnIndex]]->isEmpty == false) {
            newRow->cells.Insert(RakNet::OP_NEW_4<Table::Cell>(_FILE_AND_LINE_, row->cells[columnIndices[columnIndex]]->i,
                                                               row->cells[columnIndices[columnIndex]]->c, row->cells[columnIndices[columnIndex]]->ptr,
...
void Table::QueryTable(unsigned *columnIndicesSubset, unsigned numColumnSubset, FilterQuery *inclusionFilters, unsigned numInclusionFilters, unsigned *rowIds,
                       unsigned numRowIDs, Table *result) {
    unsigned i;
...
void Table::QueryRow(DataStructures::List<unsigned> &inclusionFilterColumnIndices, DataStructures::List<unsigned> &columnIndicesToReturn, unsigned key,
    ...
    if (inclusionFilterColumnIndices.Size() == 0) {
        ...
        for (j = 0; j < inclusionFilterColumnIndices.Size(); j++) {
            ...
            if (columnIndex != (unsigned)-1 && row->cells[columnIndex]->isEmpty == false) {
                if (columns[inclusionFilterColumnIndices[j]].columnType == STRING && (row->cells[columnIndex]->c == 0 || inclusionFilters[j].cellValue->c == 0))
                    continue;
...
const DataStructures::BPlusTree<unsigned, Table::Row *, _TABLE_BPLUS_TREE_ORDER> &Table::GetRows(void) const { return rows; }
DataStructures::Page<unsigned, DataStructures::Table::Row *, _TABLE_BPLUS_TREE_ORDER> *Table::GetListHead(void) { return rows.GetListHead(); }
