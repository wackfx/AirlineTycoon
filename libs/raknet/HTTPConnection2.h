...
class RAK_DLL_EXPORT HTTPConnection2 : public PluginInterface2 {
  public:
    ...
    bool TransmitRequest(const char *stringToTransmit, const char *host, unsigned short port = 80, bool useSSL = false, int ipVersion = 4,
                         SystemAddress useAddress = UNASSIGNED_SYSTEM_ADDRESS, void *userData = 0);
    ...
    /// HTTPConnection2 will store Packet::data and return the response to you either when the connection to the webserver is lost, or enough data has been
    /// received() This will only potentially return true after a call to ProcessTCPPacket() or OnLostConnection() \param[out] stringTransmitted The original
    /// string transmitted \param[out] hostTransmitted The parameter of the same name passed to TransmitRequest() \param[out] responseReceived The response, if
    /// any \param[out] hostReceived The SystemAddress from ProcessTCPPacket() or OnLostConnection() \param[out] contentOffset The offset from the start of
    /// responseReceived to the data body. Equivalent to searching for \r\n\r\n in responseReceived. \param[out] userData Whatever you passed to TransmitRequest
