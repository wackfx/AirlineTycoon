...
/// \file
/// \brief A class you can derive from to make it easier to represent every networked object with an integer.  This way you can refer to objects over the
/// network.
///
...
/// \details A class you can derive from to make it easier to represent every networked object with an integer.  This way you can refer to objects over the
/// network. One system should return true for IsNetworkIDAuthority() and the rest should return false.  When an object needs to be created, have the the one
/// system create the object. Then have that system send a message to all other systems, and include the value returned from GetNetworkID() in that packet.  All
/// other systems should then create the same class of object, and call SetNetworkID() on that class with the NetworkID in the packet. \see the manual for more
/// information on this.
class RAK_DLL_EXPORT NetworkIDObject {
  public:
    ...
    /// \retval UNASSIGNED_NETWORK_ID UNASSIGNED_NETWORK_ID is returned IsNetworkIDAuthority() is false and SetNetworkID() was not previously called.  This is
    /// also returned if you call this function in the constructor. \retval 0-65534 Any other value is a valid NetworkID.  NetworkIDs start at 0 and go to
    /// 65534, wrapping at that point.
    virtual NetworkID GetNetworkID(void);

    /// Sets the NetworkID for this instance.  Usually this is called by the clients and determined from the servers.  However, if you save multiplayer games
    /// you would likely use This on load as well.
