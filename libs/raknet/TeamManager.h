...
/// Teams contain properties including the number of team members per team, whether or not tagged teams must have equal numbers of members, and if a team is
/// locked or not to certain entry conditions Team members contain properties including which teams they are on and which teams they want to join if a team is
/// not immediately joinable Advanced functionality includes the ability for a team member to be on multiple teams simultaneously, the ability to swap teams
/// with other members, and the ability to resize the number of members supported per team
///
...
/// \brief Automates networking and list management for teams
/// \details When used with ReplicaManager3 and FullyConnectedMesh2, provides a complete solution to managing a distributed list of teams and team member
/// objects with support for host migration. \ingroup PLUGINS_GROUP
...
struct TeamSelection {
    ...
    /// \details ID_TEAM_BALANCER_TEAM_ASSIGNED, ID_TEAM_BALANCER_REQUESTED_TEAM_FULL, or ID_TEAM_BALANCER_REQUESTED_TEAM_LOCKED will be returned to all
    /// systems.
    ...
    /// \details ID_TEAM_BALANCER_TEAM_ASSIGNED, ID_TEAM_BALANCER_REQUESTED_TEAM_FULL, or ID_TEAM_BALANCER_REQUESTED_TEAM_LOCKED will be returned to all
    /// systems. \param[in] specificTeamToJoin Which team to attempt to join.
...
/// When using with ReplicaManager3, call TM_TeamMember::ReferenceTeamMember() in Replica3::DeserializeConstruction() and
/// TM_TeamMember::DeserializeConstruction() in Replica3::PostDeserializeConstruction() There is otherwise no need to manually serialize the class, as
/// operations are networked internally. \ingroup TEAM_MANAGER_GROUP
class RAK_DLL_EXPORT TM_TeamMember {
  public:
    ...
    /// On success, every system will get ID_TEAM_BALANCER_TEAM_ASSIGNED. Use TeamManager::DecomposeTeamAssigned() to get details of which team member the
    /// message refers to. On failure, all systems will get ID_TEAM_BALANCER_REQUESTED_TEAM_FULL or ID_TEAM_BALANCER_REQUESTED_TEAM_LOCKED. Use
    /// TeamManager::DecomposeTeamFull() and TeamManager::DecomposeTeamLocked() to get details of which team member the message refers to. \note Joining a
    /// specific team with this function may result in being on more than one team at once, even if you call the function while locally only on one team. If
    /// your game depends on only being on one team at a team, use RequestTeamSwitch() instead with the parameter teamToLeave set to 0 \param[in]
    /// TeamSelection::AnyAvailable(), TeamSelection::SpecificTeam(), or TeamSelection::NoTeam() \return false On invalid or unnecessary operation. Otherwise
    /// returns true
    ...
    /// \note This is not guaranteed to work due to latency. To clarify, If the host switches your team at the same time you call CancelRequestTeam() you may
    /// still get ID_TEAM_BALANCER_TEAM_ASSIGNED for the team you tried to cancel. \param[in] specificTeamToCancel Which team to no longer join. Use 0 for all.
    /// \return false On invalid or unnecessary operation. Otherwise returns true
    ...
    /// ReplicaManager3::PostSerializeConstruction() and ReplicaManager3::PostDeserializeConstruction() will ensure this. \param[out] constructionBitstream This
    /// object serialized to a BitStream
...
/// When using with ReplicaManager3, call TM_Team::ReferenceTeam() in Replica3::DeserializeConstruction() and TM_Team::DeserializeConstruction() in
/// Replica3::PostDeserializeConstruction() There is otherwise no need to manually serialize the class, as operations are networked internally. \ingroup
/// TEAM_MANAGER_GROUP
class RAK_DLL_EXPORT TM_Team {
  public:
    ...
    /// Setting the limit higher allows members to join in. If a member has a pending request to join this team, they join automatically and
    /// ID_TEAM_BALANCER_TEAM_ASSIGNED will be returned for those members. \param[in] _teamMemberLimit The new limit \param[in] noTeamSubcategory Which
    /// noTeamSubcategory to assign to members that now have no team. \return false On invalid or unnecessary operation. Otherwise returns true
    bool SetMemberLimit(TeamMemberLimit _teamMemberLimit, NoTeamId noTeamSubcategory);

    /// \return If team balancing is on, the most members that can be on this team that would not either unbalance it or exceed the value passed to
    /// SetMemberLimit(). If team balancing is off, the same as GetMemberLimitSetting()
    ...
    /// For an invite-only team, use ALLOW_JOIN_SPECIFIC_TEAM only and only allow the requester to call TM_TeamMember::RequestTeam() upon invitiation through
    /// your game code. Defaults to allow all \param[in] _joinPermissions Bitwise combination of ALLOW_JOIN_ANY_AVAILABLE_TEAM, ALLOW_JOIN_SPECIFIC_TEAM,
    /// ALLOW_JOIN_REBALANCING \return false On invalid or unnecessary operation. Otherwise returns true
...
class TM_World {
  public:
    ...
    /// \param[in] networkId Identifies this instance. This value is independent of values used by NetworkIDManager. You can use the same value as the object
    /// that contains this instance. \param[in] applyBalancing Whether or not to include this team for balancing when calling SetBalanceTeams().
    ...
    /// If a player on the desired team also wants to switch, then both players will switch simultaneously. Otherwise, ID_TEAM_BALANCER_REQUESTED_TEAM_FULL will
    /// be returned to the requester and switching will occur when possible. If balanceTeams is true and later set to false, players waiting on
    /// ID_TEAM_BALANCER_REQUESTED_TEAM_FULL will be able to join the desired team immediately provided it is not full. \param[in] balanceTeams Whether to
    /// activate or deactivate team balancing. \param[in] noTeamSubcategory If a player is kicked off a team and is no longer on any team, his or her
    /// noTeamSubcategory is set to this value
    ...
    /// \details Operations that can cause conflicts due to latency, such as joining teams, are operated on by the host. The result is sent to all systems added
    /// with AddParticipant() For a client/server game, call SetHost() with the server's RakNetGUID value on all systems (including the server itself). If you
    /// call TeamManager::SetTopology(TM_CLIENT_SERVER), the server will also relay messages between participants. For a peer to peer game, call SetHost() on
    /// the same peer when host migration occurs. Use TeamManager::SetTopology(TM_PEER_TO_PEER) in this case. \note If using FullyConnectedMesh2, SetHost() is
    /// called automatically when ID_FCM2_NEW_HOST is returned. \param[in] _hostGuid The host, which is the system that will serialize and resolve team disputes
    /// and calculate team balancing.
...
/// locked or not to certain entry conditions Team members contain properties including which teams they are on and which teams they want to join if a team is
/// not immediately joinable Advanced functionality includes the ability for a team member to be on multiple teams simultaneously, the ability to swap teams
/// with other members, and the ability to resize the number of members supported per team The architecture is designed for easy integration with
/// ReplicaManager3
///
/// Usage:<BR>
...
/// 2. Have those game classes contain a corresponding TM_Team or TM_TeamMember instance. Operations on teams will be performed by those instances. Use
/// SetOwner() to refer to the parent object when using composition.<BR>
...
/// 6. When sending world state to a new connection, for example in ReplicaManager3::SerializeConstruction(), call TM_SerializeConstruction() on the
/// corresponding TM_TeamMember and TM_Team objects. TM_Team instances on the new connection must be created before TM_TeamMember instances.<BR>
/// 7. Call TM_DeserializeConstruction() on your new corresponding TM_TeamMember and TM_Team instances.<BR>
/// 8. Execute team operations. ID_TEAM_BALANCER_REQUESTED_TEAM_FULL, ID_TEAM_BALANCER_REQUESTED_TEAM_LOCKED, ID_TEAM_BALANCER_TEAM_REQUESTED_CANCELLED, and
/// ID_TEAM_BALANCER_TEAM_ASSIGNED are returned to all systems when the corresponding event occurs for a team member.<BR>
...
class RAK_DLL_EXPORT TeamManager : public PluginInterface2 {
  public:
    ...
    /// \details If topology is set to TM_PEER_TO_PEER, the host assumes the original message source was connected to all other participants and does not relay
    /// messages. \note If TM_PEER_TO_PEER, this plugin will listen for ID_FCM2_NEW_HOST and call SetHost() on all worlds automatically \note Defaults to
    /// TM_PEER_TO_PEER \param[in] _topology Topology to use
    ...
    void DecomposeTeamFull(Packet *packet, TM_World **world, TM_TeamMember **teamMember, TM_Team **team, uint16_t &currentMembers,
                           uint16_t &memberLimitIncludingBalancing, bool &balancingIsActive, JoinPermissions &joinPermissions);
    ...
    /// \return true on success, false on read error
    void DecomposeTeamLocked(Packet *packet, TM_World **world, TM_TeamMember **teamMember, TM_Team **team, uint16_t &currentMembers,
                             uint16_t &memberLimitIncludingBalancing, bool &balancingIsActive, JoinPermissions &joinPermissions);
  ...
  protected:
    ...
    void DecomposeTeamFullOrLocked(RakNet::BitStream *bsIn, TM_World **world, TM_TeamMember **teamMember, TM_Team **team, uint16_t &currentMembers,
                                   uint16_t &memberLimitIncludingBalancing, bool &balancingIsActive, JoinPermissions &joinPermissions);
    ...
    /// \param[in] A packet where packet->data[0]==ID_TEAM_BALANCER_TEAM_ASSIGNED
    /// \return true on success, false on read error
    void DecodeTeamAssigned(RakNet::BitStream *bsIn, TM_World **world, TM_TeamMember **teamMember, NoTeamId &noTeamSubcategory, JoinTeamType &joinTeamType,
                            DataStructures::List<TM_Team *> &newTeam, DataStructures::List<TM_Team *> &teamsLeft, DataStructures::List<TM_Team *> &teamsJoined);
