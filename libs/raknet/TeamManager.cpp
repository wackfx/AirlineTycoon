...
bool TM_TeamMember::RequestTeam(TeamSelection teamSelection) {
    if (teamSelection.joinTeamType == JOIN_NO_TEAM) {
        ...
        // - JoinAnyTeam(requesterGuid): Attempt to join any team immediately.  If fails, send to all except requestGuid UpdateTeamsRequestedToAny(). Else sends
        // out new team, including to caller. On remote host, execute JoinAnyTeamCheck(). If fails, this was because you were added to a team simultaneously on
        // host. This is OK, just ignore the call. Assuming JoinAnyTeamCheck() passed on remote host, call UpdateTeamsRequestedToAny() for this player. execute
        // JoinAnyTeam(packet->guid).
        ...
        // - JoinSpecificTeam(requesterGuid): Attempt to join specific team immediately. If fails, send to all except requesterGuid to execute
        // AddSpecificToRequested(). Else sends out new team, including to caller. On remote host, execute JoinSpecificTeamCheck(). If fails, just ignore.
...
bool TM_TeamMember::LeaveAllTeams(NoTeamId noTeamSubcategory) { return RequestTeam(TeamSelection::NoTeam(noTeamSubcategory)); }
...
void TM_Team::LeaveTeam(TM_TeamMember *teamMember, NoTeamId noTeamSubcategory) { teamMember->LeaveTeam(this, noTeamSubcategory); }
        ...
        if (teamMembers[i]->GetCurrentTeamCount() == 1) {
            ...
            if (j != (unsigned int)-1) {
                if (teamMembers[i]->teamsRequested[j].isTeamSwitch &&
                    (teamMembers[i]->teamsRequested[j].teamToLeave == 0 || teamMembers[i]->teamsRequested[j].teamToLeave == teamMembers[i]->teams[0]))
                    return i;
            }
        }
    }
...
void TM_World::FillRequestedSlots(void) {
    ...
    for (joinRequestIndex = 0; joinRequestIndex < joinRequests.Size(); joinRequestIndex++) {
        ...
        if (teamMember->teamsRequested.Size() == 0) {
            ...
            if (team->GetTeamMembersCount() < balancedTeamLimit && team->GetTeamMembersCount() < team->GetMemberLimitSetting() &&
...
unsigned int TM_World::GetAvailableTeamIndexWithFewestMembers(TeamMemberLimit secondaryLimit, JoinPermissions joinPermissions) {
    ...
    for (teamIndex = 0; teamIndex < teams.Size(); teamIndex++) {
        if (teams[teamIndex]->GetTeamMembersCount() < secondaryLimit && teams[teamIndex]->GetTeamMembersCount() < teams[teamIndex]->GetMemberLimitSetting() &&
            teams[teamIndex]->GetTeamMembersCount() < lowestTeamMembers && (joinPermissions & teams[teamIndex]->GetJoinPermissions()) != 0) {
...
TM_Team *TM_World::JoinAnyTeam(TM_TeamMember *teamMember, int *resultCode) {
    ...
    if (idx == (unsigned int)-1) {
        ...
        for (idx = 0; idx < teams.Size(); idx++) {
            if ((teams[idx]->GetTeamMembersCount() >= balancedLimit || teams[idx]->GetTeamMembersCount() >= teams[idx]->GetMemberLimitSetting()) &&
                teams[idx]->GetMemberLimitSetting() != 0 && (ALLOW_JOIN_ANY_AVAILABLE_TEAM & teams[idx]->GetJoinPermissions()) != 0) {
                // Full
                *resultCode = -2;
                return teams[idx];
            }
...
void TeamManager::DecomposeTeamFull(Packet *packet, TM_World **world, TM_TeamMember **teamMember, TM_Team **team, uint16_t &currentMembers,
...
void TeamManager::DecomposeTeamFullOrLocked(RakNet::BitStream *bsIn, TM_World **world, TM_TeamMember **teamMember, TM_Team **team, uint16_t &currentMembers,
                                            uint16_t &memberLimitIncludingBalancing, bool &balancingIsActive, JoinPermissions &joinPermissions) {
    WorldId worldId;
...
void TeamManager::DecomposeTeamLocked(Packet *packet, TM_World **world, TM_TeamMember **teamMember, TM_Team **team, uint16_t &currentMembers,
                                      uint16_t &memberLimitIncludingBalancing, bool &balancingIsActive, JoinPermissions &joinPermissions) {
    BitStream bsIn(packet->data, packet->length, false);
    bsIn.IgnoreBytes(sizeof(MessageID));
    DecomposeTeamFullOrLocked(&bsIn, world, teamMember, team, currentMembers, memberLimitIncludingBalancing, balancingIsActive, joinPermissions);
...
void TeamManager::DecodeTeamAssigned(BitStream *bsIn, TM_World **world, TM_TeamMember **teamMember, NoTeamId &noTeamId, JoinTeamType &joinTeamType,
