...
template <class templateType> void DeletePtr_RakNet(templateType &ptr, const char *file, unsigned int line) { RakNet::OP_DELETE(ptr, file, line); }

/// Can be used with Multilist::ForEach
/// Assuming the Multilist holds pointers, will delete those pointers
template <class templateType> void DeletePtr(templateType &ptr) { delete ptr; }
...
template <class templateType> class MLKeyRef {
  public:
...
template <const MultilistType _MultilistType, class _DataType, class _KeyType = _DataType, class _IndexType = DefaultIndexType> class RAK_DLL_EXPORT Multilist {
  public:
    Multilist();
    ~Multilist();
    ...
    /// \details However, if \a force is true, it will also resort the ordered list, useful if the comparison operator between _KeyType and _DataType would now
    /// return different results Once the list is sorted, further operations to lookup by key will be log2(n) until the list is modified
...
template <const MultilistType _MultilistType, class _DataType, class _KeyType, class _IndexType>
void Multilist<_MultilistType, _DataType, _KeyType, _IndexType>::ForEach(void (*func)(_DataType &item, const char *file, unsigned int line), const char *file,
                                                                         unsigned int line) {
    _IndexType i;
    for (i = 0; i < dataSize; i++)
...
template <const MultilistType _MultilistType, class _DataType, class _KeyType, class _IndexType>
void Multilist<_MultilistType, _DataType, _KeyType, _IndexType>::FindIntersection(Multilist &source1, Multilist &source2, Multilist &intersection,
