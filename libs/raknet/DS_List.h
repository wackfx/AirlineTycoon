...
template <class list_type> class RAK_DLL_EXPORT List {
  public:
    /// Default constructor
    List();
...
template <class list_type> List<list_type>::List() {
    allocation_size = 0;
    listArray = 0;
    list_size = 0;
}
...
template <class list_type> List<list_type>::List(const List &original_copy) {
...
template <class list_type> List<list_type> &List<list_type>::operator=(const List &original_copy) {
...
template <class list_type> inline list_type &List<list_type>::operator[](const unsigned int position) const {
#ifdef _DEBUG
...
template <class list_type> inline list_type &List<list_type>::Get(const unsigned int position) const { return listArray[position]; }

template <class list_type> void List<list_type>::Push(const list_type &input, const char *file, unsigned int line) { Insert(input, file, line); }

template <class list_type> inline list_type &List<list_type>::Pop(void) {
#ifdef _DEBUG
...
template <class list_type> void List<list_type>::Insert(const list_type &input, const unsigned int position, const char *file, unsigned int line) {
#ifdef _DEBUG
    if (position > list_size) {
        RakAssert(position <= list_size);
    }
...
template <class list_type> void List<list_type>::Insert(const list_type &input, const char *file, unsigned int line) {
    // Reallocate list if necessary
...
template <class list_type> inline void List<list_type>::Replace(const list_type &input) {
    if (list_size > 0)
        listArray[list_size - 1] = input;
}

template <class list_type> void List<list_type>::RemoveAtIndex(const unsigned int position) {
#ifdef _DEBUG
    if (position >= list_size) {
        RakAssert(position < list_size);
        return;
...
template <class list_type> void List<list_type>::RemoveAtIndexFast(const unsigned int position) {
#ifdef _DEBUG
    if (position >= list_size) {
        RakAssert(position < list_size);
        return;
...
template <class list_type> inline void List<list_type>::RemoveFromEnd(const unsigned num) {
...
template <class list_type> unsigned int List<list_type>::GetIndexOf(const list_type &input) const {
...
template <class list_type> inline unsigned int List<list_type>::Size(void) const { return list_size; }

template <class list_type> void List<list_type>::Clear(bool doNotDeallocateSmallBlocks, const char *file, unsigned int line) {
    if (allocation_size == 0)
        return;
...
template <class list_type> void List<list_type>::Compress(const char *file, unsigned int line) {
    list_type *new_array;
...
template <class list_type> void List<list_type>::Preallocate(unsigned countNeeded, const char *file, unsigned int line) {
