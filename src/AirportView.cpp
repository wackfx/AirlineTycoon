...
AirportView::AirportView(BOOL bHandy, ULONG PlayerNum) : CStdRaum(bHandy, PlayerNum, "", 0) {
   ...
   SDL_UpdateWindowSurface(FrameWnd->m_hWnd);
    ...
    if (bHandy == 0) {
        AmbientManager.SetGlobalVolume(100);
    }
...
void AirportView::FocusCameraOnPos(XY Pos, BOOL Speed) {
    ...
    if (gMouseScroll == 0) {
        if (Sim.Options.OptionFlipping != 0) {
            ...
            if (Pos.y > 440 - (WinP2.y - WinP1.y - StatusLineSizeY)) {
                Pos.y = 440 - (WinP2.y - WinP1.y - StatusLineSizeY);
            }
            ...
            if (Tmp > 0) {
                if (Tmp > CameraSpeed.y + 3 || (Tmp > CameraSpeed.y && Tmp < 4)) {
                    CameraSpeed.y++;
                }
                if (Tmp < CameraSpeed.y) {
                    CameraSpeed.y = Tmp;
                }
            } else if (Tmp < 0) {
                if (Tmp < CameraSpeed.y - 3 || (Tmp < CameraSpeed.y && Tmp > -4)) {
...
void AirportView::MoveCamera() {
    ...
    if (Editor == 0) {
        ...
        if (Sim.FocusPerson != -1) {
            if (Sim.Persons.IsInAlbum(Sim.FocusPerson) == 0) {
                Sim.FocusPerson = -1;
...
void AirportView::OnPaint() {
    ...
    for (c = 0; c < static_cast<ULONG>(Sim.AirportSmacks.AnzEntries()); c++) {
        if (Sim.AirportSmacks[static_cast<SLONG>(c)].pSmack != nullptr) {
            if (Sim.AirportSmacks[static_cast<SLONG>(c)].Next(&Sim.AirportSmacks[static_cast<SLONG>(c)].Bitmap) == 0) {
                ...
                cnt++;
                Bricks[SLONG(0x10000000 + Sim.AirportSmacks[static_cast<SLONG>(c)].BrickId)].Bitmap[0].BlitFrom(
                    Sim.AirportSmacks[static_cast<SLONG>(c)].Bitmap, Sim.AirportSmacks[static_cast<SLONG>(c)].Offset.x,
                    Sim.AirportSmacks[static_cast<SLONG>(c)].Offset.y);
            }
        }
    }
    if (cnt == 0) {
    ...
    if (Sim.Date > 0 && Sim.GetHour() == 9 && Sim.GetMinute() == 0 && MouseWait > 0) {
        return;
    }
    ...
    // DutyFree:
    if ((Sim.Weekday == 5 || Sim.Weekday == 6) && SLONG(Sim.Time) > timeDutyClose + 2000) {
        DoorOpenTab[0] = 0;
    } else if ((Sim.Weekday == 5 || Sim.Weekday == 6) && SLONG(Sim.Time) > timeDutyClose && SLONG(Sim.Time) <= timeDutyClose + 2000) {
        DoorOpenTab[0] = 256 - (Sim.Time - timeDutyClose) * 256 / 2000;
    } else if (SLONG(Sim.Time) < timeDutyOpen - 2000) {
        DoorOpenTab[0] = 0;
    } else if (SLONG(Sim.Time) > timeDutyOpen) {
        DoorOpenTab[0] = 256;
    } else {
        DoorOpenTab[0] = (Sim.Time - (timeDutyOpen - 2000)) * 256 / 2000;
    }

    // ArabAir
    if (SLONG(Sim.Time) < timeArabOpen - 2000 || Sim.Weekday == 6) {
        DoorOpenTab[1] = 0;
    } else if (SLONG(Sim.Time) > timeArabOpen) {
        DoorOpenTab[1] = 256;
    } else {
        DoorOpenTab[1] = (Sim.Time - (timeArabOpen - 2000)) * 256 / 2000;
    }

    // Museum
    if (SLONG(Sim.Time) < timeMuseOpen - 2000 || Sim.Weekday == 5 || Sim.Weekday == 6) {
        DoorOpenTab[3] = 0;
    } else if (SLONG(Sim.Time) > timeMuseOpen) {
        DoorOpenTab[3] = 256;
    } else {
        DoorOpenTab[3] = (Sim.Time - (timeMuseOpen - 2000)) * 256 / 2000;
    }
    ...
    // Werbung
    if (SLONG(Sim.Time) < timeWerbOpen || Sim.Weekday == 5 || Sim.Weekday == 6) {
        DoorOpenTab[6] = 0;
    } else {
        DoorOpenTab[6] = 256;
    }
    ...
    if (RobotUse(ROBOT_USE_TRAVELHOLDING)) {
        ...
        if (SLONG(Sim.Time) > timeLastClose + 2000 || Sim.Weekday == 5) {
            DoorOpenTab[2] = 0;
        } else if (SLONG(Sim.Time) < timeLastClose) {
            DoorOpenTab[2] = 256;
        } else {
        ...
        if (SLONG(Sim.Time) > timeReisClose + 2000) {
            ...
            DoorOpenTab[4] = 256 - (Sim.Time - timeReisClose) * 256 / 2000;
        }
    } else {
        // Closed
        DoorOpenTab[2] = 0;
        ...
        if (bActive != 0) {
            ...
            if (Editor == EDITOR_BUILDS && EditObject == 0xffffffff) {
                for (SLONG c = Airport.Builds.AnzEntries() - 1; c >= 0; c--) {
                    if ((Airport.Builds.IsInAlbum(c) != 0) && (Bricks.IsInAlbum(Airport.Builds[c].BrickId) != 0)) {
                        if (gMousePosition.x + 2 + ViewPos.x >= Airport.Builds[c].ScreenPos.x &&
                            gMousePosition.x + 2 + ViewPos.x <= Airport.Builds[c].ScreenPos.x + Bricks[Airport.Builds[c].BrickId].GetBitmapDimension().x) {
                            if (gMousePosition.y + 2 + ViewPos.y >= Airport.Builds[c].ScreenPos.y &&
                                gMousePosition.y + 2 + ViewPos.y <= Airport.Builds[c].ScreenPos.y + Bricks[Airport.Builds[c].BrickId].GetBitmapDimension().y) {
                                if (Bricks[Airport.Builds[c].BrickId].IsGlasAt(gMousePosition.x + 2 - Airport.Builds[c].ScreenPos.x + ViewPos.x,
                                                                               gMousePosition.y + 2 - Airport.Builds[c].ScreenPos.y + ViewPos.y) == 0) {
                                    UnderCursor = c;
                                    break;
                                }
                            }
            ...
            // Parallax Horizont mit Clipping einbauen:
            {
                ...
                // Video-Support:
                static XY LastViewPosA;
                static XY LastViewPosB;
                            ...
                            for (d = 640 / (sizes[0] + sizes[1]) + 2; d >= 0; d--) {
                                ...
                                Bricks[ParallaxIndex[0]].BlitAt(PrimaryBm, 0,
                                                                ((100000 - (ViewPos.x * 16)) >> 6) % (sizes[0] + sizes[1]) - (sizes[0] + sizes[1]) +
                                                                    d * (sizes[0] + sizes[1]),
                                                                WinP1.y - 18 + 18);
                                Bricks[ParallaxIndex[1]].BlitAt(PrimaryBm, 0,
                                                                ((100000 - (ViewPos.x * 16)) >> 6) % (sizes[0] + sizes[1]) - (sizes[0] + sizes[1]) + sizes[0] +
                                                                    d * (sizes[0] + sizes[1]),
                                                                WinP1.y - 18 + 18);
                        ...
                        for (d = 0; d < Sim.Players.Players[static_cast<SLONG>(c)].Planes.AnzEntries(); d++) {
                            if (Sim.Players.Players[static_cast<SLONG>(c)].Planes.IsInAlbum(d) != 0) {
                                if (Sim.Players.Players[static_cast<SLONG>(c)].Planes[static_cast<SLONG>(d)].Ort == -1 ||
                                    Sim.Players.Players[static_cast<SLONG>(c)].Planes[static_cast<SLONG>(d)].Ort == -2) {
                        ...
                        for (d = 0; d < Sim.Players.Players[static_cast<SLONG>(c)].Planes.AnzEntries(); d++) {
                            if (Sim.Players.Players[static_cast<SLONG>(c)].Planes.IsInAlbum(d) != 0) {
                                if (Sim.Players.Players[static_cast<SLONG>(c)].Planes[static_cast<SLONG>(d)].Ort == -3) {
                                    if (Sim.Players.Players[static_cast<SLONG>(c)].Planes[static_cast<SLONG>(d)].TypeId != -1) {
                                        PlaneTypes.BlitPlaneAt(PrimaryBm, Sim.Players.Players[static_cast<SLONG>(c)].Planes[static_cast<SLONG>(d)].TypeId, 2,
                                                               Sim.Players.Players[static_cast<SLONG>(c)].Planes[static_cast<SLONG>(d)].AirportPos -
                                                                   XY(ViewPos.x, ViewPos.y - 5),
                                                               c);
                                    } else {
                                        Sim.Players.Players[static_cast<SLONG>(c)].Planes[static_cast<SLONG>(d)].XPlane.BlitPlaneAt(
                                            PrimaryBm, 2,
            ...
            while ((c < pBuilds->AnzEntries() && (pBuilds->IsInAlbum(c) == 0)) ||
                   (Editor == EDITOR_NONE && (pBuilds->IsInAlbum(c) != 0) && (*pBuilds)[c].BrickId >= 0x10000000 + 2000)) {
                c++;
            }
            ...
            if (d < Sim.Persons.AnzEntries()) {
                if (Sim.Persons[d].ScreenPos.y < 170) {
                    t2 = (60 << 16) + Sim.Persons[d].ScreenPos.y;
                } else {
                    t2 = (20 << 16) + Sim.Persons[d].ScreenPos.y;
                }
            } else {
                t2 = 0x7fffffff;
            }
            ...
            do {
                        ...
                        if ((Sim.Options.OptionPassengers != 0) || Clans[static_cast<SLONG>(Sim.Persons[d].ClanId)].Type >= CLAN_PLAYER1) {
                            ...
                            if (Sim.DontDisplayPlayer == -1 || qClan.Type != CLAN_PLAYER1 + Sim.DontDisplayPlayer) {
                                if ((qClan.Type < CLAN_PLAYER1 && (qPerson.LookDir == 6 || qPerson.LookDir == 7)) || qClan.Group > 30) {
                                    qClan.BlitAt(PrimaryBm, qPerson.LookDir, qPerson.Phase, qPerson.ScreenPos - ViewPos + WinP1 - XY(8 - 1, 0 - 1),
                                                 qPerson.Running);
                                } else {
                                    qClan.BlitAt(
                                ...
                                if ((Sim.Options.OptionThinkBubbles != 0) && (qPerson.MoodCountdown != 0U)) {
                                    ...
                                    if (qPerson.MoodCountdown <= MOODCOUNT_START && (qPerson.ScreenPos.y > 219 || qPerson.ScreenPos.y < 150)) {
                                        ...
                                        if (Grade >= -1 && Grade <= 8) {
                                            ...
                                            ColorFX.BlitTrans(MoodBms[qPerson.Mood].pBitmap, &PrimaryBm.PrimaryBm,
                                                              qPerson.ScreenPos - ViewPos +
                                                                  XY(8 - 1 - MoodBms[qPerson.Mood].Size.x * 4 / 5,
                                                                     -MoodBms[qPerson.Mood].Size.y - qClan.Phasen[0][0].Size.y - 1),
                                                              nullptr, Grade);
                                        }
                                    }
                                }
                                ...
                                if (qClan.Type >= CLAN_PLAYER1 && qClan.Type <= CLAN_PLAYER4) {
                                    ...
                                    if (qPlayer.PlayerStinking != 0) {
                                        for (SLONG c = 0; c < 20; c++) {
                                            if (qPlayer.Smoke[c].TTL != 0) {
                                                ColorFX.BlitTrans(gStenchBms[0].pBitmap, &PrimaryBm.PrimaryBm, qPlayer.Smoke[c].Position - ViewPos, nullptr,
                                                                  8 - min(qPlayer.Smoke[c].TTL / 2, 8));
                                        ...
                                        for (SLONG c = 0; c < 20; c++) {
                                            if (qPlayer.Smoke[c].TTL != 0) {
                                                ColorFX.BlitTrans(gSmokeBms[0].pBitmap, &PrimaryBm.PrimaryBm, qPlayer.Smoke[c].Position - ViewPos, nullptr,
                                                                  8 - min(qPlayer.Smoke[c].TTL / 2, 8));
                ...
                while (t1 < t2 && t1 != 0x7fffffff) {
                    if (Editor != EDITOR_BUILDS || UnderCursor != c || (FlackerCount & 1) == 0) {
                        if ((pBuilds->IsInAlbum(c) != 0) && (Bricks.IsInAlbum((*pBuilds)[c].BrickId) != 0)) {
                            ...
                            // Bench.AdminTime.Stop();
                            if (qBrick.Triggered == 1 && (Editor == 0)) {
                                if (qBuild.BrickId == KioskerIndex) {
                                    ...
                                    if ((Airport.Triggers[static_cast<SLONG>(qBuild.Par)].Winkel != 0) &&
                                        (Sim.TickerTime - Airport.Triggers[static_cast<SLONG>(qBuild.Par)].Winkel) / qBrick.AnimSpeed >=
                            ...
                            } else if (qBrick.Triggered == 2 && (Editor == 0)) {
                                RECT rect;
                                RECT r2;
                                ...
                                qBrick.BlitAt(PrimaryBm, 0, qBuild.ScreenPos - ViewPos + WinP1,
                                              (((qBuild.ScreenPos.x + qBuild.ScreenPos.y / 2) / 44 + qBuild.ScreenPos.y / 22)) % 7 + 100);
                                ...
                                if (Editor != EDITOR_BUILDS && BrickId == AbflugIndex && Airport.GateMapper[static_cast<SLONG>(qBuild.Par)] != -1) {
                                    PrimaryBm.BlitFrom(SmallLogoBms[Airport.GateMapper[static_cast<SLONG>(qBuild.Par)]],
                                                       qBuild.ScreenPos - ViewPos + XY(17, 38 + 38));
                                    ...
                                    if (Airport.GateMapper[static_cast<SLONG>(qBuild.Par)] != -1) {
                                        PrimaryBm.BlitFromT(TextBricks[static_cast<SLONG>(qBuild.Par) + TextBricks.AnzEntries() / 2],
                                                            qBuild.ScreenPos - ViewPos + XY(43, 5));
                                        PrimaryBm.BlitFrom(TinyLogoBms[Airport.GateMapper[static_cast<SLONG>(qBuild.Par)]],
                                                           qBuild.ScreenPos - ViewPos + XY(20, 5));
                                        PrimaryBm.BlitFrom(TinyLogoBms[Airport.GateMapper[static_cast<SLONG>(qBuild.Par)]],
                                                           qBuild.ScreenPos - ViewPos + XY(134, 5));
                                    }
                                }
                            }
                            ...
                            if (Editor != EDITOR_BUILDS && BrickId == ScannerIndex) {
                                ...
                                for (SLONG d = 0; d < SLONG(Sim.Persons.AnzEntries()); d++) {
                                    if ((Sim.Persons.IsInAlbum(d) != 0) && (Clans.IsInAlbum(Sim.Persons[d].ClanId) != 0)) {
                                        PERSON &qPerson = Sim.Persons[d];
                                        CLAN &qClan = Clans[static_cast<SLONG>(qPerson.ClanId)];
                                        XY pp = qPerson.ScreenPos - ViewPos + WinP1 - XY(8 - 1, 0 - 1) +
                                                XY(qPerson.FlightPlaneIndex & 3, (qPerson.FlightPlaneIndex >> 2) & 3);
                            ...
                            if (Editor == EDITOR_BUILDS &&
                                ((BrickId >= 0x10000000 + 2030 && BrickId <= 0x10000000 + 2035) || BrickId == 0x10000000 + 2045 ||
                                 BrickId == 0x10000000 + 2001 || BrickId == 0x10000000 + 2003 || BrickId == 0x10000000 + RUNE_AREALO ||
                                 BrickId == 0x10000000 + RUNE_AREARU || BrickId == 0x10000000 + 500 || BrickId == 0x10000000 + 521 ||
                                 BrickId == 0x10000000 + 522 || BrickId == 0x10000000 + 520 || BrickId == 0x10000000 + 492 ||
                                 (BrickId >= 0x10000000 + 760 && BrickId <= 0x10000000 + 774) || BrickId == 0x10000000 + RUNE_WAITPLANE ||
                                 BrickId == 0x10000000 + RUNE_DROPSUITCASE || BrickId == 0x10000000 + RUNE_WAYPOINT ||
            ...
            if (Editor == 0) {
                ...
                if ((Editor == 0) && Sim.Persons[PlayerIndex].Position.y >= 5000) {
            ...
            if (LastAnzPeopleOnScreen == -1) {
                ...
                if (LastAnzPeopleOnScreen < AnzPeopleOnScreen) {
                    LastAnzPeopleOnScreen++;
                }
                if (LastAnzPeopleOnScreen > AnzPeopleOnScreen) {
                    LastAnzPeopleOnScreen--;
                }
                ...
                for (c = 0; c < static_cast<ULONG>(Airport.PlateDimension.x); c++) {
                    for (d = 0; d < 15; d++) {
                        if (d != 3 && d != 4) {
                            ...
                            if ((Airport.iPlate[SLONG(d + (c << 4))] & 128) == 0) {
                                PrimaryBm.BlitFromT(RuneBms[2], Pos);
                            }
                            if ((Airport.iPlate[SLONG(d + (c << 4))] & 64) == 0) {
                                PrimaryBm.BlitFromT(RuneBms[3], Pos);
                            }
                            if ((Airport.iPlate[SLONG(d + (c << 4))] & 32) == 0) {
                                PrimaryBm.BlitFromT(RuneBms[4], Pos);
                            }
                            if ((Airport.iPlate[SLONG(d + (c << 4))] & 16) == 0) {
                                PrimaryBm.BlitFromT(RuneBms[5], Pos);
                            }
                        }
                    }
                }
            ...
            // if (IsWindowEnabled())
            {
                if (Editor == EDITOR_BUILDS && (bCursorCaptured != 0) && EditObject != 0xffffffff && (Bricks.IsInAlbum(EditObject) != 0)) {
            ...
            if ((IsDialogOpen() == 0) && (MenuIsOpen() == 0) && Editor == EDITOR_NONE && (MouseWait == 0)) {
                ...
                if (c >= ROOM_FLIGHTDISPLAY && c < ROOM_FLIGHTDISPLAY + 60) {
                    ...
                    if (c >= ROOM_FLIGHTDISPLAY && c < ROOM_FLIGHTDISPLAY + 30) {
                        ...
                        SetMouseLook(CURSOR_HOT, 5000 + c, TextBrickTexts[SLONG(c - ROOM_FLIGHTDISPLAY - 30 + TextBrickTexts.AnzEntries() / 2)], ROOM_AIRPORT,
                                     10);
                    }
                } else if (c >= ROOM_BURO_A && c <= ROOM_BURO_D && c % 10 == 0 && c / 10 == ULONG(Sim.localPlayer + 1)) {
                    if (gLanguage == LANGUAGE_D) {
                        SetMouseLook(CURSOR_HOT, 5000 + c,
                                     CString(bprintf(StandardTexte.GetS(TOKEN_TOOLTIP, 5000 + c), (LPCTSTR)Sim.Players.Players[SLONG(c / 10 - 1)].AirlineX)) +
                                         " (B)",
                                     ROOM_AIRPORT, 10);
                    } else {
                        SetMouseLook(CURSOR_HOT, 5000 + c,
                    ...
                    SetMouseLook(CURSOR_HOT, 5000 + c, ROOM_AIRPORT, 10);
                }
            }
...
void AirportView::OnLButtonDown(UINT nFlags, CPoint point) {
    ...
    if (Sim.Players.Players[PlayerNum].RunningToToilet != 0) {
        return;
    }
    if ((Editor == 0) && gMousePosition.y < 440 && (IsDialogOpen() == 0) && (MenuIsOpen() == 0)) {
    ...
    if (Sim.DontDisplayPlayer == Sim.localPlayer) {
        return;
    }

    if (Sim.FocusPerson != -1 && gMousePosition.y < 440 && (IsDialogOpen() == 0) && (MenuIsOpen() == 0)) {
        Sim.FocusPerson = -1;
    }
    ...
    if (point.x >= WinP1.x && point.x <= WinP2.x && point.y >= WinP1.y && point.y <= WinP2.y - StatusLineSizeY * static_cast<int>(Editor == 0)) {
        ...
        // if (IsWindowEnabled())
        {
            ...
            if (Editor != 0) {
                ...
                if ((GetAsyncKeyState(VK_SHIFT) / 256) != 0) {
                    ...
                    for (SLONG d = SLONG(Sim.Persons.AnzEntries()) - 1; d >= 0; d--) {
                        if ((Sim.Persons.IsInAlbum(d) != 0) && (Clans.IsInAlbum(Sim.Persons[d].ClanId) != 0)) {
                            ...
                            if (gMousePosition.IfIsWithin(p.x - (qClan.Phasen[1])[0].Size.x / 2, p.y - (qClan.Phasen[1])[0].Size.y,
                                                          p.x + (qClan.Phasen[1])[0].Size.x / 2, p.y)) {
                ...
                } else if (Sim.Players.Players[PlayerNum].IsWalking2Player == -1 && (PreLButtonDown(point) == 0)) {
                    if (Sim.Tutorial == 1310 && (Sim.IsTutorial != 0)) {
                        return;
                    }
                    ...
                    if ((MenuIsOpen() != 0) || qPlayerPerson.StatePar == ROOM_STAIRS1UP || qPlayerPerson.StatePar == ROOM_STAIRS2UP ||
                        ...
                        if (Sim.bNoTime == FALSE && gMousePosition.y < 440 && (MouseWait == 0) && Sim.Players.Players[PlayerNum].IsWalking2Player == -1 &&
                            ...
                            for (SLONG d = SLONG(Sim.Persons.AnzEntries()) - 1; d >= 0; d--) {
                                if ((Sim.Persons.IsInAlbum(d) != 0) && (Sim.Persons[d].StatePar == 0) && PlayerNum != Sim.Persons[d].State &&
                                    (Clans.IsInAlbum(Sim.Persons[d].ClanId) != 0)) {
                                    PERSON &qPerson = Sim.Persons[d];
                                    CLAN &qClan = Clans[SLONG(qPerson.ClanId)];
                                    SLONG Type = qClan.Type;
                                    ...
                                    if (Type >= CLAN_PLAYER1 && Type <= CLAN_PLAYER4) {
                                        ...
                                        if (gMousePosition.IfIsWithin(p.x - (qClan.Phasen[0])[0].Size.x / 2, p.y - (qClan.Phasen[0])[0].Size.y,
                                                                      p.x + (qClan.Phasen[0])[0].Size.x / 2, p.y)) {
                                                ...
                                                if (abs(qPerson.Position.y - qPlayerPerson.Position.y) < 640) {
                                                    if (qPlayer.GetRoom() == ROOM_AIRPORT) {
                                                        if (qPlayer.CallItADay == FALSE) {
                                                            if ((qPlayer.IsStuck == 0) && (qPlayer.IsTalking == 0)) {
                                                                XY ArrayPos;
                                                                ...
                                                                if (qPerson.Position.y < 4000) {
                                                                    ArrayPos.y = qPerson.Position.y / 22 + 5;
                                                                } else {
                                                                ...
                                                                if (qPerson.Position.x > qPlayerPerson.Position.x) {
                                                                    if ((Airport.iPlate[ArrayPos.y + (ArrayPos.x << 4)] & 16) != 0) {
                                                                        if ((Airport.iPlate[ArrayPos.y + ((ArrayPos.x - 1) << 4)] & 16) != 0) {
                                                                    ...
                                                                    if ((Airport.iPlate[ArrayPos.y + (ArrayPos.x << 4)] & 64) != 0) {
                                                                        if ((Airport.iPlate[ArrayPos.y + ((ArrayPos.x + 1) << 4)] & 64) != 0) {
                                                                            Target = XY(ArrayPos.x + 2, ArrayPos.y);
                                                                        } else {
                                                                            Target = XY(ArrayPos.x + 1, ArrayPos.y);
                                                                        }
                                                                ...
                                                                if (qPlayerPerson.LookDir == 9) // Warte-Gimmick
                                                                {
                                                                    ...
                                                                    if (qPerson.Position.y > qPlayerPerson.Position.y) {
                                                                        qPlayerPerson.LookDir = 2;
                                                                    }
                                                                    if (qPerson.Position.x < qPlayerPerson.Position.x) {
                                                                        qPlayerPerson.LookDir = 3;
                                                                    }
                                                                    if (qPerson.Position.x > qPlayerPerson.Position.x) {
                                                                        qPlayerPerson.LookDir = 1;
                                                                    }
                                                                ...
                                                                if (Target.x != -100) {
                                                                    ...
                                                                    if (qPlayer.Owner == 1) {
                                                                        ...
                                                                        if (Sim.bNetwork != 0) {
                                                                            ...
                                                                            SIM::SendSimpleMessage(ATNET_DIALOG_LOCK, 0,
                                                                                                   Sim.Players.Players[Sim.localPlayer].PlayerNum);
                                                                        ...
                                                                        // Netzwerkspieler:
                                                                        Message << Phase << PlayerNum << qPerson.Position.x << qPerson.Position.y;
...
void AirportView::OnLButtonUp(UINT /*nFlags*/, CPoint point) {
    ...
    if (Sim.Players.Players[PlayerNum].IsWalking2Player != -1 || (IsDialogOpen() != 0) || (IgnoreNextLButtonUp != 0)) {
...
//--------------------------------------------------------------------------------------------
void AirportView::OnLButtonDblClk(UINT /*nFlags*/, CPoint point) {
    if (Sim.Time <= 9 * 60000) {
        return;
    }
    ...
    if (point.x >= WinP1.x && point.x <= WinP2.x && point.y >= WinP1.y && point.y <= WinP2.y && (Editor == 0)) {
        ...
        if (Sim.bNoTime == FALSE && (IsDialogOpen() == 0) && Sim.DontDisplayPlayer != Sim.localPlayer) {
            if (Sim.Players.Players[PlayerNum].IsStuck == 0) {
...
//--------------------------------------------------------------------------------------------
void AirportView::OnToggleEditor() { Editor ^= 1; }
        ...
        if (Editor == EDITOR_BUILDS) {
            if (nChar >= '0' && nChar <= '9' && UnderCursor != 0xffffffff && Airport.Builds.IsInAlbum(UnderCursor)) {
                ...
                if ((brickid >= 2030 && brickid <= 2039) || brickid <= 2001 || brickid <= 2045 || brickid == 2003 || brickid == RUNE_AREALO ||
                    ...
                    brickid == RUNE_DROPSUITCASE || brickid == RUNE_WAYPOINT || brickid == RUNE_WAYPOINT_WAIT || brickid == RUNE_WAYPOINT_START ||
                    brickid == RUNE_WAYPOINT_G || brickid == RUNE_CONDBLOCK || brickid == 337 || brickid == 338) {
                    while (Airport.Builds[UnderCursor].Par > 99)
                        Airport.Builds[UnderCursor].Par -= 100;
                    while (Airport.Builds[UnderCursor].Par > 25)
                        Airport.Builds[UnderCursor].Par -= 10;
...
XY AIRPORT::GetRandomBirthplace(BOOL Type, SLONG PlayerNum, TEAKRAND *pRand) {
    if (Type == 0) {
        return (GetRandomTypedRune(RUNE_CREATION, 0, false, pRand));
    }
...
void AIRPORT::GetRandomShop(XY &ReturnPosition, SLONG &ReturnStatePar, SLONG ClanType, UBYTE *pMood, TEAKRAND *pRand) {
    ...
    if (ReturnStatePar >= ROOM_LASTMIN_X1 && ReturnStatePar <= ROOM_LASTMIN_X4) {
        ReturnStatePar = ROOM_LAST_MINUTE;
    }
    if (ReturnStatePar >= ROOM_REISE_X1 && ReturnStatePar <= ROOM_REISE_X4) {
        ReturnStatePar = ROOM_REISEBUERO;
    }
    if (ReturnStatePar >= ROOM_MONITOR1 && ReturnStatePar <= ROOM_MONITOR9) {
        ReturnStatePar = ROOM_MONITOR1;
    }

    for (c = 0; c < SLONG(Runes.AnzEntries()); c++) {
        if (Runes[c].BrickId == (RUNE_2SHOP | 0x10000000) &&
            ((Runes[c].Par >= ROOM_SHOP1 && Runes[c].Par <= ROOM_SHOP4) || Runes[c].Par == ROOM_MONITOR1 || Runes[c].Par == ROOM_MUSEUM ||
             Runes[c].Par == ROOM_BANK || Runes[c].Par == ROOM_REISEBUERO || Runes[c].Par == ROOM_LAST_MINUTE ||
             (Runes[c].Par == ROOM_WC_F && ClanType == CLAN_FEMALE) || (Runes[c].Par == ROOM_WC_M && ClanType == CLAN_MALE))) {
            Buffer[Anz++] = c;
            if (Anz == 20) {
...
nofreetravelagents:
    ...
    if (Runes[static_cast<SLONG>(Buffer[c])].Par == ROOM_SHOP1 &&
        (SLONG(Sim.Time) < timeDutyOpen || ((Sim.Weekday == 5 || Sim.Weekday == 6) && SLONG(Sim.Time) > timeDutyClose - 60000))) {
        goto again;
    }
    if (Runes[static_cast<SLONG>(Buffer[c])].Par == ROOM_LAST_MINUTE && (SLONG(Sim.Time) > timeLastClose - 60000 || Sim.Weekday == 5)) {
        goto again;
    }
    if (Runes[static_cast<SLONG>(Buffer[c])].Par == ROOM_MUSEUM && (SLONG(Sim.Time) < timeMuseOpen || Sim.Weekday == 5 || Sim.Weekday == 6)) {
        goto again;
    }
    if (Runes[static_cast<SLONG>(Buffer[c])].Par == ROOM_REISEBUERO && (SLONG(Sim.Time) > timeReisClose - 60000 || Sim.Weekday == 6)) {
        goto again;
    }
            ...
            if (Sim.RoomBusy[ROOM_REISE_X1 + ((d + n) & 3)] == 0) {
                for (c = 0; c < SLONG(Runes.AnzEntries()); c++) {
                    if (Runes[c].BrickId == (RUNE_2SHOP | 0x10000000) && Runes[c].Par == ROOM_REISE_X1 + ((d + n) & 3)) {
                        ...
                        if (pMood != nullptr) {
                            (*pMood) = static_cast<UBYTE>(MoodPersonReise);
                        }
                        return;
                    }
                }
            ...
            if (Sim.RoomBusy[ROOM_MONITOR1 + ((d + n) % 6)] == 0) {
                for (c = 0; c < SLONG(Runes.AnzEntries()); c++) {
                    if (Runes[c].BrickId == (RUNE_2SHOP | 0x10000000) && Runes[c].Par == ROOM_MONITOR1 + ((d + n) & 3)) {
                        ...
                        if (pMood != nullptr) {
                            (*pMood) = static_cast<UBYTE>(MoodPersonScreen);
                        }
                        return;
                    }
                }
    ...
    if (pMood != nullptr) {
        ...
        } else if (ReturnStatePar == ROOM_MUSEUM) {
            (*pMood) = static_cast<UBYTE>(MoodPersonMuseum);
        } else if (ReturnStatePar == ROOM_WC_F || ReturnStatePar == ROOM_WC_M) {
            (*pMood) = static_cast<UBYTE>(MoodPersonToilet);
        }
    }
}
    ...
    if (NumFound != 0) {
        ...
        if (d >= ROOM_FLIGHTDISPLAY && d < ROOM_FLIGHTDISPLAY + 60) {
            if (d < ROOM_FLIGHTDISPLAY + 30) {
                ...
                if (TextBrickTexts[d - ROOM_FLIGHTDISPLAY - 30 + TextBrickTexts.AnzEntries() / 2].GetLength() == 0) {
                    return (0);
                }
            }
        }
...
XY AIRPORT::GetNextWaypointRune(UBYTE StartingWaypoint, UBYTE *CurrentWaypoint, ULONG *Gimmick) {
    ...
    for (c = SLONG(Runes.AnzEntries()) - 1; c >= 0; c--) {
        if ((Runes[c].BrickId == static_cast<SLONG>(RUNE_WAYPOINT | 0x10000000) || Runes[c].BrickId == static_cast<SLONG>(RUNE_WAYPOINT_WAIT | 0x10000000) ||
            ...
            Runes[c].Par == *CurrentWaypoint + 1) {
            *CurrentWaypoint = *CurrentWaypoint + 1;
            if (*Gimmick <= 1) {
                *Gimmick = static_cast<ULONG>(Runes[c].BrickId == static_cast<SLONG>(RUNE_WAYPOINT_G | 0x10000000));
            }
...
void AIRPORT::LoadAirport(SLONG LeftEnd, SLONG CheckIn, SLONG Office, SLONG Entry, SLONG Shops, SLONG Cafe, SLONG Security, SLONG Suitcase, SLONG WaitZone,
    ...
    for (c = d = 0; c < 10; c++) {
        ...
        if (c > 0) {
            ...
            // hprintf (0, "Merge Level %li with Dilatation %li (NewSize: %li Bricks / %li new)", c, Diletation[c], localBuilds[c-1].AnzEntries(),
            // localBuilds[c].AnzEntries());
        }
    }
    ...
    while (true) {
        BestD = 0;
        for (d = 1; d < 10; d++) {
            if (Count[d] >= localBuilds[d].AnzEntries()) {
                continue;
            }
            ...
            if (Count[BestD] >= localBuilds[BestD].AnzEntries()) {
                ...
                BestD = d;
                       ...
                       localBuilds[d][Count[d]].ScreenPos.y + Bricks[localBuilds[d][Count[d]].BrickId].GetBitmapDimension().y +
                               Bricks[localBuilds[d][Count[d]].BrickId].FloorOffset <
                           localBuilds[BestD][Count[BestD]].ScreenPos.y + Bricks[localBuilds[BestD][Count[BestD]].BrickId].GetBitmapDimension().y +
                               Bricks[localBuilds[BestD][Count[BestD]].BrickId].FloorOffset) {
                BestD = d;
            } else if (Bricks[localBuilds[d][Count[d]].BrickId].Layer == Bricks[localBuilds[BestD][Count[BestD]].BrickId].Layer &&
                       localBuilds[d][Count[d]].ScreenPos.y + Bricks[localBuilds[d][Count[d]].BrickId].GetBitmapDimension().y +
                               Bricks[localBuilds[d][Count[d]].BrickId].FloorOffset ==
                           localBuilds[BestD][Count[BestD]].ScreenPos.y + Bricks[localBuilds[BestD][Count[BestD]].BrickId].GetBitmapDimension().y +
                               Bricks[localBuilds[BestD][Count[BestD]].BrickId].FloorOffset &&
                       localBuilds[d][Count[d]].ScreenPos.x < localBuilds[BestD][Count[BestD]].ScreenPos.x) {
                BestD = d;
            }
        }
...
void AIRPORT::NewDay() {
    ...
    if (Sim.Players.Players[1].IsOut == 0) {
        Sim.Persons += PERSON(Clans.GetPlayerId(CLAN_PLAYER2), Airport.GetRandomBirthplace(TRUE, 2), 0, 99, 0, 0);
    }
    if (Sim.Players.Players[2].IsOut == 0) {
        Sim.Persons += PERSON(Clans.GetPlayerId(CLAN_PLAYER3), Airport.GetRandomBirthplace(TRUE, 3), 0, 99, 0, 0);
    }
    if (Sim.Players.Players[3].IsOut == 0) {
        Sim.Persons += PERSON(Clans.GetPlayerId(CLAN_PLAYER4), Airport.GetRandomBirthplace(TRUE, 4), 0, 99, 0, 0);
    }
    ...
    for (c = d = 0; c < Builds.AnzEntries(); c++) {
        ...
        // Sim.Persons+=PERSON (Clans.GetCustomerIdByGroup (Builds[c].Par), Builds[c].ScreenPos, REASON_WAYPOINT, Builds[c].Par, Builds[c].Par, 0,
        // (Builds[c].Par==240)?MoodPersonBone:0);
    }
}
...
SLONG AIRPORT::CalcPlateXPosition(BUILD &qBuild, SLONG BrickXOffset, SLONG Alignment) const {
    ...
    case -1:
        ...
        break; // Linke Seite
    case 0:
        AddX = Bricks[qBuild.BrickId].GetBitmapDimension().x / 2;
        break; // Mitte
    case +1:
        AddX = Bricks[qBuild.BrickId].GetBitmapDimension().x;
        break; // Rechte Seite
    default:
        TeakLibW_Exception(FNL, ExcNever);
    }
    ...
    if (Bricks[qBuild.BrickId].GetBitmapDimension().y - 2 + qBuild.ScreenPos.y < 210) {
        ...
        rc = (AddX + qBuild.ScreenPos.x - PlateOffset.x + 11 + BrickXOffset +
              (Bricks[qBuild.BrickId].GetBitmapDimension().y + qBuild.ScreenPos.y - 2 - 93) / 2) /
                 44 -
             1;
    } else {
        // untere Ebene:
        rc = (AddX + qBuild.ScreenPos.x - PlateOffset.x + 11 + BrickXOffset +
              (Bricks[qBuild.BrickId].GetBitmapDimension().y + qBuild.ScreenPos.y - 2 - 220) / 2) /
                 44 -
             1;
    }

    if (rc < 0 || rc >= PlateDimension.x) { /*hprintf (0, "(x=%li) in Line %li", rc, __LINE__);*/
        return (0);
    }
...
#define FUCK(a)                                                                                                                                                \
    ...
    for (d = e = 0, BestD = BestE = -1; d < SLONG(Builds.AnzEntries()); d++) {
        if (Builds.IsInAlbum(d) != 0 /*&& Builds[d].BrickId >= 100+0x10000000 && Builds[d].BrickId <= 109+0x10000000*/) {
            ...
            if (BestE == -1 || Builds[d].ScreenPos.x < Builds[BestE].ScreenPos.x) {
                BestE = d;
            }
        }
    }
            ...
            case OBST_RUNE:
                ...
                if (y >= 0 && y <= 15 && x >= 0) {
                    ...
                    case RUNE_BELT_RIGHT:
                        iPlate[y + (x << 4)] = UBYTE((iPlate[y + (x << 4)] & (~176)) | 1);
                        break;
                    case RUNE_BELT_LEFT:
                        iPlate[y + (x << 4)] = UBYTE((iPlate[y + (x << 4)] & (~224)) | 1);
                        break;

                    case RUNE_WESTEAST:
                        iPlate[y + (x << 4)] &= (~160);
                        break;
                    case RUNE_NORTHSOUTH:
                        iPlate[y + (x << 4)] &= (~80);
                        break;
...
void AIRPORT::TryDoor(XY ArrayPos, BOOL Player, SLONG PlayerNum) {
    ...
    for (c = Doors.AnzEntries() - 1; c >= 0; c--) {
        if (Doors[c].ArabDoor == 2) // Frachttür
        {
            ...
            if (Sim.Players.Players[PlayerNum].ExRoom != ROOM_FRACHT && Sim.Players.Players[PlayerNum].DirectToRoom != ROOM_FRACHT) {
                continue;
            }
        }
        ...
        if (Doors[c].ArabDoor == TRUE) {
            ...
            if (Doors[c].ArrayPos.x == ArrayPos.x &&
                (Doors[c].ArrayPos.y == ArrayPos.y - 1 || Doors[c].ArrayPos.y == ArrayPos.y || Doors[c].ArrayPos.y == ArrayPos.y + 1)) {
        ...
        } else if ((Doors[c].ArrayPos.x == ArrayPos.x && ((Doors[c].ArrayPos.y == ArrayPos.y - 1 && (Player == 0)) || Doors[c].ArrayPos.y == ArrayPos.y)) ||
                                    ...
                                    for (SLONG d = Sim.Persons.AnzEntries() - 1; d >= 0; d--) {
                                        if ((Sim.Persons.IsInAlbum(d) != 0) && Clans[static_cast<SLONG>(Sim.Persons[d].ClanId)].Type >= CLAN_PLAYER1 &&
                                            ...
                                            if ((Runes[c].ScreenPos - XY(-100, 4900) - Sim.Persons[d].ScreenPos).abs() < 100) {
                                                ...
                                                if (Sim.Persons[d].Position.y < 5000) {
                                                    Sim.Persons[d].ScreenPos.x -= 5;
                                                }
...
void AIRPORT::UpdateStaticDoorImage() {
    ...
    for (c = 0; c < 4; c++) {
        ...
        if (Sim.Players.Players[c].OfficeState == 2 || (Editor != 0)) {
            ...
            Bricks[0x10000000 + 734 + c].Bitmap[0].Destroy();
        }
    }
}
    ...
    for (c = 0; c < SLONG(Builds.AnzEntries()); c++) {
        if ((Builds.IsInAlbum(c) != 0) && Bricks[Builds[c].BrickId].ObstacleType == OBST_RUNE) {
            if ((Builds[c].BrickId >= 0x10000000 + RUNE_CREATION && Builds[c].BrickId <= 0x10000000 + RUNE_PCREATION2) ||
                ...
                Builds[c].BrickId == 0x10000000 + RUNE_AREALO || Builds[c].BrickId == 0x10000000 + RUNE_AREARU ||
                Builds[c].BrickId == 0x10000000 + RUNE_WAITPLANE || Builds[c].BrickId == 0x10000000 + RUNE_DROPSUITCASE ||
                Builds[c].BrickId == 0x10000000 + RUNE_EXIT_SUITCASE || Builds[c].BrickId == 0x10000000 + RUNE_CREATE_SUITCASE ||
                Builds[c].BrickId == 0x10000000 + RUNE_2CHECKIN_EXIT || Builds[c].BrickId == 0x10000000 + RUNE_WAYPOINT_START ||
                Builds[c].BrickId == 0x10000000 + RUNE_WAYPOINT || Builds[c].BrickId == 0x10000000 + RUNE_WAYPOINT_G ||
                Builds[c].BrickId == 0x10000000 + RUNE_WAYPOINT_WAIT || Builds[c].BrickId == 0x10000000 + RUNE_CONDBLOCK ||
                Builds[c].BrickId == 0x10000000 + RUNE_DURCHLEUCHTER) {
                AnzRunes++;
            }
        }
    }
            ...
            if ((Builds[c].BrickId >= 0x10000000 + RUNE_CREATION && Builds[c].BrickId <= 0x10000000 + RUNE_PCREATION2) ||
                ...
                if (Builds[c].BrickId != 0x10000000 + RUNE_CONDBLOCK) {
                    ...
                    if (Bricks[Runes[AnzRunes].BrickId].GetBitmapDimension().y - 2 + Runes[AnzRunes].ScreenPos.y < 170) {
                        ...
                        x = (Bricks[Runes[AnzRunes].BrickId].GetBitmapDimension().x / 2 + Runes[AnzRunes].ScreenPos.x - PlateOffset.x + 11 +
                             (Bricks[Runes[AnzRunes].BrickId].GetBitmapDimension().y + Runes[AnzRunes].ScreenPos.y - 2 - 93) / 2) /
                                44 -
                            1;
                        ...
                        x = (Bricks[Runes[AnzRunes].BrickId].GetBitmapDimension().x / 2 + Runes[AnzRunes].ScreenPos.x - PlateOffset.x + 11 +
                             (Bricks[Runes[AnzRunes].BrickId].GetBitmapDimension().y + Runes[AnzRunes].ScreenPos.y - 2 - 220) / 2) /
                                44 -
                            1;
...
char *AIRPORT::GetHallFilename() {
    if (HallNum == 0) {
        return (bprintf(HallFilenames[HallNum], HallLevel[0], HallLevel[1], HallLevel[2], HallLevel[3], HallLevel[4], HallLevel[5], HallLevel[6], HallLevel[7],
                        HallLevel[8]));
    }
            ...
            if (Sim.Players.Players[c].Planes.IsInAlbum(d) != 0) {
                ...
                if (e != -1) {
                    ...
                    if ((Plan->Flug[e].ObjectType == 1 || Plan->Flug[e].ObjectType == 2) && Plan->Flug[e].Gate >= 0) {
                        // Check-In:
                        if (Plan->Flug[e].Startdate == Sim.Date && (Plan->Flug[e].Startzeit - 1 == Sim.GetHour() || Plan->Flug[e].Startzeit == Sim.GetHour()) &&
                            Plan->Flug[e].VonCity == static_cast<ULONG>(Sim.HomeAirportId)) {
                            if (Plan->Flug[e].Startzeit + 1 < TextPrinted[Plan->Flug[e].Gate]) {
                                TextBricks[Plan->Flug[e].Gate].FillWith(0);
                                TextBricks[Plan->Flug[e].Gate].PrintAt(bprintf("%s-%s %li:00", (LPCTSTR)Cities[Plan->Flug[e].VonCity].Kuerzel,
                                                                               (LPCTSTR)Cities[Plan->Flug[e].NachCity].Kuerzel, Plan->Flug[e].Startzeit + 1),
                                                                       FontSmallWhiteX, TEC_FONT_LEFT, 0, 0, 132, 12);

                                TextBrickTexts[Plan->Flug[e].Gate] = bprintf("%s-%s %li:00", (LPCTSTR)Cities[Plan->Flug[e].VonCity].Name,
                                                                             (LPCTSTR)Cities[Plan->Flug[e].NachCity].Name, Plan->Flug[e].Startzeit + 1);
                                TextPrinted[Plan->Flug[e].Gate] = Plan->Flug[e].Startzeit + 1;
                            }
                        }
                        // Abflug:
                        if (Plan->Flug[e].Startdate == Sim.Date && (Plan->Flug[e].Startzeit == Sim.GetHour() || Plan->Flug[e].Startzeit - 1 == Sim.GetHour()) &&
                            Plan->Flug[e].VonCity == static_cast<ULONG>(Sim.HomeAirportId)) {
                            if (Plan->Flug[e].Startzeit + 1 < TextPrinted[TextBricks.AnzEntries() / 2 + Plan->Flug[e].Gate]) {
                                ...
                                TextBricks[TextBricks.AnzEntries() / 2 + Plan->Flug[e].Gate].PrintAt(
                                    bprintf("%s-%s %li:00", (LPCTSTR)Cities[Plan->Flug[e].VonCity].Kuerzel, (LPCTSTR)Cities[Plan->Flug[e].NachCity].Kuerzel,
                                            Plan->Flug[e].Startzeit + 1),
                                    FontSmallWhiteX, TEC_FONT_LEFT, 0, 0, 132, 12);

                                TextBrickTexts[TextBricks.AnzEntries() / 2 + Plan->Flug[e].Gate] =
                                    bprintf("%s-%s %li:00", (LPCTSTR)Cities[Plan->Flug[e].VonCity].Name, (LPCTSTR)Cities[Plan->Flug[e].NachCity].Name,
                                            Plan->Flug[e].Startzeit + 1);
                ...
                if (e != -1) {
                    // Dadurch wird's unten kompakter:Eintrag vorhanden?
                    if ((Plan->Flug[e].ObjectType == 1 || Plan->Flug[e].ObjectType == 2) && Plan->Flug[e].Gate >= 0) {
                        if (Plan->Flug[e].Landedate == Sim.Date && Plan->Flug[e].Landezeit == Sim.GetHour() &&
                            Plan->Flug[e].NachCity == static_cast<ULONG>(Sim.HomeAirportId)) {
                            if (Plan->Flug[e].Landezeit < TextPrinted[TextBricks.AnzEntries() / 2 + Plan->Flug[e].Gate]) {
                                TextBricks[TextBricks.AnzEntries() / 2 + Plan->Flug[e].Gate].FillWith(0);
                                TextBricks[TextBricks.AnzEntries() / 2 + Plan->Flug[e].Gate].PrintAt(
                                    bprintf("%s-%s %li:00", (LPCTSTR)Cities[Plan->Flug[e].VonCity].Kuerzel, (LPCTSTR)Cities[Plan->Flug[e].NachCity].Kuerzel,
                                            Plan->Flug[e].Landezeit),
                                    FontSmallWhiteX, TEC_FONT_LEFT, 0, 0, 132, 12);
