...
class RoomsParticipant {
  public:
    ...
    void SetInQuickJoin(bool b) {
        inQuickJoin = b;
        if (b)
            room = 0;
    }

    // Name is used for persistent invites and bans. Name should be unique among all participants or else the invites and bans will be applied to the wrong
    // players
...
struct RoomDescriptor {
    ...
    DataStructures::Table::Cell *GetProperty(int index) { return roomProperties.GetRowByIndex(0, 0)->cells[index]; }
...
struct RoomCreationParameters {
    ...
    RoomsErrorCode Validate(const DataStructures::List<RakNet::RakString> &otherRoomNames, ProfanityFilter *profanityFilter) const;
};
...
struct RoomQuery {
    ...
    void AddQuery_BINARY(const char *columnName, const char *input, int inputLength,
                         DataStructures::Table::FilterQueryType op = DataStructures::Table::QF_EQUAL);
...
class AllGamesRoomsContainer {
  public:
    ...
    RoomsErrorCode CreateRoom(RoomCreationParameters *roomCreationParameters, ProfanityFilter *profanityFilter);

    // Enters a room based on the search queries. If no rooms are available to join, will create a room instead
    RoomsErrorCode EnterRoom(RoomCreationParameters *roomCreationParameters, RoomMemberMode roomMemberMode, ProfanityFilter *profanityFilter, RoomQuery *query,
                             JoinedRoomResult *joinRoomResult);

    // Attempts to join a room by search query filters
    // Returns REC_JOIN_BY_FILTER_*
    RoomsErrorCode JoinByFilter(const GameIdentifier &gameIdentifier, RoomMemberMode roomMemberMode, RoomsParticipant *roomsParticipant, RoomID lastRoomJoined,
                                RoomQuery *query, JoinedRoomResult *joinRoomResult);
    ...
    static RoomsErrorCode SendInvite(RoomsParticipant *roomsParticipant, RoomsParticipant *inviteeId, bool inviteToSpectatorSlot,
    ...
    RoomsErrorCode GetReadyStatus(RoomID roomId, Room **room, DataStructures::List<RoomsParticipant *> &readyUsers,
    ...
    void GetRoomProperties(RoomID roomId, Room **room, DataStructures::Table *table) const;
    ...
    // 3. For each of these rooms, record that this member can potentially join by storing a copy of the pointer into quickJoinWorkingList, if minimumPlayers =>
    // total room slots For all rooms:
    ...
    RoomsErrorCode ProcessQuickJoins(DataStructures::List<QuickJoinUser *> &timeoutExpired, DataStructures::List<JoinedRoomResult> &joinedRoomMembers,
    ...
    bool IsInQuickJoin(RoomsParticipant *roomsParticipant) const;

    // Get all rooms for a certain title
    static int RoomsSortByName(Room *const &key, Room *const &data);
    RoomsErrorCode SearchByFilter(const GameIdentifier &gameIdentifier, RoomsParticipant *roomsParticipant, RoomQuery *roomQuery,
...
class PerGameRoomsContainer {
  ...
  protected:
    RoomsErrorCode CreateRoom(RoomCreationParameters *roomCreationParameters, ProfanityFilter *profanityFilter, RoomID lobbyRoomId, bool validate);
    RoomsErrorCode LeaveRoom(RoomsParticipant *roomsParticipant, bool *gotNewModerator);
    RoomsErrorCode JoinByFilter(RoomMemberMode roomMemberMode, RoomsParticipant *roomsParticipant, RoomID lastRoomJoined, RoomQuery *query,
                                JoinedRoomResult *joinRoomResult);
    ...
    static bool IsInQuickJoin(RoomsParticipant *roomsParticipant);
    unsigned int GetQuickJoinIndex(RoomsParticipant *roomsParticipant) const;
    void GetRoomNames(DataStructures::List<RakNet::RakString> &roomNames) const;
    ...
    unsigned ProcessQuickJoins(DataStructures::List<QuickJoinUser *> &timeoutExpired, DataStructures::List<JoinedRoomResult> &joinedRoomMembers,
                               DataStructures::List<QuickJoinUser *> &dereferencedPointers, RakNet::TimeMS elapsedTime, RoomID startingRoomId);

    // Sort an input list of rooms
    // Rooms are sorted by time created (longest is higher priority). If within one minute, then subsorted by total playable slot count (lower is higher
    // priority). When using EnterRoom or JoinByFilter, record the last roomOutput joined, and try to avoid rejoining the same roomOutput just left
    void RoomPrioritySort(RoomsParticipant *roomsParticipant, RoomQuery *roomQuery,
                          DataStructures::OrderedList<Room *, Room *, RoomsSortByTimeThenTotalSlots> &roomsOutput);

    RoomsErrorCode SearchByFilter(RoomsParticipant *roomsParticipant, RoomQuery *roomQuery,
                                  DataStructures::OrderedList<Room *, Room *, AllGamesRoomsContainer::RoomsSortByName> &roomsOutput, bool onlyJoinable);
...
class Room {
  public:
    ...
    ~Room();
    RoomsErrorCode SendInvite(RoomsParticipant *roomsParticipant, RoomsParticipant *inviteeId, bool inviteToSpectatorSlot, const RakNet::RakString &subject,
    ...
    RoomsErrorCode GetReadyStatus(DataStructures::List<RoomsParticipant *> &readyUsers, DataStructures::List<RoomsParticipant *> &unreadyUsers) const;
