        ...
        JoinedRoomResult *joinRoomResult)
{
    ...
    PerGameRoomsContainer *perGameRoomsContainer = perGamesRoomsContainers.Get(roomCreationParameters->gameIdentifier);
    RoomsErrorCode roomsErrorCode = perGameRoomsContainer->JoinByFilter(roomMemberMode, roomCreationParameters->firstUser, static_cast<RakNet::RoomID>(-1), query, joinRoomResult);
...
{
    return static_cast<int>(roomsParticipant->GetInQuickJoin())!=0;
}
...
Room::Room( RoomID _roomId, RoomCreationParameters *roomCreationParameters, DataStructures::Table::Row *_row, RoomsParticipant* roomsParticipant )
{
    ...
    tableRow->cells[DefaultRoomColumns::TC_DESTROY_ON_MODERATOR_LEAVE]->Set(static_cast<int>(roomCreationParameters->networkedRoomCreationParameters.destroyOnModeratorLeave));
...
RoomsErrorCode Room::SendInvite(RoomsParticipant* roomsParticipant, RoomsParticipant* inviteeId, bool inviteToSpectatorSlot, const RakNet::RakString& subject, const RakNet::RakString& body)
{
    ...
    if (pcjrr==PCJRR_NO_PUBLIC_SLOTS && !inviteToSpectatorSlot) { return REC_SEND_INVITE_NO_SLOTS;
}
...
bool Room::IsRoomDead() const
{
    return roomMemberList.Size()==0 || roomDestroyed;
}
...
bool Room::IsRoomLockedToSpectators() const
{
    return roomLockState==RLS_ALL_LOCKED;
}
bool Room::IsRoomLockedToPlayers() const
{
...
bool Room::AreAllPlayableSlotsFilled() const
{
    return !HasOpenPublicSlots() && !HasOpenReservedSlots();
}
bool Room::HasOpenPublicSlots() const
{
    return GetNumericProperty(DefaultRoomColumns::TC_REMAINING_PUBLIC_SLOTS)!=0.0;
}
bool Room::HasOpenReservedSlots() const
{
    return GetNumericProperty(DefaultRoomColumns::TC_REMAINING_RESERVED_SLOTS)!=0.0;
}
bool Room::HasOpenSpectatorSlots() const
{
