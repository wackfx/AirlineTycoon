...
RoomMember::RoomMember() {
    isReady = false;
    newMemberNotificationProcessed = false;
    joinTime = 0;
    roomMemberMode = RMM_PUBLIC;
    roomsParticipant = nullptr;
}
...
RemoveUserResult::~RemoveUserResult() = default;
...
void RoomDescriptor::Serialize(bool writeToBitstream, RakNet::BitStream *bitStream) {
    ...
    unsigned int i = 0;
    unsigned int roomMemberListSize = 0;
    unsigned int banListSize = 0;
...
RoomCreationParameters::~RoomCreationParameters() = default;
RoomsErrorCode RoomCreationParameters::Validate(const DataStructures::List<RakNet::RakString> &otherRoomNames, ProfanityFilter *profanityFilter) const {
    ...
    if ((profanityFilter != nullptr) && profanityFilter->HasProfanity(networkedRoomCreationParameters.roomName.C_String())) {
        return REC_ROOM_CREATION_PARAMETERS_ROOM_NAME_HAS_PROFANITY;
    }
    if (otherRoomNames.GetIndexOf(networkedRoomCreationParameters.roomName) != static_cast<unsigned>(-1)) {
    ...
    if (networkedRoomCreationParameters.roomName.SubStr(0, static_cast<unsigned int>(QUICK_JOIN_ROOM_NAME_LENGTH)) == QUICK_JOIN_ROOM_NAME) {
...
void RoomQuery::AddQuery_STRING(const char *columnName, const char *charValue, DataStructures::Table::FilterQueryType op) {
    if (charValue == nullptr || charValue[0] == 0) {
        return;
    }
...
void RoomQuery::AddQuery_BINARY(const char *columnName, const char *input, int inputLength, DataStructures::Table::FilterQueryType op) {
    if (input == nullptr || input[0] == 0 || inputLength == 0) {
        return;
    }
    SetupNextQuery(columnName, op);
...
void RoomQuery::SetQueriesToStatic() {
    queries = (DataStructures::Table::FilterQuery *)fq;
    for (int i = 0; i < 32; i++) {
...
AllGamesRoomsContainer::~AllGamesRoomsContainer() = default;
RoomsErrorCode AllGamesRoomsContainer::CreateRoom(RoomCreationParameters *roomCreationParameters, ProfanityFilter *profanityFilter) {
    if (roomCreationParameters->firstUser->GetRoom() != nullptr) {
        return REC_CREATE_ROOM_CURRENTLY_IN_A_ROOM;
    }
    if (roomCreationParameters->firstUser->GetInQuickJoin()) {
        return REC_CREATE_ROOM_CURRENTLY_IN_QUICK_JOIN;
    }

    if (!perGamesRoomsContainers.Has(roomCreationParameters->gameIdentifier)) {
...
RoomsErrorCode AllGamesRoomsContainer::EnterRoom(RoomCreationParameters *roomCreationParameters, RoomMemberMode roomMemberMode,
                                                 ProfanityFilter *profanityFilter, RoomQuery *query, JoinedRoomResult *joinRoomResult) {
    roomCreationParameters->roomOutput = nullptr;
    joinRoomResult->roomOutput = nullptr;
    ...
    RoomsErrorCode roomsErrorCode =
        perGameRoomsContainer->JoinByFilter(roomMemberMode, roomCreationParameters->firstUser, static_cast<RakNet::RoomID>(-1), query, joinRoomResult);
...
RoomsErrorCode AllGamesRoomsContainer::JoinByFilter(const GameIdentifier &gameIdentifier, RoomMemberMode roomMemberMode, RoomsParticipant *roomsParticipant,
                                                    RoomID lastRoomJoined, RoomQuery *query, JoinedRoomResult *joinRoomResult) {
    (void)lastRoomJoined;

    if (roomsParticipant->GetRoom() != nullptr) {
    ...
    joinRoomResult->agrc = this;
    RoomsErrorCode rec = perGameRoomsContainer->JoinByFilter(roomMemberMode, roomsParticipant, static_cast<RakNet::RoomID>(-1), query, joinRoomResult);
    joinRoomResult->roomDescriptor.FromRoom(joinRoomResult->roomOutput, this);
    return rec;
}
...
RoomsErrorCode AllGamesRoomsContainer::AddUserToQuickJoin(const GameIdentifier &gameIdentifier, QuickJoinUser *quickJoinMember) const {
    ...
    if (quickJoinMember->roomsParticipant->GetInQuickJoin()) {
...
RoomsErrorCode AllGamesRoomsContainer::RemoveUserFromQuickJoin(RoomsParticipant *roomsParticipant, QuickJoinUser **qju) const {
    RoomsErrorCode roomsErrorCode = REC_SUCCESS;
    unsigned int i = 0;
...
bool AllGamesRoomsContainer::IsInQuickJoin(RoomsParticipant *roomsParticipant) const {
    unsigned int i = 0;
    for (i = 0; i < perGamesRoomsContainers.Size(); i++) {
...
RoomsErrorCode AllGamesRoomsContainer::SearchByFilter(const GameIdentifier &gameIdentifier, RoomsParticipant *roomsParticipant, RoomQuery *roomQuery,
                                                      DataStructures::OrderedList<Room *, Room *, RoomsSortByName> &roomsOutput, bool onlyJoinable) const {
...
void AllGamesRoomsContainer::DestroyRoomIfDead(Room *room) const {
    if (room == nullptr || !room->IsRoomDead()) {
        return;
    }
    unsigned int i = 0;
    for (i = 0; i < perGamesRoomsContainers.Size(); i++) {
...
RoomsErrorCode AllGamesRoomsContainer::GetInvitesToParticipant(RoomsParticipant *roomsParticipant, DataStructures::List<InvitedUser *> &invites) const {
    unsigned int i = 0;
...
RoomsErrorCode AllGamesRoomsContainer::SendInvite(RoomsParticipant *roomsParticipant, RoomsParticipant *inviteeId, bool inviteToSpectatorSlot,
                                                  const RakNet::RakString &subject, const RakNet::RakString &body) {
    if (roomsParticipant->GetRoom() == nullptr) {
...
RoomsErrorCode AllGamesRoomsContainer::AcceptInvite(RoomID roomId, Room **room, RoomsParticipant *roomsParticipant,
                                                    const RakNet::RakString &inviteSender) const {
...
RoomsErrorCode AllGamesRoomsContainer::GrantModerator(RoomsParticipant *roomsParticipant, RoomsParticipant *newModerator,
...
RoomsErrorCode AllGamesRoomsContainer::SetCustomRoomProperties(RoomsParticipant *roomsParticipant, DataStructures::Table *table) const {
    if (roomsParticipant->GetRoom() == nullptr) {
    ...
    unsigned int perGamesRoomContainersIndex = 0;
    for (perGamesRoomContainersIndex = 0; perGamesRoomContainersIndex < perGamesRoomsContainers.Size(); perGamesRoomContainersIndex++) {
        if (perGamesRoomsContainers[perGamesRoomContainersIndex]->GetRoomByLobbyRoomID(roomsParticipant->GetRoom()->GetID()) != nullptr) {
            break;
        }
    }
    ...
    unsigned int newTableIndex = 0;
    unsigned int oldTableIndex = 0;
    DataStructures::Table *oldTable = &perGameRoomsContainer->roomsTable;
    DataStructures::Table::Row *row = nullptr;
    for (newTableIndex = 0; newTableIndex < table->GetColumnCount(); newTableIndex++) {
        ...
        if (oldTableIndex == static_cast<unsigned int>(-1)) {
            if (oldTable->GetColumnCount() < static_cast<unsigned int>(MAX_CUSTOM_QUERY_FIELDS)) {
        ...
        *(row->cells[oldTableIndex]) = *(table->GetRowByIndex(0, nullptr)->cells[newTableIndex]);
    }
    return REC_SUCCESS;
}
void AllGamesRoomsContainer::GetRoomProperties(RoomID roomId, Room **room, DataStructures::Table *table) const {
    table->Clear();
    unsigned int perGamesRoomContainersIndex = 0;
    for (perGamesRoomContainersIndex = 0; perGamesRoomContainersIndex < perGamesRoomsContainers.Size(); perGamesRoomContainersIndex++) {
...
RoomsErrorCode AllGamesRoomsContainer::ChangeRoomName(RoomsParticipant *roomsParticipant, const RakNet::RakString &newRoomName,
    ...
    for (i = 0; i < perGamesRoomsContainers.Size(); i++) {
        ...
        if (roomNames.GetIndexOf(newRoomName) != static_cast<unsigned int>(-1)) {
...
RoomsErrorCode AllGamesRoomsContainer::GetReadyStatus(RoomID roomId, Room **room, DataStructures::List<RoomsParticipant *> &readyUsers,
                                                      DataStructures::List<RoomsParticipant *> &unreadyUsers) const {
    *room = GetRoomByLobbyRoomID(roomId);
...
RoomsErrorCode AllGamesRoomsContainer::AreAllMembersReady(RoomID roomId, Room **room, bool *allReady) const {
    *room = GetRoomByLobbyRoomID(roomId);
    ...
    return (*room)->AreAllMembersReady(static_cast<unsigned int>(-1), allReady);
}
 ...
 */
Room *AllGamesRoomsContainer::GetRoomByLobbyRoomID(RoomID lobbyRoomID) const {
    unsigned int i = 0;
    Room *room = nullptr;
    for (i = 0; i < perGamesRoomsContainers.Size(); i++) {
        room = perGamesRoomsContainers[i]->GetRoomByLobbyRoomID(lobbyRoomID);
...
RoomsErrorCode AllGamesRoomsContainer::ProcessQuickJoins(DataStructures::List<QuickJoinUser *> &timeoutExpired,
                                                         DataStructures::List<JoinedRoomResult> &joinedRoomMembers,
                                                         DataStructures::List<QuickJoinUser *> &dereferencedPointers, RakNet::TimeMS elapsedTime) {
    unsigned int numRoomsCreated = 0;
    unsigned int i = 0;
...
PerGameRoomsContainer::~PerGameRoomsContainer() = default;
RoomsErrorCode PerGameRoomsContainer::CreateRoom(RoomCreationParameters *roomCreationParameters, ProfanityFilter *profanityFilter, RoomID lobbyRoomId,
                                                 bool validate) {
    if (validate) {
...
RoomsErrorCode PerGameRoomsContainer::JoinByFilter(RoomMemberMode roomMemberMode, RoomsParticipant *roomsParticipant, RoomID lastRoomJoined, RoomQuery *query,
...
RoomsErrorCode PerGameRoomsContainer::AddUserToQuickJoin(QuickJoinUser *quickJoinMember) {
    if (GetQuickJoinIndex(quickJoinMember->roomsParticipant) != static_cast<unsigned int>(-1)) {
        return REC_ADD_TO_QUICK_JOIN_ALREADY_THERE;
    }
...
RoomsErrorCode PerGameRoomsContainer::RemoveUserFromQuickJoin(RoomsParticipant *roomsParticipant, QuickJoinUser **qju) {
    ...
    *qju = nullptr;
    if (quickJoinIndex == static_cast<unsigned int>(-1)) {
        return REC_REMOVE_FROM_QUICK_JOIN_NOT_THERE;
    }
    ...
    *qju = quickJoinList[quickJoinIndex];
    ...
    roomsParticipant->SetPerGameRoomsContainer(nullptr);
    return REC_SUCCESS;
}
bool PerGameRoomsContainer::IsInQuickJoin(RoomsParticipant *roomsParticipant) { return static_cast<int>(roomsParticipant->GetInQuickJoin()) != 0; }
...
int PerGameRoomsContainer::RoomsSortByTimeThenTotalSlots(Room *const &key, Room *const &data) {
    ...
    int diff = static_cast<int>(abs(keyCreationTime - dataCreationTime));
    if (diff < 30 * 1000) {
...
unsigned PerGameRoomsContainer::ProcessQuickJoins(DataStructures::List<QuickJoinUser *> &timeoutExpired,
    ...
    if (!nextQuickJoinProcess.UpdateInterval(elapsedTime)) {
        return 0;
    }
    ...
    RoomsErrorCode roomsErrorCode = REC_SUCCESS;
    Room *room = nullptr;
    double totalRoomSlots = NAN;
    double remainingRoomSlots = NAN;
    ...
    while (true) {
        ...
        // 3. For each of these rooms, record that this member can potentially join by storing a copy of the pointer into quickJoinWorkingList, if
        // minimumPlayers => total room slots
        for (quickJoinIndex = 0; quickJoinIndex < quickJoinList.Size(); quickJoinIndex++) {
        ...
        // For all rooms:
        // 4. For each room where there are enough potential quick join members to fill the room, join all those members at once. Remove these members from the
        // quick join list. Go to 1.
        for (roomIndex = 0; roomIndex < allRooms.Size(); roomIndex++) {
            ...
            if (remainingRoomSlots > 0 && room->quickJoinWorkingList.Size() >= static_cast<unsigned int>(remainingRoomSlots)) {
                ...
                for (quickJoinIndex = 0; quickJoinIndex < static_cast<int>(room->quickJoinWorkingList.Size()); quickJoinIndex++) {
                    quickJoinMemberTimeSort.Insert(room->quickJoinWorkingList[quickJoinIndex], room->quickJoinWorkingList[quickJoinIndex], true,
                                                   _FILE_AND_LINE_);
                }

                for (quickJoinIndex = 0; quickJoinIndex < static_cast<unsigned>(remainingRoomSlots); quickJoinIndex++) {
    ...
    for (quickJoinIndex = 0; quickJoinIndex < quickJoinList.Size(); quickJoinIndex++) {
        if (quickJoinList[quickJoinIndex]->networkedQuickJoinUser.minimumPlayers <= static_cast<int>(quickJoinList.Size())) {
    ...
    unsigned quickJoinIndex2 = 0;
    DataStructures::Table resultTable;
    QuickJoinUser *quickJoinMember = nullptr;
    RoomCreationParameters roomCreationParameters;
    ...
    while (quickJoinIndex < quickJoinList.Size()) {
        ...
        for (queryIndex = 0; queryIndex < quickJoinMember->networkedQuickJoinUser.query.numQueries; queryIndex++) {
            ...
            if ((quickJoinMember->networkedQuickJoinUser.query.queries[queryIndex].operation == DataStructures::Table::QF_EQUAL) &&
                !DefaultRoomColumns::HasColumnName(quickJoinMember->networkedQuickJoinUser.query.queries[queryIndex].columnName) &&
                potentialNewRoom.ColumnIndex(quickJoinMember->networkedQuickJoinUser.query.queries[queryIndex].columnName) == -1 &&
                !quickJoinMember->networkedQuickJoinUser.query.queries[queryIndex].cellValue->isEmpty) {
                potentialNewRoom.AddColumn(quickJoinMember->networkedQuickJoinUser.query.queries[queryIndex].columnName,
                                           quickJoinMember->networkedQuickJoinUser.query.queries[queryIndex].cellValue->EstimateColumnType());
        ...
        for (quickJoinIndex2 = quickJoinIndex + 1; quickJoinIndex2 < quickJoinList.Size(); quickJoinIndex2++) {
            ...
            unsigned int subQueryCount = 0;
            unsigned int subQueryIndex = 0;
            for (subQueryIndex = 0, subQueryCount = 0; subQueryIndex < quickJoinList[quickJoinIndex2]->networkedQuickJoinUser.query.numQueries;
                 subQueryIndex++) {
            ...
            potentialNewRoom.QueryTable(columnIndices, 1, subQueries, subQueryCount, nullptr, 0, &resultTable);
            if (resultTable.GetRowCount() > 0) {
                potentialNewRoommates.Insert(quickJoinList[quickJoinIndex2], _FILE_AND_LINE_);
                if (potentialNewRoommates.Size() >= static_cast<unsigned int>(quickJoinMember->networkedQuickJoinUser.minimumPlayers) - 1) {
                    ...
                    roomsErrorCode = CreateRoom(&roomCreationParameters, nullptr, startingRoomId + numRoomsCreated, false);
                    ...
                    for (quickJoinIndex2 = 0; quickJoinIndex2 < potentialNewRoommates.Size(); quickJoinIndex2++) {
                        roomsErrorCode = roomCreationParameters.roomOutput->JoinByQuickJoin(potentialNewRoommates[quickJoinIndex2]->roomsParticipant,
                                                                                            RMM_PUBLIC, &joinedRoomResult);
                        RakAssert(roomsErrorCode == REC_SUCCESS);
...
RoomsErrorCode PerGameRoomsContainer::SearchByFilter(RoomsParticipant *roomsParticipant, RoomQuery *roomQuery,
    ...
    roomsTable.QueryTable(columnIndices, 1, roomQuery->queries, roomQuery->numQueries, nullptr, 0, &resultTable);
    ...
    while (cur != nullptr) {
        for (i = 0; i < cur->size; i++) {
            ...
            room = static_cast<Room *>(cur->data[i]->cells[0]->ptr);
            if ((!onlyJoinable || room->ParticipantCanJoinRoom(roomsParticipant, false, true) == PCJRR_SUCCESS) &&
                !room->IsHiddenToParticipant(roomsParticipant)) {
...
void PerGameRoomsContainer::RoomPrioritySort(RoomsParticipant *roomsParticipant, RoomQuery *roomQuery,
                                             DataStructures::OrderedList<Room *, Room *, RoomsSortByTimeThenTotalSlots> &roomsOutput) {
    int i = 0;
    // If you don't care about room filters, just join any room
    if (roomQuery == nullptr || roomQuery->numQueries == 0 || roomQuery->queries == nullptr) {
        DataStructures::List<Room *> rooms;
        GetAllRooms(rooms);
        for (i = 0; static_cast<unsigned>(i) < rooms.Size(); i++) {
            Room *room = rooms[i];
            if (room->ParticipantCanJoinRoom(roomsParticipant, false, true) == PCJRR_SUCCESS && !room->IsHiddenToParticipant(roomsParticipant)) {
    ...
    // Of those that pass room query, sort by time (lower is first). If within one minute of each other, sort by number of users in playable slots (higher if
    // first) Be sure to return columns TC_LOBBY_ROOM_PTR, TC_USED_PUBLIC_PLUS_RESERVED_SLOTS, TC_LOBBY_ROOM_PTR
...
Room *PerGameRoomsContainer::GetRoomByLobbyRoomID(RoomID lobbyRoomID) const {
    DataStructures::Table::Row *row = roomsTable.GetRowByID(lobbyRoomID);
    if (row == nullptr) {
        return nullptr;
    }

    return static_cast<Room *>(roomsTable.GetRowByID(lobbyRoomID)->cells[DefaultRoomColumns::TC_LOBBY_ROOM_PTR]->ptr);
}
...
void PerGameRoomsContainer::GetAllRooms(DataStructures::List<Room *> &rooms) const {
    ...
    while (cur != nullptr) {
        for (i = 0; i < cur->size; i++) {
            rooms.Insert(static_cast<Room *>(cur->data[i]->cells[DefaultRoomColumns::TC_LOBBY_ROOM_PTR]->ptr), _FILE_AND_LINE_);
        }
        cur = cur->next;
    }
}
void PerGameRoomsContainer::GetRoomNames(DataStructures::List<RakNet::RakString> &roomNames) const {
    DataStructures::Page<unsigned, DataStructures::Table::Row *, _TABLE_BPLUS_TREE_ORDER> *cur = roomsTable.GetRows().GetListHead();
    int i = 0;
...
unsigned int PerGameRoomsContainer::GetQuickJoinIndex(RoomsParticipant *roomsParticipant) const {
    unsigned int i = 0;
...
Room::Room(RoomID _roomId, RoomCreationParameters *roomCreationParameters, DataStructures::Table::Row *_row, RoomsParticipant *roomsParticipant) {
    ...
    auto *roomMember = new RoomMember;
    ...
    tableRow->cells[DefaultRoomColumns::TC_ROOM_PASSWORD]->Set("ASS");
    tableRow->cells[DefaultRoomColumns::TC_CREATION_TIME]->Set(static_cast<int>(RakNet::GetTimeMS()));
    tableRow->cells[DefaultRoomColumns::TC_DESTROY_ON_MODERATOR_LEAVE]->Set(
        static_cast<int>(roomCreationParameters->networkedRoomCreationParameters.destroyOnModeratorLeave));
...
void Room::UpdateUsedSlots(DataStructures::Table::Row *tableRow, Slots *totalSlots, Slots *usedSlots) {
    ...
    tableRow->cells[DefaultRoomColumns::TC_REMAINING_SLOTS]->Set(totalSlots->publicSlots + totalSlots->reservedSlots + totalSlots->spectatorSlots -
                                                                 usedSlots->publicSlots - usedSlots->reservedSlots - usedSlots->spectatorSlots);
    tableRow->cells[DefaultRoomColumns::TC_REMAINING_PUBLIC_PLUS_RESERVED_SLOTS]->Set(totalSlots->publicSlots + totalSlots->reservedSlots -
                                                                                      usedSlots->publicSlots - usedSlots->reservedSlots);
...
void Room::UpdateUsedSlots(Slots *totalSlots, Slots *usedSlots) { UpdateUsedSlots(tableRow, totalSlots, usedSlots); }
Slots Room::GetTotalSlots() const {
    Slots totalSlots;
    totalSlots.publicSlots = static_cast<int>(GetNumericProperty(DefaultRoomColumns::TC_TOTAL_PUBLIC_SLOTS));
    totalSlots.reservedSlots = static_cast<int>(GetNumericProperty(DefaultRoomColumns::TC_TOTAL_RESERVED_SLOTS));
    totalSlots.spectatorSlots = static_cast<int>(GetNumericProperty(DefaultRoomColumns::TC_TOTAL_SPECTATOR_SLOTS));
    return totalSlots;
}
...
Slots Room::GetUsedSlots() const {
    ...
    for (i = 0; i < roomMemberList.Size(); i++) {
        ...
        } else if (roomMemberList[i]->roomMemberMode == RMM_SPECTATOR_PUBLIC || roomMemberList[i]->roomMemberMode == RMM_SPECTATOR_RESERVED) {
...
RoomsErrorCode Room::SendInvite(RoomsParticipant *roomsParticipant, RoomsParticipant *inviteeId, bool inviteToSpectatorSlot, const RakNet::RakString &subject,
                                const RakNet::RakString &body) {
    ...
    if (roomsParticipantIndex == -1) {
        return REC_SEND_INVITE_INVITOR_NOT_IN_ROOM;
    }
    ...
    if (IsInRoom(inviteeId)) {
        return REC_SEND_INVITE_INVITEE_ALREADY_IN_THIS_ROOM;
    }

    // Does inviteeId already have an invite from roomsParticipant (either slot type)?
    if (HasInvite(inviteeId->GetName())) {
        return REC_SEND_INVITE_INVITEE_ALREADY_INVITED;
    }

    // Can inviteeId join?
    ParticipantCanJoinRoomResult pcjrr = ParticipantCanJoinRoom(inviteeId, inviteToSpectatorSlot, false);
    if (pcjrr == PCJRR_BANNED) {
        return REC_SEND_INVITE_INVITEE_BANNED;
    }
    if (pcjrr == PCJRR_NO_PUBLIC_SLOTS && !inviteToSpectatorSlot) {
        return REC_SEND_INVITE_NO_SLOTS;
    }
    if (pcjrr == PCJRR_NO_PUBLIC_OR_RESERVED_SLOTS) {
        return REC_SEND_INVITE_NO_SLOTS;
    }
    if (pcjrr == PCJRR_LOCKED) {
        return REC_SEND_INVITE_ROOM_LOCKED;
    }
    ...
    if (memberMode == RMM_MODERATOR) {
        ...
        if (relevantPermission == NetworkedRoomCreationParameters::INVITE_MODE_ANYONE_CAN_INVITE) {
            canInvite = true;
        } else if ((relevantPermission == NetworkedRoomCreationParameters::INVITE_MODE_PUBLIC_SLOTS_CAN_INVITE ||
            ...
            canInvite = true;
        } else if ((relevantPermission == NetworkedRoomCreationParameters::INVITE_MODE_RESERVED_SLOTS_CAN_INVITE ||
                    relevantPermission == NetworkedRoomCreationParameters::INVITE_MODE_MODERATOR_OR_PUBLIC_OR_RESERVED_SLOTS_CAN_INVITE) &&
                   memberMode == RMM_RESERVED) {
            canInvite = true;
        } else if (relevantPermission == NetworkedRoomCreationParameters::INVITE_MODE_SPECTATOR_SLOTS_CAN_INVITE &&
                   (memberMode == RMM_SPECTATOR_PUBLIC || memberMode == RMM_SPECTATOR_RESERVED)) {
            canInvite = true;
        } else if (relevantPermission == NetworkedRoomCreationParameters::INVITE_MODE_MODERATOR_OR_PUBLIC_SLOTS_CAN_INVITE &&
                   (memberMode == RMM_SPECTATOR_PUBLIC || memberMode == RMM_SPECTATOR_RESERVED)) {
            canInvite = true;
        } else {
...
RoomsErrorCode Room::AcceptInvite(RoomsParticipant *roomsParticipant, const RakNet::RakString &inviteSender) {
    ...
    if (inviteIndex == -1) {
        return REC_ACCEPT_INVITE_NO_SUCH_INVITE;
    }
    ...
    if (pcjrr == PCJRR_BANNED) {
        return REC_ACCEPT_INVITE_BANNED;
    }
    if (pcjrr == PCJRR_NO_PUBLIC_SLOTS || pcjrr == PCJRR_NO_PUBLIC_OR_RESERVED_SLOTS) {
        return REC_ACCEPT_INVITE_NO_SLOTS;
    }
    if (pcjrr == PCJRR_LOCKED) {
        return REC_ACCEPT_INVITE_ROOM_LOCKED;
    }
    RakAssert(pcjrr == PCJRR_SUCCESS);
...
RoomsErrorCode Room::StartSpectating(RoomsParticipant *roomsParticipant) {
    ...
    unsigned int roomsParticipantIndex = GetRoomIndex(roomsParticipant);
    if (roomsParticipantIndex == -1) {
        return REC_START_SPECTATING_NOT_IN_ROOM;
    }
    ...
    if (roomMemberList[roomsParticipantIndex]->roomMemberMode == RMM_RESERVED || roomMemberList[roomsParticipantIndex]->roomMemberMode == RMM_MODERATOR) {
        roomMemberList[roomsParticipantIndex]->roomMemberMode = RMM_SPECTATOR_RESERVED;
    } else {
...
RoomsErrorCode Room::StopSpectating(RoomsParticipant *roomsParticipant) {
    ...
    unsigned int roomsParticipantIndex = GetRoomIndex(roomsParticipant);
    if (roomsParticipantIndex == -1) {
        return REC_STOP_SPECTATING_NOT_IN_ROOM;
    }
    if (roomMemberList[roomsParticipantIndex]->roomMemberMode != RMM_SPECTATOR_PUBLIC &&
        roomMemberList[roomsParticipantIndex]->roomMemberMode != RMM_SPECTATOR_RESERVED) {
    ...
    if (pcjrr == PCJRR_NO_PUBLIC_OR_RESERVED_SLOTS) {
        return REC_STOP_SPECTATING_NO_SLOTS;
    }
    if (pcjrr == PCJRR_LOCKED) {
        return REC_STOP_SPECTATING_ROOM_LOCKED;
    }
    RakAssert(pcjrr == PCJRR_SUCCESS);
...
RoomsErrorCode Room::GrantModerator(RoomsParticipant *roomsParticipant, RoomsParticipant *newModerator, DataStructures::List<InvitedUser> &clearedInvites) {
    ...
    unsigned int roomsParticipantIndex = GetRoomIndex(roomsParticipant);
    if (roomsParticipantIndex == -1) {
        return REC_GRANT_MODERATOR_NOT_IN_ROOM;
    }
    ...
    if (newModeratorIndex == -1) {
        return REC_GRANT_MODERATOR_NEW_MODERATOR_NOT_IN_ROOM;
    }
...
RoomsErrorCode Room::ChangeSlotCounts(RoomsParticipant *roomsParticipant, Slots slots) {
    ...
    unsigned int roomsParticipantIndex = GetRoomIndex(roomsParticipant);
    if (roomsParticipantIndex == -1) {
        return REC_CHANGE_SLOT_COUNTS_NOT_IN_ROOM;
    }
...
RoomsErrorCode Room::SetCustomRoomProperties(RoomsParticipant *roomsParticipant, DataStructures::Table *table) {
    ...
    unsigned int roomsParticipantIndex = GetRoomIndex(roomsParticipant);
    if (roomsParticipantIndex == -1) {
        return REC_SET_CUSTOM_ROOM_PROPERTIES_NOT_IN_ROOM;
    }
...
RoomsErrorCode Room::ChangeRoomName(RoomsParticipant *roomsParticipant, const RakNet::RakString &newRoomName, ProfanityFilter *profanityFilter) {
    ...
    unsigned int roomsParticipantIndex = GetRoomIndex(roomsParticipant);
    if (roomsParticipantIndex == -1) {
        return REC_CHANGE_ROOM_NAME_NOT_IN_ROOM;
    }
...
RoomsErrorCode Room::SetHiddenFromSearches(RoomsParticipant *roomsParticipant, bool _hiddenFromSearches) {
    ...
    unsigned int roomsParticipantIndex = GetRoomIndex(roomsParticipant);
    if (roomsParticipantIndex == -1) {
        return REC_SET_HIDDEN_FROM_SEARCHES_NOT_IN_ROOM;
    }
...
RoomsErrorCode Room::SetDestroyOnModeratorLeave(RoomsParticipant *roomsParticipant, bool destroyOnModeratorLeave) {
    ...
    unsigned int roomsParticipantIndex = GetRoomIndex(roomsParticipant);
    if (roomsParticipantIndex == -1) {
        return REC_SET_DESTROY_ON_MODERATOR_LEAVE_NOT_IN_ROOM;
    }
    ...
    tableRow->cells[DefaultRoomColumns::TC_DESTROY_ON_MODERATOR_LEAVE]->Set(static_cast<int>(destroyOnModeratorLeave));
    return REC_SUCCESS;
}
RoomsErrorCode Room::SetReadyStatus(RoomsParticipant *roomsParticipant, bool isReady) {
    ...
    unsigned int roomsParticipantIndex = GetRoomIndex(roomsParticipant);
    if (roomsParticipantIndex == -1) {
        return REC_SET_READY_STATUS_NOT_IN_ROOM;
    }

    // Is roomsParticipant a player?
    if (roomMemberList[roomsParticipantIndex]->roomMemberMode != RMM_MODERATOR && roomMemberList[roomsParticipantIndex]->roomMemberMode != RMM_PUBLIC &&
        roomMemberList[roomsParticipantIndex]->roomMemberMode != RMM_RESERVED) {
    ...
    if (autoLockReadyStatus && !isReady && (AreAllPlayableSlotsFilled() || IsRoomLockedToPlayers()) && allReady) {
...
RoomsErrorCode Room::GetReadyStatus(DataStructures::List<RoomsParticipant *> &readyUsers, DataStructures::List<RoomsParticipant *> &unreadyUsers) const {
    RakAssert(roomDestroyed == false);
...
RoomsErrorCode Room::SetRoomLockState(RoomsParticipant *roomsParticipant, RoomLockState _roomLockState) {
    ...
    unsigned int roomsParticipantIndex = GetRoomIndex(roomsParticipant);
    if (roomsParticipantIndex == -1) {
        return REC_SET_ROOM_LOCK_STATE_NOT_IN_ROOM;
    }
    ...
    if (static_cast<int>(roomLockState) < 0 || static_cast<int>(roomLockState) > static_cast<int>(RLS_ALL_LOCKED)) {
...
RoomsErrorCode Room::AreAllMembersReady(unsigned int exceptThisIndex, bool *allReady) const {
    RakAssert(roomDestroyed == false);

    // Is the ready status for all members true?
    unsigned int i = 0;
    for (i = 0; i < roomMemberList.Size(); i++) {
        if (!roomMemberList[i]->isReady && i != exceptThisIndex) {
            *allReady = false;
            return REC_SUCCESS;
        }
    }
...
RoomsErrorCode Room::KickMember(RoomsParticipant *roomsParticipant, RoomsParticipant *kickedParticipant, const RakNet::RakString &reason) {
    ...
    unsigned int roomsParticipantIndex = GetRoomIndex(roomsParticipant);
    if (roomsParticipantIndex == -1) {
        return REC_KICK_MEMBER_NOT_IN_ROOM;
    }
    ...
    kickedParticipant->SetRoom(nullptr);
...
RoomsErrorCode Room::UnbanMember(RoomsParticipant *roomsParticipant, const RakNet::RakString &name) {
    ...
    unsigned int roomsParticipantIndex = GetRoomIndex(roomsParticipant);
    if (roomsParticipantIndex == -1) {
        return REC_UNBAN_MEMBER_NOT_IN_ROOM;
    }
...
RoomsErrorCode Room::JoinByFilter(RoomsParticipant *roomsParticipant, RoomMemberMode roomMemberMode, JoinedRoomResult *joinRoomResult) {
    ...
    if (pcjrr == PCJRR_BANNED) {
        return REC_JOIN_BY_FILTER_BANNED;
    }
    if (pcjrr == PCJRR_NO_PUBLIC_SLOTS || pcjrr == PCJRR_NO_PUBLIC_OR_RESERVED_SLOTS) {
        return REC_JOIN_BY_FILTER_NO_SLOTS;
    }
    if (pcjrr == PCJRR_LOCKED) {
        return REC_JOIN_BY_FILTER_ROOM_LOCKED;
    }
    RakAssert(pcjrr == PCJRR_SUCCESS);
    ...
    if (firstInviteIndex != -1) {
        ...
        joinRoomResult->acceptedInvitor = nullptr;
    }
...
RoomsErrorCode Room::JoinByQuickJoin(RoomsParticipant *roomsParticipant, RoomMemberMode roomMemberMode, JoinedRoomResult *joinRoomResult) {
    ...
    if (roomsErrorCode != REC_SUCCESS) {
        int val = static_cast<int>(roomsErrorCode);
        val += static_cast<int>(REC_JOIN_BY_QUICK_JOIN_CANNOT_JOIN_AS_MODERATOR) - static_cast<int>(REC_JOIN_BY_FILTER_CANNOT_JOIN_AS_MODERATOR);
        roomsErrorCode = static_cast<RoomsErrorCode>(val);
    } else {
        // Moderator does not count towards a slot
        UpdateUsedSlots();
        RakAssert(GetNumericProperty(DefaultRoomColumns::TC_USED_SLOTS) == roomMemberList.Size() - 1);
...
ParticipantCanJoinRoomResult Room::ParticipantCanJoinAsPlayer(RoomsParticipant *roomsParticipant, bool asSpectator, bool checkHasInvite) {
    // Is the room locked?
    if (roomLockState == RLS_PLAYERS_LOCKED && !asSpectator) {
        return PCJRR_LOCKED;
    }
    ...
    if (!asSpectator) {
        ...
        if (!publicAvailable && !reservedAvailable) {
            return PCJRR_NO_PUBLIC_OR_RESERVED_SLOTS;
        }
    } else {
        if (!HasOpenSpectatorSlots()) {
...
bool Room::IsRoomDead() const { return roomMemberList.Size() == 0 || roomDestroyed; }
RoomsErrorCode Room::GetInvitesToParticipant(RoomsParticipant *roomsParticipant, DataStructures::List<InvitedUser *> &invites) const {
    RakAssert(roomDestroyed == false);
...
RoomsParticipant *Room::GetModerator() const {
    unsigned int i = 0;
    for (i = 0; i < roomMemberList.Size(); i++) {
...
RoomID Room::GetID() const { return lobbyRoomId; }
double Room::GetNumericProperty(int index) const { return tableRow->cells[index]->i; }
const char *Room::GetStringProperty(int index) const { return tableRow->cells[index]->c; }
void Room::SetNumericProperty(int index, double value) { tableRow->cells[index]->Set(value); }
void Room::SetStringProperty(int index, const char *value) { tableRow->cells[index]->Set(value); }
RoomsErrorCode Room::RemoveUser(RoomsParticipant *roomsParticipant, RemoveUserResult *removeUserResult) {
    ...
    if (roomMemberList[roomsParticipantIndex]->roomMemberMode == RMM_MODERATOR) {
        ...
        if ((destroyOnModeratorLeave != 0) || roomMemberList.Size() == 1) {
            ...
            unsigned int i = 0;
            for (i = 0; i < roomMemberList.Size(); i++) {
                roomMemberList[i]->roomsParticipant->SetRoom(nullptr);
            }
            roomDestroyed = true;
            removeUserResult->roomDestroyed = true;
    ...
    roomsParticipant->SetRoom(nullptr);
...
bool Room::IsRoomLockedToSpectators() const { return roomLockState == RLS_ALL_LOCKED; }
bool Room::IsRoomLockedToPlayers() const { return roomLockState == RLS_PLAYERS_LOCKED || roomLockState == RLS_ALL_LOCKED; }
bool Room::IsInRoom(RoomsParticipant *roomsParticipant) const { return GetRoomIndex(roomsParticipant) != -1; }
bool Room::HasInvite(const RakNet::RakString &roomsParticipant) { return GetFirstInviteIndex(roomsParticipant) != -1; }
...
bool Room::AreAllPlayableSlotsFilled() const { return !HasOpenPublicSlots() && !HasOpenReservedSlots(); }
bool Room::HasOpenPublicSlots() const { return GetNumericProperty(DefaultRoomColumns::TC_REMAINING_PUBLIC_SLOTS) != 0.0; }
bool Room::HasOpenReservedSlots() const { return GetNumericProperty(DefaultRoomColumns::TC_REMAINING_RESERVED_SLOTS) != 0.0; }
bool Room::HasOpenSpectatorSlots() const { return GetNumericProperty(DefaultRoomColumns::TC_REMAINING_SPECTATOR_SLOTS) != 0.0; }
bool Room::IsHiddenToParticipant(RoomsParticipant *roomsParticipant) const {
    if (!hiddenFromSearches) {
        return false;
    }
...
void AllGamesRoomsContainer::UnitTest() {
    ...
    roomCreationParameters.networkedRoomCreationParameters.inviteToSpectatorSlotPermission =
        NetworkedRoomCreationParameters::INVITE_MODE_RESERVED_SLOTS_CAN_INVITE;
    ...
    roomsErrorCode = agrc.EnterRoom(&roomCreationParameters, RMM_ANY_PLAYABLE, nullptr, nullptr, &joinedRoomResult);
    RakAssert(roomsErrorCode == REC_SUCCESS);
    RakAssert(roomsParticipant1.GetRoom() != 0);
    ...
    RakAssert(cells[0].c);
    RakAssert(strcmp(roomsParticipant1.GetRoom()->GetStringProperty(agrc.GetPropertyIndex(roomsParticipant1.GetRoom()->GetID(), "LevelName")), cells[0].c) ==
              0);
    ...
    // Join by filter
    roomsErrorCode = agrc.JoinByFilter(gameIdentifier1, RMM_ANY_PLAYABLE, &roomsParticipant2, static_cast<RakNet::RoomID>(-1), &roomQuery, &joinedRoomResult);
