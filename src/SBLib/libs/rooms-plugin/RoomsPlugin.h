...
struct JoinByFilter_Func : public RoomsPluginFunc {
    ...
    void PrintResult(void) {
        if (resultCode != REC_SUCCESS)
            ...
            printf("Joined room %s with id %i and %.0f used slots\n", joinedRoomResult.roomDescriptor.GetProperty(DefaultRoomColumns::TC_ROOM_NAME)->c,
                   joinedRoomResult.roomDescriptor.lobbyRoomId, joinedRoomResult.roomDescriptor.GetProperty(DefaultRoomColumns::TC_USED_SLOTS)->i);
            for (unsigned int i = 0; i < joinedRoomResult.roomDescriptor.roomMemberList.Size(); i++) {
                printf("%i. %s (%s)\n", i + 1, joinedRoomResult.roomDescriptor.roomMemberList[i].name.C_String(),
                       joinedRoomResult.roomDescriptor.roomMemberList[i].systemAddress.ToString());
            }
        }
    }
...
/// \details Spectators do not count towards room ready states. The moderator can lock the room, preventing users from spectating, or not allow spectators at
/// all \ingroup ROOMS_COMMANDS
...
/// \brief Stop spectating. This will rejoin the room as a player, using a reserved slot if we were invited, and a public slot if not or if there are no
/// reserved slots \details This may fail if the moderator locked the room, or if no slots are available. \ingroup ROOMS_COMMANDS
...
/// \brief Change the allowed slot counts for the room. Setting fewer slot counts than the number of players does not kick out players, though it may prevent
/// changing spectator status \ingroup ROOMS_COMMANDS
...
struct GetRoomProperties_Func : public RoomsPluginFunc {
    ...
    void PrintResult(void) {
        ...
        if (resultCode != REC_SUCCESS) {
            ...
            char out[8096];
            printf("room %s has %i columns and %i used slots\n", roomName.C_String(), roomDescriptor.roomProperties.GetColumnCount(),
...
/// \brief Kick a member out of the room. This will also automatically ban that member from rejoining as long as the moderator does not change, or the member is
/// unbanned \ingroup ROOMS_COMMANDS
...
/// If no rooms are available to join, but enough quick join members are present to create a room instead, this will be done. The room custom properties will be
/// any equality comparisons with networkedQuickJoinUser.query \ingroup ROOMS_COMMANDS
...
/// <LI>Process the callback, which will contain the original input parameters, plus the new output parameters. All structures contain resultCode, which
/// indicates if the operation was successful (REC_SUCCESS) or not (Anything else)
/// </OL>
/// <BR>
/// To use as a server:
/// <OL>
...
/// <LI>If you want other systems to be able to call RoomsPlugin::LoginRoomsParticipant(), call RoomsPlugin::AddLoginServerAddress() with the addresses of those
/// systems <LI>As users go online, call RoomsPlugin::LoginRoomsParticipant(). Login and Logoff is up to you to implement (or rely on other systems, such as
/// Lobby2) <LI>As users go offline, call RoomsPlugin::LogoffRoomsParticipant();
/// </OL>
...
class RAK_DLL_EXPORT RoomsPlugin : public PluginInterface2, public RoomsCallback {
  public:
    ...
    /// \param[in] loginServerAddress The server adding this user. Use RakNet::UNASSIGNED_SYSTEM_ADDRESS for not applicable. Otherwise, the address must
    /// previously have been added using AddLoginServerAddress() or the function will fail.
    ...
    /// \param[in] userName A unique string identifying the user
    /// \param[in] loginServerAddress The server removing. Use RakNet::UNASSIGNED_SYSTEM_ADDRESS for not applicable. Otherwise, the address must previously have
    /// been added using AddLoginServerAddress() or the function will fail.
