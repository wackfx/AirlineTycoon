...
ULONG GetPhysicalCdRomBitlist(void) {
    ...
    if (ERROR_SUCCESS == RegCreateKeyEx(HKEY_LOCAL_MACHINE, Subkey, 0, NULL, REG_OPTION_NON_VOLATILE, KEY_ENUMERATE_SUB_KEYS | KEY_EXECUTE | KEY_QUERY_VALUE,
                                ...
                                if (ERROR_SUCCESS == RegQueryValueEx(hKey4, strClass, NULL, NULL, (UBYTE *)Temp, &TempSize)) {
                                    if (stricmp(Temp, strCDROM) == 0) {
                                        ...
                                        if (ERROR_SUCCESS == RegQueryValueEx(hKey4, strDeviceDesc, NULL, NULL, (UBYTE *)Temp, &TempSize)) {
                                            strlwr(Temp);
                                            if (strstr(Temp, strVirtual) == 0 && strstr(Temp, strFakeCD) == 0 && strstr(Temp, strFastCD) == 0 &&
                                                strstr(Temp, strFakeCD2) == 0 && strstr(Temp, strPsuedoCD) == 0 && strstr(Temp, strCDSubst) == 0) {
                                                TempSize = 500;
...
bool CheckCDFile(int _filepos, int _filesize, int _code) {
    ...
    lstrcpy(fname, (LPCTSTR)(CString)gCDPath);
    fname[3] = 0;
...
void CheckSomeCdFileLength(void) {
    ...
    if (Sim.bNetwork && Sim.bIsHost == false)
        return;

    switch (NumNewDay++) {
    case 5:
        if (!CHECKCDFILE0001)
            CreditsFilesAreMissing = TRUE;
        break;
    case 8:
        if (!CHECKCDFILE0002)
            CreditsFilesAreMissing = TRUE;
