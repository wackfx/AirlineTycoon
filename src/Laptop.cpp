...
CLaptop::CLaptop(BOOL bHandy, ULONG PlayerNum) : CPlaner(bHandy, PlayerNum, Sim.Players.Players[static_cast<SLONG>(PlayerNum)].EarthAlpha, TRUE) {
    ...
    KommVarLampe = -1;
    SP_Lampe.ReSize(4);
    SP_Lampe.Clips[0].ReSize(0, "lapligv2.smk", "", XY(80, 403), SPM_IDLE, CRepeat(1, 1), CPostWait(30, 30), SMACKER_CLIP_CANCANCEL, nullptr, SMACKER_CLIP_SET,
                             0, &KommVarLampe, "A1", 0);
    SP_Lampe.Clips[1].ReSize(1, "lapligh.smk", "", XY(80, 403), SPM_IDLE, CRepeat(1, 1), CPostWait(0, 0), SMACKER_CLIP_DONTCANCEL, &KommVarLampe,
                             SMACKER_CLIP_SET | SMACKER_CLIP_PRE, -1, nullptr, "A3A1", 0, 2);
    SP_Lampe.Clips[2].ReSize(2, "lapligw1.smk", "", XY(80, 403), SPM_IDLE, CRepeat(1, 1), CPostWait(10, 30), SMACKER_CLIP_DONTCANCEL, &KommVarLampe,
                             SMACKER_CLIP_SET | SMACKER_CLIP_PRE, -1, nullptr, "A1", 0);
    SP_Lampe.Clips[3].ReSize(3, "lapligw1.smk", "", XY(80, 403), SPM_IDLE, CRepeat(1, 1), CPostWait(5, 5), SMACKER_CLIP_DONTCANCEL, &KommVarLampe,
                             SMACKER_CLIP_SET | SMACKER_CLIP_PRE, -1, nullptr, "A1", 0);

    SP_Antivir.ReSize(1);
    SP_Antivir.Clips[0].ReSize(1, "antivir.smk", "", XY(522, 57), SPM_IDLE, CRepeat(1, 1), CPostWait(0, 0), SMACKER_CLIP_CANCANCEL, nullptr, SMACKER_CLIP_SET,
                               0, nullptr, "A1", 0);
    ...
    if (qPlayer.LaptopVirus != 0) {
        ...
        SP_Buttons[1].Clips[0].ReSize(0, "tdmstand.smk", "", XY(88, 53), SPM_IDLE, CRepeat(1, 1), CPostWait(30, 30), SMACKER_CLIP_CANCANCEL, nullptr,
                                      SMACKER_CLIP_SET, 0, &KommVarButtons[1], "A1", 0);
        ...
        SP_Buttons[0].Clips[0].ReSize(0, "tdmstand.smk", "", XY(67, 128), SPM_IDLE, CRepeat(1, 1), CPostWait(30, 30), SMACKER_CLIP_CANCANCEL, nullptr,
                                      SMACKER_CLIP_SET, 0, &KommVarButtons[0], "A1", 0);
        ...
        SP_Buttons[4].Clips[0].ReSize(0, "tdmstand.smk", "", XY(58, 207), SPM_IDLE, CRepeat(1, 1), CPostWait(30, 30), SMACKER_CLIP_CANCANCEL, nullptr,
                                      SMACKER_CLIP_SET, 0, &KommVarButtons[4], "A1", 0);
        ...
        SP_Buttons[3].Clips[0].ReSize(0, "virf_glu.smk", "", XY(68, 278), SPM_IDLE, CRepeat(1, 1), CPostWait(0, 0), SMACKER_CLIP_CANCANCEL, nullptr,
                                      SMACKER_CLIP_SET, 0, &KommVarButtons[3], "A1", 0);
        SP_Buttons[3].Clips[1].ReSize(1, "virf_tou.smk", "", XY(68, 278), SPM_IDLE, CRepeat(1, 1), CPostWait(0, 0), SMACKER_CLIP_DONTCANCEL, &KommVarButtons[3],
                                      SMACKER_CLIP_SET | SMACKER_CLIP_PRE, -1, nullptr, "A1", 0);

        SP_Buttons[2].Clips[0].ReSize(0, "virr_glu.smk", "", XY(94, 349), SPM_IDLE, CRepeat(1, 1), CPostWait(0, 0), SMACKER_CLIP_CANCANCEL, nullptr,
                                      SMACKER_CLIP_SET, 0, &KommVarButtons[2], "A1", 0);
        SP_Buttons[2].Clips[1].ReSize(1, "virr_tou.smk", "", XY(94, 349), SPM_IDLE, CRepeat(1, 1), CPostWait(0, 0), SMACKER_CLIP_DONTCANCEL, &KommVarButtons[2],
                                      SMACKER_CLIP_SET | SMACKER_CLIP_PRE, -1, nullptr, "A1", 0);
    ...
    PlaneRouteBms.ReSize(pRoomLib, GFX_PLANE, GFX_PLANE_B0, GFX_PLANE_B1, GFX_PLANE_V0, GFX_PLANE_V1, GFX_PLANE_R0, GFX_PLANE_R1, GFX_PLANE_G0, GFX_PLANE_G1,
                         GFX_ROUTE, GFX_ROUTE_B0, GFX_ROUTE_B1, GFX_ROUTE_B2, GFX_ROUTE_V0, GFX_ROUTE_V1, GFX_ROUTE_V2, GFX_ROUTE_R0, GFX_ROUTE_R1,
...
void CLaptop::OnLButtonDown(UINT nFlags, CPoint point) {
    ...
    if ((PreLButtonDown(gMousePosition) != 0) || qPlayer.LaptopBattery <= 0) {
        return;
    }
    ...
    if (CurrentBlock != -1) {
        ...
        if (MouseClickArea == ROOM_LAPTOP && MouseClickId == 100) {
            ...
            if (MouseClickPar1 == 31) {
                ...
                ScrollOffset = gMousePosition - qPlayer.Blocks[CurrentBlock].ScreenPos -
                               XY(5, 43 + (138 - 43) * pBlock->Page / pBlock->PageSize / (pBlock->AnzPages - 1)) - XY(0, 10);
                ScrollBlock = CurrentBlock;
                ScrollSide = 0;
            } else if (MouseClickPar1 == 89) {
                ScrollOffset = gMousePosition - qPlayer.Blocks[CurrentBlock].ScreenPos -
                               XY(376, 43 + (138 - 43) * pBlock->PageB / pBlock->PageSizeB / (pBlock->AnzPagesB - 1)) - XY(0, 10);
                ScrollBlock = CurrentBlock;
                ScrollSide = 1;
        ...
        if (Sim.Players.Players[PlayerNum].LaptopVirus == 0) {
            if (MouseClickPar1 == 90) {
                FensterVisible ^= 1;
            } else if (MouseClickPar1 == 70) {
                qPlayer.DisplayPlanes[0] ^= 1;
            } else if (MouseClickPar1 == 71) {
                qPlayer.DisplayPlanes[1] ^= 1;
            } else if (MouseClickPar1 == 72) {
                qPlayer.DisplayPlanes[2] ^= 1;
            } else if (MouseClickPar1 == 73) {
                qPlayer.DisplayPlanes[3] ^= 1;
            } else if (MouseClickPar1 == 80) {
                qPlayer.DisplayRoutes[0] = UBYTE((qPlayer.DisplayRoutes[0] + 1) % 3);
            } else if (MouseClickPar1 == 81) {
                qPlayer.DisplayRoutes[1] = UBYTE((qPlayer.DisplayRoutes[1] + 1) % 3);
            } else if (MouseClickPar1 == 82) {
                qPlayer.DisplayRoutes[2] = UBYTE((qPlayer.DisplayRoutes[2] + 1) % 3);
            } else if (MouseClickPar1 == 83) {
                qPlayer.DisplayRoutes[3] = UBYTE((qPlayer.DisplayRoutes[3] + 1) % 3);
            }
            ...
            if (MouseClickPar1 / 10 == 7) {
                KommVarButtons[3] = 1;
            }
            if (MouseClickPar1 / 10 == 8) {
                KommVarButtons[2] = 1;
            }
        }
    }
...
void CLaptop::UpdateWarningLightModes() {
    ...
    if (qPlayer.Auftraege.GetNumOpen() != 0) {
        WarningLightModes[0] = 1;
        if (qPlayer.Auftraege.GetNumDueToday() != 0) {
            WarningLightModes[0] = 2;
        }
    } else {
        WarningLightModes[0] = 0;
    }

    if (qPlayer.Frachten.GetNumOpen() != 0) {
        WarningLightModes[1] = 1;
        if (qPlayer.Frachten.GetNumDueToday() != 0) {
            WarningLightModes[1] = 2;
        }
    } else {
        WarningLightModes[1] = 0;
    }

    if (qPlayer.RentRouten.GetNumUsed() != 0) {
