            ...
            for (c = 0; c < 10 + 4; c++) {
                ...
                if (ShadowPass == 1) {
                    ...
                    if (PhaseListsNumbers[c] == -1) {
                        ...
                        if (id >= 0x01ff00ff00ff00ff) {
                            add = 7;
                        } else if (id >= 0x01ff00ff00ff00) {
                            add = 6;
                        } else if (id >= 0x01ff00ff00ff) {
                            add = 5;
        ...
        if (IsInAlbum(n) != 0) {
            ...
            for (c = 0; c < 10 + 4; c++) {
                ...
                if (gUpdatingPools != 0) {
                    ...
                    if (c == 4) {
                        qClan.Phasen[c].ReSize(&qClan.ClanGimmick, qClan.PhasenIds[c]);
                    } else if (c == 9) {
                        qClan.Phasen[c].ReSize(&qClan.ClanWarteGimmick, qClan.PhasenIds[c]);
                    } else {
                        qClan.Phasen[c].ReSize(&qClan.ClanPool, qClan.PhasenIds[c]);
                    }
...
void CLAN::BlitAt(SBPRIMARYBM &Offscreen, SLONG Dir, SLONG Phase, XY ScreenPos, UBYTE Running) {
    ...
    if (Dir < 8 || Dir == 9) {
        if (Phase / localFaktor >= Phasen[localDir].AnzEntries()) {
            Phase = 0;
        }
        pbm = &(Phasen[localDir])[Phase / localFaktor];
    } else {
        if (Phase >= Phasen[8].AnzEntries()) {
            Phase = 0;
        }
        pbm = &(Phasen[8])[Phase];
    }
    ...
    if (pbm != nullptr) {
        if (ScreenPos.x > -60 && ScreenPos.x < RightAirportClip + 50) {
            ...
            if (Type >= CLAN_PLAYER1 && Type <= CLAN_PLAYER4) {
                if (Sim.Players.Players[Type - CLAN_PLAYER1].DirectToRoom == ROOM_SECURITY ||
                    Sim.Players.Players[Type - CLAN_PLAYER1].ExRoom == ROOM_SECURITY) {
                    bNotSecurity = false;
                }
            }
            ...
            if (Type != 30 && (Sim.Options.OptionSchatten != 0) && localDir != 5 && bNotSecurity) {
                ...
                if ((pbm != nullptr) && (pbm->Size.x != 0) && (pbm->Size.y != 0)) {
                    Size = XY(pbm->Size.x / 2, pbm->Size.y - 1);

                    ColorFX.BlitAlpha(pbm->pBitmap, &PrimaryBm.PrimaryBm,
                                      ScreenPos - Size + ShadowOffset /*qPerson.ScreenPos-ViewPos+XY(-7-gShadowBm.Size.x/2,-2)*/);
                }
            }
...
void CLAN::BlitSkelettAt(SBPRIMARYBM &Offscreen, SLONG Dir, SLONG Phase, XY ScreenPos) {
    ...
    if (Dir < 8) {
        if (Phase / Faktor >= Skelett[Dir].AnzEntries()) {
            Phase = 0;
        }
        pbm = &(Skelett[Dir])[Phase / Faktor];
    } else {
        if (Phase >= Skelett[8].AnzEntries()) {
            Phase = 0;
        }
        pbm = &(Skelett[8])[Phase];
    }
...
void CLAN::BlitLargeAt(SBBM &Offscreen, SLONG Dir, SLONG Phase, XY ScreenPos) {
    ...
    if (Dir < 8 || Dir == 9) {
        if (Phase / Faktor >= Phasen[Dir].AnzEntries()) {
            Phase = 0;
        }
...
TEAKFILE &operator>>(TEAKFILE &File, CLAN &Clan) {
    ...
    if ((tmp != 0) && (Clan.TodayInGame == 0)) {
        Clan.ClanPool.Load();
        Clan.ClanGimmick.Load();
        Clan.ClanWarteGimmick.Load();
    } else if ((tmp != 0) && (Clan.TodayInGame == 0)) {
...
UBYTE CLANS::GetCustomerId(SLONG Browned, SLONG Koffer, TEAKRAND *pRand) {
    ...
    if (CheatMoreNuns != 0) {
        ...
        if (CheatMoreNuns == 2) {
            return (GetCustomerIdByGroup(10));
        }
        if (CheatMoreNuns == 3) {
            return (GetCustomerIdByGroup(11));
        }
    }
                    ...
                    if ((*this)[c].TodayInGame != 0) {
                        ...
                        if ((*this)[c].HasSuitcase != 0) {
                            for (d = 0; d < AnzEntries(); d++) {
                                if ((IsInAlbum(d) != 0) && (*this)[d].Type <= 11 && (*this)[d].Group <= 0 &&
                                    (*this)[c].HasSuitcase == -(*this)[d].HasSuitcase) {
                                    if (FirstDay == 0) {
...
PERSON::PERSON(UBYTE ClanId, XY Position, UBYTE Reason, UBYTE FlightAirline, SLONG FlightPlaneId, UBYTE FlightPlaneIndex, UBYTE Mood, UBYTE FirstClass) {
    ...
    case REASON_SHOPPING:
        ...
        Airport.GetRandomShop(Target, StatePar, Clans[static_cast<SLONG>(ClanId)].Type, &(PERSON::Mood),
                              &PersonRandCreate); // Der Shop wird in StatePar gespeichert
        break;
        ...
        if (State != 255) {
            if (Smokers[State].Smoke.AnzEntries() != 0) {
                for (x = -1; x <= 1; x++) {
                    ...
                    if (ArrayPos.y > 5) {
                        Airport.iPlate[ArrayPos.y - 1 + ((ArrayPos.x + x) << 4)] |= 1;
                    }
                    if (ArrayPos.y < 15) {
                        Airport.iPlate[ArrayPos.y + 1 + ((ArrayPos.x + x) << 4)] |= 1;
                    }
...
void PERSON::DoOneCustomerStep() {
    ...
    if (qClan.Group == 90 && (Sim.GetHour() < 13 || Sim.Weekday == 5 || Sim.Weekday == 6)) {
        return;
    }

    if (((State & PERSON_WAITFLAG) != 0) && (State & (~PERSON_WAITFLAG)) == PERSON_BOARDING && WaitCount <= 250) {
        return;
    }
    ...
    // Stinkt es hier?
    if (ArrayPos.y >= 0 && ArrayPos.y <= 9 && Mood != MoodPersonSick && ((Airport.iPlate[ArrayPos.y + 5 + (ArrayPos.x << 4)] & 1) != 0)) {
        if (Mood == MoodPersonBone || Mood == MoodPersonStenchDog) {
            Mood = UBYTE(MoodPersonStenchDog);

            if (MoodCountdown != 0U) {
                MoodCountdown = max(MoodCountdown, MOODCOUNT_START - 16);
            } else {
                MoodCountdown = MOODCOUNT_START + rand() % 7;
            }
    ...
    if ((NewPos.x + 4422) / 44 != (Position.x + 4422) / 44 || (NewPos.y + 2211) / 22 != (Position.y + 2211) / 22 || Dir == 4) {
        ...
        if (ArrayPos.y < 2) {
            Airport.TryDoor(XY(ArrayPos.x, ArrayPos.y + 5), 0, 0);
        }
        ...
        if ((State & (~PERSON_WAITFLAG) & (~PERSON_BROWSEFLAG)) == PERSON_WAITING) {
            ...
            if (ArrayPos.y > 0 && ArrayPos.y < 7) {
                ...
                if ((Airport.SeatsTaken[ArrayPos.x] & (1 << ((ArrayPos.y + 1) * 2))) == 0) {
        ...
        // Am Check-In warten?
        if ((State & (~PERSON_WAITFLAG) & (~PERSON_BROWSEFLAG)) == PERSON_CHECKINGIN && abs(Position.y - Target.y) <= StepSize.y &&
            abs(Position.x - Target.x) <= StepSize.x) {
            Dir = 4;
            if (PersonalRandWalk.Rand(100) == 0) {
                PersonReachedTarget();
            }
        }
        ...
        DirtyWaypoingShortcut:
            ...
            case PERSON_WAITING:
                if (ArrayPos.y == 4 && ((NewPossibleDirs & 64) != 0) && ((Airport.SeatsTaken[ArrayPos.x + 3] & (1 << ((ArrayPos.y - 1) * 2 + 1))) == 0)) {
                    Dir = 1;
                } else if (ArrayPos.y == 2 && ((NewPossibleDirs & 64) != 0) &&
                           (((Airport.SeatsTaken[ArrayPos.x + 3] & (1 << ((ArrayPos.y + 1) * 2))) == 0) ||
                            ((Airport.SeatsTaken[ArrayPos.x + 2] & (1 << ((ArrayPos.y + 1) * 2))) == 0))) {
                    Dir = 1;
                } else {
                    do {
                    waiting_again:
                ...
                if (ArrayPos.y == 4 && Dir == 2) {
                    goto waiting_again;
                }
                if (ArrayPos.y == 0 && Dir == 0) {
                    goto waiting_again;
                }
                break;
            ...
            case PERSON_2SHOP:
                if (Position.x == Target.x) {
                    ...
                    if (Position.x < Target.x) // nach rechts
                    {
                        ...
                        BOOL bAussenErlaubt = static_cast<BOOL>((static_cast<BOOL>((Airport.iPlate[14 + (ArrayPos.x << 4)] & 64) != 0) != 0) &&
                                                                ((Airport.iPlate[14 + ((ArrayPos.x + 1) << 4)] & 64) != 0) &&
                                                                ((Airport.iPlate[14 + ((ArrayPos.x - 1) << 4)] & 64)) != 0);

                        if ((bFast != 0) && (bAussenErlaubt != 0) &&
                            (ArrayPos.y + 5 == 14 || (((Airport.iPlate[13 + ((ArrayPos.x + 1) << 4)] & 2) != 0) && Target.x - Position.x > 88))) {
                            ty = 209; // Schnelle Clans überholen außen, wenn jemand vor ihnen ist...
                        } else {
                            if (Airport.iPlateDir[13 + ((ArrayPos.x) << 4)] == 8 || Airport.iPlateDir[13 + ((ArrayPos.x + 1) << 4)] == 8 ||
                                ((bFast != 0) && ((Airport.iPlate[13 + ((ArrayPos.x + 1) << 4)] & 2) != 0) && Target.x - Position.x > 88) ||
                                (StepSize.x > 1 && (Airport.iPlateDir[13 + ((ArrayPos.x) << 4)] == 254 ||
                                                    Airport.iPlateDir[13 + ((ArrayPos.x + 1) << 4)] == 254))) // Falls der Weg blockiert ist....
                            {
                                if (bAussenErlaubt != 0) {
                                    ...
                                    for (c = 0; c < 5; c++) {
                                        if (((Airport.iPlateDir[12 + ((ArrayPos.x + c) << 4)] == 3 || Airport.iPlateDir[12 + ((ArrayPos.x + c) << 4)] == 254) &&
                                             ((Airport.iPlate[12 + ((ArrayPos.x + c) << 4)] & 2) != 0)) ||
                                            (Airport.iPlate[12 + ((ArrayPos.x + c) << 4)] & 64) == 0) {
                                            break;
                                        }
                                    }
                            ...
                            {
                                if (ArrayPos.y + 5 == 14 && Dir == 1 && (bAussenErlaubt != 0) && (Airport.iPlate[14 + ((ArrayPos.x + 1) << 4)] & 2) == 0) {
                                    ty = 209; // Es ist Platz, also überholen wir außen...
                                } else if (ArrayPos.y + 5 == 12 && (Dir == 1 || Dir == 0)) {
                                    // Prüfen, ob auf der gemeinsamen Innenspur Platz ist...
                                    for (c = 0; c < 5; c++) {
                                        if ((Airport.iPlateDir[12 + ((ArrayPos.x + c) << 4)] == 3 &&
                                             ((Airport.iPlate[12 + ((ArrayPos.x + c) << 4)] & 2) != 0)) ||
                                            (Airport.iPlate[12 + ((ArrayPos.x + c) << 4)] & 64) == 0) {
                                            break;
                                        }
                    ...
                    {
                        BOOL bFast = static_cast<BOOL>(StepSize.y > 4);
                        BOOL bAussenErlaubt = static_cast<BOOL>((static_cast<BOOL>((Airport.iPlate[10 + (ArrayPos.x << 4)] & 64) != 0) != 0) &&
                                                                ((Airport.iPlate[10 + ((ArrayPos.x - 1) << 4)] & 64) != 0) &&
                                                                ((Airport.iPlate[10 + ((ArrayPos.x + 1) << 4)] & 64)) != 0);

                        if ((bFast != 0) && (bAussenErlaubt != 0) &&
                            (ArrayPos.y + 5 == 10 || (((Airport.iPlate[11 + ((ArrayPos.x - 1) << 4)] & 2) != 0) && Position.x - Target.x > 88))) {
                            ty = 121; // Schnelle Clans überholen außen, wenn jemand vor ihnen ist...
                        } else {
                            if (Airport.iPlateDir[11 + ((ArrayPos.x) << 4)] == 8 || Airport.iPlateDir[11 + ((ArrayPos.x - 1) << 4)] == 8 ||
                                ((bFast != 0) && ((Airport.iPlate[11 + ((ArrayPos.x - 1) << 4)] & 2) != 0) && Position.x - Target.x > 88) ||
                                (StepSize.x > 1 && (Airport.iPlateDir[11 + ((ArrayPos.x) << 4)] == 254 ||
                                                    Airport.iPlateDir[11 + ((ArrayPos.x - 1) << 4)] == 254))) // Falls der Weg blockiert ist....
                            {
                                if (bAussenErlaubt != 0) {
                                    ...
                                    SLONG c = 0;
                                    for (; c < 5; c++) {
                                        if (((Airport.iPlateDir[12 + ((ArrayPos.x - c) << 4)] == 1 || Airport.iPlateDir[12 + ((ArrayPos.x - c) << 4)] == 254) &&
                                             ((Airport.iPlate[12 + ((ArrayPos.x - c) << 4)] & 2) != 0)) ||
                                            (Airport.iPlate[12 + ((ArrayPos.x - c) << 4)] & 16) == 0) {
                                            break;
                                        }
                                    }
                            ...
                            {
                                if (ArrayPos.y + 5 == 10 && Dir == 3 && (bAussenErlaubt != 0) && (Airport.iPlate[10 + ((ArrayPos.x - 1) << 4)] & 2) == 0) {
                                    ty = 209; // Es ist Platz, also überholen wir außen...
                                } else if (ArrayPos.y + 5 == 12 && (Dir == 3 || Dir == 2)) {
                                    // Prüfen, ob auf der gemeinsamen Innenspur Platz ist...
                                    SLONG c = 0;
                                    for (; c < 5; c++) {
                                        if ((Airport.iPlateDir[12 + ((ArrayPos.x - c) << 4)] == 1 &&
                                             ((Airport.iPlate[12 + ((ArrayPos.x - c) << 4)] & 2) != 0)) ||
                                            (Airport.iPlate[12 + ((ArrayPos.x - c) << 4)] & 16) == 0) {
                                            break;
                                        }
            ...
            case PERSON_LEAVINGPL:
                ...
                if (Position.x >= Target.x) {
                    ...
                    if (Mood == MoodPersonSick) {
                        ...
                        if (qClan.HasSuitcase < 0 && (Clans.IsInAlbum(ClanId - 1) != 0) &&
                            Clans[static_cast<SLONG>(ClanId - 1)].HasSuitcase == -qClan.HasSuitcase) {
                            ...
                            if (Sim.RoomBusy[ROOM_BELT_X1 + n] == 0 || Sim.RoomBusy[ROOM_BELT_X1 + n] == 1) {
                                ...
                                for (c = 0; c < Airport.Runes.AnzEntries(); c++) {
                                    if (Airport.Runes[c].BrickId == (RUNE_2SHOP | 0x10000000) && Airport.Runes[c].Par == ROOM_BELT_X1 + n) {
                                        ...
                                        // Koffer erzeugen:
                                        Sim.PersonQueue.AddPerson(Clans.GetSuitcaseId(-qClan.HasSuitcase),
                                                                  Airport.GetRandomTypedRune(RUNE_CREATE_SUITCASE, 0, false, &PersonalRandWalk),
                                                                  REASON_SUITCASE_EXIT, 0, 0, 0);
                                        break;
                                    }
                                }
            ...
            case PERSON_BOARDING:
                if (Position.x == Target.x || ((NewPossibleDirs & 128) != 0)) {
                    if ((NewPossibleDirs & 128) != 0) {
                        Dir = 0;
                    } else {
                        Dir = 1;
                    }
                } else if (Position.x < Target.x) {
                    if ((NewPossibleDirs & 64) != 0) {
                        Dir = 1;
                    } else {
                        Dir = 0;
                    }
                    ...
                    if ((NewPossibleDirs & 16) != 0) {
                        Dir = 3;
                    } else {
                        Dir = 0;
                    }
                    if (abs(Position.x - Target.x) <= StepSize.x && ((NewPossibleDirs & 128) != 0)) {
                        Position.x = Target.x;
                        Dir = 4;
                    }
                }
    ...
    DirtyLabel:
        ...
        if (NewArrayPos.y >= 0 && NewArrayPos.y <= 9 && (State & PERSON_BROWSEFLAG) == 0) {
            ...
            if ((Airport.iPlate[Index] & 2) == 0 || WaitCount >= 200 || (State & (~PERSON_WAITFLAG) & (~PERSON_BROWSEFLAG)) == PERSON_2CHECKIN_EXIT ||
                (Dir == 1 && Airport.iPlateDir[Index] == 3) || (Dir == 3 && Airport.iPlateDir[Index] == 1)) {
                ...
                if (ArrayPos.y >= 1 + 2 && ArrayPos.y < 9 && Dir == 2 && Position.x > Target.x &&
                    (Airport.iPlate[ArrayPos.y + 5 + ((ArrayPos.x - 1) << 4)] & 2) == 0 && ((Airport.iPlate[ArrayPos.y + 5 + (ArrayPos.x << 4)] & 16) != 0) &&
                    abs(Position.x - Target.x) > StepSize.x * 4) {
                    Dir = 3; // Wir weichen nach links aus
                    StepSize.x += 4;
                    goto DirtyLabel;
                }
                //...oder nach rechts?
                else if (ArrayPos.y > 1 + 1 && ArrayPos.y < 9 && Dir == 2 && Position.x < Target.x &&
                         (Airport.iPlate[ArrayPos.y + 5 + ((ArrayPos.x + 1) << 4)] & 2) == 0 &&
                         ((Airport.iPlate[ArrayPos.y + 5 + (ArrayPos.x << 4)] & 64) != 0) && abs(Position.x - Target.x) > StepSize.x * 4) {
        ...
        // Sind wir noch im Bodenplatten-Array?
        if (ArrayPos.y >= 0 && ArrayPos.y <= 9 && (State & PERSON_BROWSEFLAG) == 0 && (State & (~PERSON_WAITFLAG) & (~PERSON_BROWSEFLAG)) != PERSON_BOARDING &&
            (State & (~PERSON_WAITFLAG) & (~PERSON_BROWSEFLAG)) != PERSON_2CHECKIN_EXIT &&
            (State & (~PERSON_WAITFLAG) & (~PERSON_BROWSEFLAG)) != PERSON_WAITING) {
            if ((Airport.iPlate[ArrayPos.y + 5 + (ArrayPos.x << 4)] & 8) == 8) {
                Airport.iPlate[ArrayPos.y + 5 + (ArrayPos.x << 4)] |= 2;
    ...
    if (ArrayPos.y >= 0 && ArrayPos.y <= 9 && ((Airport.iPlate[ArrayPos.y + 5 + (ArrayPos.x << 4)] & 2) != 0)) {
        if (StepSize.x == 1) {
...
void PERSON::DoOnePlayerStep() {
    ...
    if (qPlayer.IsStuck != 0) {
        ...
        Running = 0U;
        Phase = UBYTE((Phase + 1) %
                      (Clans[static_cast<SLONG>(ClanId)].Phasen[static_cast<SLONG>(LookDir)].AnzEntries() * Clans[static_cast<SLONG>(ClanId)].Faktor));
    ...
    } else if (LookDir == 9) // Warte-Gimmick-Animation
    {
        ...
        // Obere oder untere Ebene?
        if (Position.y < 4000) {
            ArrayPos.y = Position.y / 22 + 5; // unten!
        } else {
            ArrayPos.y = (Position.y - 5000) / 22; // oben!
        }
    ...
    switch (Dir) {
    case 0:
        NewPos.y = Position.y - StepSize.y * 2;
        break;
    case 1:
        NewPos.y = Position.y - StepSize.y;
        NewPos.x = Position.x + StepSize.x;
    ...
    if (qPlayer.PlayerStinking > 0 && Position.y < 4000) {
        for (SLONG x = -2; x <= 2; x++) {
            ...
            if (ArrayPos.y - 1 > 6) {
                Airport.iPlate[ArrayPos.y - 2 + ((ArrayPos.x + x) << 4)] &= ~1;
            }
            if (ArrayPos.y + 1 < 14) {
                Airport.iPlate[ArrayPos.y + 2 + ((ArrayPos.x + x) << 4)] &= ~1;
            }
    ...
    if ((NewPos.x + 4422) / 44 != (Position.x + 4422) / 44 || (NewPos.y + 2211) / 22 != (Position.y + 2211) / 22 || Dir == 8) {
        ...
        if (ArrayPos.y >= 0 && ArrayPos.y < 15) {
            if (ArrayPos.y > 0 && (Airport.iPlate[ArrayPos.y - 1 + (ArrayPos.x << 4)] & 4) == 0) {
                NewPossibleDirs &= (~128);
            }
            if (ArrayPos.y < 14 && (Airport.iPlate[ArrayPos.y + 1 + (ArrayPos.x << 4)] & 4) == 0) {
                NewPossibleDirs &= (~32);
            }
            if ((Airport.iPlate[ArrayPos.y + ((ArrayPos.x - 1) << 4)] & 4) == 0) {
                NewPossibleDirs &= (~16);
            }
            if ((Airport.iPlate[ArrayPos.y + ((ArrayPos.x + 1) << 4)] & 4) == 0) {
                NewPossibleDirs &= (~64);
        ...
        if (StatePar != 0) {
            ...
            if (StatePar < ROOM_STAIRSVON || StatePar > ROOM_STAIRSBIS) {
                ...
                if (Position.x > Target.x && (StatePar == ROOM_WERKSTATT || (StatePar == -1 && Position.x > Airport.RightEnd - 600))) {
                    Dir = 6;
                } else if (Position.x > Target.x && (StatePar == ROOM_FRACHT || (StatePar == -1 && Position.x < Airport.LeftEnd + 600))) {
                    Dir = 6;
                } else if ((Position.y + 1000) > ((Target.y + 1000) % 5000)) {
                    Dir = 0;
                } else if (Position.x > Target.x) {
                    Dir = 6;
                } else if (Position.x < Target.x) {
            ...
            if (StatePar == ROOM_WERKSTATT || (StatePar == -1 && Position.x > Airport.RightEnd - 600)) {
                if (Dir == 2 && (Airport.iPlate[ArrayPos.y + (ArrayPos.x << 4)] & 64) == 0) {
                    Dir = 3;
                }
                if (Dir == 6 && (Airport.iPlate[ArrayPos.y + (ArrayPos.x << 4)] & 16) == 0 &&
                    ((Airport.iPlate[ArrayPos.y + (ArrayPos.x << 4)] & 64) == 0 || (NewPos.x + 4444) / 44 < 33)) {
                    Dir = 7;
                }
            ...
            if (ArrayPos.y >= 5 && ArrayPos.y < 15) {
                switch (Dir) {
                case 2:
                    if (ArrayPos.y > Target.y / 22 + 5 && Target.x / 44 - ArrayPos.x <= ArrayPos.y - (Target.y / 22 + 5) &&
                        ((Airport.iPlate[ArrayPos.y + (ArrayPos.x << 4)] & 128) != 0) && ((Airport.iPlate[ArrayPos.y + (ArrayPos.x << 4)] & 64) != 0) &&
                        ((Airport.iPlate[ArrayPos.y - 1 + (ArrayPos.x << 4)] & 64) != 0) &&
                        ((Airport.iPlate[ArrayPos.y + ((ArrayPos.x + 1) << 4)] & 128) != 0) && ((Airport.iPlate[ArrayPos.y + (ArrayPos.x << 4)] & 4) != 0) &&
                        ((Airport.iPlate[ArrayPos.y - 1 + (ArrayPos.x << 4)] & 4) != 0) && ((Airport.iPlate[ArrayPos.y + ((ArrayPos.x + 1) << 4)] & 4) != 0)) {
                        Dir = 1; // Nord-Ost
                    } else if (ArrayPos.y < Target.y / 22 + 5 && Target.x / 44 - ArrayPos.x <= (Target.y / 22 + 5) - ArrayPos.y &&
                               ((Airport.iPlate[ArrayPos.y + (ArrayPos.x << 4)] & 32) != 0) && ((Airport.iPlate[ArrayPos.y + (ArrayPos.x << 4)] & 64) != 0) &&
                               ((Airport.iPlate[ArrayPos.y + 1 + (ArrayPos.x << 4)] & 64) != 0) &&
                               ((Airport.iPlate[ArrayPos.y + ((ArrayPos.x + 1) << 4)] & 32) != 0) &&
                               ((Airport.iPlate[ArrayPos.y + (ArrayPos.x << 4)] & 4) != 0) && ((Airport.iPlate[ArrayPos.y + 1 + (ArrayPos.x << 4)] & 4) != 0) &&
                               ((Airport.iPlate[ArrayPos.y + ((ArrayPos.x + 1) << 4)] & 4) != 0)) {
                        Dir = 3; // Süd-Ost
                    }
                    break;

                case 6:
                    if (ArrayPos.y > Target.y / 22 + 5 && ArrayPos.x - Target.x / 44 <= ArrayPos.y - (Target.y / 22 + 5) &&
                        ((Airport.iPlate[ArrayPos.y + (ArrayPos.x << 4)] & 128) != 0) && ((Airport.iPlate[ArrayPos.y + (ArrayPos.x << 4)] & 16) != 0) &&
                        ((Airport.iPlate[ArrayPos.y - 1 + (ArrayPos.x << 4)] & 16) != 0) &&
                        ((Airport.iPlate[ArrayPos.y + ((ArrayPos.x - 1) << 4)] & 128) != 0) && ((Airport.iPlate[ArrayPos.y + (ArrayPos.x << 4)] & 4) != 0) &&
                        ((Airport.iPlate[ArrayPos.y - 1 + (ArrayPos.x << 4)] & 4) != 0) && ((Airport.iPlate[ArrayPos.y + ((ArrayPos.x - 1) << 4)] & 4) != 0)) {
                        Dir = 7; // Nord-West
                    } else if (ArrayPos.y < Target.y / 22 + 5 && ArrayPos.x - Target.x / 44 <= (Target.y / 22 + 5) - ArrayPos.y &&
                               ((Airport.iPlate[ArrayPos.y + (ArrayPos.x << 4)] & 32) != 0) && ((Airport.iPlate[ArrayPos.y + (ArrayPos.x << 4)] & 16) != 0) &&
                               ((Airport.iPlate[ArrayPos.y + 1 + (ArrayPos.x << 4)] & 16) != 0) &&
                               ((Airport.iPlate[ArrayPos.y + ((ArrayPos.x - 1) << 4)] & 32) != 0) &&
                               ((Airport.iPlate[ArrayPos.y + (ArrayPos.x << 4)] & 4) != 0) && ((Airport.iPlate[ArrayPos.y + 1 + (ArrayPos.x << 4)] & 4) != 0) &&
                               ((Airport.iPlate[ArrayPos.y + ((ArrayPos.x - 1) << 4)] & 4) != 0)) {
        ...
        if (((128 >> (Dir >> 1)) & NewPossibleDirs) == 0 && !(StatePar == ROOM_FRACHT || StatePar == ROOM_WERKSTATT || StatePar == ROOM_NASA ||
                                                              qPlayer.ExRoom == ROOM_WERKSTATT || qPlayer.ExRoom == ROOM_FRACHT)) {
            if (qPlayer.Owner == 0 && Dir != 8) {
                CWait c;
            }

            if (qPlayer.iWalkActive != 0) {
                ...
                if (ArrayPos.y >= 5 && ArrayPos.y <= 14) {
                    ...
                    if (Dir == 2 && ArrayPos.y > Target.y / 22 + 5 && ArrayPos.y > 5 && Target.x / 44 - ArrayPos.x <= ArrayPos.y - (Target.y / 22 + 5) &&
                        ((Airport.iPlate[ArrayPos.y + (ArrayPos.x << 4)] & 128) != 0) && ((Airport.iPlate[ArrayPos.y - 1 + (ArrayPos.x << 4)] & 64) != 0) &&
                        ((Airport.iPlate[ArrayPos.y + (ArrayPos.x << 4)] & 4) != 0) && ((Airport.iPlate[ArrayPos.y - 1 + (ArrayPos.x << 4)] & 4) != 0)) {
                        Dir = 0;
                    }
                    // Sonderregelung: Nach oben ausweichen, wenn nahebei:
                    else if (Dir == 6 && ArrayPos.y > Target.y / 22 + 5 && ArrayPos.y > 5 && ArrayPos.x - Target.x / 44 <= ArrayPos.y - (Target.y / 22 + 5) &&
                             ((Airport.iPlate[ArrayPos.y + (ArrayPos.x << 4)] & 128) != 0) &&
                             ((Airport.iPlate[ArrayPos.y - 1 + (ArrayPos.x << 4)] & 16) != 0) && ((Airport.iPlate[ArrayPos.y + (ArrayPos.x << 4)] & 4) != 0) &&
                             ((Airport.iPlate[ArrayPos.y - 1 + (ArrayPos.x << 4)] & 4) != 0)) {
                        Dir = 0;
                    } else {
                        SLONG ty = ArrayPos.y;
                        ...
                        if (ty < ArrayPos.y && (((128 >> 0) & NewPossibleDirs) != 0)) {
                            Dir = 0;
                        } else if (ty > ArrayPos.y && (((128 >> 2) & NewPossibleDirs) != 0)) {
                            Dir = 4;
                        } else {
                            Dir = 8;
                        }
                    ...
                    if (Dir == 2 && (((128 >> 2) & NewPossibleDirs) != 0) && (UBYTE(Airport.iPlate[ArrayPos.y + 1 + (ArrayPos.x << 4)] & (128 >> 1)) != 0U)) {
                        Dir = 4;
                    } else if (Dir == 2 && (((128 >> 0) & NewPossibleDirs) != 0) &&
                               (UBYTE(Airport.iPlate[ArrayPos.y - 1 + (ArrayPos.x << 4)] & (128 >> 1)) != 0U)) {
                        Dir = 0;
                    } else if (Dir == 6 && (((128 >> 2) & NewPossibleDirs) != 0) &&
                               (UBYTE(Airport.iPlate[ArrayPos.y + 1 + (ArrayPos.x << 4)] & (128 >> 3)) != 0U)) {
                        Dir = 4;
                    } else if (Dir == 6 && (((128 >> 0) & NewPossibleDirs) != 0) &&
                               (UBYTE(Airport.iPlate[ArrayPos.y - 1 + (ArrayPos.x << 4)] & (128 >> 3)) != 0U)) {
                        Dir = 0;
                    } else if (p.y > qPlayer.TertiaryTarget.y && (((128 >> 0) & NewPossibleDirs) != 0)) {
                        Dir = 0;
                    } else if (p.y < qPlayer.TertiaryTarget.y && (((128 >> 2) & NewPossibleDirs) != 0)) {
                        Dir = 4;
                        if (StatePar == -1) {
                            StatePar = 0;
                        }
                    } else if ((((128 >> 2) & NewPossibleDirs) != 0) && ArrayPos.y == 0) {
                        Dir = 4;
                    } else if ((((128 >> 0) & NewPossibleDirs) != 0) && ArrayPos.y == 2) {
                        Dir = 0;
                    } else {
                        Dir = 8;
                    }
        ...
        if (StatePar == 0) {
            ...
            if (c >= ROOM_PHONE1 && c <= ROOM_PHONE4) {
            ...
            } else if ((c != 0) && (c < ROOM_REISE_X1 || c > ROOM_MONITOR6) && c != ROOM_AIRPORT_EXIT &&
                       ((c != ROOM_KIOSK && c != ROOM_INSEL && c != ROOM_SECURITY && c != ROOM_RUSHMORE && c != ROOM_WELTALL && c != ROOM_RICKS &&
                         c != ROOM_ROUTEBOX && c != ROOM_SECURITY) ||
                        c == qPlayer.DirectToRoom)) {
                ...
                if (StatePar < ROOM_STAIRSVON || StatePar > ROOM_STAIRSBIS) {
                    ...
                    if (StatePar != ROOM_WC_F && StatePar != ROOM_WC_M) {
                    ...
                    if (StatePar >= ROOM_BURO_A && StatePar <= ROOM_PERSONAL_D && StatePar / 10 != State + 1) {
                        ...
                        if (StatePar != ROOM_WC_F && StatePar != ROOM_WC_M && Sim.RoomBusy[StatePar] > 0) {
                            Sim.RoomBusy[StatePar]--;
                        }
                    ...
                    // Doch nicht in einen Raum gehen, falls Dialog offen ist oder eröffnet wird:
                        ...
                        qPlayer.TertiaryTarget.y = (Target.y - 5000 - 11) / 22 + 1;
                        qPlayer.WalkStop();
                    } else if ((Sim.RoomBusy[StatePar] > 1 || qPlayer.LocationForbidden == StatePar) && (Sim.bNoTime == 0) &&
                               (StatePar < ROOM_BURO_A || StatePar > ROOM_PERSONAL_D)) {
                        // Raum (wenn keine Toilette) als besetzt entmarkieren:
                        if (StatePar != ROOM_WC_F && StatePar != ROOM_WC_M && Sim.RoomBusy[StatePar] > 0) {
                            Sim.RoomBusy[StatePar]--;
                        }
                        ...
                        if (qPlayer.Owner == 0) {
                            if ((qPlayer.LocationWin != nullptr) && ((*qPlayer.LocationWin).IsDialogOpen() == 0) &&
                                ((*qPlayer.LocationWin).MenuIsOpen() == 0)) {
                                qPlayer.ThrownOutOfRoom = UWORD(StatePar);

                                if (StatePar == ROOM_RICKS || StatePar == ROOM_KIOSK || StatePar == ROOM_LAST_MINUTE || StatePar == ROOM_ARAB_AIR ||
                                    StatePar == ROOM_REISEBUERO || StatePar == ROOM_INSEL || StatePar == ROOM_RUSHMORE || StatePar == ROOM_ROUTEBOX ||
                                    StatePar == ROOM_WELTALL) {
                                    (*qPlayer.LocationWin)
                                        .MenuStart(MENU_REQUEST, (qPlayer.LocationForbidden != StatePar) ? MENU_REQUEST_THROWNOUT : MENU_REQUEST_FORBIDDEN);
                                } else {
                                    (*qPlayer.LocationWin)
                                        .MenuStart(MENU_REQUEST, (qPlayer.LocationForbidden != StatePar) ? MENU_REQUEST_THROWNOUT : MENU_REQUEST_FORBIDDEN);
                                }
                        ...
                        Target = Airport.GetRandomTypedRune(RUNE_SHOP, static_cast<UBYTE>(StatePar), false);
                    }
                }
            }
        }
    } else if (ArrayPos.y == 2 && Dir == 0) {
        Airport.TryDoor(ArrayPos, 1, State);
    }
    ...
    if (c == ROOM_ELECTRO || c == ROOM_ROUTEBOX || ROOM_WELTALL) {
        c = 0;
    }

    if (!(StatePar == 0 && qPlayer.GetRoom() == ROOM_AIRPORT && c != 0 && (c < ROOM_REISE_X1 || c > ROOM_MONITOR9) && c != ROOM_ELECTRO && c != ROOM_ROUTEBOX &&
          c != ROOM_RICKS)) {
        // Spätenstens jetzt stimmt die Richtung:
        switch (Dir) {
        case 0:
            Position.y -= StepSize.y * 2;
            break;
        case 1:
    ...
    if (StatePar != 0) {
        ...
        if (StatePar >= ROOM_STAIRSVON && StatePar <= ROOM_STAIRSBIS) {
            ...
            if (abs(Position.y - Target.y) <= StepSize.y * 2) {
                Position.y = Target.y;
            }
            ...
            if (StatePar != -1) {
                ...
                if (c == ROOM_WC_F) {
                    ...
                    if (State != Sim.localPlayer || (Sim.CallItADay == 0)) {
                        if (qPlayer.Owner != 2) {
                            if (Upfloor != 0) {
                                Position.y += 5000;
                            }
                            qPlayer.EnterRoom(c);
                            if (qPlayer.Owner == 0 && (Sim.bNetwork != 0) && (Sim.bIsHost != 0)) {
                    ...
                    if (c == ROOM_LAST_MINUTE) {
                        LookDir = 1;
                    } else if (c == ROOM_REISEBUERO) {
                        LookDir = 3;
                    } else if (c == ROOM_ARAB_AIR) {
                        LookDir = 0;
                    } else if (c == ROOM_KIOSK) {
                        LookDir = 0;
                    } else if (c == ROOM_RICKS) {
                        LookDir = 0;
                    } else if (c == ROOM_ROUTEBOX) {
                        LookDir = 0;
                    }
            ...
            if (Position.x == Target.x && Position.y + Upfloor * 5000 == Target.y) {
                StatePar = 0;
                qPlayer.ExRoom = 0;
                bDoBroadcastPosition = true;
            }
    ...
    if (StatePar == 0 && qPlayer.GetRoom() == ROOM_AIRPORT) {
        c = Airport.GetRuneParNear(XY(NewPos.x, NewPos.y + Upfloor * 5000), XY(StepSize.x, StepSize.y * 2), RUNE_SHOP);
        if (c == ROOM_ELECTRO || c == ROOM_ROUTEBOX || c == ROOM_WELTALL) {
            c = 0;
        }

        if ((c != 0) && (c != ROOM_ELECTRO || c == qPlayer.DirectToRoom) && (c < ROOM_REISE_X1 || c > ROOM_MONITOR9) && c != ROOM_RICKS) // ex:ROOM_MONITOR6
        {
            ...
            if (c < ROOM_STAIRSVON || c > ROOM_STAIRSBIS) {
                ...
                if (RunePar == ROOM_NASA || RunePar == ROOM_FRACHT) {
                    SLONG Room = RunePar;
                    if (Room != 0) {
                        Target = Airport.GetRandomTypedRune(RUNE_2SHOP, static_cast<UBYTE>(Room)) + XY(44, 0);
                    }
                } else if (RunePar == ROOM_WELTALL || RunePar == ROOM_RUSHMORE || RunePar == ROOM_INSEL || RunePar == ROOM_SECURITY) {
                    SLONG Room = RunePar;
                    if (Room != 0) {
                        Target = Airport.GetRandomTypedRune(RUNE_2SHOP, static_cast<UBYTE>(Room));
                    }
                    qPlayer.DirectToRoom = 0;
                } else if (RunePar != ROOM_WERKSTATT) {
                    SLONG Room = RunePar;
                    if (Room != 0) {
                        Target = Airport.GetRandomTypedRune(RUNE_2SHOP, static_cast<UBYTE>(Room)) + XY(0, 22);
                    }
                } else {
                    SLONG Room = RunePar;
                    if (Room != 0) {
    ...
    if (Dir != 8) {
        ...
        // Handy-Stehphasen:
        if (State == Sim.localPlayer && (qPlayer.LocationWin != nullptr) && ((qPlayer.LocationWin)->IsDialogOpen() != 0) &&
            (qPlayer.LocationWin)->DialogMedium == MEDIUM_HANDY) {
            Phase += 4;
        }
        ...
        if (qPlayer.IsWalking2Player != -1) {
            ...
            if (Position.y + Upfloor * 5000 > p.y + 7) {
                Phase = 0;
            } else if (Position.y + Upfloor * 5000 < p.y - 7) {
                Phase = 2;
            } else if (Position.x > p.x + 7) {
                Phase = 3;
            } else if (Position.x < p.x - 7) {
                Phase = 1;
            }
            ...
            if (qPlayer.PlayerDialogState == -1) {
                ...
                if ((Sim.bNetwork != 0) && Sim.Players.Players[qPlayer.IsWalking2Player].Owner == 2) {
                    TEAKFILE Message;
                    ...
                    Message << ATNET_DIALOG_START << qPlayer.PlayerNum << Position.x << Position.y << Phase << LookDir;
                    if (Upfloor != 0) {
                        Position.y -= 5000;
                    }
    ...
    case 7:
        ...
        break;
    case 8:
        if (Position.x == Target.x && (Position.y % 5000) == (Target.y % 5000) && qPlayer.PrimaryTarget.x == (Target.x - 22) / 44 &&
            qPlayer.PrimaryTarget.y == ((Target.y + 22 + Upfloor * 5000) - 5000 - 11) / 22 + 1) {
            if (StatePar == 0) {
        ...
        } else if (Position.x == Target.x && (Position.y % 5000) > (Target.y % 5000) && qPlayer.PrimaryTarget.x == (Target.x - 22) / 44 &&
                   qPlayer.PrimaryTarget.y == ((Target.y + 22 + Upfloor * 5000) - 5000 - 11) / 22 + 1) {
    ...
    if (Dir != 8) {
        if ((Running != 0U) && Dir < 8 && (Position.y / 22 + 5 != 13 || (Airport.iPlate[(Position.y / 22 + 5) + ((Position.x / 44) << 4)] & 240) != 80)) {
            Phase = UBYTE((Phase + 1) % (Clans[static_cast<SLONG>(ClanId)].Phasen[static_cast<SLONG>(LookDir + 10)].AnzEntries() *
                                         Clans[static_cast<SLONG>(ClanId)].Faktor * 2));
        } else {
            Phase = UBYTE((Phase + 1) %
                          (Clans[static_cast<SLONG>(ClanId)].Phasen[static_cast<SLONG>(LookDir)].AnzEntries() * Clans[static_cast<SLONG>(ClanId)].Faktor));
        }
    ...
    if (Dir == 8 && LastDir != 8 && qPlayer.PrimaryTarget.x == (Target.x - 22) / 44 &&
        qPlayer.PrimaryTarget.y == ((Target.y + 22 + Upfloor * 5000) - 5000 - 11) / 22 + 1) {
        bDoBroadcastPosition = true;
        qPlayer.iWalkActive = 0;
        qPlayer.NewDir = 8;
    ...
    if (bDoBroadcastPosition && (Sim.bNetwork != 0)) {
        qPlayer.BroadcastPosition();
    }
...
void PERSON::PersonReachedTarget() {
    ...
    case REASON_FLYING: {
        ...
        case PERSON_2WAITROOM: {
            if (FlightAirline == Sim.localPlayer && Sim.Tutorial == 1603 && (Sim.IsTutorial != 0)) {
                Sim.Tutorial = 1604;
                Sim.Players.Players[Sim.localPlayer].Messages.NextMessage();
            ...
            if (Sim.Players.Players[static_cast<SLONG>(FlightAirline)].Planes[FlightPlaneId].Ort == -3 &&
                ...
                if (fpe->Gate == -1) {
                    ...
                    Target = Airport.GetRandomTypedRune(RUNE_DURCHLEUCHTER, static_cast<UBYTE>(fpe->Gate), false, &PersonalRand);
                }
...
void PERSONS::RemoveAnimationNear(XY Position) {
    ...
    for (SLONG c = 0; c < Max; c++) {
        if (IsInAlbum(c) != 0) {
            ...
            if (abs(qPerson.Position.x - Position.x) < 44 && abs(qPerson.Position.y - Position.y) < 22 &&
                Clans[static_cast<SLONG>(qPerson.ClanId)].Type == CLAN_ANIMATION) {
...
void PERSONS::DoOneStep() {
    ...
    if (Sim.AnyPersonsInAirport == FALSE && Sim.Time < 9 * 60000) {
        return;
    }
                        ...
                        if ((Sim.CallItADay == 0) && (Sim.Time > 8 * 60000 && Sim.Time < 18 * 60000)) {
                            if ((qPerson.State & PERSON_TALKING) == 0) {
                                if (qPerson.State == (PERSON_EXSHOP | PERSON_BROWSEFLAG | PERSON_WAITFLAG)) {
                                    // Im Geschäft warten
                                    qPerson.WaitCount -= 5;
                                    if (qPerson.WaitCount < 20) {
                                        qPerson.State = (PERSON_EXSHOP | PERSON_BROWSEFLAG);
                                    }
                                } else {
                                    // if ((qPerson.State & PERSON_WAITFLAG) && (qPerson.State & (~PERSON_WAITFLAG) & (~PERSON_BROWSEFLAG))!=PERSON_CHECKINGIN
                                    // && (qPerson.State & (~PERSON_WAITFLAG) & (~PERSON_BROWSEFLAG))!=PERSON_BOARDING)
                                    if (((qPerson.State & PERSON_WAITFLAG) != 0) &&
                                        (qPerson.State & (~PERSON_WAITFLAG) & (~PERSON_BROWSEFLAG)) != PERSON_CHECKINGIN) {
                                        if (qPerson.WaitCount <= 250) {
                                            qPerson.WaitCount++;
                                        }
                                    ...
                                    if (((qPerson.State & PERSON_WAITFLAG) == 0) || (((Waiting + qPerson.ClanId) & 3) == 0)) {
                    ...
                    case CLAN_PLAYER4: // Falls Person auf Flughafensicht, dann laufen:
                    {
                        ...
                        if (qPlayer.GetRoom() == ROOM_AIRPORT && (qPlayer.IsTalking == 0) &&
                            (qPlayer.LocationWin == nullptr ||
                             ((*qPlayer.LocationWin).CurrentMenu != MENU_WC_F && (*qPlayer.LocationWin).CurrentMenu != MENU_WC_M))) {
                            if (qPlayer.Stunned == 0) {
                                if (Sim.Difficulty != DIFF_TUTORIAL || Sim.Tutorial >= 1100 || (Sim.IsTutorial == 0)) {
                                    qPerson.DoOnePlayerStep();

                                    if (qPlayer.IsStuck == 0 && (qPerson.Running != 0U) &&
...
SLONG PERSONS::GetNumShoppers() {
    ...
    for (c = AnzEntries() - 1; c >= 0; c--) {
        if (IsInAlbum(c) != 0) {
            if (Clans[static_cast<SLONG>((*this)[c].ClanId)].Type < CLAN_PLAYER1 && (*this)[c].Reason == REASON_SHOPPING) {
                n++;
            }
        }
    }
            ...
            if (Indexes[c] != -1) {
                for (d = 0; d < 4; d++) {
                    if (c != d && Indexes[d] != -1) {
                        ...
                        if ((p.x < 80 && p.y < 60) || (CheatBubbles != 0)) {
                            if (Sim.Players.Players[c].IsStuck != 0) {
                                ...
                                if (s > 50) {
                                    (*this)[Indexes[c]].Mood = 0;
                                } else if (s > 10) {
                                    (*this)[Indexes[c]].Mood = 1;
                                } else if (s > -10) {
                                    (*this)[Indexes[c]].Mood = 2;
                                } else if (s > -50) {
                                    (*this)[Indexes[c]].Mood = 3;
                                } else {
                                    (*this)[Indexes[c]].Mood = 4;
                                }
        ...
        for (c = AnzEntries() - 1; c >= 0; c--) {
            if (IsInAlbum(c) != 0) {
                ...
                if ((p.x < 40 && p.y < 80) || (CheatBubbles != 0)) {
                    ...
                    } else if ((Clans[static_cast<SLONG>(qPerson.ClanId)].Type < 30 && Clans[static_cast<SLONG>(qPerson.ClanId)].Group < 30) ||
                               Clans[static_cast<SLONG>(qPerson.ClanId)].Group == 240) {
                        if (qPerson.Mood != 0U) {
                            if (qPerson.MoodCountdown != 0U) {
                                if (qPerson.MoodCountdown < 16) {
                                    qPerson.MoodCountdown = MOODCOUNT_START - qPerson.MoodCountdown;
...
//--------------------------------------------------------------------------------------------
void CPersonQueue::AddPerson(UBYTE ClanId, XY Position, UBYTE Reason, UBYTE FlightAirline, SLONG FlightPlaneId, UBYTE FlightPlaneIndex, UBYTE Mood,
                             UBYTE FirstClass) {
    SLONG c = 0;
    SLONG d = 0;
        ...
        if (Spots[c].Position == Position) {
            ...
            for (d = 0; d < Person.AnzEntries(); d++) {
                ...
                if (Person[d].TimeSlice == -1) {
                    ...
                    // log: hprintf ("CPersonQueue::AddPerson (%li, %li|%li, d=%li, TimeSlice=%li (vs %li)...", ClanId, Position.x, Position.y, (SLONG)d,
                    // (SLONG)Person[d].TimeSlice, Sim.TimeSlice);
    ...
    do {
        ...
        for (c = Person.AnzEntries() - 1; c >= 0; c--) {
            if (Person[c].TimeSlice != -1 && Person[c].TimeSlice <= Sim.TimeSlice /*&& Person[c].Priority==MinPriority*/) {
                ...
                Sim.Persons *= PERSON(Person[c].ClanId, Person[c].Position, Person[c].Reason, Person[c].FlightAirline, Person[c].FlightPlaneId,
                                      Person[c].FlightPlaneIndex, Person[c].Mood, Person[c].FirstClass);
