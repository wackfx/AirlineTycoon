...
CSabotage::CSabotage(BOOL bHandy, ULONG PlayerNum) : CStdRaum(bHandy, PlayerNum, "sabotage.gli", GFX_SABOTAGE) {
    ...
    SP_Araber.ReSize(12);
    SP_Araber.Clips[0].ReSize(0, "s_clic.smk", "", XY(413, 255), SPM_IDLE, CRepeat(1, 1), CPostWait(0, 0), SMACKER_CLIP_CANCANCEL, nullptr, SMACKER_CLIP_SET, 0,
                              nullptr, // Warten
                              "A9A4E1", 0, 10, 1);
    SP_Araber.Clips[10].ReSize(10, "s_clicb.smk", "", XY(413, 255), SPM_IDLE, CRepeat(1, 1), CPostWait(0, 0), SMACKER_CLIP_CANCANCEL, nullptr, SMACKER_CLIP_SET,
                               0, nullptr, // Warten
                               "A9E1", 0, 1);

    SP_Araber.Clips[1].ReSize(1, "s_turn.smk", "s_turn.raw", XY(275, 101), SPM_IDLE, CRepeat(1, 1), CPostWait(0, 0), SMACKER_CLIP_DONTCANCEL, nullptr,
                              SMACKER_CLIP_SET, 0, nullptr, // Dreht sich um
                              "E1E1", 2, 3);
    SP_Araber.Clips[2].ReSize(2, "s_rede.smk", "", XY(155, 102), SPM_TALKING, CRepeat(8, 8), CPostWait(0, 0), SMACKER_CLIP_CANCANCEL, nullptr, SMACKER_CLIP_SET,
                              0, nullptr, "A9E1E1", 2, 3, 6);
    SP_Araber.Clips[3].ReSize(3, "s_wait.smk", "", XY(155, 102), SPM_LISTENING, CRepeat(2, 2), CPostWait(20, 20), SMACKER_CLIP_CANCANCEL, nullptr,
                              SMACKER_CLIP_SET, 0, nullptr, "A9A1A1E1E1", 3, 4, 5, 2, 6);
    SP_Araber.Clips[4].ReSize(4, "s_left.smk", "", XY(155, 102), SPM_LISTENING, CRepeat(1, 1), CPostWait(0, 0), SMACKER_CLIP_DONTCANCEL, nullptr,
                              SMACKER_CLIP_SET, 0, nullptr, "A9E1E1", 3, 2, 6);
    SP_Araber.Clips[5].ReSize(5, "s_rigt.smk", "", XY(155, 102), SPM_LISTENING, CRepeat(1, 1), CPostWait(0, 0), SMACKER_CLIP_DONTCANCEL, nullptr,
                              SMACKER_CLIP_SET, 0, nullptr, "A9E1E1", 3, 2, 6);
    SP_Araber.Clips[6].ReSize(6, "s_raus.smk", "", XY(155, 102), SPM_IDLE, CRepeat(1, 1), CPostWait(0, 0), SMACKER_CLIP_DONTCANCEL, &PlayEyeAnim,
                              SMACKER_CLIP_SET | SMACKER_CLIP_PRE, 1, nullptr, "A9E1E1", 7, 2, 3);
    SP_Araber.Clips[7].ReSize(3, "s_wait.smk", "", XY(155, 102), SPM_IDLE, CRepeat(1, 1), CPostWait(20, 20), SMACKER_CLIP_CANCANCEL, nullptr, SMACKER_CLIP_SET,
                              0, nullptr, "A9A1A1A1A1E1E1", 7, 11, 6, 8, 9, 2, 3);
    SP_Araber.Clips[11].ReSize(11, "s_waitk.smk", "", XY(155, 102), SPM_IDLE, CRepeat(1, 1), CPostWait(1, 1), SMACKER_CLIP_CANCANCEL, nullptr, SMACKER_CLIP_SET,
                               0, nullptr, // Blinzeln
                               "A9", 7);

    SP_Araber.Clips[8].ReSize(4, "s_left.smk", "", XY(155, 102), SPM_IDLE, CRepeat(1, 1), CPostWait(0, 0), SMACKER_CLIP_DONTCANCEL, nullptr, SMACKER_CLIP_SET,
                              0, nullptr, "A9E1E1", 7, 2, 3);
    SP_Araber.Clips[9].ReSize(5, "s_rigt.smk", "", XY(155, 102), SPM_IDLE, CRepeat(1, 1), CPostWait(0, 0), SMACKER_CLIP_DONTCANCEL, nullptr, SMACKER_CLIP_SET,
                              0, nullptr, "A9E1E1", 7, 2, 3);
...
void CSabotage::OnPaint() {
    ...
    if (Sim.ItemZange != 0) {
        RoomBm.BlitFrom(ZangeBm, 369, 188);
    }
    ...
    if ((IsDialogOpen() == 0) && (MenuIsOpen() == 0)) {
        if (gMousePosition.IfIsWithin(86, 98, 209, 439)) {
            SetMouseLook(CURSOR_EXIT, 0, ROOM_SABOTAGE, 999);
        } else if (gMousePosition.IfIsWithin(138, 9, 211, 120) && (qPlayer.HasItem(ITEM_DART) == 0) && (qPlayer.HasItem(ITEM_DISKETTE) == 0) &&
                   qPlayer.WerbungTrust == FALSE) {
            SetMouseLook(CURSOR_HOT, 0, ROOM_SABOTAGE, 800);
        } else {
            if (SP_Araber.GetClip() != 0 && SP_Araber.GetClip() != 1 && SP_Araber.GetClip() != -1) {
                if (gMousePosition.IfIsWithin(376, 106, 546, 440)) {
                    SetMouseLook(CURSOR_HOT, 0, ROOM_SABOTAGE, 10);
                }
                if (gMousePosition.IfIsWithin(288, 227, 631, 323)) {
                    SetMouseLook(CURSOR_HOT, 0, ROOM_SABOTAGE, 10);
                }
            } else {
                if (gMousePosition.IfIsWithin(422, 265, 548, 439)) {
                    SetMouseLook(CURSOR_HOT, 0, ROOM_SABOTAGE, 10);
                }
            }
        }
...
void CSabotage::OnLButtonDown(UINT nFlags, CPoint point) {
    ...
    if (PreLButtonDown(point) == 0) {
        if (MouseClickArea == ROOM_SABOTAGE && MouseClickId == 999) {
            Sim.Players.Players[PlayerNum].LeaveRoom();
        } else if (MouseClickArea == ROOM_SABOTAGE && MouseClickId == 800) {
            StartDialog(TALKER_SABOTAGE, MEDIUM_AIR, 800);
        } else if (MouseClickArea == ROOM_SABOTAGE && MouseClickId == 10) {
            StartDialog(TALKER_SABOTAGE, MEDIUM_AIR, 0);
