...
void DrawCityTip(SBBM &TipBm, ULONG CityId) {
    ...
    // Text:
    TipBm.PrintAt(bprintf("%s (%s), %s", (LPCTSTR)Cities[CityId].Name, (LPCTSTR)Cities[CityId].Kuerzel, (LPCTSTR)Cities[CityId].Lage), FontSmallWhite,
                  TEC_FONT_LEFT, 16, 4, 265, 82);

    if (CityPos.y < 17 + 160 - 60) {
        CityPos.y += 5;
    } else {
        CityPos.y -= 60;
    }
    ...
    if (Sim.Players.Players[Sim.localPlayer].RentCities.RentCities[Cities(CityId)].Rang == 0) {
        TipBm.PrintAt(Einheiten[EINH_DM].bString(Cities[CityId].BuroRent), FontSmallGrey, TEC_FONT_LEFT, CityPos.x + 90, CityPos.y + 11, 278, 181);
    } else {
        TipBm.PrintAt(Einheiten[EINH_DM].bString(Sim.Players.Players[Sim.localPlayer].RentCities.RentCities[Cities(CityId)].Miete), FontSmallGrey,
                      TEC_FONT_LEFT, CityPos.x + 90, CityPos.y + 11, 278, 181);
    }
    ...
    for (c = 0; c < Sim.Players.AnzPlayers; c++) {
        if (Cities.GetIdFromIndex(CityId) != static_cast<ULONG>(Sim.HomeAirportId)) {
            if (Sim.Players.Players[c].RentCities.RentCities[Cities(CityId)].Rang != 0) {
                TipBm.PrintAt(bprintf("%li.%s", Sim.Players.Players[c].RentCities.RentCities[Cities(CityId)].Rang, (LPCTSTR)Sim.Players.Players[c].Airline),
                              FontSmallGrey, TEC_FONT_LEFT, CityPos.x, CityPos.y + 11 * (Sim.Players.Players[c].RentCities.RentCities[Cities(CityId)].Rang + 1),
                              278, 181);
            }
        } else {
...
void DrawCityTipContents(SBBM &TipBm, ULONG CityId, XY Headline, XY Contents, XY Map, SB_CFont *pHeadFont, SB_CFont *pFont) {
    ...
    // Text:
    TipBm.PrintAt(bprintf("%s (%s)", (LPCTSTR)Cities[CityId].Name, (LPCTSTR)Cities[CityId].Kuerzel), *pHeadFont, TEC_FONT_LEFT, Headline.x, Headline.y,
                  TipBm.Size.x, TipBm.Size.y);
    ...
    if (Sim.Players.Players[Sim.localPlayer].RentCities.RentCities[Cities(CityId)].Rang == 0) {
        TipBm.PrintAt(Einheiten[EINH_DM].bString(Cities[CityId].BuroRent), *pFont, TEC_FONT_LEFT, Contents.x + 90, Contents.y + 131, TipBm.Size.x,
                      TipBm.Size.y);
    } else {
        TipBm.PrintAt(Einheiten[EINH_DM].bString(Sim.Players.Players[Sim.localPlayer].RentCities.RentCities[Cities(CityId)].Miete), *pFont, TEC_FONT_LEFT,
                      Contents.x + 90, Contents.y + 131, TipBm.Size.x, TipBm.Size.y);
    }
    ...
    for (c = 0; c < Sim.Players.AnzPlayers; c++) {
        if (Cities.GetIdFromIndex(Cities(CityId)) != static_cast<ULONG>(Sim.HomeAirportId)) {
            if (Sim.Players.Players[c].RentCities.RentCities[Cities(CityId)].Rang != 0) {
                TipBm.PrintAt(bprintf("%li.%s", Sim.Players.Players[c].RentCities.RentCities[Cities(CityId)].Rang, (LPCTSTR)Sim.Players.Players[c].Airline),
                              *pFont, TEC_FONT_LEFT, Contents.x,
                              Contents.y + 122 + 11 * (Sim.Players.Players[c].RentCities.RentCities[Cities(CityId)].Rang + 1), TipBm.Size.x, TipBm.Size.y);
            }
        } else {
            TipBm.PrintAt(bprintf("%s", (LPCTSTR)Sim.Players.Players[c].Airline), *pFont, TEC_FONT_LEFT, Contents.x, Contents.y + 122 + 11 * (c + 2),
                          TipBm.Size.x, TipBm.Size.y);
        }
    }
}
...
void DrawPlaneTipContents(SBBM &TipBm, CPlaneType *PlaneType, CPlane *Plane, XY Headline, XY Contents, SB_CFont *pHeadFont, SB_CFont *pFont, BOOL OwnPlane,
    ...
    if (Plane != nullptr) {
        if (OwnedByComputer != 0) {
            TipBm.PrintAt(bprintf(StandardTexte.GetS(TOKEN_PLANE, 1014), Plane->ptAnzPiloten, Plane->ptAnzPiloten), *pFont, TEC_FONT_LEFT, Contents.x + 70,
                          Contents.y + 66, Max.x, Max.y);
            TipBm.PrintAt(bprintf(StandardTexte.GetS(TOKEN_PLANE, 1014), Plane->ptAnzBegleiter, Plane->ptAnzBegleiter), *pFont, TEC_FONT_LEFT, Contents.x + 70,
                          Contents.y + 77, Max.x, Max.y);
        } else {
            TipBm.PrintAt(bprintf(StandardTexte.GetS(TOKEN_PLANE, 1014), Plane->AnzPiloten, Plane->ptAnzPiloten), *pFont, TEC_FONT_LEFT, Contents.x + 70,
                          Contents.y + 66, Max.x, Max.y);
            TipBm.PrintAt(bprintf(StandardTexte.GetS(TOKEN_PLANE, 1014), Plane->AnzBegleiter, Plane->ptAnzBegleiter), *pFont, TEC_FONT_LEFT, Contents.x + 70,
                          Contents.y + 77, Max.x, Max.y);
        }
    } else {
...
void DrawRouteTipContents(SBBM &TipBm, SLONG PlayerNum, ULONG RouteId, SLONG Gate, SLONG Passagiere, SLONG PassagiereFC, SLONG Costs, SLONG Ticketpreis,
    ...
    if (TipBm.Size.x == 0 || TipBm.pBitmap == nullptr) {
        return;
    }
    ...
    if (TipBm.TryPrintAt(bprintf("%s - %s", (LPCTSTR)Cities[Routen[RouteId].VonCity].Name, (LPCTSTR)Cities[Routen[RouteId].NachCity].Name), *pFont,
        ...
        TipBm.PrintAt(bprintf("%s - %s", (LPCTSTR)Cities[Routen[RouteId].VonCity].Name, (LPCTSTR)Cities[Routen[RouteId].NachCity].Name), *pFont, TEC_FONT_LEFT,
                      Contents.x, Contents.y, Contents.x + 170, Contents.y + 170);
    } else if (TipBm.TryPrintAt(bprintf("%s - %s", (LPCTSTR)Cities[Routen[RouteId].VonCity].Name, (LPCTSTR)Cities[Routen[RouteId].NachCity].Kuerzel), *pFont,
                                TEC_FONT_LEFT, Contents.x, Contents.y, Contents.x + 170, Contents.y + 170) < 12) {
        TipBm.PrintAt(bprintf("%s - %s", (LPCTSTR)Cities[Routen[RouteId].VonCity].Name, (LPCTSTR)Cities[Routen[RouteId].NachCity].Kuerzel), *pFont,
                      TEC_FONT_LEFT, Contents.x, Contents.y, Contents.x + 170, Contents.y + 170);
    } else {
        TipBm.PrintAt(bprintf("%s - %s", (LPCTSTR)Cities[Routen[RouteId].VonCity].Kuerzel, (LPCTSTR)Cities[Routen[RouteId].NachCity].Kuerzel), *pFont,
                      TEC_FONT_LEFT, Contents.x, Contents.y, Contents.x + 170, Contents.y + 170);
    }
    ...
    if (Costs != 0) {
        ...
        // Block 1 Inhalt:
        TipBm.PrintAt(Einheiten[EINH_KM].bString(Cities.CalcDistance(Routen[RouteId].VonCity, Routen[RouteId].NachCity) / 1000), *pFont, TEC_FONT_LEFT,
                      Contents.x + 85, Contents.y + 44, Contents.x + 170, Contents.y + 170);
        TipBm.PrintAt((Passagiere + PassagiereFC) > 0 ? (CString(bitoa(Passagiere)) + "/" + CString(bitoa(PassagiereFC))) : "?", *pFont, TEC_FONT_LEFT,
                      Contents.x + 85, Contents.y + 55, Contents.x + 170, Contents.y + 170);
        TipBm.PrintAt(CString(bitoa(Ticketpreis)) + "/" + CString(Einheiten[EINH_DM].bString(TicketpreisFC)), *pFont, TEC_FONT_LEFT, Contents.x + 85,
                      Contents.y + 66, Contents.x + 170, Contents.y + 170);
        ...
        // Block 2 Inhalt:
        TipBm.PrintAt((Passagiere + PassagiereFC) > 0 ? Einheiten[EINH_DM].bString(tmp) : "?", *pFont, TEC_FONT_LEFT, Contents.x + 85, Contents.y + 88,
                      Contents.x + 170, Contents.y + 170);
        ...
        TipBm.PrintAt((Passagiere + PassagiereFC) > 0 ? Einheiten[EINH_DM].bString(tmp - Costs) : "?", *pFont, TEC_FONT_LEFT, Contents.x + 85, Contents.y + 110,
                      Contents.x + 170, Contents.y + 170);
        ...
        if (Gate == -1) {
            ...
            TipBm.PrintAt(bprintf(StandardTexte.GetS(TOKEN_SCHED, 2001), Gate + 1), *pFont, TEC_FONT_LEFT, Contents.x, Contents.y + 132, Contents.x + 170,
                          Contents.y + 170);
        }
    ...
    } else // abstrakte Version
    {
        ...
        TipBm.PrintAt(Einheiten[EINH_KM].bString(Cities.CalcDistance(Routen[RouteId].VonCity, Routen[RouteId].NachCity) / 1000), *pFont, TEC_FONT_LEFT,
                      Contents.x + 85, Contents.y + 33, Contents.x + 170, Contents.y + 170);
        TipBm.PrintAt(CString(bitoa(Ticketpreis)) + "/" + CString(Einheiten[EINH_DM].bString(TicketpreisFC)), *pFont, TEC_FONT_LEFT, Contents.x + 85,
        ...
        // Block 2 Inhalt:
        TipBm.PrintAt(bitoa(Sim.Players.Players[PlayerNum].AnzPlanesOnRoute(RouteId)), *pFont, TEC_FONT_LEFT, Contents.x + 85, Contents.y + 66,
                      Contents.x + 170, Contents.y + 170);
        TipBm.PrintAt(CString(Einheiten[EINH_P].bString(qRRoute.Auslastung)) + " / " + CString(Einheiten[EINH_P].bString(qRRoute.AuslastungFC)), *pFont,
                      TEC_FONT_LEFT, Contents.x + 85, Contents.y + 77, Contents.x + 170, Contents.y + 170);
        ...
        for (c = 0; c < Sim.Players.AnzPlayers; c++) {
            if (Sim.Players.Players[c].RentRouten.RentRouten[Routen(RouteId)].Rang != 0) {
                TipBm.PrintAt(Sim.Players.Players[c].Airline, *pFont, TEC_FONT_LEFT, Contents.x + 11,
                              Contents.y + 121 + 11 * (Sim.Players.Players[c].RentRouten.RentRouten[Routen(RouteId)].Rang), Contents.x + 170, Contents.y + 170);
            }
        }
    }
}
...
void DrawAuftragTip(SLONG Player, SBBM &TipBm, SBBMS *pPlaneTips, SBBM *pTipOriginBm, CAuftrag *Auftrag, SLONG Costs, SLONG Okay, XY MapOffset) {
    ...
    if (Auftrag != nullptr) {
        DrawAuftragTipContents(Player, TipBm, pPlaneTips, Auftrag, -2, Costs, Okay, XY(32, 3), XY(32, 20), MapOffset, &FontSmallBlack, &FontSmallBlack);
    }
}
...
void DrawAuftragTipContents(SLONG PlayerNum, SBBM &TipBm, SBBMS *pPlaneTips, CAuftrag *Auftrag, SLONG Gate, SLONG Costs, SLONG Okay, XY Headline, XY Contents,
    ...
    if (TipBm.TryPrintAt(bprintf("%s - %s", (LPCTSTR)Cities[Auftrag->VonCity].Name, (LPCTSTR)Cities[Auftrag->NachCity].Name), *pFont, TEC_FONT_LEFT, Contents.x,
        ...
        TipBm.PrintAt(bprintf("%s - %s", (LPCTSTR)Cities[Auftrag->VonCity].Name, (LPCTSTR)Cities[Auftrag->NachCity].Name), *pFont, TEC_FONT_LEFT, Contents.x,
                      Contents.y + 0, Contents.x + 170, Contents.y + 15);
    } else if (TipBm.TryPrintAt(bprintf("%s - %s", (LPCTSTR)Cities[Auftrag->VonCity].Name, (LPCTSTR)Cities[Auftrag->NachCity].Kuerzel), *pFont, TEC_FONT_LEFT,
                                Contents.x, Contents.y + 0, Contents.x + 170, Contents.y + 15) < 12) {
        TipBm.PrintAt(bprintf("%s - %s", (LPCTSTR)Cities[Auftrag->VonCity].Name, (LPCTSTR)Cities[Auftrag->NachCity].Kuerzel), *pFont, TEC_FONT_LEFT, Contents.x,
                      Contents.y + 0, Contents.x + 170, Contents.y + 15);
    } else {
        TipBm.PrintAt(bprintf("%s - %s", (LPCTSTR)Cities[Auftrag->VonCity].Kuerzel, (LPCTSTR)Cities[Auftrag->NachCity].Kuerzel), *pFont, TEC_FONT_LEFT,
                      Contents.x, Contents.y + 0, Contents.x + 170, Contents.y + 15);
    }
    ...
    if (Sim.Date == Auftrag->BisDate) {
        ...
        TipBm.PrintAt(StandardTexte.GetS(TOKEN_SCHED, 3010 + (Auftrag->Date + Sim.StartWeekday) % 7), *pFont, TEC_FONT_LEFT, Contents.x + 85, Contents.y + 22,
                      Contents.x + 170, Contents.y + 170);
    } else {
        TipBm.PrintAt(CString(StandardTexte.GetS(TOKEN_SCHED, 3009)) + " " +
                          CString(StandardTexte.GetS(TOKEN_SCHED, 3010 + (Auftrag->BisDate + Sim.StartWeekday) % 7)),
                      *pFont, TEC_FONT_LEFT, Contents.x + 85, Contents.y + 22, Contents.x + 170, Contents.y + 170);
    }

    TipBm.PrintAt(CString(Einheiten[EINH_KM].bString(Cities.CalcDistance(Auftrag->VonCity, Auftrag->NachCity) / 1000)), *pFont, TEC_FONT_LEFT, Contents.x + 85,
                  Contents.y + 44, Contents.x + 170, Contents.y + 170);
    ...
    // Kostenrechnung:
        ...
        if (Costs != 0) {
            ...
            TipBm.PrintAt(Einheiten[EINH_DM].bString(Costs), *pFont, TEC_FONT_LEFT, Contents.x + 85, Contents.y + 99, Contents.x + 170, Contents.y + 170);
            TipBm.PrintAt(Einheiten[EINH_DM].bString(Auftrag->Praemie - Costs), *pFont, TEC_FONT_LEFT, Contents.x + 85, Contents.y + 110, Contents.x + 170,
                          Contents.y + 170);
        }
    ...
    if (PlayerNum != -1 && pPlaneTips != nullptr) {
        ...
        if (bAnyValid != 0) {
            red = TipBm.pBitmap->GetHardwarecolor(0xffff00);
        } else {
            red = TipBm.pBitmap->GetHardwarecolor(0x404040);
        }
...
void DrawFrachtTip(SLONG Player, SBBM &TipBm, SBBMS *pPlaneTips, SBBM *pTipOriginBm, CFracht *Fracht, SLONG TonsThis, SLONG Costs, SLONG Income, SLONG Okay,
    ...
    if (Fracht != nullptr) {
        DrawFrachtTipContents(Player, TipBm, pPlaneTips, Fracht, TonsThis, Costs, Income, Okay, XY(32, 3), XY(32, 20), MapOffset, &FontSmallBlack,
                              &FontSmallBlack);
    }
}
...
void DrawFrachtTipContents(SLONG PlayerNum, SBBM &TipBm, SBBMS *pPlaneTips, CFracht *Fracht, SLONG TonsThis, SLONG Costs, SLONG Income, SLONG Okay, XY Headline,
    ...
    if (TipBm.TryPrintAt(bprintf("%s - %s", (LPCTSTR)Cities[Fracht->VonCity].Name, (LPCTSTR)Cities[Fracht->NachCity].Name), *pFont, TEC_FONT_LEFT, Contents.x,
        ...
        TipBm.PrintAt(bprintf("%s - %s", (LPCTSTR)Cities[Fracht->VonCity].Name, (LPCTSTR)Cities[Fracht->NachCity].Name), *pFont, TEC_FONT_LEFT, Contents.x,
                      Contents.y + 0, Contents.x + 170, Contents.y + 15);
    } else if (TipBm.TryPrintAt(bprintf("%s - %s", (LPCTSTR)Cities[Fracht->VonCity].Name, (LPCTSTR)Cities[Fracht->NachCity].Kuerzel), *pFont, TEC_FONT_LEFT,
                                Contents.x, Contents.y + 0, Contents.x + 170, Contents.y + 15) < 12) {
        TipBm.PrintAt(bprintf("%s - %s", (LPCTSTR)Cities[Fracht->VonCity].Name, (LPCTSTR)Cities[Fracht->NachCity].Kuerzel), *pFont, TEC_FONT_LEFT, Contents.x,
                      Contents.y + 0, Contents.x + 170, Contents.y + 15);
    } else {
        TipBm.PrintAt(bprintf("%s - %s", (LPCTSTR)Cities[Fracht->VonCity].Kuerzel, (LPCTSTR)Cities[Fracht->NachCity].Kuerzel), *pFont, TEC_FONT_LEFT,
                      Contents.x, Contents.y + 0, Contents.x + 170, Contents.y + 15);
    }
    ...
    if (Sim.Date == Fracht->BisDate) {
        ...
        TipBm.PrintAt(StandardTexte.GetS(TOKEN_SCHED, 3010 + (Fracht->Date + Sim.StartWeekday) % 7), *pFont, TEC_FONT_LEFT, Contents.x + 85, Contents.y + 22,
                      Contents.x + 170, Contents.y + 170);
    } else {
        TipBm.PrintAt(CString(StandardTexte.GetS(TOKEN_SCHED, 3009)) + " " +
                          CString(StandardTexte.GetS(TOKEN_SCHED, 3010 + (Fracht->BisDate + Sim.StartWeekday) % 7)),
                      *pFont, TEC_FONT_LEFT, Contents.x + 85, Contents.y + 22, Contents.x + 170, Contents.y + 170);
    }

    TipBm.PrintAt(CString(Einheiten[EINH_KM].bString(Cities.CalcDistance(Fracht->VonCity, Fracht->NachCity) / 1000)), *pFont, TEC_FONT_LEFT, Contents.x + 85,
                  Contents.y + 44, Contents.x + 170, Contents.y + 170);

    if (TonsThis != 0) {
        TipBm.PrintAt(CString(Einheiten[EINH_T].bString(TonsThis)) + " / " + Einheiten[EINH_T].bString(Fracht->Tons), *pFont, TEC_FONT_LEFT, Contents.x + 85,
                      Contents.y + 55, Contents.x + 170, Contents.y + 170);
    } else {
    ...
    TipBm.PrintAt(Einheiten[EINH_DM].bString(Fracht->Strafe), *pFont, TEC_FONT_LEFT, Contents.x + 85, Contents.y + 66 + Offset, Contents.x + 170,
                  Contents.y + 170);
    ...
    if (Okay == 0) {
        ...
        if (((Costs == 0) && Fracht->Praemie == 0) || ((Costs != 0) && Income == 0)) {
            TipBm.PrintAt(StandardTexte.GetS(TOKEN_AUFTRAG, 1011), *pFont, TEC_FONT_LEFT, Contents.x + 85, Contents.y + 88 + Offset, Contents.x + 170,
                          Contents.y + 170);
        } else {
            TipBm.PrintAt(Einheiten[EINH_DM].bString(Costs != 0 ? Income : Fracht->Praemie), *pFont, TEC_FONT_LEFT, Contents.x + 85, Contents.y + 88 + Offset,
                          Contents.x + 170, Contents.y + 170);
        }
    }
    ...
    // Kommentar:
    if ((Okay != 0) && Okay >= 1 && Okay <= 3) {
        TipBm.PrintAt(StandardTexte.GetS(TOKEN_AUFTRAG, 2320 + Okay), *pFont, TEC_FONT_LEFT, Contents.x, Contents.y + 100 + Offset, Contents.x + 170,
                      Contents.y + 178);
    }
    // Kostenrechnung:
    else {
        if (Costs != 0) {
            TipBm.PrintAt(StandardTexte.GetS(TOKEN_ROUTE, 1009), *pFont, TEC_FONT_LEFT, Contents.x, Contents.y + 99 + Offset, Contents.x + 170,
                          Contents.y + 170);
            TipBm.PrintAt(StandardTexte.GetS(TOKEN_AUFTRAG, 1005), *pFont, TEC_FONT_LEFT, Contents.x, Contents.y + 110 + Offset, Contents.x + 170,
                          Contents.y + 170);
            TipBm.PrintAt(Einheiten[EINH_DM].bString(Costs), *pFont, TEC_FONT_LEFT, Contents.x + 85, Contents.y + 99 + Offset, Contents.x + 170,
                          Contents.y + 170);
            TipBm.PrintAt(Einheiten[EINH_DM].bString(Income - Costs), *pFont, TEC_FONT_LEFT, Contents.x + 85, Contents.y + 110 + Offset, Contents.x + 170,
                          Contents.y + 170);
        }
        ...
        if (Unlocked == 0) {
            ...
            TipBm.PrintAt(StandardTexte.GetS(TOKEN_SCHED, 2005), *pFont, TEC_FONT_LEFT, Contents.x + 10, Contents.y + 132 + 25, Contents.x + 170,
                          Contents.y + 180);
...
void DrawAutoflugTipContents(SBBM &TipBm, SLONG Costs, SLONG NotPassengers, SLONG VonCity, SLONG NachCity, XY Headline, XY Contents, SB_CFont *pHeadFont,
    ...
    if (TipBm.TryPrintAt(bprintf("%s - %s", (LPCTSTR)Cities[VonCity].Name, (LPCTSTR)Cities[NachCity].Name), *pFont, TEC_FONT_LEFT, Contents.x, Contents.y + 0,
        ...
        TipBm.PrintAt(bprintf("%s - %s", (LPCTSTR)Cities[VonCity].Name, (LPCTSTR)Cities[NachCity].Name), *pFont, TEC_FONT_LEFT, Contents.x, Contents.y + 0,
                      Contents.x + 170, Contents.y + 15);
    } else if (TipBm.TryPrintAt(bprintf("%s - %s", (LPCTSTR)Cities[VonCity].Name, (LPCTSTR)Cities[NachCity].Kuerzel), *pFont, TEC_FONT_LEFT, Contents.x,
                                Contents.y + 0, Contents.x + 170, Contents.y + 15) < 12) {
        TipBm.PrintAt(bprintf("%s - %s", (LPCTSTR)Cities[VonCity].Name, (LPCTSTR)Cities[NachCity].Kuerzel), *pFont, TEC_FONT_LEFT, Contents.x, Contents.y + 0,
                      Contents.x + 170, Contents.y + 15);
    } else {
        TipBm.PrintAt(bprintf("%s - %s", (LPCTSTR)Cities[VonCity].Kuerzel, (LPCTSTR)Cities[NachCity].Kuerzel), *pFont, TEC_FONT_LEFT, Contents.x,
                      Contents.y + 0, Contents.x + 170, Contents.y + 15);
    }
    ...
    TipBm.PrintAt(Einheiten[EINH_KM].bString(Cities.CalcDistance(VonCity, NachCity) / 1000), *pFont, TEC_FONT_LEFT, Contents.x + 85, Contents.y + 44,
                  Contents.x + 170, Contents.y + 170);
    TipBm.PrintAt(CString(Einheiten[EINH_KG].bString(NotPassengers * 30)), *pFont, TEC_FONT_LEFT, Contents.x + 85, Contents.y + 55, Contents.x + 170,
                  Contents.y + 170);
    ...
    TipBm.PrintAt(CString(Einheiten[EINH_DM].bString(Einnahmen - Costs)), *pFont, TEC_FONT_LEFT, Contents.x + 85, Contents.y + 110, Contents.x + 170,
                  Contents.y + 170);
...
void DrawKursTipContents(SBBM &TipBm, SLONG PlayerView, SLONG PlayerAktie, SB_CFont *pHeadFont, SB_CFont *pFont) {
    ...
    TipBm.PrintAt(
        Einheiten[EINH_P].bString(static_cast<SLONG>(__int64(Sim.Players.Players[PlayerAktie].Dividende) * 100 / Sim.Players.Players[PlayerAktie].Kurse[0])),
        *pFont, TEC_FONT_LEFT, 98, 53, 170, 160);
    ...
    TipBm.PrintAt(Einheiten[EINH_DM].bString64(__int64(Sim.Players.Players[PlayerAktie].Kurse[0]) * Sim.Players.Players[PlayerView].OwnsAktien[PlayerAktie]),
                  *pFont, TEC_FONT_LEFT, 98, 108, 200, 160);
    ...
    for (c = 0; c < 4; c++) {
        ...
        TipBm.PrintAt(
            Einheiten[EINH_P].bString(SLONG(Sim.Players.Players[c].OwnsAktien[PlayerAktie] * __int64(100) / Sim.Players.Players[PlayerAktie].AnzAktien)),
            *pFont, TEC_FONT_LEFT, 150, 141 + c * 11, 180, 210);
    }
    ...
    for (c = 0; c < 9; c++) {
        TipBm.Line(141 + 60 * c / 9, SLONG(79 - Sim.Players.Players[PlayerAktie].Kurse[9 - c] * 61 / Max), 141 + 60 * (c + 1) / 9,
                   SLONG(79 - Sim.Players.Players[PlayerAktie].Kurse[9 - (c + 1)] * 61 / Max), AktienKursLineColor[PlayerAktie]);
    }
}
...
void DrawMoneyTip(SBBM &TipBm, SLONG PlayerNum, SLONG Page) {
    ...
    // Fenster-Überschrift:
    {
        ...
        TipBm.PrintAt(bprintf((LPCTSTR)CString(StandardTexte.GetS(TOKEN_MONEY, 1000)),
                              (LPCTSTR)CString(StandardTexte.GetS(TOKEN_SCHED, 3010 + (pTimeStruct->tm_wday + 6) % 7)), pTimeStruct->tm_mday,
                              pTimeStruct->tm_mon + 1),
                      FontBankRed, TEC_FONT_LEFT, 28, 11, 400, 214);
    }

    for (c = 0; c < Page * 10; c++) {
        if (Sim.Players.Players[PlayerNum].History.HistoryLine[c].Description != "*") {
            break;
        }
    }
    ...
    for (c = Page * 10; c < Page * 10 + 10; c++) {
        if (Sim.Players.Players[PlayerNum].History.HistoryLine[c].Description != "*" &&
            strncmp(Sim.Players.Players[PlayerNum].History.HistoryLine[c].Description, "----", 4) != 0) {
            TipBm.PrintAt((LPCTSTR)Sim.Players.Players[PlayerNum].History.HistoryLine[c].Description, FontBankBlack, TEC_FONT_LEFT, 36,
                          72 + (c - Page * 10) * 11, 378, 214);
            TipBm.PrintAt(Einheiten[EINH_DM].bString64(Sim.Players.Players[PlayerNum].History.HistoryLine[c].Money), FontBankBlack, TEC_FONT_RIGHT, 36,
                          72 + (c - Page * 10) * 11, 378, 214);
            money += Sim.Players.Players[PlayerNum].History.HistoryLine[c].Money;
        }
    }
...
void DrawPlanesNotepad(SBBM &NotepadBm, CDataTable *Table, SLONG CountFrom, SBBMS *pMenuBms, BOOL DisplayWartungskosten) {
    ...
    for (c = CountFrom; c < CountFrom + 13 && c < Table->AnzRows; c++) {
        ...
        if (Table->ValueFlags[0 + c * Table->AnzColums] != 0U) {
            ...
            pFont1 = &FontSmallBlack;
        }
        if (DisplayWartungskosten != 0) {
            Text2 = Table->Values[1 + c * Table->AnzColums];
        } else {
            Text2 = Table->Values[1 + c * Table->AnzColums];
        }

        for (SLONG x = 170; x >= 0; x -= 4) {
            if (NotepadBm.TryPrintAt(Text2, FontSmallBlack, TEC_FONT_RIGHT, XY(216 + x, 25 + (c - CountFrom) * 13),
                                     XY((*pMenuBms)[0].Size.x - 2, 25 + (c - CountFrom) * 13 + 13)) < 13) {
                NotepadBm.PrintAt(Text2, FontSmallBlack, TEC_FONT_RIGHT, XY(216 + x, 25 + (c - CountFrom) * 13),
                                  XY((*pMenuBms)[0].Size.x - 2, 25 + (c - CountFrom) * 13 + 13));

                if (NotepadBm.TryPrintAt(Table->Values[0 + c * Table->AnzColums], *pFont1, TEC_FONT_LEFT, XY(216, 25 + (c - CountFrom) * 13),
                                         XY(216 + x, 25 + (c - CountFrom) * 13 + 13)) < 13) {
                    NotepadBm.PrintAt(Table->Values[0 + c * Table->AnzColums], *pFont1, TEC_FONT_LEFT, XY(216, 25 + (c - CountFrom) * 13),
                                      XY(216 + x, 25 + (c - CountFrom) * 13 + 13));
                } else {
                    x -= 10;
                    ...
                    while (str.GetLength() > 3) {
                        ...
                        if (NotepadBm.TryPrintAt(str, *pFont1, TEC_FONT_LEFT, XY(216, 25 + (c - CountFrom) * 13), XY(216 + x, 25 + (c - CountFrom) * 13 + 13)) <
                            13) {
    ...
    if (CountFrom > 0) {
        NotepadBm.BlitFrom((*pMenuBms)[1], 196, 191);
    }
    if (CountFrom + 13 < Table->AnzRows) {
        NotepadBm.BlitFrom((*pMenuBms)[2], 374, 199);
    }
}
