...
template <class T, SLONG Precision> class FIXPOINT {
  ...
  public:
    ...
    FIXPOINT operator*(SLONG p) const {
        FIXPOINT rc;
        rc.Value = Value * p;
        return (rc);
    }
    ...
    FIXPOINT operator*(const FIXPOINT &p) const {
        FIXPOINT rc;
        rc.Value = ((Value >> (Precision >> 1)) * (p.Value >> (Precision >> 1)));
        return (rc);
    }
    FIXPOINT operator/(const FIXPOINT &p) const {
        FIXPOINT rc;
        rc.Value = ((Value / p.Value) >> Precision);
        return (rc);
    }
    FIXPOINT operator+=(const FIXPOINT &p) {
        Value += p.Value;
        return (*this);
    }
    FIXPOINT operator-=(const FIXPOINT &p) {
