...
//--------------------------------------------------------------------------------------------
CShadesBlend::CShadesBlend(LPDIRECTDRAWSURFACE pSurface1, LPDIRECTDRAWSURFACE pSurface2, LPDIRECTDRAWSURFACE pTmpSurface, long AnzHLines, bool Direction,
                           bool fTransparent)
    : CBlenden(pSurface1, pSurface2, pTmpSurface) {
    _AnzHLines = AnzHLines;
    _dwFlags = (fTransparent) ? DDBLT_KEYSRC | DDBLT_WAIT : DDBLT_WAIT;
...
void CShadesBlend::UpdateTmpSurface(double Prozent) {
    ...
    for (cy = 0; cy < _AnzHLines; cy++) {
        // altes Bild:
        if (_Direction)
            h = long(Prozent * 2 - 100 + (_AnzHLines - 1 - cy) * 100 / _AnzHLines);
        else
            h = long(Prozent * 2 - 100 + cy * 100 / _AnzHLines);
        if (h < 0)
            h = 0;
        if (h > 100)
            h = 100;
...
void CTerminatorBlend::UpdateTmpSurface(double Prozent) {
    ...
    if (Prozent < 55)
        y = cos(Prozent * 3.14159 / 2 / 55) * _SizeY / 2;
    if (Prozent >= 55 && Prozent < 85)
        y = sin((Prozent - 55) * 3.14159 / 30) * _SizeY / 8;
    if (Prozent >= 85)
        y = sin((Prozent - 85) * 3.14159 / 15) * _SizeY / 20;
