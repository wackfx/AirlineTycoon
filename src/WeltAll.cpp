...
CWeltAll::CWeltAll(BOOL bHandy, ULONG PlayerNum) : CStdRaum(bHandy, PlayerNum, "", 0) {
    ...
    SP_Flags[0].Clips[0].ReSize(0, "flagb.smk", "", XY(142, 63), SPM_IDLE, CRepeat(9, 9), CPostWait(0, 0), SMACKER_CLIP_CANCANCEL, nullptr, SMACKER_CLIP_SET, 0,
                                nullptr, "A9", 0);
    SP_Flags[1].Clips[0].ReSize(0, "flagg.smk", "", XY(442, 163), SPM_IDLE, CRepeat(9, 9), CPostWait(0, 0), SMACKER_CLIP_CANCANCEL, nullptr, SMACKER_CLIP_SET,
                                0, nullptr, "A9", 0);
    SP_Flags[2].Clips[0].ReSize(0, "flagr.smk", "", XY(782, 63), SPM_IDLE, CRepeat(9, 9), CPostWait(0, 0), SMACKER_CLIP_CANCANCEL, nullptr, SMACKER_CLIP_SET, 0,
                                nullptr, "A9", 0);
    SP_Flags[3].Clips[0].ReSize(0, "flagy.smk", "", XY(1072, 103), SPM_IDLE, CRepeat(9, 9), CPostWait(0, 0), SMACKER_CLIP_CANCANCEL, nullptr, SMACKER_CLIP_SET,
                                0, nullptr, "A9", 0);

    SP_Ufo.ReSize(1);
    SP_Ufo.Clips[0].ReSize(0, "ufo.smk", "", XY(0, 0), SPM_IDLE, CRepeat(9, 9), CPostWait(0, 0), SMACKER_CLIP_CANCANCEL, nullptr, SMACKER_CLIP_SET, 0, nullptr,
                           "A9", 0);
...
void CWeltAll::OnPaint() {
    ...
    for (c = 0; c < 20; c++) {
        if (Sternschnuppen[c].StartIndex != -1) {
            ...
            if ((Sternschnuppen[c].Position.y >> 8) < 500) {
                RoomBm.BlitFromT(StarBms[Sternschnuppen[c].StartIndex], (Sternschnuppen[c].Position.x >> 8) - 150 - ViewPos.x,
                                 (Sternschnuppen[c].Position.y >> 8) - 20);
            } else {
    ...
    if (((Time / 1000) != (LastTime / 1000)) && ((rand() % 3) == 0)) {
        for (c = 0; c < 20; c++) {
            if (Sternschnuppen[c].StartIndex == -1) {
                ...
                case 0:
                    Sternschnuppen[c].Velocity = XY(5, 9) * SLONG(4);
                    break;
                case 1:
                    Sternschnuppen[c].Velocity = XY(-11, 13) * SLONG(4);
                    break;
                case 2:
    ...
    for (c = 0; c < 4; c++) {
        ...
        case 0:
            ...
            break;
        case 1:
            StationPos = XY(370, 160);
            break;
        case 2:
            StationPos = XY(710, 60);
            break;
        case 3:
...
void CWeltAll::OnLButtonDown(UINT nFlags, CPoint point) {
    ...
    if (PreLButtonDown(point) == 0) {
        if (MouseClickArea == ROOM_WELTALL && MouseClickId == 999) {
