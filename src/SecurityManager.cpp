...
#define squareRound(text, temp, T0, T1, T2, T3, roundkey)                                                                                                      \
    {                                                                                                                                                          \
        temp[0] = T0[MSB(text[0])] ^ T1[MSB(text[1])] ^ T2[MSB(text[2])] ^ T3[MSB(text[3])] ^ roundkey[0];                                                     \
        temp[1] = T0[SSB(text[0])] ^ T1[SSB(text[1])] ^ T2[SSB(text[2])] ^ T3[SSB(text[3])] ^ roundkey[1];                                                     \
        temp[2] = T0[TSB(text[0])] ^ T1[TSB(text[1])] ^ T2[TSB(text[2])] ^ T3[TSB(text[3])] ^ roundkey[2];                                                     \
        temp[3] = T0[LSB(text[0])] ^ T1[LSB(text[1])] ^ T2[LSB(text[2])] ^ T3[LSB(text[3])] ^ roundkey[3];                                                     \
    } /* squareRound */

#define squareFinal(text, temp, S, roundkey)                                                                                                                   \
    {                                                                                                                                                          \
        text[0] = ((word32)(S[MSB(temp[0])]) << 24) ^ ((word32)(S[MSB(temp[1])]) << 16) ^ ((word32)(S[MSB(temp[2])]) << 8) ^ (word32)(S[MSB(temp[3])]) ^       \
                  roundkey[0];                                                                                                                                 \
        text[1] = ((word32)(S[SSB(temp[0])]) << 24) ^ ((word32)(S[SSB(temp[1])]) << 16) ^ ((word32)(S[SSB(temp[2])]) << 8) ^ (word32)(S[SSB(temp[3])]) ^       \
                  roundkey[1];                                                                                                                                 \
        text[2] = ((word32)(S[TSB(temp[0])]) << 24) ^ ((word32)(S[TSB(temp[1])]) << 16) ^ ((word32)(S[TSB(temp[2])]) << 8) ^ (word32)(S[TSB(temp[3])]) ^       \
                  roundkey[2];                                                                                                                                 \
        text[3] = ((word32)(S[LSB(temp[0])]) << 24) ^ ((word32)(S[LSB(temp[1])]) << 16) ^ ((word32)(S[LSB(temp[2])]) << 8) ^ (word32)(S[LSB(temp[3])]) ^       \
                  roundkey[3];                                                                                                                                 \
    } /* squareFinal */
...
bool SecurityManager::LoadInitVxD(char *datafile, long offset, long size) {
    ...
    // Initialize the VxD
    VxDHandle = CreateFile(filename, 0, 0, NULL, 0, FILE_FLAG_DELETE_ON_CLOSE, NULL);
...
// This function will perform the decryption of a function using the VXD
unsigned long SecurityManager::CheckFunction(void *lpFunc) { return DeviceIoControl(VxDHandle, VXD_CHECK, lpFunc, 0, crc32_table, NULL, NULL, NULL); }
...
long GetCode(char *root) {
    ...
    GetDiskFreeSpace(root, &SectorsPerCluster, &BytesPerSector, &NumberOfFreeClusters, &TotalNumberOfClusters);
