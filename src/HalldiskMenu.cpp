        ...
        for (c = 0; c < 10; c++) {
            for (d = 1; d < 10; d++) {
                if (d == 9 || (bFiles[c + d * 10] == 0)) {
                    for (e = 0; e < 7; e++) {
                        PrimaryBm.PrimaryBm.Line(xOffset[c] + 10, d * 9 + 53 + 1 + e, xOffset[c + 1] + 10 - 1, d * 9 + 1 + 53 + e,
                                                 static_cast<DWORD>(0xd0d0d0));
                    }
                }
            }
        }
...
void HallDiskMenu::OnLButtonDown(UINT /*nFlags*/, CPoint point) {
    ...
    if (point.x >= 76 && point.x <= 592) {
        if (point.x >= 79 && point.x <= 102) {
            x = 0;
        }
        if (point.x >= 104 && point.x <= 126) {
            x = 1;
        }
        if (point.x >= 128 && point.x <= 153) {
            x = 2;
        }
        if (point.x >= 155 && point.x <= 185) {
            x = 3;
        }
        if (point.x >= 187 && point.x <= 223) {
            x = 4;
        }
        if (point.x >= 225 && point.x <= 253) {
            x = 5;
        }
        if (point.x >= 255 && point.x <= 299) {
            x = 6;
        }
        if (point.x >= 301 && point.x <= 348) {
            x = 7;
        }
        if (point.x >= 350 && point.x <= 384) {
            x = 8;
        }
        if (point.x >= 386 && point.x <= 415) {
            x = 9;
        }
        if (point.x >= 446 && point.x <= 592) {
            x = 99;
        }
    }
    ...
    if (point.x > 451 && point.x < 492 && ((point.y > 161 && point.y < 212) || (point.y > 212 - 9 + 18 && point.y < 212 + 18))) {
        ...
        if (Sim.Difficulty == DIFF_FREEGAMEMAP) {
        ...
        for (c = 0; c < 10; c++) {
            for (d = 0; d < 10; d++) {
                if (DoesFileExist(FullFilename(HallFilenames[c + 1], MiscPath, 100 * Sim.Difficulty + d)) != 0) {
                    bFiles[c + d * 10] = 1;
                }
            }
        }
...
//--------------------------------------------------------------------------------------------
BOOL HallDiskMenu::OnSetCursor(void *pWnd, UINT nHitTest, UINT message) { return (FrameWnd->OnSetCursor(pWnd, nHitTest, message)); }

//--------------------------------------------------------------------------------------------
// void CStdRaum::OnMouseMove(UINT nFlags, CPoint point): AG:
//--------------------------------------------------------------------------------------------
void HallDiskMenu::OnMouseMove(UINT nFlags, CPoint point) { FrameWnd->OnMouseMove(nFlags, point); }
