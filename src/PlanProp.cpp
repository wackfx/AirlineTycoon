...
static XY FoodOffsets[] = {XY(116, 98),  XY(104, 999), XY(116, 90),  XY(116, 286), XY(116, 102), XY(116, 330), XY(116, 108), XY(116, 292), XY(116, 108),
                           XY(116, 303), XY(116, 109), XY(116, 330), XY(135, 110), XY(135, 330), XY(135, 112), XY(135, 325), XY(135, 118), XY(135, 999)};

static XY StarOffsets[] = {XY(498, 8), XY(498, 87), XY(498, 167), XY(498, 284), XY(66, 8), XY(66, 87), XY(66, 167), XY(66, 246)};
...
void CPlaneProps::OnPaint() {
    ...
    if (ActivePage >= 50) {
        ...
        if (PlaneIndex != -1) {
            ...
            RoomBm.BlitFromT(Food[qPlane.EssenTarget * 2], SeatOffsets[qPlane.SitzeTarget] + FoodOffsets[(qPlane.SitzeTarget * 3 + qPlane.TablettsTarget) * 2] -
                                                               Food[qPlane.EssenTarget * 2].Size / SLONG(2) + ScrollOffset);
            RoomBm.BlitFromT(Food[qPlane.EssenTarget * 2 + 1], SeatOffsets[qPlane.SitzeTarget] +
                                                                   FoodOffsets[(qPlane.SitzeTarget * 3 + qPlane.TablettsTarget) * 2 + 1] -
                                                                   Food[qPlane.EssenTarget * 2 + 1].Size / SLONG(2) + ScrollOffset);
            ...
            if (qPlane.Sitze != qPlane.SitzeTarget) {
                RoomBm.BlitFromT(qCursor, StarOffsets[0] + XY(qPlane.SitzeTarget * 47 - 1, -1) + ScrollOffset);
            }
            if (qPlane.Tabletts != qPlane.TablettsTarget) {
                RoomBm.BlitFromT(qCursor, StarOffsets[1] + XY(qPlane.TablettsTarget * 47 - 1, -1) + ScrollOffset);
            }
            if (qPlane.Deco != qPlane.DecoTarget) {
                RoomBm.BlitFromT(qCursor, StarOffsets[2] + XY(qPlane.DecoTarget * 47 - 1, -1) + ScrollOffset);
            }
            if (qPlane.Essen != qPlane.EssenTarget) {
            ...
            for (SLONG c = 0; c < 3; c++) {
                for (SLONG d = 0; d < 4; d++) {
                    if (gMousePosition.IfIsWithin(StarOffsets[d].x + c * 47 + ScrollOffsetX, StarOffsets[d].y, StarOffsets[d].x + 43 + c * 47 + ScrollOffsetX,
                        ...
                        switch (d) {
                        case 0:
                            Backup = qPlane.SitzeTarget;
                            qPlane.SitzeTarget = static_cast<UBYTE>(c);
                            break;
                        case 1:
                            Backup = qPlane.TablettsTarget;
                            qPlane.TablettsTarget = static_cast<UBYTE>(c);
                            break;
                        case 2:
                            Backup = qPlane.DecoTarget;
                            qPlane.DecoTarget = static_cast<UBYTE>(c);
                            break;
                        case 3:
            ...
            if (qPlane.SitzeTarget != qPlane.Sitze) {
                RoomBm.PrintAt(bprintf("%s %s", (LPCTSTR)CString(StandardTexte.GetS(TOKEN_SCHED, 1800)), (LPCTSTR)CString(Einheiten[EINH_DM].bString(cost1))),
                               FontSmallBlack, TEC_FONT_RIGHT, XY(502, 58) + ScrollOffset, XY(631, 72) + ScrollOffset);
            } else {
                cost1 = 0;
            }
            if (qPlane.TablettsTarget != qPlane.Tabletts) {
                RoomBm.PrintAt(bprintf("%s %s", (LPCTSTR)CString(StandardTexte.GetS(TOKEN_SCHED, 1800)), (LPCTSTR)CString(Einheiten[EINH_DM].bString(cost2))),
                               FontSmallBlack, TEC_FONT_RIGHT, XY(502, 138) + ScrollOffset, XY(631, 155) + ScrollOffset);
            } else {
                cost2 = 0;
            }
            if (qPlane.DecoTarget != qPlane.Deco) {
                RoomBm.PrintAt(bprintf("%s %s", (LPCTSTR)CString(StandardTexte.GetS(TOKEN_SCHED, 1800)), (LPCTSTR)CString(Einheiten[EINH_DM].bString(cost3))),
                               FontSmallBlack, TEC_FONT_RIGHT, XY(502, 218) + ScrollOffset, XY(631, 305) + ScrollOffset);
            } else {
                cost3 = 0;
            }
            if (cost1 + cost2 + cost3 > 0) {
                RoomBm.PrintAt(bprintf("%s %s", (LPCTSTR)CString(StandardTexte.GetS(TOKEN_SCHED, 1800)),
                                       (LPCTSTR)CString(Einheiten[EINH_DM].bString(cost1 + cost2 + cost3))),
                               FontSmallBlack, TEC_FONT_RIGHT, XY(502, 252) + ScrollOffset, XY(631, 271) + ScrollOffset);
            }
            RoomBm.PrintAt(bprintf("%s %s", (LPCTSTR)CString(Einheiten[EINH_DM].bString(qPlane.ptPassagiere * FoodCosts[qPlane.EssenTarget])),
                                   (LPCTSTR)StandardTexte.GetS(TOKEN_SCHED, 1801)),
                           FontSmallBlack, TEC_FONT_RIGHT, XY(502, 334) + ScrollOffset, XY(631, 353) + ScrollOffset);
            // RoomBm.PrintAt (bprintf ("%s %s", (LPCTSTR)(CString)Einheiten[EINH_DM].bString
            ...
            for (SLONG c = 0; c < 3; c++) {
                for (SLONG d = 0; d < 4; d++) {
                    if (gMousePosition.IfIsWithin(StarOffsets[d].x + c * 47 + ScrollOffsetX, StarOffsets[d].y, StarOffsets[d].x + 43 + c * 47 + ScrollOffsetX,
                        ...
                        switch (d) {
                        case 0:
                            qPlane.SitzeTarget = Backup;
                            break;
                        case 1:
                            qPlane.TablettsTarget = Backup;
                            break;
                        case 2:
                            qPlane.DecoTarget = Backup;
                            break;
                        case 3:
            ...
            if ((IsDialogOpen() == 0) && (MenuIsOpen() == 0) && ActivePage == 100) {
                ...
                if (Costs>0)
                Sim.Players.Players[(SLONG)PlayerNum].Messages.AddMessage (BERATERTYP_GIRL, bprintf (StandardTexte.GetS (TOKEN_SCHED, 1820),
                Insert1000erDots(Costs)), MESSAGE_COMMENT); else if (Costs<0) Sim.Players.Players[(SLONG)PlayerNum].Messages.AddMessage (BERATERTYP_GIRL,
                bprintf (StandardTexte.GetS (TOKEN_SCHED, 1821), Insert1000erDots(-Costs)), MESSAGE_COMMENT);
                }
            ...
            // Die Stewardess Anzeige:
            {
                ...
                if (n != 0) {
                    RoomBm.BlitFromT(StewardessBms[0], px, 408);
                    px += StewardessBms[0].Size.x;
                    for (; n > 1; n--) {
                        RoomBm.BlitFromT(StewardessBms[1], px, 408);
                        px += StewardessBms[1].Size.x;
                    }
                    RoomBm.BlitFromT(StewardessBms[2], px, 408);
                    px += StewardessBms[2].Size.x + 3;
                }
                ...
                if (n != 0) {
                    RoomBm.BlitFromT(StewardessBms[3], px, 408);
                    px += StewardessBms[3].Size.x;
                    for (; n > 1; n--) {
                        RoomBm.BlitFromT(StewardessBms[4], px, 408);
                        px += StewardessBms[4].Size.x;
                    }
                    RoomBm.BlitFromT(StewardessBms[5], px, 408);
                    px += StewardessBms[5].Size.x;
                }
                ...
                if (n != 0) {
                    if (m != 0) {
                        px -= 9;
                        RoomBm.BlitFromT(StewardessBms[6], px, 408);
                        px += StewardessBms[6].Size.x;
                    } else {
                        RoomBm.BlitFromT(StewardessBms[7], px, 408);
                        px += StewardessBms[7].Size.x;
                    }

                    for (; n > 1; n--) {
                        RoomBm.BlitFromT(StewardessBms[8], px, 408);
                        px += StewardessBms[8].Size.x;
                    }
                    RoomBm.BlitFromT(StewardessBms[9], px, 408);
                    px += StewardessBms[9].Size.x + 3;
                }
        ...
        if (PlaneIndex != -1) {
            ...
            if (qPlane.TriebwerkTarget != 0U) {
                RoomBm.BlitFrom(TurbinenBms[qPlane.TriebwerkTarget - 1], 444 + ScrollOffsetX, 42);
            }
            if (qPlane.SicherheitTarget != 0U) {
                RoomBm.BlitFrom(SosBms[qPlane.SicherheitTarget - 1], 305 + ScrollOffsetX, 247);
            }
            if (qPlane.ElektronikTarget != 0U) {
                RoomBm.BlitFrom(CockpitBms[qPlane.ElektronikTarget - 1], 442 + ScrollOffsetX, 237);
            }
            ...
            if (qPlane.Reifen != qPlane.ReifenTarget) {
                RoomBm.BlitFromT(qCursor, StarOffsets[4] + XY(qPlane.ReifenTarget * 47 - 1, -1) + ScrollOffset);
            }
            if (qPlane.Triebwerk != qPlane.TriebwerkTarget) {
                RoomBm.BlitFromT(qCursor, StarOffsets[5] + XY(qPlane.TriebwerkTarget * 47 - 1, -1) + ScrollOffset);
            }
            if (qPlane.Sicherheit != qPlane.SicherheitTarget) {
                RoomBm.BlitFromT(qCursor, StarOffsets[6] + XY(qPlane.SicherheitTarget * 47 - 1, -1) + ScrollOffset);
            }
                ...
                for (SLONG c = 0; c < 3; c++) {
                    for (SLONG d = 0; d < 4; d++) {
                        if (gMousePosition.IfIsWithin(StarOffsets[d + 4].x + c * 47, StarOffsets[d + 4].y, StarOffsets[d + 4].x + 43 + c * 47,
                                                      StarOffsets[d + 4].y + 36)) {
                            switch (d) {
                            case 0:
                                Backup = qPlane.ReifenTarget;
                                qPlane.ReifenTarget = static_cast<UBYTE>(c);
                                break;
                            case 1:
                                Backup = qPlane.TriebwerkTarget;
                                qPlane.TriebwerkTarget = static_cast<UBYTE>(c);
                                break;
                            case 2:
                                Backup = qPlane.SicherheitTarget;
                                qPlane.SicherheitTarget = static_cast<UBYTE>(c);
                                break;
                            case 3:
            ...
            if (qPlane.Reifen != qPlane.ReifenTarget) {
                ...
                RoomBm.PrintAt(bprintf("%s %s", (LPCTSTR)CString(StandardTexte.GetS(TOKEN_SCHED, 1800)), (LPCTSTR)CString(Einheiten[EINH_DM].bString(cost1))),
                               FontSmallBlack, TEC_FONT_LEFT, XY(8, 58) + ScrollOffset, XY(205, 72) + ScrollOffset);
            }
            if (qPlane.Triebwerk != qPlane.TriebwerkTarget) {
                cost2 = (TriebwerkCosts[qPlane.TriebwerkTarget] - TriebwerkCosts[qPlane.Triebwerk] / 2);
                RoomBm.PrintAt(bprintf("%s %s", (LPCTSTR)CString(StandardTexte.GetS(TOKEN_SCHED, 1800)), (LPCTSTR)CString(Einheiten[EINH_DM].bString(cost2))),
                               FontSmallBlack, TEC_FONT_LEFT, XY(8, 137) + ScrollOffset, XY(205, 151) + ScrollOffset);
            }
            if (qPlane.Sicherheit != qPlane.SicherheitTarget) {
                cost3 = (SicherheitCosts[qPlane.SicherheitTarget] - SicherheitCosts[qPlane.Sicherheit] / 2);
                RoomBm.PrintAt(bprintf("%s %s", (LPCTSTR)CString(StandardTexte.GetS(TOKEN_SCHED, 1800)), (LPCTSTR)CString(Einheiten[EINH_DM].bString(cost3))),
                               FontSmallBlack, TEC_FONT_LEFT, XY(8, 216) + ScrollOffset, XY(205, 230) + ScrollOffset);
            }
            if (qPlane.Elektronik != qPlane.ElektronikTarget) {
                cost4 = (ElektronikCosts[qPlane.ElektronikTarget] - ElektronikCosts[qPlane.Elektronik] / 2);
                RoomBm.PrintAt(bprintf("%s %s", (LPCTSTR)CString(StandardTexte.GetS(TOKEN_SCHED, 1800)), (LPCTSTR)CString(Einheiten[EINH_DM].bString(cost4))),
                               FontSmallBlack, TEC_FONT_LEFT, XY(8, 295) + ScrollOffset, XY(205, 309) + ScrollOffset);
            }

            if (cost1 + cost2 + cost3 + cost4 > 0) {
                RoomBm.PrintAt(bprintf("%s %s", (LPCTSTR)CString(StandardTexte.GetS(TOKEN_SCHED, 1800)),
                                       (LPCTSTR)CString(Einheiten[EINH_DM].bString(cost1 + cost2 + cost3 + cost4))),
                               FontSmallBlack, TEC_FONT_LEFT, XY(8, 329) + ScrollOffset, XY(205, 345) + ScrollOffset);
            }
            ...
            /*RoomBm.BlitFrom (DigitBms[qPlane.AnzPutzcrew%10], 133+ScrollOffsetX, 374);
              ...
              RoomBm.PrintAt (bprintf ("%s %s", (LPCTSTR)(CString)StandardTexte.GetS (TOKEN_SCHED, 1802), (LPCTSTR)(CString)Einheiten[EINH_DM].bString
              (qPlane.AnzPutzcrew*250)), FontSmallBlack, TEC_FONT_LEFT, XY(58,329+87)+ScrollOffset, XY(205,345+87)+ScrollOffset);*/
            ...
            // Erster-Klasse Anzahl blitten:
            {
                ...
                if (prozent > 9) {
                    RoomBm.BlitFrom(DigitBms[prozent / 10], 117 + ScrollOffsetX, 374);
                }
                ...
                for (c = 0; c < 3; c++) {
                    for (d = 0; d < 4; d++) {
                        if (gMousePosition.IfIsWithin(StarOffsets[d + 4].x + c * 47, StarOffsets[d + 4].y, StarOffsets[d + 4].x + 43 + c * 47,
                            ...
                            switch (d) {
                            case 0:
                                qPlane.ReifenTarget = Backup;
                                break;
                            case 1:
                                qPlane.TriebwerkTarget = Backup;
                                break;
                            case 2:
                                qPlane.SicherheitTarget = Backup;
                                break;
                            case 3:
    ...
    if ((IsDialogOpen() == 0) && (MenuIsOpen() == 0) && (ActivePage == 0 || ActivePage == 100)) {
        ...
        if (gMousePosition.IfIsWithin(134, 26, 164, 44) && PlaneIndex > 0) {
        ...
        } else if ((gMousePosition.IfIsWithin(172, 50, 192, 55) || gMousePosition.IfIsWithin(178, 56, 195, 69) ||
                    gMousePosition.IfIsWithin(182, 60, 207, 67))) {
            if (ActivePage == 100) {
                SetMouseLook(CURSOR_HOT, 3105, ROOM_PLANEPROPS, 90);
                RoomBm.BlitFromT(MenuBms[7], 172 + HeadlineOffset, 50);
            }
        } else if ((gMousePosition.IfIsWithin(224, 48, 243, 52) || gMousePosition.IfIsWithin(218, 53, 239, 56) ||
                    gMousePosition.IfIsWithin(210, 57, 234, 64))) {
            if (ActivePage == 0) {
            ...
            if (Costs > 0) {
                Sim.Players.Players[PlayerNum].Messages.AddMessage(
                    BERATERTYP_GIRL, bprintf(StandardTexte.GetS(TOKEN_SCHED, 1820), Insert1000erDots(Costs).c_str()), MESSAGE_COMMENT);
            } else if (Costs < 0) {
                Sim.Players.Players[PlayerNum].Messages.AddMessage(
                    BERATERTYP_GIRL, bprintf(StandardTexte.GetS(TOKEN_SCHED, 1821), Insert1000erDots(-Costs).c_str()), MESSAGE_COMMENT);
            }
    ...
    if (ActivePage != 100) {
        RoomBm.BlitFromT(MenuBms[9], 172 + HeadlineOffset, 50);
    }
    if (ActivePage != 0) {
        RoomBm.BlitFromT(MenuBms[10], 210 + HeadlineOffset, 50);
    }
...
void CPlaneProps::OnLButtonDown(UINT nFlags, CPoint point) {
    ...
    if (PreLButtonDown(point) == 0) {
        ...
        if (MouseClickArea == ROOM_PLANEPROPS && MouseClickId == 999) {
            qPlayer.LeaveRoom();
            ...
            if (qPlane.MaxBegleiter > qPlane.ptAnzBegleiter) {
                qPlane.MaxBegleiter--;
            }
            ...
            if (qPlane.MaxBegleiter < qPlane.ptAnzBegleiter * 2) {
                qPlane.MaxBegleiter++;
            }
            qPlayer.MapWorkers(FALSE);
            qPlayer.NetUpdatePlaneProps(PlaneId);
